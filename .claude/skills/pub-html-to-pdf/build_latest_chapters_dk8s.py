#!/usr/bin/env python3
"""
build_latest_chapters_dk8s.py — 도커·쿠버네티스 책의 머릿말+프롤로그+챕터 1~6 최신본+에필로그+맺음말을
PDF로 변환하고, 각 번호 챕터 앞에 구분 표지 페이지를 끼워 하나로 병합한다.

- 표지 데이터: projects/특이점이-온-개발자-도커-쿠버네티스/book/dividers.json
- 판권지 데이터: projects/특이점이-온-개발자-도커-쿠버네티스/book/colophon.json
- 표지 이미지: projects/특이점이-온-개발자-도커-쿠버네티스/assets/cover.jpg
- build_pdf.render_pdf 재사용.
사용법: python .claude/skills/pub-html-to-pdf/build_latest_chapters_dk8s.py

build_latest_chapters_msa.py를 복사해 PROJECT·CHAPTERS·_TOP_LABELS·출력 파일명만 교체.
"""
...
# 물리 순서: 표지 → 판권지 → 머릿말 → 프롤로그 → 목차 → 챕터1~6 → 에필로그 → 맺음말
CHAPTERS = [
    "머릿말",
    "prologue",
    "01-왜-컨테이너인가-v9",
    "02-Docker-이해하기-v9",
    "03-Docker-다루기-v10",
    "04-Kubernetes-시작하기-v11",
    "05-Kubernetes-네트워킹-v11",
    "06-Kubernetes-운영하기-v9",
    "epilogue",
    "afterword",
]
...
_TOP_LABELS = {"머릿말": "머릿말", "prologue": "프롤로그",
               "epilogue": "에필로그", "afterword": "맺음말"}
