#!/usr/bin/env python3
"""
build_latest_chapters_msa.py — MSA 책의 프롤로그+챕터 1~5 최신본+에필로그+마치며를
PDF로 변환하고, 각 번호 챕터 앞에 구분 표지 페이지를 끼워 하나로 병합한다.

- 표지 데이터: projects/특이점이-온-개발자-MSA/book/dividers.json
- 표지 스타일: 풀페이지 중앙 (CHAPTER + 번호 + 제목/부제 + '이번 챕터가 끝나면' goal)
- build_pdf.render_pdf 재사용.
사용법: python .claude/skills/pub-html-to-pdf/build_latest_chapters_msa.py
"""
...
# 물리 순서: 표지 → 머릿말 → 프롤로그 → 목차 → 챕터1~5 → 에필로그 → 마치며
CHAPTERS = [
    "preface",
    "prologue",
    "01-MSA란-무엇인가-v5",
    "02-동기식-MSA-구현-v5",
    "03-클린-아키텍처와-Kubernetes-v5",
    "04-비동기-MSA-Kafka-v6",
    "05-실시간-알림-WebSocket-v7",
    "epilogue",
    "afterword",
]
...
_TOP_LABELS = {"preface": "머릿말", "prologue": "프롤로그",
               "epilogue": "에필로그", "afterword": "마치며"}
...
def _finalize(src, dst, phys, toc_entries):
    ...
    raw = []
    if toc_phys:
        raw.append((1, "목차", toc_phys))
    for lvl, label, bp in toc_entries:
        if bp in body_to_phys:
            raw.append((lvl, label, body_to_phys[bp]))
    raw.sort(key=lambda e: (e[2], e[0]))
    outline, prev = [], 0
    for lvl, label, pno in raw:
...
def main() -> int:
    ...
    dividers = _load_dividers()
    import fitz

    cover = _cover_pdf()

    # 1) 모든 파트 렌더 (numbering 순서: 머릿말, 프롤로그, [표지+챕터]*, 에필로그, 마치며)
    rendered = []
    for name in CHAPTERS:
        ...
        if name[:2].isdigit():
            ...
            rendered.append({"pdf": pdf, "lvl": 1,
                             "label": _TOP_LABELS.get(name), "sub": None})
    ...
    # 4) 물리 조립: 표지 + 머릿말 + 프롤로그 + 목차 + (챕터 파트들) + 에필로그 + 마치며
    colophon = _colophon_pdf()
    phys = []
    if cover:
    ...
    print(f"[OK] 병합 완료(표지·머릿말·프롤로그·목차 + 계층 목차/번호): {merged}")
    return 0
