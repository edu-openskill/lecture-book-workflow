#!/usr/bin/env python3
"""컨셉 3 — '시리즈물형' 일관된 상징물형
시리즈 통일 레이아웃 + 권별 컬러코딩 + 큰 넘버링 + 공통 심볼"""
...
f_title    = F(44, bold=True)
f_sub      = F(14, bold=True)
f_author   = F(10, bold=True)
f_pub      = F(8)
f_series_s = F(6)
f_vol_num  = F(80, bold=True)   # 큰 넘버링
f_vol_lbl  = F(8)
f_tagline  = F(7)
...
def draw_symbol(d, cx, cy, r, symbol_type, fill_color, outline_color, lw):
    """시리즈 공통 심볼 (권별로 다른 도형)"""
...
def draw_series_dots(d, cx, y, active_idx, colors, r):
    """시리즈 진행 인디케이터 (4개 점)"""
    gap = r * 4
    start_x = cx - int(1.5 * gap)
    for i in range(4):
        dx = start_x + i * gap
        if i == active_idx:
            d.ellipse([(dx-r, y-r), (dx+r, y+r)], fill=colors[i])
        else:
            d.ellipse([(dx-r, y-r), (dx+r, y+r)], outline=colors[i],
                      width=max(1, mm(0.3)))


BOOK_COLORS = [(0,128,128), (75,60,180), (120,50,160), (180,120,30)]
...
def create_spread(b):
    ...
    c1, c2 = b["c1"], b["c2"]
    idx = [bb["id"] for bb in BOOKS].index(b["id"])

    c1_very_light = tuple(c1[i] + int((255 - c1[i]) * 0.92) for i in range(3))
    c1_light = tuple(c1[i] + int((255 - c1[i]) * 0.75) for i in range(3))
    ...
    # 상단 컬러 밴드 (시리즈 통일 위치, 권별 색상)
    band_h = mm(45)
    d.rectangle([(fx, 0), (fx+FRONT, band_h)], fill=c1)

    # 밴드 안에 시리즈명 (흰색)
    stxt = "특이점이 온 개발자"
    sw = tw(d, stxt, f_series)
    d.text((fx + (FRONT-sw)//2, mm(8)), stxt, fill=(255,255,255,200), font=f_series)

    # 밴드 안에 VOL 넘버 (큰 숫자, 반투명)
    vol_w = tw(d, b["vol"], f_vol_num)
    vol_color = tuple(min(255, v + 40) for v in c1)
    d.text((fx + FRONT - vol_w - mm(8), mm(2)), b["vol"],
           fill=vol_color, font=f_vol_num)

    # VOL 라벨
    vlbl = f"VOL.{b['vol']}"
    d.text((fx + mm(8), band_h - mm(12)), vlbl, fill="white", font=f_vol_lbl)

    # 심볼 (중앙)
    sym_cy = int(H * 0.40)
    draw_symbol(d, fx + FRONT//2, sym_cy, mm(30),
                b["symbol_type"], c1_very_light, c1_light, mm(0.8))
    # 심볼 안에 작은 심볼 (이중)
    draw_symbol(d, fx + FRONT//2, sym_cy, mm(18),
                b["symbol_type"], "white", c1, mm(0.5))

    # 제목 (심볼 아래)
    title_y = sym_cy + mm(38)
    title_w = tw(d, b["title"], f_title)
    d.text((fx + (FRONT-title_w)//2, title_y), b["title"], fill=(30,30,30), font=f_title)

    # 부제
    sub_y = title_y + th(d, b["title"], f_title) + mm(4)
    sub_w = tw(d, b["sub"], f_sub)
    d.text((fx + (FRONT-sub_w)//2, sub_y), b["sub"], fill=c1, font=f_sub)

    # 태그라인
    tag_y = sub_y + th(d, b["sub"], f_sub) + mm(6)
    ...
    # 시리즈 진행 인디케이터 (4개 점)
    dot_y = int(H * 0.82)
    draw_series_dots(d, fx + FRONT//2, dot_y, idx, BOOK_COLORS, mm(2.5))
...
for b in BOOKS:
    img = create_spread(b)
    p = os.path.join(OUT, f"spread-concept3-{b['id']}.png")
    img.save(p, dpi=(DPI,DPI))
    print(f"OK {p} ({img.size[0]}x{img.size[1]})")

print(f"\n컨셉 3 '시리즈물형' — {len(BOOKS)}권 완료")
