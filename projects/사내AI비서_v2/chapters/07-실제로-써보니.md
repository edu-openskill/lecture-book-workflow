...
> 한 줄 요약: AI 비서에게 기억력과 업무 일지를 줬다. 같은 질문엔 바로 답하고, 하루 몇 건 처리했는지 기록한다.  
...
CH05에서 AI 비서에게 대화용 메모장(WindowMemory)을 제공했습니다. 최근 5턴을 기억하는 메모장이었습니다. 이번에는 다른 종류의 메모장을 추가해 보겠습니다.

**답변 메모장 (ResponseCache)**

누군가 같은 질문을 또 했습니다. AI 비서가 이미 한 번 검색해서 답했던 질문이라면 다시 벡터 DB를 뒤질 필요가 없습니다. 메모장에 적어둔 답을 바로 읽어주면 됩니다. 다만 메모장에는 유통기한이 있습니다. 1시간이 지나면 메모를 지우게 됩니다. 사내 규정이 바뀌었을 수도 있기 때문입니다. 이를 **TTL(Time To Live)** 이라고 부릅니다.

**임베딩 메모장 (EmbeddingCache)**

AI 비서가 문서를 검색하려면 질문 문장을 벡터(숫자 배열)로 바꿔야 합니다. 이 변환에도 시간이 걸립니다. 같은 문장을 여러 번 변환할 필요 없이 한 번 계산한 벡터를 파일로 저장해 두면 다음에는 바로 꺼내 쓸 수 있습니다.
...
AI 비서에 모니터링 시스템이 없다고 생각해 봅니다. 하루 종일 질문에 답하는데 팀장이 "이번 달 LLM 비용이 얼마야?"라고 물으면 대답을 못 할 것입니다. 기록이 없으니까요.

우리 시스템도 마찬가지입니다. 동료 B가 "비용이 얼마나 나가는 건지 모르겠어요"라고 했을 때 답할 수가 없을 것입니다. LLM 호출 한 번에 토큰이 얼마나 들고 하루에 총 몇 건 처리하는지 아무도 모를 것입니다.

그래서 AI 비서에게 업무 일지를 작성하도록 해보겠습니다.
...
실무에서는 **Langfuse**나 LangSmith 같은 전문 모니터링 도구가 이 역할을 합니다. 호출별 토큰 수와 비용, 지연 시간을 자동으로 수집해서 대시보드로 보여주게 됩니다. 하지만 도구를 붙이기 전에 "무엇을 측정해야 하는지"부터 이해해야 합니다. 그래서 이 책에서는 **TokenTracker**라는 간단한 클래스를 직접 만들어 보겠습니다. 매 호출마다 한 줄씩 기록이 쌓이고 나중에 "이번 달 토큰 비용이 얼마야?"라고 물으면 바로 답할 수 있게 됩니다. 운영을 개선하려면 먼저 측정부터 해야 합니다.


### 1.4 Retry 로직

ex06의 AI 비서는 LLM 호출이 실패하면 "지금 못 찾겠어요"라고 말하고 멈췄습니다. 사용자는 같은 질문을 처음부터 다시 해야 했습니다.

네트워크 오류나 타임아웃, 파싱 실패 등으로 LLM이 가끔 응답을 못 합니다. 이제 AI 비서에게 규칙을 하나 알려주겠습니다. "한 번 실패하면 **3번까지 다시 시도해.** 시도 사이에는 2초씩 쉬고." 일시적인 오류는 잠깐 뒤에 다시 시도하면 해결되는 경우가 많습니다. 이렇게 캐시와 업무 일지, 재시도까지 운영에 필요한 안정 장치를 모두 갖추게 됩니다.
...
| 기능 | 비유 | 코드 |
|------|------|------|
| 응답 캐시 | AI 비서의 답변 메모장 (1시간 유효) | `cache.py` |
| 임베딩 캐시 | 단어→벡터 변환 결과 저장 | `cache.py` |
| 토큰 추적 | AI 비서의 업무 일지 | `monitoring.py` |
...
| 캐시+재시도가 붙은 AI 비서 | **ConnectHRAgent** | ex06의 IntegratedAgent에 캐시, 모니터링, 재시도를 통합한 운영용 에이전트 |
