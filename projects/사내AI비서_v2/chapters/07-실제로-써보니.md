...
누군가 같은 질문을 또 했습니다. AI 사서가 이미 한 번 검색해서 답했던 질문이라면 다시 벡터 DB를 뒤질 필요가 없습니다. 메모장에 적어둔 답을 바로 읽어주면 됩니다. 다만 메모장에는 유통기한이 있습니다. 1시간이 지나면 메모를 지우게 됩니다. 사내 규정이 바뀌었을 수도 있기 때문입니다. 이를 **TTL(Time To Live)** 이라고 부릅니다.

**임베딩 메모장 (EmbeddingCache)**

AI 사서가 문서를 검색하려면 질문 문장을 벡터(숫자 배열)로 바꿔야 합니다. 이 변환에도 시간이 걸립니다. 같은 문장을 여러 번 변환할 필요 없이 한 번 계산한 벡터를 파일로 저장해 두면 다음에는 바로 꺼내 쓸 수 있습니다.
...
AI 사서에 모니터링 시스템이 없다고 생각해 봅니다. 하루 종일 질문에 답하는데 팀장이 "이번 달 LLM 비용이 얼마야?"라고 물으면 대답을 못 할 것입니다. 기록이 없으니까요.
...
ex06의 AI 사서는 LLM 호출이 실패하면 "지금 못 찾겠어요"라고 말하고 멈췄습니다. 사용자는 같은 질문을 처음부터 다시 해야 했습니다.

네트워크 오류나 타임아웃, 파싱 실패 등으로 LLM이 가끔 응답을 못 합니다. 이제 AI 사서에게 규칙을 하나 알려주겠습니다. "한 번 실패하면 **3번까지 다시 시도해.** 시도 사이에는 2초씩 쉬고." 일시적인 오류는 잠깐 뒤에 다시 시도하면 해결되는 경우가 많습니다. 이렇게 캐시와 업무 일지, 재시도까지 운영에 필요한 안정 장치를 모두 갖추게 됩니다.
