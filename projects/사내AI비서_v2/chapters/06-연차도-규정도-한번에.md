...
`ex06/src/agent.py`를 열어 TODO의 `pass`를 지우고 아래 코드를 작성합니다.

#### 1단계: 프롬프트 및 도구 임포트

`ex06/src/agent.py`를 열어 두 함수의 TODO의 `pass`를 지우고 아래 코드를 작성합니다.

`IntegratedAgent`는 앞에서 만든 안내데스크(QueryRouter) + 도구(mcp_tools)를 하나로 합친 통합 비서 클래스입니다. **시스템 프롬프트(System Prompt)** 는 LLM에게 "당신은 누구이고, 무엇을 할 수 있고, 어떤 규칙을 지켜야 하는지"를 알려주는 지시문입니다. 파일을 열어 `SYSTEM_PROMPT` 내용을 확인해 보시기 바랍니다.
