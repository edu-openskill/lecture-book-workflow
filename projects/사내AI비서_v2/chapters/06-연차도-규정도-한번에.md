...
AI 비서도 이렇게 되어야 합니다. 질문을 보고 DB가 필요한지 문서가 필요한지 아니면 둘 다인지 판단하게 됩니다. 판단이 끝나면 각 담당자(도구)에게 작업을 맡기고 결과를 하나로 묶어서 답해 주게 됩니다. 이번 챕터에서 만들 **통합 에이전트**가 바로 이 안내데스크입니다.
...
*사용자의 질문 목적지에 맞게 알맞은 담당자를 배정하는 안내데스크.*
...
*한 번의 질문으로 안내데스크가 담당자들을 알아서 찾아 정보를 조합해준다.*
...
sequenceDiagram
    사용자->>안내데스크: 질문
    안내데스크->>안내데스크: 질문 분류 (정형/비정형/혼합)
    안내데스크->>에이전트: 분류 결과 전달
    에이전트->>도구: 필요한 도구 호출 (DB조회/문서검색)
    도구-->>에이전트: 결과
    에이전트->>에이전트: 결과를 종합하여 답변 생성
    에이전트-->>사용자: 답변
```

*CH06 전체 흐름입니다. 질문이 안내데스크(QueryRouter)를 거쳐 에이전트로 전달되고, 에이전트가 도구를 호출하여 답변을 생성합니다.*
...
*파란색이 이번 실습에서 만드는 부분입니다. 질문을 정형/비정형/혼합으로 분류하는 안내데스크를 만듭니다.*
...
`IntegratedAgent` 클래스는 앞에서 만든 안내데스크(QueryRouter)와 도구(mcp_tools)를 연결하는 껍데기입니다. 클래스 선언과 `__init__`은 이미 파일에 있으니 열어서 확인해 보시기 바랍니다. 파일 상단의 **SYSTEM_PROMPT**도 함께 보면 좋습니다. LLM에게 "당신은 누구이고, 무엇을 할 수 있고, 어떤 규칙을 지켜야 하는지"를 알려주는 지시문입니다.
