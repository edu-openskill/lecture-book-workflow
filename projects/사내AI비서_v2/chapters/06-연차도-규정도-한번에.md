...
*CH06 전체 흐름. 질문이 안내데스크(QueryRouter)를 거쳐 에이전트로 전달되고, 에이전트가 도구를 호출하여 답변을 생성한다.*
...
*파란색이 이번 실습에서 만드는 부분이다. 질문을 정형/비정형/혼합으로 분류하는 안내데스크를 만든다.*
...
사원이나 매출처럼 숫자로 된 데이터는 `structured`로, 회사 정책이나 규정처럼 문장으로 된 문서 지식은 `unstructured` 키워드로 나눕니다.
...
```python
# router.py — TODO: classify_query 메서드 구현
...
`QueryRouter` 클래스와 `__init__`은 이미 파일에 준비되어 있습니다.
...
*파란색이 이번 실습에서 만드는 부분이다. 에이전트가 호출할 도구(DB조회, 문서검색)를 @tool로 정의한다.*

에이전트가 데이터베이스나 문서 저장소에 접근할 때 쓰는 도구입니다. `@tool` 데코레이터만 붙이면 에이전트가 스스로 판단해서 도구를 사용합니다.

`ex06/src/mcp_tools.py`를 열어 TODO의 `pass`를 지우고 아래 코드를 작성합니다.
...
`leave_balance` 함수의 TODO의 `pass`를 지우고 아래 코드를 작성합니다.

```python
# mcp_tools.py — TODO: leave_balance 함수 구현
...
```python
# mcp_tools.py — TODO: sales_sum 함수 구현
...
```python
# mcp_tools.py — TODO: list_employees 함수 구현
...
```python
# mcp_tools.py — TODO: search_documents 함수 구현
...
*파란색이 이번 실습에서 만드는 부분이다. 라우터와 도구를 조립하여 ReAct 에이전트가 답변을 생성한다.*
...
앞에서 설명한 도구 호출(Tool Calling)을 실제로 구현하는 단계입니다. `create_tool_calling_agent` 는 LLM에게 "이 도구를 쓸 수 있다"고 알려주는 에이전트를 만들고 **AgentExecutor** 는 이 에이전트의 "생각 -> 도구 호출 -> 결과 확인" 루프를 반복 실행하는 실행기입니다. `_build_agent_executor` 메서드의 TODO의 `pass`를 지우고 아래 코드를 작성합니다.

```python
# agent.py — TODO: _build_agent_executor 메서드 구현
