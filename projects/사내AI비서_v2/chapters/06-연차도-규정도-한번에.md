...
> 한 줄 요약: CH05의 사서가 질문 분류와 도구 호출을 배워 AI 사서로 진화한다.  
...
CH05의 사서도 이런 안내 역할을 했습니다. 문서를 찾아서 답해주는 것까지는 잘 해냈죠. 그런데 질문이 다양해지면서 사서 혼자서는 감당이 안 되기 시작합니다. "연차 며칠 남았어?"는 문서가 아니라 DB를 뒤져야 하고, "연차 잔여 + 신청 절차"는 DB와 문서를 동시에 봐야 합니다.

사서가 질문을 분류하는 체계를 갖추고 필요한 도구를 직접 호출할 수 있게 되면 어떨까요? 질문의 종류를 판단하고(QueryRouter), 맞는 도구를 골라 실행하고(Tool), 결과를 종합해서 답하는(Agent) 시스템을 갖추는 것입니다.

**AI 사서**: "인사팀은 3층이에요. 엘리베이터 내리셔서 오른쪽입니다."

**AI 사서**: "교육 신청은 3층 인사팀이고, 노트북은 2층 IT 지원팀이에요. 두 곳 다 가셔야 해요."

AI 비서도 이렇게 되어야 합니다. 질문을 보고 DB가 필요한지 문서가 필요한지 아니면 둘 다인지 판단하게 됩니다. 판단이 끝나면 각 담당자(도구)에게 작업을 맡기고 결과를 하나로 묶어서 답해 주게 됩니다. CH05의 사서가 질문 분류 + 도구 호출을 배워서 **AI 사서**로 진화하는 것, 이번 챕터에서 만들 **통합 에이전트**가 바로 이 AI 사서입니다.



### 1.4 QueryRouter -- 3단계 질문 분류

에이전트 안에는 **QueryRouter**라는 AI 사서의 '판단 규칙'이 들어 있습니다. 모든 질문은 3단계를 거쳐 분류되는데, 베테랑 사서가 질문을 받고 어느 서가로 갈지 판단하는 방식과 똑같습니다.
...
Center: A minimalist librarian desk icon labeled "AI 사서".
...
*사용자의 질문 유형에 맞게 알맞은 도구를 배정하는 AI 사서.*
...
*대부분의 질문은 1단계 키워드 매칭에서 끝난다. 모호할 때만 더 비싼 방법을 쓴다.*
...
독스트링이 중요한 이유가 여기 있습니다. 함수 설명이 불명확하면 에이전트가 엉뚱한 도구를 고릅니다. AI 사서에게 각 도구가 무슨 일을 하는지 제대로 알려줘야 제대로 골라 쓰는 것과 마찬가지입니다.
...
*질문이 들어오면 QueryRouter가 분류하고, ReAct 에이전트가 필요한 도구를 순서대로 호출한다.*
...
*한 번의 질문으로 AI 사서가 필요한 도구를 알아서 골라 정보를 조합해준다.*
...
그럼 지금부터 이 AI 사서, **통합 에이전트**를 직접 만들어 보겠습니다.
...
| 기능 | 비유 | 코드 |
|------|------|------|
| 3단계 질문 분류기 | AI 사서의 판단 기준 | `router.py` |
| 도구(Tool) 4개 | 각 담당 도구 | `mcp_tools.py` |
| ReAct 에이전트 | AI 사서 전체 | `agent.py` |
...
| 비유 | 기술 용어 | 정식 정의 |
|------|---------|----------|
| AI 사서 전체 | **에이전트 (Agent)** | 사용자의 질문을 받아 스스로 판단하고, 필요한 도구를 선택·실행해서 최종 답변을 만들어내는 자율적 프로그램 |
| AI 사서의 분류 기준 | **QueryRouter** | 사용자 질문을 분석해서 처리 경로(structured / unstructured / hybrid)를 결정하는 분류기 |
...
sequenceDiagram
    사용자->>AI 사서: 질문
    AI 사서->>AI 사서: 질문 분류 (정형/비정형/혼합)
    AI 사서->>도구: 필요한 도구 호출 (DB조회/문서검색)
    도구-->>AI 사서: 결과
    AI 사서->>AI 사서: 결과를 종합하여 답변 생성
    AI 사서-->>사용자: 답변
```

*CH06 전체 흐름입니다. 질문이 AI 사서(QueryRouter)를 거쳐 분류되고, 에이전트가 도구를 호출하여 답변을 생성합니다.*
...
*파란색이 이번 실습에서 만드는 부분입니다. 질문을 정형/비정형/혼합으로 분류하는 AI 사서의 질문 분류기를 만듭니다.*
...
*파란색이 이번 실습에서 만드는 부분입니다. 에이전트가 호출할 도구(DB조회, 문서검색)를 @tool로 정의합니다.*

에이전트가 데이터베이스나 문서 저장소에 접근할 때 쓰는 도구입니다. `@tool` 데코레이터만 붙이면 에이전트가 스스로 판단해서 도구를 사용합니다. `ex06/src/mcp_tools.py`를 열어 네 함수의 TODO의 `pass`를 지우고 아래 코드를 작성합니다.
...
*파란색이 이번 실습에서 만드는 부분입니다. 라우터와 도구를 조립하여 ReAct 에이전트가 답변을 생성합니다.*
...
`IntegratedAgent` 클래스는 앞에서 만든 AI 사서의 질문 분류기(QueryRouter)와 도구(mcp_tools)를 연결하는 껍데기입니다. 클래스 선언과 `__init__`은 이미 파일에 있으니 열어서 확인해 보시기 바랍니다. 파일 상단의 **SYSTEM_PROMPT**도 함께 보면 좋습니다. LLM에게 "당신은 누구이고, 무엇을 할 수 있고, 어떤 규칙을 지켜야 하는지"를 알려주는 지시문입니다.
...
*연차 잔여(DB)와 신청 규정(문서)을 동시에 가져온다.*
...
- **사서가 AI 사서로 진화했습니다.** 질문을 분류하고 도구를 호출하여 답변합니다.
