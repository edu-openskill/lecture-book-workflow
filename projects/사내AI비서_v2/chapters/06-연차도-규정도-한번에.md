...
도구가 준비됐습니다. 이제 에이전트가 이 도구를 어떻게 쓰는지 살펴보겠습니다. 에이전트의 두뇌는 **ReAct 패턴**으로 동작합니다. Reason(추론) + Act(행동). 이름이 거창해 보이지만 원리는 단순합니다. 생각하고 행동하는 걸 반복합니다.
...
이 과정을 최대 10회까지 반복할 수 있습니다. 한 번에 답을 못 찾으면 다시 생각하고 다시 행동합니다. 도중에 오류가 나도 포기하지 않고 다른 방법을 시도합니다.
...
3단계는 순서가 중요합니다. 확실한 신호(키워드)를 먼저 확인하고 모호할 때만 더 비싼 방법(LLM 호출)을 씁니다. 대부분은 1단계에서 끝납니다.
