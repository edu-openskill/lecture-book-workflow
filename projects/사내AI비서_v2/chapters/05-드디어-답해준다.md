...
```python
# rag_chain.py — TODO: 각 Document에서 source, page 메타데이터와 page_content를 꺼내어
...
```python
# rag_chain.py — TODO: 캐시가 비어 있으면 build_rag_chain()으로 초기화 후 반환
...
`chat_api.py`에서 매 요청마다 이 함수를 호출합니다. 매번 체인을 새로 만들지 않고 캐시된 것을 재사용합니다.
