...
`__init__`에 이미 준비된 코드를 보겠습니다.

```python
# conversation.py — __init__ (파일에 이미 작성되어 있음)

self._turns = deque(maxlen=k)  # k=5: 최근 5턴만 유지
```

`deque(maxlen=k)`가 핵심입니다. `maxlen`을 설정하면 새 항목이 들어올 때 가장 오래된 항목이 자동으로 빠집니다. 메모장에 6번째 메모를 붙이면 1번째가 떨어집니다. `get_history()`가 반환하는 텍스트가 프롬프트의 `{history}`에 들어갑니다.
