...
**두 가지 방법을 섞어 쓰는 사서** — "연차 신청"이라는 질문에 벡터 검색은 "휴가 사용"이라는 의미적으로 비슷한 문서를 찾아옵니다. BM25는 "연차"와 "신청"이라는 단어가 정확히 들어간 문서를 찾아옵니다. 둘을 합치면? 의미도 맞고 키워드도 맞는 문서가 상위에 올라옵니다. 이것이 **하이브리드 검색** 입니다.
...
가장 먼저 확인할 건 "몇 자로 자를 것인가"입니다. 같은 문서를 300자, 500자, 1000자로 잘라보고 결과를 비교해보십시오. 고정 크기 청킹의 핵심 코드입니다.
...
`--percentile` 옵션으로 검문소 높이를 직접 바꿔서 실험해보십시오:
...
k값(3, 5, 10), threshold(0.0~0.5), metadata 필터(부서별, 문서유형별)의 결과가 표로 출력됩니다. 결과를 확인한 뒤 `--k`, `--threshold`, `--department` 옵션으로 직접 값을 바꿔가며 실험해보십시오.
...
- **청킹은 의미 단위로 하십시오.** Fixed-size 청킹은 빠르지만 주제가 섞입니다. Semantic Chunking이 이상적이고 Recursive Character 청킹이 현실적인 균형점입니다.
- **리랭킹은 2단계 전략으로 진행하십시오.** 넓게 가져와서(top_k=10~20) Cross-Encoder로 좁게 정제(top_k=5)합니다.
- **하이브리드 검색이 단일 검색보다 낫습니다.** BM25(키워드) + Vector(의미)를 alpha로 가중 결합하면 한쪽만 쓸 때의 약점을 보완해줍니다.
- 다음 챕터에서는 검색이 아니라 **질문 자체** 를 다듬습니다. 사용자가 "규정 알려줘"처럼 모호하게 물어봤을 때 질문을 구체화하는 Query Rewriting을 다룹니다.
