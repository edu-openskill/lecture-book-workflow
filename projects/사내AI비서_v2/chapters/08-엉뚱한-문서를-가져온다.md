...
*AI 비서의 청킹 전략이 진화하는 과정. 가위로 자르던 방식에서 문단을 보고, 의미를 읽는 방식으로.*
...
**리랭킹(ReRanking)** — 검색 결과를 10개 가져왔다고 합시다. 이 중에 진짜 관련 있는 건 3~4개이고 나머지는 비슷하게 생겼지만 관련 없는 문서입니다. AI 비서가 10개를 펼쳐놓고 질문을 다시 읽으면서 "이건 맞고, 이건 아니고"를 하나하나 확인합니다. **Cross-Encoder**라는 모델이 질문과 문서를 함께 읽고 관련도를 다시 매기는 겁니다.

**하이브리드 검색** — 지금 AI 비서는 한 가지 방법으로만 찾습니다. 의미가 비슷한 문서를 찾는 벡터 검색입니다. 하지만 **BM25**라는 정확한 키워드 매칭 방식도 있습니다. "연차 신청"이라는 질문에 벡터 검색은 "휴가 사용"이라는 의미적으로 비슷한 문서를 찾아옵니다. BM25는 "연차"와 "신청"이라는 단어가 정확히 들어간 문서를 찾아옵니다. 둘을 합치면? 의미도 맞고 키워드도 맞는 문서가 상위에 올라옵니다.
...
Left part — 'リランキング': A list of 10 documents, Cross-Encoder icon re-scoring them,
  resulting in a reordered list with top 3 highlighted.
Right part — 'ハイブリッド検索': Two parallel search paths (Vector search + BM25 keyword search)
