...
**동료**: "그거 왜 만들어요? ChatGPT한테 그냥 물어보면 되지 않나."
...
*그림 1-3. LLM은 세상의 공개 데이터는 학습했지만, 우리 회사 내부 문서는 읽은 적이 없다*
...
*그림 1-4. `step2_context.py` 실행 결과. 문서를 직접 넣으니 정확하게 답한다*
...
*그림 1-5. 문서를 통째로 넣는 방식의 한계. 문서가 늘어나면 프롬프트 창이 넘친다*
...
`OllamaEmbeddings`는 각 문서를 수백 차원의 숫자 배열(벡터)로 변환합니다. 의미가 비슷한 문서일수록 벡터 공간에서 가까이 위치하게 됩니다. `Chroma.from_documents()`가 이 벡터를 ChromaDB에 저장합니다. `k=3`은 "질문과 가장 비슷한 문서 3개를 가져오라"는 설정입니다.
...
"참고 정보를 바탕으로 질문에 답하세요"라는 한 줄이 핵심입니다. LLM에게 제공된 문서 안에서만 답하도록 제약을 거는 거예요.
...
*그림 1-8. `step3_rag.py` 실행 결과. [인사규정] 문서를 찾아서 답변하고 출처까지 보여준다*
...
*그림 1-9. 청킹 여부에 따른 검색 결과 비교. 조각으로 나누면 관련 문서만 정확히 찾는다*
...
## 1.5 복잡한 질문 던져보기: Chain-of-Thought 추론
...
:::term-box
**Chain-of-Thought란?** "단계별 추론"이라는 뜻. LLM이 최종 답을 곧바로 내놓지 않고, 먼저 문제를 작게 쪼개어 하나씩 따져본 뒤 답을 내놓게 하는 방식입니다. 연차 계산처럼 여러 단계가 필요한 질문에서 답의 정확도가 크게 올라갑니다.
:::
  ...
  <div class="jf-group">
    ...
    <div class="jf-items">
      <div class="jf-item">
        ...
        <div class="jf-desc"><span class="jf-title">통합 에이전트</span><span class="jf-hint">Query Router · MCP Tools · ReAct Agent</span></div>
      </div>
      <div class="jf-item">
