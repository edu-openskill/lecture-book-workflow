...
그래서 이번 챕터에서는 AI 비서보다 먼저 **사내 시스템**을 실행해봅니다. 코드를 하나하나 뜯어보진 않을 것입니다. 완성된 시스템을 띄워보고 "이런 데이터를 이렇게 조회할 수 있구나"를 확인하는 것이 목표입니다.
...
> 기본 환경(Python 3.12, Docker)이 없다면 **교육자료**를 먼저 확인하십시오.
...
> **Apple Silicon(M1/M2/M3) 사용자**: psycopg2-binary 설치가 실패하면 `brew install libpq` 를 먼저 실행하십시오.
...
서버를 실행하고 Swagger UI(`/docs`)에서 API를 직접 호출해보고, 웹 UI(`/admin/`)에서 일반 사용자 화면도 확인해 보십시오.
...
직원, 연차, 매출 — 세 영역의 API가 보입니다. 직접 눌러보십시오.

POST로 직원을 등록하고 GET으로 조회하면 방금 등록한 데이터가 돌아옵니다. 수정(PATCH)이나 삭제(DELETE)도 가능합니다. 이야기 파트에서 말한 CRUD 네 가지가 전부 동작하는 것입니다.
Swagger UI는 개발자용입니다. 하지만 이 시스템에는 일반 사용자를 위한 웹 UI도 있습니다. 브라우저에서 `http://localhost:8000/admin/`을 열어보십시오.
