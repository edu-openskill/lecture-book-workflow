...
*그림 2-1. 문서 검색만으로는 답할 수 없는 질문이 있다*
...
**팀장**: "문서 검색만 되면 안 되지."
**팀장**: "'김대리 연차 며칠 남았어?', '이번 달 개발팀 매출 얼마야?' 이런 것도 답해야지."
...
매출도 같습니다. 작년 매출 보고서 PDF는 있지만 "이번 달"은 DB에만 있습니다. 실시간 숫자는 문서에 없습니다. 규정집을 덮고 의자에 기댔습니다. 천장 형광등이 한 번 깜빡였습니다.
...
화이트보드 앞에 나란히 섰습니다. 팀장이 마커를 집어 들고 왼쪽부터 박스 세 개를 그려 나갔습니다. 마커 냄새가 한 번 훅 올라왔습니다.

**팀장**: "세 개면 충분해. 직원, 연차, 매출."

**직원(Employee)**. 사번, 이름, 부서, 직급, 입사일. 예: `EMP001 김철수 개발팀 대리 2023-03-01`.

**연차(LeaveBalance)**. 누가, 몇 년도에, 총 며칠이고, 사용한 게 며칠인지. 예: `김민수 2025년 총 15일, 사용 3일, 잔여 12일`.

**매출(Sale)**. 어느 부서가, 언제, 얼마를, 뭘 팔았는지. 예: `개발팀 2025-03-01 5,000,000원 SI프로젝트`.
...
**팀장**: "프로젝트에 이름이 없으면 회의할 때 불편해. 우리 회사가 **커넥트**잖아. HR 데이터를 다루는 AI 에이전트니까… **커넥트HR 에이전트** 어때?"
...
*그림 2-5. 직원 관리 목록. 맨 아래에 홍길동(E011)이 추가된 모습입니다. API 호출법을 몰라도 CRUD가 됩니다*

지금까지 만든 부분이 전체 시스템에서 어디에 놓이는지 한번 보겠습니다. 아래는 이 책 전체에서 만들 **커넥트HR 에이전트**의 구성도입니다. 진하게 표시된 두 박스가 이번 챕터에서 띄운 부분입니다. 나머지는 다음 챕터들에서 하나씩 채워 갑니다.
...
<div class="arch-fullmap">
  ...
  <div class="afm-zone">
    ...
    <div class="afm-row afm-three">
      <div class="afm-box afm-faint">
        <div class="afm-label">Query Router</div>
        <div class="afm-sub">규칙·스키마·LLM</div>
      </div>
      <div class="afm-box afm-faint">
        <div class="afm-label">LCEL Agent Chain</div>
        <div class="afm-sub">ReAct 패턴</div>
      </div>
      <div class="afm-box afm-faint">
        <div class="afm-label">MCP Tools</div>
        <div class="afm-sub">DB 조회 도구</div>
      </div>
    </div>
  </div>
  ...
  <div class="afm-note">
    오늘 띄운 건 <b>FastAPI + PostgreSQL</b> 두 박스입니다. 나머지는 챕터 3부터 하나씩 채워집니다. 챕터를 지날 때마다 이 그림의 빈 박스가 하나씩 진해지는 걸 확인해 주세요.
  </div>
</div>
