...
4단계를 순서대로 밟으면 됩니다. `load_embedding_model`이 ko-sroberta를 메모리에 올리고, `embed_chunks`가 청크 텍스트를 768차원 숫자 벡터로 변환합니다. `collection.upsert`는 같은 ID가 이미 있으면 덮어쓰므로, 파이프라인을 여러 번 실행해도 데이터가 중복되지 않습니다.

`search_chroma`는 검색 함수입니다. 저장할 때와 같은 모델로 질문을 벡터로 바꾸고, ChromaDB에서 가장 가까운 청크를 찾아옵니다.
...
`model.encode([query])`로 질문을 벡터로 변환한 뒤, `collection.query()`가 ChromaDB에서 코사인 거리가 가까운 순서대로 `top_k`개를 가져옵니다. 반환되는 `distance`는 0(완전 일치)~2(완전 반대) 범위의 코사인 거리입니다.
...
`main()` 함수에도 TODO가 있습니다. step1과 step2를 순서대로 호출합니다. `pass`를 지우고 아래 코드를 작성합니다.
