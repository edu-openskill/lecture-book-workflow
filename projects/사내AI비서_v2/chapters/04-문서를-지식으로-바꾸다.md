...
```mermaid
graph LR
    A["사내 문서<br/>data/docs/"] --> B["파싱<br/>extractor.py"]
    B --> C["청킹<br/>chunker.py ✏️"]
    C --> D["임베딩+저장<br/>store.py ✏️"]
    D --> E["파이프라인 조립<br/>main.py ✏️"]
    E --> F["검색 테스트<br/>cli_search.py ✏️"]
```

코드를 작성하고(1~4), 파싱부터 단계별로 확인합니다(5~7). ✏️가 학생이 직접 작성하는 파일입니다.
