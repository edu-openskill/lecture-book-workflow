...
4. `main.py --step 1` — 파싱 결과 확인
5. `main.py` — 전체 파이프라인 실행
6. `cli_search.py` — 검색 테스트

먼저 코드를 작성하고(1~3), 파싱 결과를 확인한 뒤(4), 전체 파이프라인을 돌리고(5), 검색이 되는지 테스트합니다(6). **파싱 결과 확인 단계를 건너뛰지 않기를 권장합니다.**


### 2.5 실습 1: 파싱부터 확인합니다 (main.py --step 1)

> 이 실습을 실행하려면 **2.6~2.8의 TODO를 먼저 채워야** 합니다. 코드를 먼저 작성한 뒤 여기로 돌아와 실행해 주시기 바랍니다.
...
파싱이 잘 되는 걸 확인했으니, 이제 핵심인 **청킹 코드**를 직접 작성합니다. `ex04/src/chunker.py`를 열어 `split_text_into_chunks` TODO의 `pass`를 지우고 아래 코드를 작성합니다.
...
`main()` 함수에도 TODO가 3개 있습니다. step1과 step2를 순서대로 호출하고, step1 결과가 없으면 자동으로 먼저 실행하는 로직을 넣어줍니다. `pass`를 지우고 아래 코드를 작성합니다.
