...
파싱이 잘 되는 걸 확인했으니, 이제 핵심인 **청킹 코드**를 직접 작성합니다. `ex04/src/chunker.py`를 열어 `split_text_into_chunks` 함수의 TODO를 아래 코드로 채워줍니다.

```python
# chunker.py — split_text_into_chunks
...
```python
# cli_search.py — run_single_query
...
```python
# cli_search.py — run_interactive_mode
