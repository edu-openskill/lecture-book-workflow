...
마크다운 표 형식을 확인할 수 있습니다. 표의 생김새가 일반 표보다 삐뚤빼뚤하고 다소 끊어진 것처럼 보일 수 있습니다. 하지만 **벡터 DB(임베딩 모델) 입장에서는 오히려 이 방식이 엑셀 원본보다 훨씬 이해하기 좋습니다.** 헤더 행과 구분선(`---`)이 존재하고 행/열의 문맥이 텍스트 흐름(파이프 `|` 기호 등)으로 유지되기 때문에 "경영지원본부 예산"을 검색하면 그 표의 데이터가 정확하게 검색됩니다.
...
파싱이 잘 되는 걸 확인했으니, 이제 핵심인 **청킹 코드**를 직접 작성합니다. `ex04/src/chunker.py`를 열어 `split_text_into_chunks` 함수의 `pass`를 지우고 아래 코드를 작성합니다.
...
청킹이 되면 임베딩해서 ChromaDB에 넣어야 합니다. `ex04/src/store.py`를 열어 두 함수의 `pass`를 지우고 아래 코드를 각각 작성합니다.
...
chunker와 store를 채웠으면 이제 **파이프라인을 조립**합니다. `ex04/src/main.py`를 열어 세 함수의 `pass`를 지우고 아래 코드를 각각 작성합니다.
...
파이프라인을 돌렸으면 이제 **검색이 작동하는지 직접 확인해 보겠습니다.** `ex04/src/cli_search.py`를 열어 두 함수의 `pass`를 지우고 아래 코드를 각각 작성합니다.
