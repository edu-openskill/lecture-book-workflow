...
코드를 모두 작성한 뒤(1~4), 파싱부터 단계별로 확인합니다(5~7).
...
```mermaid
graph LR
    A["split_text_into_chunks()"] -->|"호출됨"| B["_chunk_utils.py<br/>chunk_extract_result()"]
    B -->|"청크 생성"| C["_chunk_utils.py<br/>build_text_chunk()"]
    style A fill:#fff3cd,stroke:#ffc107,stroke-width:2px
```

*`split_text_into_chunks`는 `_chunk_utils.py`의 `chunk_extract_result`에서 호출됩니다. 학생이 작성할 함수는 노란색입니다.*
...
```mermaid
graph LR
    A["store_chunks_to_chroma()"] -->|"1"| B["_store_utils.py<br/>load_embedding_model()"]
    A -->|"2"| C["chromadb<br/>PersistentClient()"]
    A -->|"3"| D["_store_utils.py<br/>embed_chunks()"]
    A -->|"4"| E["collection.upsert()"]
    F["search_chroma()"] -->|"1"| B
    F -->|"2"| G["collection.query()"]
    style A fill:#fff3cd,stroke:#ffc107,stroke-width:2px
    style F fill:#fff3cd,stroke:#ffc107,stroke-width:2px
```

*학생이 작성할 두 함수(노란색)가 `_store_utils.py`의 보조 함수와 ChromaDB API를 호출합니다.*
...
```mermaid
graph TD
    M["main()"] -->|"step 1"| S1["step1_python_parsing()"]
    M -->|"step 2"| S2["step2_embed_and_store()"]
    S1 --> E["extractor.py<br/>extract_all_from_directory()"]
    S1 --> MD["_pipeline_utils.py<br/>save_results_as_markdown()"]
    S2 --> CH["chunker.py<br/>chunk_all_documents()"]
    S2 --> ST["store.py<br/>store_chunks_to_chroma()"]
    style M fill:#fff3cd,stroke:#ffc107,stroke-width:2px
    style S1 fill:#fff3cd,stroke:#ffc107,stroke-width:2px
    style S2 fill:#fff3cd,stroke:#ffc107,stroke-width:2px
```

*`main()`이 step1과 step2를 순서대로 호출합니다. 학생이 작성할 세 함수(노란색)가 앞에서 만든 chunker, store를 연결합니다.*
...
```mermaid
graph LR
    M["main()"] -->|"--query"| SQ["run_single_query()"]
    M -->|"없으면"| IM["run_interactive_mode()"]
    IM -->|"반복"| SQ
    SQ --> SC["store.py<br/>search_chroma()"]
    SQ --> PR["_search_utils.py<br/>print_search_result()"]
    style SQ fill:#fff3cd,stroke:#ffc107,stroke-width:2px
    style IM fill:#fff3cd,stroke:#ffc107,stroke-width:2px
```

*`main()`이 쿼리 유무에 따라 단일/대화형 모드를 선택합니다. 학생이 작성할 두 함수(노란색)가 앞에서 만든 `search_chroma`를 호출합니다.*
...
### 2.11 [설명] store.py — 임베딩 + ChromaDB 저장
