...
`main.py`는 이야기 파트에서 설명한 손질(파싱) → 다지기(청킹) → 양념(임베딩) → 냉장고 정리(저장) 네 단계를 한 번에 실행하는 진입점입니다. 하지만 **전체를 한 번에 돌리기 전에 파싱만 먼저** 확인해 보겠습니다.
...
`--step 1`은 손질(파싱)과 마크다운 변환을 실행합니다. 파싱이 끝나면 `data/markdown/` 폴더에 마크다운 파일 결과물이 생성됩니다. 이 파일을 열어보는 것이 핵심입니다. **파싱이 잘 됐는지 눈으로 확인하는 과정**, 이 단계를 건너뛰지 않도록 주의해야 합니다.
...
> 파싱은 RAG의 첫 단추입니다. 여기서 텍스트가 깨지면 뒤에서 아무리 고쳐도 소용이 없습니다. RAG 업계에는 쓰레기가 들어가면 쓰레기가 나온다는 **Garbage In, Garbage Out** 데이터 속성 법칙이 관용어처럼 따라붙을 정도입니다. 생성된 `data/markdown/` 폴더 결과물을 반드시 확인하여 원문과 꼼꼼히 대조해 보시길 적극 권장합니다.
...
### 2.6 실습 2: 청킹 핵심 로직 (chunker.py)

파싱이 잘 되는 걸 확인했으니, 이제 핵심인 **청킹 코드**를 직접 작성합니다. `ex04/src/chunker.py`를 열어봅니다.

```python
# chunker.py — TODO를 채워주세요

def split_text_into_chunks(text, chunk_size=500, overlap=100):
    """텍스트를 chunk_size 단위로 자르되, overlap만큼 겹치게 합니다."""
    # TODO: step = chunk_size - overlap
    #       while start < len(text): text[start:start+chunk_size]
    pass
```

이야기 파트에서 본 "500자 자르기 + 100자 오버랩" 그대로입니다. `step = chunk_size - overlap`으로 다음 청크 시작 위치를 계산하면 앞뒤 조각이 겹칩니다.


### 2.7 실습 3: 벡터DB 저장+검색 (store.py)

청킹이 되면 임베딩해서 ChromaDB에 넣어야 합니다. `ex04/src/store.py`를 열어봅니다.

```python
# store.py — TODO를 채워주세요

def store_chunks_to_chroma(chunks, chroma_dir, collection_name, embedding_model_name):
    """1. 모델 로드 → 2. ChromaDB 초기화 → 3. 임베딩 → 4. upsert"""
    # TODO: load_embedding_model() → PersistentClient → embed_chunks()
    #       collection.upsert(ids=..., documents=..., embeddings=..., metadatas=...)
    pass

def search_chroma(query, chroma_dir, collection_name, embedding_model_name, top_k=5):
    """쿼리 임베딩 → collection.query() → 결과 정리"""
    # TODO: model.encode([query]) → collection.query(query_embeddings=..., n_results=top_k)
    pass
```

`store_chunks_to_chroma`가 이야기 파트의 "냉장고 정리"에 해당합니다. `embed_chunks()`로 768차원 벡터를 만들고, `collection.upsert()`로 ChromaDB에 넣습니다. `search_chroma`는 나중에 CLI 검색에서 호출됩니다.


### 2.8 실습 4: 파이프라인 조립 + 실행 (main.py)

chunker와 store를 채웠으면 이제 **파이프라인을 조립**합니다. `ex04/src/main.py`의 TODO를 채워 `extractor`, `chunker`, `store` 모듈을 연결합니다.

```python
# main.py — TODO를 채워주세요

def step1_python_parsing(docs_dir):
    """extract_all_from_directory()로 문서 추출 → 마크다운 저장"""
    # TODO: results = extract_all_from_directory(docs_dir)
    #       save_results_as_markdown(results)
    pass

def step2_embed_and_store(python_results, chroma_dir, ...):
    """chunk_all_documents()로 청킹 → store_chunks_to_chroma()로 저장"""
    # TODO: all_chunks = chunk_all_documents(python_results, chunk_size, overlap)
    #       store_chunks_to_chroma(all_chunks, ...)
    pass
```
...
`--chunk-size` 옵션으로 청크 크기를 바꿀 수 있습니다. 기본값(500자, 100자 오버랩)으로 실행하면 파이프라인이 두 단계를 순서대로 실행합니다.
...
### 2.9 실습 5: 벡터 검색 CLI 도구 (cli_search.py)

파이프라인을 돌렸으면 이제 **검색이 작동하는지 직접 확인해 보겠습니다.** `ex04/src/cli_search.py`의 TODO를 채운 뒤 실행합니다.

```python
# cli_search.py — TODO를 채워주세요

def run_single_query(query, top_k, chroma_dir, collection_name, embedding_model_name):
    """search_chroma()로 검색 → 결과를 print_search_result()로 출력"""
    # TODO: results = search_chroma(query, ...) → 각 result 출력
    pass

def run_interactive_mode(top_k, chroma_dir, collection_name, embedding_model_name):
    """while 루프로 input() 반복 → run_single_query() 호출"""
    # TODO: while True: query = input() → 'quit'이면 break
    pass
```

ChromaDB에서 검색 결과를 가져와 유사도와 함께 표시하는 로직입니다.
...
### 2.10 [설명] extractor.py — 형식별 텍스트 추출
...
### 2.12 [설명] store.py — 임베딩 + ChromaDB 저장
...
`PersistentClient`가 핵심입니다. 메모리가 아닌 디스크(`data/chroma_db/`)에 저장하므로 프로그램을 종료해도 데이터가 남아 있게 됩니다. `hnsw:space: "cosine"`으로 코사인 유사도를 사용합니다.
