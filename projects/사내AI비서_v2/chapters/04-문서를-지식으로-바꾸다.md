...
파싱 결과가 깨끗한 걸 확인했으면 이제 **전체 파이프라인을 돌려보겠습니다.** `ex04/src/main.py`의 TODO를 채워 `extractor`, `chunker`, `store` 모듈을 조합합니다. 각 함수의 호출 순서와 인자는 앞에서 설명한 흐름(파싱 → 청킹 → 저장) 그대로입니다.
...
파이프라인을 돌렸으면 이제 **검색이 작동하는지 직접 확인해 보겠습니다.** `ex04/src/cli_search.py`의 TODO를 채운 뒤 실행합니다. ChromaDB에서 검색 결과를 가져와 유사도와 함께 표시하는 로직입니다.
