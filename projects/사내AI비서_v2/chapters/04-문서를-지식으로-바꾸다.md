...
파싱이 잘 되는 걸 확인했으니, 이제 핵심인 **청킹 코드**를 직접 작성합니다. `ex04/src/chunker.py`를 열면 아래 TODO가 보입니다.

```python
# chunker.py — 현재 상태 (rag-start)
...
아래 코드를 `pass` 자리에 작성합니다.

```python
# chunker.py — split_text_into_chunks 정답
...
이야기 파트에서 본 "500자 자르기 + 100자 오버랩" 그대로입니다. `step = chunk_size - overlap`이 핵심입니다. 500 - 100 = 400자씩 이동하니까, 앞 청크의 마지막 100자가 다음 청크의 처음 100자와 겹칩니다.


### 2.7 실습 3: 벡터DB 저장+검색 (store.py)

청킹이 되면 임베딩해서 ChromaDB에 넣어야 합니다. `ex04/src/store.py`를 열면 두 개의 TODO가 보입니다.

```python
# store.py — 현재 상태 (rag-start)
...
`store_chunks_to_chroma`가 이야기 파트의 "냉장고 정리"에 해당합니다. `embed_chunks()`로 768차원 벡터를 만들고, `collection.upsert()`로 ChromaDB에 넣습니다. 아래 코드를 각 `pass` 자리에 작성합니다.

```python
# store.py — store_chunks_to_chroma 정답
...
`search_chroma`는 검색 함수입니다. 쿼리를 임베딩하고 ChromaDB에서 유사한 청크를 찾아옵니다. 두 번째 `pass` 자리에 작성합니다.

```python
# store.py — search_chroma 정답
...
chunker와 store를 채웠으면 이제 **파이프라인을 조립**합니다. `ex04/src/main.py`를 열면 세 함수에 TODO가 있습니다.

```python
# main.py — 현재 상태 (rag-start)
...
각 `pass` 자리에 아래 코드를 작성합니다.

```python
# main.py — step1_python_parsing 정답
...
`main()` 함수에서는 step1과 step2를 순서대로 호출합니다. step2를 실행하려면 step1의 결과가 필요하니까, step1 결과가 없으면 자동으로 step1을 먼저 실행하는 로직도 넣어줍니다.
...
파이프라인을 돌렸으면 이제 **검색이 작동하는지 직접 확인해 보겠습니다.** `ex04/src/cli_search.py`를 열면 두 함수에 TODO가 있습니다.

```python
# cli_search.py — 현재 상태 (rag-start)
...
```python
# cli_search.py — run_single_query 정답
...
```python
# cli_search.py — run_interactive_mode 정답
