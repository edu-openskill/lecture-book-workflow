...
└── src/
    ├── main.py         [설명] 파이프라인 실행 진입점  ◀ 먼저 돌려보세요!
    ├── cli_search.py   [설명] 벡터 검색 CLI 도구     ◀ 검색 결과 확인!
...
실습은 코드를 실행하고 결과를 관찰하는 방식으로 진행합니다. `[설명]` 파일의 코드를 읽으면서 각 모듈이 어떤 역할을 하는지 이해하고, `main.py`와 `cli_search.py`를 실행하여 파이프라인 동작을 직접 확인합니다.
...
파싱 결과가 깨끗한 걸 확인했으면 이제 **전체 파이프라인을 돌려보겠습니다.** `ex04/src/main.py`는 위에서 살펴본 `extractor`, `chunker`, `store` 모듈을 조합하여 파싱 → 청킹 → 저장을 한 번에 실행합니다.
...
파이프라인을 돌렸으면 이제 **검색이 작동하는지 직접 확인해 보겠습니다.** `ex04/src/cli_search.py`는 ChromaDB에서 검색 결과를 가져와 유사도와 함께 표시합니다.
