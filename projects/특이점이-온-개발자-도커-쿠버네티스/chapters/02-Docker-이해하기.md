...
:::term-box
**하이퍼바이저 가상화**: 하이퍼바이저(가상화 소프트웨어)가 하드웨어 위에 OS를 통째로 따로 올려 독립된 컴퓨터처럼 쓰게 해 주는 방식입니다. 이 방식으로 만들어진 결과물을 **가상 머신(VM)** 이라 부릅니다. 격리가 완전한 대신 OS가 매번 통째로 뜨므로 무겁고 기동이 느립니다.
:::

:::term-box
**컨테이너 가상화**: 호스트 OS의 커널(운영체제의 핵심)을 여러 컨테이너가 공유하고, 파일시스템·네트워크·프로세스 공간만 컨테이너별로 분리하는 방식입니다. 이 방식으로 만들어진 결과물을 **컨테이너**라 부릅니다.
:::
...
:::term-box
**커널(Kernel)**: 운영체제(OS)의 핵심입니다. 프로세스 생성, 메모리 관리, 격리 공간 생성을 담당합니다. Docker의 모든 컨테이너는 호스트 PC의 **같은 리눅스 커널**을 공유합니다. 그 하나의 커널이 각 컨테이너에 격리 공간을 할당합니다.
:::
...
원리는 잡혔습니다. 이제 Docker를 설치하고 직접 띄워 봅니다.
...
Client와 Server 정보가 둘 다 뜨면 설치가 완료된 겁니다. 오픈이 화면에도 두 블록이 나란히 찍혔습니다.
