...
설치 로그가 마무리되며 Nginx 설치가 끝났습니다. 오픈이는 설정이 제대로 반영되었는지 확인하기 위해 현재 활성화된 포트를 점검해 보기로 합니다. `net-tools` 패키지를 추가로 설치한 뒤, 명령어를 입력해 Nginx가 네트워크상에서 정상적으로 대기 중인지 확인합니다.
...
반면 ubuntu는 **'사용자에게 명령어를 입력받는 것'** 이 목적입니다. 그래서 메인 프로세스인 **bash**는 사용자의 입력을 받는 터미널이 있어야 합니다. 하지만 -d 옵션처럼 입력해 줄 터미널이 연결되지 않은 상태라면, bash는 더 이상 할 일이 없다고 판단해 종료됩니다.
...
지금까지 오픈이는 남이 올려둔 이미지(ubuntu, nginx, tomcat)를 Docker Hub에서 받아 쓰기만 했습니다. 반대로, 직접 수정한 컨테이너를 이미지로 만들어 Docker Hub에 올릴 수도 있습니다. 그러면 다른 컴퓨터에서도 이 이미지 받으면 같은 상태로 쓸 수 있습니다.
...
컨테이너가 실행된 이후에 새로 쌓이는 데이터들은 이미지에 저장되지 않고, 컨테이너가 삭제되는 순간 함께 사라져 버립니다. 만약 데이터베이스처럼 실시간으로 데이터를 쌓아야 하는 컨테이너가 종료된다면, 그동안 저장했던 데이터들도 통째로 날아가게 됩니다.
...
먼저 해본 방식은 호스트 PC의 특정 폴더를 컨테이너 내부의 경로와 연결하는 **바인드 마운트**입니다. PC의 폴더가 USB라면, 컨테이너는 그 USB를 꽂아서 사용하는 방식입니다. 

양쪽이 같은 데이터를 공유하기 때문에 어느 한쪽에서 파일을 수정하면 반대쪽에서도 즉시 확인됩니다. 무엇보다 USB를 뽑는다고 데이터가 사라지지 않듯, 컨테이너를 삭제해도 파일의 실체는 호스트 PC에 안전하게 남습니다.
...
컨테이너 안에서 /app/bind/a.txt를 생성한 뒤, 호스트의 폴더를 확인해 보았습니다.
