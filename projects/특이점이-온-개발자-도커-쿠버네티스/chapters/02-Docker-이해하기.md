...
이렇게 하나의 주방을 여러 명이서 사용하는 것처럼 하나의 컴퓨터를 여러 대로 나눠서 사용하는 IT 기술을 **가상화(Virtualization)** 라고 부릅니다. 
...
구조의 핵심은 **호스트 OS의 커널 하나를 모든 컨테이너가 공유**한다는 점입니다. Docker 엔진은 "**새 격리 공간을 만들어 달라**"는 요청만 보냅니다. 실제로 프로세스를 격리하는 일은 **커널**이 합니다. 그래서 다른 컨테이너가 다른 앱과 다른 라이브러리를 가져도 충돌하지 않고 한쪽이 런타임을 업그레이드해도 옆 컨테이너는 영향을 받지 않습니다.
...
<svg viewBox="0 0 760 200" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Docker Hub와 이미지, 컨테이너 사이의 내려받기·올리기·실행·저장 전체 흐름">
  ...
  <line x1="158" y1="80" x2="341" y2="80" stroke="#475569" stroke-width="1.6" marker-end="url(#dk-p)"/>
  <text x="249" y="73" text-anchor="middle" font-size="11" font-weight="600" fill="#0f172a">내려받기</text>
  <line x1="341" y1="108" x2="158" y2="108" stroke="#475569" stroke-width="1.6" marker-end="url(#dk-p)"/>
  <text x="249" y="124" text-anchor="middle" font-size="11" font-weight="600" fill="#0f172a">올리기</text>
  <line x1="419" y1="80" x2="633" y2="80" stroke="#475569" stroke-width="1.6" marker-end="url(#dk-p)"/>
  <text x="526" y="73" text-anchor="middle" font-size="11" font-weight="600" fill="#0f172a">실행</text>
  <line x1="633" y1="108" x2="419" y2="108" stroke="#475569" stroke-width="1.6" marker-end="url(#dk-p)"/>
...
이미지는 한 덩어리가 아니라 여러 **레이어**가 층층이 쌓인 구조입니다. 아래쪽에는 OS 기본 라이브러리나 런타임처럼 잘 안 바뀌는 부분이 있고, 위쪽에는 애플리케이션 코드처럼 자주 바뀌는 부분이 있습니다. 한 번 받은 아래 레이어는 저장해 두고 다시 쓰기 때문에, 코드만 바뀐 새 버전이 나오면 맨 위 레이어만 새로 받으면 됩니다.
:::
...
*그림 2-12. 컨테이너 통신 한눈에 보기 — 외부 진입과 컨테이너끼리 IP 통신*
...
서버 설치를 마쳤더라도 운영하다 보면 설정 파일을 수정해야 할 일이 생기기 마련입니다. 리눅스에서 파일을 편집하려면 전용 편집기가 필요한데, 오픈이는 **Vim**을 선택해 설치했습니다.
...
apt install vim 도중 Geographic area와 Time zone을 선택하는 화면이 나옵니다. 이때 **Asia**, **Seoul**을 각각 선택합니다. 이 설정은 컨테이너 안의 시간 표시와 로그 시각 기준이 되어, 한국 시간으로 동작하게 만들어 줍니다.
:::
...
오픈이는 감을 잡기 위해 간단한 파일을 하나 만들어봤습니다.
...
컨테이너에서 빠져나올 때는 `exit`입니다. 그런데 `exit`를 치고 `docker ps`를 쳐보니, 방금까지 돌던 ubuntu 컨테이너가 목록에서 사라져 있었습니다.
...
오픈이는 같은 상황을 한 번 더 만들어 비교했습니다. 이름을 붙여 띄운 뒤 `exit`로 나와봤습니다.
...
이번엔 `exit` 말고 다른 방식을 찾아봤습니다. `CTRL+P → CTRL+Q`를 쓰면 컨테이너는 그대로 두고 쉘만 빠져나올 수 있습니다. 이름을 붙여 한 번 더 실행한 뒤 이 조합으로 빠져나왔습니다.
...
이번에는 ubuntu의 메인 프로세스를 유지하기 위해 메인 프로세스를 바꾸는 테스트를 해보기로 했습니다. bash 대신 **sleep 1000** 을 메인으로 지정했습니다.
...
이유는 Docker Hub에서 내려받은 Tomcat 이미지에는 `webapps/ROOT/index.html` 파일이 없기 때문입니다.
...
vim 패키지로 index.html 생성 및 저장을 마치고 **localhost:8080** 로 다시 접속했더니 방금 만든 페이지가 응답되었습니다.
...
업로드가 완료된 뒤 Docker Hub의 Repositories 탭을 열어보니 방금 올린 이미지를 확인할 수 있습니다.
...
Docker Hub에 이미지를 올리고 한숨 돌리려던 오픈이의 머릿속에 문득 한 가지 걱정이 스쳤습니다. 이미지에 담긴 것은 컨테이너의 초기 상태뿐이기 때문입니다.

컨테이너가 실행된 뒤 새로 쌓이는 데이터는 이미지에 저장되지 않습니다. 컨테이너가 삭제되는 순간 함께 사라져 버립니다. 데이터베이스처럼 실시간으로 데이터를 쌓는 컨테이너가 삭제되면, 그동안 저장한 데이터도 통째로 날아갑니다.

*'컨테이너를 내려도 데이터는 남아 있어야 하는데.'*

결국 데이터를 잃지 않으려면 컨테이너 외부에 보관해야 합니다. 이를 위해 컨테이너와 외부 저장소를 연결해주는 기능이 바로 **마운트**입니다.

Docker의 마운트는 두 가지 방식이 있습니다.

### 2.9.1 바인드 마운트: 호스트 폴더와 직접 연결

먼저 해본 방식은 호스트 PC의 특정 폴더를 컨테이너 내부의 경로와 연결하는 **바인드 마운트**입니다. USB를 컴퓨터에 꽂으면 그 안의 파일을 그대로 볼 수 있듯, 컨테이너도 호스트 PC의 폴더를 꽂아서 쓰는 방식입니다. 
...
볼륨 마운트가 그 역할을 합니다. Docker 엔진이 자기 안에 별도 저장 공간을 두고, 사용자는 볼륨에 이름만 붙여 컨테이너와 연결합니다. 실제 저장 위치는 Docker가 맡아서 관리합니다.
...
방금 전에 만들었던 `b.txt`가 새 컨테이너 안에서도 그대로 보였습니다. 컨테이너를 내려도 데이터는 사라지지 않았습니다. 처음 던졌던 걱정이 풀리는 순간이었습니다.
...
노트를 훑어보고 노트북 뚜껑을 닫았습니다. 책상 위엔 식은 커피 잔이 남아 있었습니다.
