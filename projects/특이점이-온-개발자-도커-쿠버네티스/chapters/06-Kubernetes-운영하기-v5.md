...
**팀장**: "구성 잘 잡았네요. 그럼 챕터 3에서 docker-compose로 짰던 그 통합 사이트, 이번엔 쿠버네티스 위에 올려 볼래요?"

오픈이는 펜을 든 채 잠시 멈칫했습니다. Pod 하나, Service 하나는 깔끔하게 띄울 수 있습니다. 그런데 프론트엔드·백엔드·DB가 한 덩어리로 묶인 사이트를 그대로 옮기려고 하니, 어디부터 손대야 할지 막막했습니다. docker-compose로 띄울 때 짚지 않고 지나갔던 두 가지가 한꺼번에 도드라졌습니다.

먼저 DB 비밀번호가 마음에 걸렸습니다. 챕터 3에서 MySQL 컨테이너를 띄울 때 Dockerfile의 ENV에 비밀번호를 적었더니 빌드 도중 *민감한 정보는 ENV에 넣지 말라*는 경고가 떴던 게 떠올랐습니다. 그때는 학습용이라 그대로 뒀지만, 운영에서는 이대로 갈 수 없습니다. 이미지에 박아 두면 환경이 바뀔 때마다 이미지를 다시 빌드해야 하고, 그렇다고 운영 YAML에 환경 변수로 옮겨 적으면 비밀번호가 Git 저장소에 그대로 올라갑니다. DB 주소나 접속 URL처럼 환경마다 달라지는 값도 마찬가지입니다.
...
다음 날 아침, 오픈이는 자리에 앉자마자 노트를 펼쳤습니다. 며칠 전 회의실에서 팀장이 의뢰한 챕터 3 그 사이트를 K8s 위에 올릴 차례였습니다. 매듭은 풀렸고, 도구는 쥐고 있었습니다. 이번 챕터에서는 그동안 따로따로 익힌 도구들이 한 사이트에서 한꺼번에 만납니다. 챕터 4·5에서 본 Pod·Deployment·Service·Ingress에, 이번 챕터에서 새로 더한 ConfigMap·Secret과 PV·PVC가 합쳐집니다.
