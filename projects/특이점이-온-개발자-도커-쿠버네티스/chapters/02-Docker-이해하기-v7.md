...
원리를 짚었으니 이제 Docker를 설치하고 직접 띄워 보겠습니다.
...
익숙한 nginx 환영 페이지가 떴습니다. 컨테이너를 띄울 때 `-p 80:80`으로 포트를 열어 둔 덕에, 컨테이너 안의 nginx와 브라우저가 연결된 것입니다.
...
`exit` 말고 다른 방식이 있는지 찾아봤습니다. `CTRL+P → CTRL+Q`를 쓰면 컨테이너는 그대로 두고 쉘만 빠져나올 수 있습니다. 같은 방식으로 한 번 더 띄우고 이 조합으로 빠져나왔습니다.
