...
*그림 2-4. 컨테이너 가상화의 전체 구조 (Linux 호스트 기준. Windows·macOS는 2.3에서 다룸)*
...
이 구조의 핵심은 **호스트 OS의 커널 하나를 모든 컨테이너가 공유**한다는 점입니다. VM처럼 OS를 통째로 따로 올리지 않으니 가볍고 빠르게 뜹니다. Docker 엔진이 그 커널에 "**새 격리 공간을 만들어 달라**"고 요청하면, 실제로 프로세스를 격리하는 일은 **커널**이 합니다. 그래서 컨테이너마다 다른 앱과 다른 라이브러리를 가져도 충돌하지 않고, 한쪽이 런타임을 업그레이드해도 옆 컨테이너는 영향을 받지 않습니다.
...
이미지는 붕어빵 틀에 가깝습니다. 틀(이미지) 하나로 붕어빵(컨테이너)을 여러 개 찍어낼 수 있고, 틀 자체는 변하지 않습니다. 그래서 같은 이미지에서 나온 컨테이너는 어디서 띄우든 똑같은 환경을 가집니다. 이 덕분에 "**내 PC와 서버 환경이 같다**"가 가능합니다. 로컬에서 만든 이미지를 서버에서 그대로 꺼내 실행하면 서버의 런타임 버전이 무엇이든 컨테이너는 자기 이미지 안의 런타임으로 돕니다.
...
이미지는 한 덩어리가 아니라 여러 **레이어**가 층층이 쌓여 있습니다. 아래쪽에는 OS 기본 라이브러리나 런타임처럼 잘 안 바뀌는 부분이 있고, 위쪽에는 애플리케이션 코드처럼 자주 바뀌는 부분이 있습니다. 한 번 받은 아래 레이어는 저장해 두고 다시 쓰기 때문에, 코드만 바뀐 새 버전이 나오면 맨 위 레이어만 새로 받으면 됩니다. 여기에 커널은 호스트와 공유하므로 이미지에 담지 않습니다. 이런 구조 덕에 도커 이미지는 OS 전체를 담는 VM 이미지에 비해 훨씬 가볍습니다.
:::
...
```bash [터미널] Docker 버전 확인
docker version   # Docker 버전 확인
```

Client와 Server 정보가 둘 다 뜨면 설치가 완료된 것입니다.
...
오픈이는 컨테이너 동작 확인을 위해 브라우저에 `localhost`를 입력했습니다. 화면은 뜨지 않았습니다.

*'왜 접속이 안 되지.'*

## 2.5 컨테이너 통신

왜 접속이 안 되는지 알려면 컨테이너가 외부와 어떻게 닿는지부터 짚어야 했습니다.

### 2.5.1 네트워크 한눈에 보기

오픈이가 자료를 펼치니 컨테이너 네트워크가 크게 세 가지로 나뉘어 있었습니다. **컨테이너끼리 연결하는 방법**, **외부 요청이 컨테이너로 들어오는 길**, **이름으로 컨테이너를 찾는 방법**입니다.

주방 비유로 가상화를 잡았던 것처럼, 이번에는 식당 여러 개가 모인 **푸드코트**로 그려 봤습니다.

### 2.5.2 컨테이너끼리 - 주방 모니터와 주문 전광판

A, B, C 식당이 푸드코트에 입점해 있습니다. 식당끼리는 벽으로 완전히 분리되어 있어 서로의 주방을 들여다볼 수 없습니다.

각 식당의 주방에는 **주방 모니터**가 한 대씩 있고, 푸드코트 한가운데에는 모두가 보는 **주문 전광판**이 걸려 있습니다. 주방 모니터와 전광판은 한 가닥의 **전용 통신 케이블**로 이어져 있고, 모든 식당의 케이블이 전광판 본체에 꽂혀 있습니다. 식당끼리 벽이 막혀 있어도 이 전광판을 거치면 서로의 주문 현황을 알 수 있습니다.
...
*그림 2-13. 주방 모니터·전용 통신 케이블·주문 전광판이 컨테이너 네트워크의 비유*
...
*'각자 격리되어 있는데 케이블 한 가닥으로 다 통하네. 그럼 푸드코트 바깥의 손님은 어떻게 주문하지.'*

다음으로 알아본 건 외부 손님이 식당에 어떻게 주문을 넣느냐였습니다.

### 2.5.3 외부에서 들어오기 - 입구 키오스크

docker0는 도커가 호스트에 기본으로 만들어 두는 **가상 스위치**입니다. 이 위에서 동작하는 네트워크를 **기본 네트워크**라고 부릅니다. 공유기가 집안 기기를 한 네트워크로 묶어 주듯 docker0도 같은 호스트의 컨테이너들을 이 기본 네트워크로 묶어 둡니다.

그래서 `docker run`으로 컨테이너를 띄우면 별도 옵션을 지정하지 않는 한 자동으로 이 기본 네트워크에 들어갑니다. 같은 네트워크 안에 있으니 IP만 알면 서로 통신할 수 있습니다.

문제는 공유기 너머, 즉 호스트 PC 바깥에 있는 외부 사용자입니다. 브라우저처럼 외부에서 컨테이너로 바로 접근하기는 어렵습니다. 이유는 두 가지입니다.

- Windows·macOS에서는 Docker가 가상 리눅스 안에서 돌기 때문에 docker0와 컨테이너 IP가 호스트 터미널에서는 보이지 않습니다.
...
푸드코트에 비유하면 외부 손님은 주방에 직접 들어가지 못합니다. 대신 입구의 **키오스크**에 주문을 넣으면 키오스크가 해당 식당으로 그 주문을 넘겨줍니다. 짜장면은 A 식당으로, 돈까스는 B 식당으로 주문이 전달됩니다. 어떤 메뉴를 누르느냐가 곧 어느 식당으로 갈지를 정하는 분기 기준입니다.
...
`docker run -p 8080:80`을 실행하면 Docker는 iptables에 "**호스트의 8080 포트로 들어온 요청을 컨테이너의 80 포트로 전달**"이라는 규칙을 한 줄 심어 둡니다. 이제 브라우저가 `localhost:8080`을 호출하면 iptables가 이 규칙대로 요청의 도착지를 컨테이너 80 포트로 바꿔치기하고 docker0를 거쳐 해당 컨테이너로 전달합니다. 사용자는 이 변환을 모르고 "**localhost에 접속했더니 컨테이너 안의 페이지가 나왔네**" 정도로만 경험합니다.
...
*그림 2-16. 호스트 포트가 컨테이너 포트로 연결되는 구조*

*'외부에서 들어올 땐 결국 호스트 포트로만 들어오는구나. iptables가 그 안에서 컨테이너 포트로 길을 바꿔주고, 컨테이너 IP는 외부에 노출할 필요가 없네.'*

### 2.5.4 이름으로 찾기 - 홀의 안내 지도

컨테이너끼리 통신하려면 결국 상대 IP를 알아야 합니다. 한두 개라면 IP를 적어 두고 쓸 수 있지만, 수가 늘어나면 일일이 외우는 게 번거로워집니다.
...
새 컨테이너가 뜰 때마다 이름과 IP가 자동으로 Docker DNS에 등록됩니다. 그다음 `app` 컨테이너가 `db`라는 이름으로 요청을 보내면 Docker DNS가 먼저 받아 `db`를 `172.17.0.3`으로 변환해 알려 주고, app은 그 IP로 다시 요청을 보냅니다. 컨테이너가 다시 떠서 IP가 바뀌어도 이름은 그대로라 코드를 수정할 필요가 없습니다.
...
*그림 2-18. Docker DNS가 이름을 IP로 변환 (사용자 정의 네트워크 한정)*

다만 이 자동 DNS는 **사용자 정의 네트워크(User-defined Network)** 안에서만 켜집니다. 지금까지 본 기본 네트워크(docker0)에서는 작동하지 않습니다. 그래서 사용자가 직접 네트워크를 만들어 컨테이너를 묶어 둬야 이름으로 통신할 수 있습니다. 이 차이는 다음 챕터에서 직접 만들어 보며 확인합니다.

*'naver.com처럼 컨테이너도 이름으로 부르면 되는구나. IP는 일일이 외울 필요 없겠네.'*

오픈이는 여기까지 컨테이너의 통신을 정리했습니다. 이번엔 컨테이너 안에 들어가 어떻게 생겼는지 직접 보기로 했습니다.
...
프롬프트가 `root@...#` 모양으로 바뀌었습니다. 방금까지는 호스트의 터미널이었는데, 이제는 컨테이너 안의 쉘입니다. 호스트와는 분리된 공간에 들어와 있다는 감각이 손끝에 옵니다.
...
<table>
  ...
  <thead>
    <tr><th>용도</th><th>명령</th><th>설명</th></tr>
  </thead>
  <tbody>
    <tr><td style="text-align:center">위치 확인</td><td><code>pwd</code></td><td>현재 위치</td></tr>
    <tr><td style="text-align:center">이동</td><td><code>cd &lt;경로&gt;</code></td><td>폴더 이동</td></tr>
    <tr><td style="text-align:center">목록</td><td><code>ls</code>, <code>ls -la</code></td><td>파일/폴더 목록 (숨김 포함)</td></tr>
    <tr><td style="text-align:center">폴더 생성</td><td><code>mkdir &lt;이름&gt;</code></td><td>폴더 만들기</td></tr>
    <tr><td style="text-align:center">파일 생성</td><td><code>touch &lt;이름&gt;</code></td><td>빈 파일</td></tr>
    <tr><td style="text-align:center">복사</td><td><code>cp &lt;원본&gt; &lt;사본&gt;</code></td><td>파일 복사</td></tr>
    <tr><td style="text-align:center">이동/이름변경</td><td><code>mv &lt;원본&gt; &lt;대상&gt;</code></td><td>이동 또는 rename</td></tr>
    <tr><td style="text-align:center">삭제</td><td><code>rm &lt;파일&gt;</code>, <code>rm -r &lt;폴더&gt;</code></td><td>삭제</td></tr>
    <tr><td style="text-align:center">패키지 설치</td><td style="white-space:nowrap"><code>apt update &amp;&amp; apt install -y &lt;이름&gt;</code></td><td>패키지 설치</td></tr>
    <tr><td style="text-align:center">프로세스</td><td><code>ps -ef</code>, <code>kill &lt;PID&gt;</code></td><td>실행 중 프로세스 / 종료</td></tr>
    <tr><td style="text-align:center">검색</td><td><code>find &lt;경로&gt; -name &lt;이름&gt;</code></td><td>파일 검색</td></tr>
    <tr><td style="text-align:center">로그</td><td><code>tail -n &lt;수&gt; &lt;파일&gt;</code></td><td>마지막 N줄</td></tr>
  </tbody>
</table>
...
기본 명령어를 손에 익힌 오픈이는, 앞서 도커 이미지로 받아 실행했던 nginx를 이번에는 리눅스 환경에 직접 설치해 보기로 했습니다.

컨테이너 안은 최소 구성이라 필요한 패키지는 직접 설치해야 합니다. 패키지 목록을 갱신한 뒤 nginx를 설치하고 실행했습니다.
...
설치 로그가 마무리되며 nginx 설치가 끝났습니다. 설정이 제대로 반영됐는지 확인하기 위해 현재 활성화된 포트를 점검해 보기로 했습니다. `net-tools` 패키지를 추가로 설치한 뒤, 명령어를 입력해 nginx가 네트워크상에서 정상적으로 대기 중인지 확인했습니다.
...
방금 쓴 문장이 그대로 찍혀 나왔습니다.

빈 파일은 만들고 봤으니, 이번엔 컨테이너에 깔려 있는 nginx 기본 페이지를 `find`로 찾아봤습니다.

```bash [터미널] 컨테이너 내부 - index.html 위치 검색
...
내부 구조는 익혔지만 **언제 살아 있고 언제 죽는지**는 아직 감이 없다는 뜻이었습니다. 컨테이너가 뜻밖에 꺼지면 곤란하니, 이 규칙부터 정리해 보기로 했습니다.
...
오픈이는 nginx 때 봤던 `-d` 옵션을 사용해 ubuntu를 백그라운드로 실행해 보기로 했습니다.
...
실행 중인 프로세스 목록에 ubuntu가 없었습니다. 컨테이너가 실행되자마자 바로 종료됐기 때문입니다. 반면 이전에 실습했던 nginx는 똑같은 -d 옵션으로도 잘 돌아갔습니다.

원인을 찾다 **메인 프로세스**의 개념을 알게 되었습니다. 도커 컨테이너는 시작과 동시에 실행되는 메인 프로세스가 종료되면 컨테이너도 함께 종료됩니다. 이 관점에서 두 이미지를 비교해 보니 차이가 분명했습니다.

먼저 nginx는 **'웹 서버로서 요청을 대기하는 것'** 이 목적입니다. 그래서 메인 프로세스인 **nginx**는 요청이 올 때까지 계속 깨어 있는 데몬으로 동작합니다. 백그라운드에서도 '대기'라는 할 일이 분명히 있기 때문에 프로세스가 죽지 않고 컨테이너가 계속 살아 있습니다.
...
ubuntu를 백그라운드로 살리려면 `-dit`(`-d` 백그라운드, `-i` 입력 받기, `-t` 터미널 화면)을 쓰거나 메인 프로세스를 다른 것으로 바꿔야 합니다.
...
오픈이는 백그라운드로 띄운 컨테이너에 다시 들어가는 방법이 궁금해졌습니다. 자료를 살펴보니 두 가지가 있었습니다. 메인 프로세스에 직접 연결하는 `attach`와, 새 프로세스를 하나 더 띄우는 `exec`입니다. 결과는 비슷해 보이지만 내부에서 일어나는 일이 다르다고 해서, 두 방법을 직접 비교해 봤습니다.

먼저 `attach`는 컨테이너 안에서 이미 돌고 있는 메인 프로세스를 연결합니다. 같은 프로세스를 공유하기 때문에 여기서 잘못 빠져나와 프로세스를 종료하면 컨테이너 전체가 같이 꺼집니다.
...
컨테이너의 메인 프로세스는 PID 1로 식별됩니다.

먼저 ubuntu를 백그라운드로 띄우고, 그 컨테이너에 `docker attach`로 접속했습니다.
...
ubuntu의 입력 터미널이 나타났습니다. `attach`는 **메인 프로세스**에 직접 연결되므로, 여러 터미널에서 붙어도 모두 같은 화면을 공유합니다. 다만 이 상태에서 잘못 빠져나와 메인 프로세스를 종료하면 컨테이너 전체가 꺼집니다. 그래서 `CTRL+P → CTRL+Q`로 안전하게 빠져나왔습니다.

이번에는 같은 컨테이너에 `exec`로 다시 들어가 봤습니다.
...
`exec`가 정말 새 프로세스를 만드는지 눈으로 확인하고 싶어서, 호스트에서 터미널을 하나 더 열고 컨테이너 내부의 프로세스 목록을 찍어 봤습니다.
...
*'attach는 메인 프로세스에 직접 붙는 방식이고, exec는 새 프로세스를 만들어 접근하는 방식이구나. 앞으로는 exec만 써야겠다.'*

여기까지 공부하고 나니 컨테이너의 생명 주기는 더는 헷갈리지 않았습니다. 오픈이는 의자에서 일어나 잠깐 어깨를 풀고 다시 노트북 앞에 앉았습니다. 그러다 한 가지가 마음에 걸렸습니다. 앞서 Ubuntu 컨테이너 안에 패키지를 깔고 파일도 몇 개 만들어 두었습니다. 컨테이너를 내리는 순간 그 작업이 다 사라진다면 다음에 똑같은 걸 또 깔아야 합니다.

*'내가 수정한 이 상태를 기록해 두려면 어떻게 해야 하지.'*

필요한 건 **지금 컨테이너 상태를 그대로 기록해 두는 방법**입니다.

## 2.8 commit - 실행 중인 컨테이너를 이미지로

그 일을 하는 명령이 `docker commit`입니다. 이 명령어는 컨테이너의 현재 상태를 그대로 이미지로 저장합니다.
...
자기만의 이미지를 만들기 위해 Tomcat 이미지를 받아 컨테이너를 실행했습니다.
...
:::term-box
**레포지토리(Repository)와 태그(Tag)**: `docker images` 결과의 `REPOSITORY` 칸은 이미지 이름(`coderyu5523/tomcat` 같은 사용자명/이미지명 형식), `TAG` 칸은 같은 이미지의 버전 라벨입니다. 태그를 명시하지 않으면 자동으로 `latest`가 붙습니다. 같은 레포지토리에 여러 태그를 두어 버전을 구분할 수 있습니다(예: `myapp:v1`, `myapp:v2`).
:::
...
한숨 돌리고 계속 공부하던 오픈이는 한 가지를 발견했습니다. 방금 commit으로 만든 이미지에 담기는 건 컨테이너의 초기 상태뿐이라는 점입니다.

컨테이너가 실행된 뒤 새로 쌓이는 데이터는 이미지에 저장되지 않습니다. 그래서 컨테이너가 삭제되는 순간 함께 사라져 버립니다. 데이터베이스처럼 실시간으로 데이터를 쌓는 컨테이너가 삭제되면, 그동안 저장한 데이터도 통째로 날아갑니다.
...
먼저 해 본 방식은 호스트 PC의 특정 폴더를 컨테이너 내부의 경로와 연결하는 **바인드 마운트**입니다. USB를 컴퓨터에 꽂으면 그 안의 파일을 그대로 볼 수 있듯, 컨테이너도 호스트 PC의 폴더를 꽂아서 쓰는 방식입니다.

한쪽에서 파일을 수정하면 다른 쪽에서도 즉시 확인됩니다. USB를 뽑아도 안에 든 파일이 사라지지 않듯, 컨테이너를 삭제해도 파일은 호스트 PC에 그대로 남습니다.

:::term-box
**바인드 마운트(Bind Mount)**: 호스트 PC의 특정 폴더를 컨테이너 안 경로와 직접 연결하는 마운트 방식입니다. 사용자가 지정한 폴더를 컨테이너가 그대로 빌려 쓰기 때문에, 한쪽의 변경 사항이 실시간으로 양쪽에 반영됩니다. 어느 폴더를 쓸지는 사용자가 직접 지정합니다.

`docker run -it --mount type=bind,src=<호스트 경로>,dst=<컨테이너 경로> <이미지>` 형태로 명령어를 작성합니다.
:::
...
<svg viewBox="0 0 760 240" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="바인드 마운트 — 호스트 PC의 사용자 지정 폴더와 컨테이너 안 경로를 직접 연결한 구조">
  ...
  <text x="160" y="110" text-anchor="middle" font-size="11" font-weight="600" fill="#475569">사용자 지정 경로</text>
  <rect x="80" y="125" width="160" height="22" rx="3" fill="#fff" stroke="#475569" stroke-width="1.2"/>
  <text x="160" y="140" text-anchor="middle" font-size="11" font-family="monospace" fill="#0f172a">/c/app/bind</text>
  <rect x="100" y="155" width="120" height="22" rx="3" fill="#fff" stroke="#475569" stroke-width="1.2"/>
  <text x="160" y="170" text-anchor="middle" font-size="11" font-family="monospace" fill="#0f172a">a.txt</text>
...
<tbody>
<tr><td>폴더 생성</td><td><code>mkdir -p ~/app/bind</code></td></tr>
<tr><td>마운트 실행</td><td><code>docker run -it --mount type=bind,src=$HOME/app/bind,dst=/app/bind ubuntu</code></td></tr>
<tr><td>확인</td><td><code>ls ~/app/bind</code></td></tr>
</tbody>
</table>
...
같은 파일이 양쪽에 있었습니다. 호스트에서 소스 코드를 고치면 컨테이너에 바로 반영되니까, 개발할 때 쓰기 딱 맞는 방식입니다.

### 2.9.2 볼륨 마운트: Docker가 관리하는 저장소

바인드 마운트를 써 보니 신경 쓰이는 점이 하나 있었습니다. 호스트의 어느 폴더를 쓸지, 경로를 어떻게 정리해 둘지 사용자가 직접 관리해야 한다는 점입니다. 이 일을 Docker에 맡길 수는 없을까.
...
<svg viewBox="0 0 760 240" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="볼륨 마운트 — Docker가 관리하는 호스트 PC 안 저장 공간을 컨테이너에 연결한 구조">
  ...
  <text x="160" y="110" text-anchor="middle" font-size="11" font-weight="700" fill="#7b341e">Docker 내부 저장소</text>
  <text x="160" y="140" text-anchor="middle" font-size="11" font-family="monospace" fill="#7b341e">/var/lib/docker/volumes/</text>
  <rect x="70" y="155" width="180" height="24" rx="3" fill="#fff" stroke="#ff7849" stroke-width="1.2"/>
  <text x="160" y="171" text-anchor="middle" font-size="11" font-family="monospace" fill="#7b341e">metacoding-volume</text>
  <rect x="460" y="50" width="280" height="160" rx="8" fill="#fff" stroke="#ff7849" stroke-width="1.4" stroke-dasharray="6,4"/>
  <text x="474" y="70" font-size="11" font-weight="700" fill="#7b341e">컨테이너 (Ubuntu)</text>
  <rect x="490" y="90" width="220" height="100" rx="6" fill="#fff" stroke="#fdba74" stroke-width="1.2" stroke-dasharray="4,3"/>
  <text x="600" y="110" text-anchor="middle" font-size="11" font-weight="600" fill="#7b341e">컨테이너 내부 경로</text>
...
볼륨 마운트가 그 일을 합니다. Docker 엔진이 자기 안에 별도 저장 공간을 두고, 사용자는 볼륨에 이름만 붙여 컨테이너와 연결합니다. 그래서 실제 저장 위치는 Docker가 맡아서 관리합니다.

:::term-box
**볼륨 마운트(Volume Mount)**: Docker 엔진이 자체적으로 관리하는 저장 공간(볼륨)을 컨테이너에 연결하는 마운트 방식입니다. 사용자는 볼륨 이름만 정하면 되고, 실제 저장 위치는 Docker가 알아서 관리합니다. 컨테이너가 사라져도 볼륨에 담긴 데이터는 그대로 남아 있어, 다른 컨테이너에서 같은 이름으로 다시 연결할 수 있습니다.

`docker run -it --mount type=volume,src=<볼륨 이름>,dst=<컨테이너 경로> <이미지>` 형태로 명령어를 작성합니다.
:::
...
오픈이는 하루 사이에 익힌 키워드를 정리했습니다. 격리, 이미지, 포트포워딩, CMD, 볼륨이 머릿속에 정리됐습니다. 어제 두 시간을 흘려보냈던 그 에러도 이제는 어디에서 막혔던 건지 짚을 수 있을 것 같았습니다.
...
식은 커피 잔을 옆으로 밀어두고 노트북 뚜껑을 닫았습니다. 창밖이 어느새 어두워져 있었습니다.
