...
:::term-box
**이미지(Image)**: 컨테이너를 만드는 데 필요한 파일시스템과 실행 정보가 한 덩어리로 묶인 읽기 전용 패키지입니다. 애플리케이션 코드, 라이브러리, 설정, 실행 명령이 함께 들어 있어 어디서 띄우든 동일한 환경이 만들어집니다.
:::
...
이미지는 한 덩어리가 아니라 여러 레이어가 층층이 쌓인 구조입니다. 아래쪽에는 OS 기본 라이브러리나 런타임처럼 잘 안 바뀌는 부분이 있고, 위쪽에는 애플리케이션 코드처럼 자주 바뀌는 부분이 있습니다. 한 번 받은 아래 레이어는 저장해 두고 다시 쓰기 때문에, 코드만 바뀐 새 버전이 나오면 맨 위 레이어만 새로 받으면 됩니다.
:::
...
익숙한 nginx 환영 페이지가 떴습니다. 컨테이너를 실행할 때 `-p 80:80`으로 포트를 열어 둔 덕에, 컨테이너 안의 nginx와 브라우저가 연결된 것입니다.
...
먼저 nginx는 **'웹 서버로서 요청을 대기하는 것'** 이 목적입니다. 그래서 메인 프로세스인 **nginx**는 요청이 올 때까지 계속 깨어 있는 데몬으로 동작합니다. 백그라운드에서도 '대기'라는 할 일이 명확히 있기 때문에 프로세스가 죽지 않고 컨테이너를 계속 살려두는 것입니다.
...
다음 챕터에서는 한 번에 컨테이너 한 대가 아니라, 프론트엔드·백엔드·DB가 함께 뜨는 실제 서비스 구성을 다룹니다. **Dockerfile**로 환경을 자동으로 만들고 **Docker Compose**로 여러 컨테이너를 한 번에 묶어 올려 보겠습니다.
:::
