...
*'도커 때 Nginx 한 대를 앞에 세웠던 것처럼, 변하지 않는 문 하나가 있어야 하는데.'*
...
*그림 5-12. Ingress 리소스(YAML)와 Ingress Controller(Pod) — 둘 다 있어야 라우팅이 작동합니다*
...
우편 위 **받는 곳 주소**는 사람이 눈으로 읽는 위치 정보입니다. **송장**은 우체국이 시스템에 등록한 뒤 출력해 붙이는 바코드 라벨입니다. 분류 라인·도착 우체국 코드 같은 시스템적 분류 정보가 구조화되어 있어 스캐너로만 읽힙니다. 두 정보는 같은 우편 위에 있지만 누가 읽는지와 들이는 손이 다릅니다.
...
네트워크 패킷도 이 두 정보를 닮았습니다. **Service**는 *받는 곳 주소*에 해당하는 IP·포트만 보고 일합니다. 모든 노드 커널의 iptables 규칙으로 분류·전달합니다. **Ingress**는 한 단계 더 들어가, *송장 바코드*에 해당하는 도메인·URL 경로까지 읽고 어느 Service로 보낼지 결정합니다. 별도 Pod 안 HTTP 서버가 이 일을 합니다.
...
:::term-box
**kube-proxy**: 모든 노드(서버)에 항상 떠 있는 프로그램입니다. 들어온 요청의 받는 주소가 살아있는 Pod의 주소로 바뀌도록 iptables 규칙을 미리 깔아 두고 갱신합니다.
:::
...
발신자가 우편을 들고 동네 **우체국** 입구로 들어갑니다. 우편 봉투에는 동네 우체국으로 가는 외부 주소가 적혀 있습니다. 안쪽 **창구 직원**이 그 외부 주소를 가리고 그 위에 새 **송장**을 붙입니다. 새 송장 바코드에는 우편이 **다음으로 갈 물류 센터의 라우팅 코드**가 들어 있습니다. 우편에 적힌 받는 사람·주소는 그대로 따라가며 최종 목적지를 알려 줍니다.
...
| 우체국 입구 | **NodePort** | 각 노드의 공개 포트 |
| 창구 직원 | **kube-proxy (1차)** | iptables 규칙을 미리 깔아 두고, 그 규칙으로 받는 곳을 바꿔 다음 단계로 전달 |
...
*그림 5-27. kube-proxy(2차)가 미리 받아 둔 Pod 목록에서 살아있는 Pod IP를 골라 변환해 Pod로 전달*
