...
Service YAML을 처음부터 끝까지 다시 훑어봐도 경로별 라우팅 항목은 보이지 않았습니다. Service는 라벨로 Pod를 묶을 줄만 알지, URL 경로를 보고 길을 가르는 일은 하지 않습니다.
...
도커에서 NGINX를 택배 분류 센터로 풀었습니다(CH03 3.2.2). K8s에서는 그 흐름이 더 잘게 쪼개져 들어옵니다. 본사·가맹점 비유로는 조직의 위계가 잘 보였지만, 요청이 어디로 어떻게 흘러가는지 따라가려면 우편의 흐름이 더 가깝습니다. 이 챕터 안에서는 사람의 통신(전화)에서 패킷의 통신(우편)으로 관점을 옮겨 보겠습니다.
...
오픈이는 두 리소스가 같은 트래픽을 다루는데 보는 정보가 다르다는 점이 궁금했습니다. 책상 위 택배 송장 한 장이 떠올랐습니다.

송장에는 두 가지 정보가 함께 있습니다. 위쪽엔 사람이 눈으로 읽는 **받는 곳 주소**(시·동·번지), 아래쪽엔 스캐너로 읽는 **바코드**가 있습니다. 받는 곳 주소는 단순한 위치 정보입니다. 바코드에는 더 자세한 분류 정보(분류 라인·도착 우체국 코드 등)가 인코딩되어 있어 스캐너가 있어야 읽힙니다. 같은 송장 위에 있지만 읽는 방식과 들이는 손이 다릅니다.
...
*그림 5-19. 우편 위 송장에 받는 곳 주소와 바코드가 함께 있고, Service는 받는 곳 주소만, Ingress는 바코드까지 읽습니다*

네트워크 패킷도 이 두 정보를 닮았습니다. **Service**는 송장의 *받는 곳 주소*에 해당하는 IP·포트만 보고 일합니다. 모든 노드 커널의 iptables 규칙으로 분류·전달합니다. **Ingress**는 한 단계 더 들어가, *바코드*에 해당하는 도메인·URL 경로까지 읽고 어느 Service로 보낼지 결정합니다. 별도 Pod 안 HTTP 서버가 이 일을 합니다.
...
*그림 5-20. 실제 패킷에 TCP/IP 헤더와 HTTP 페이로드가 함께 들어 있고, Service는 헤더만, Ingress는 페이로드까지 읽습니다*

| 항목 | Service | Ingress |
|:---:|:---|:---|
| 보는 정보 | 받는 곳 주소 (IP·포트) | 송장 바코드 (URL·Host) |
| 읽는 도구 | 눈 (단순 위치 정보) | 스캐너 (인코딩 해석) |
...
<svg viewBox="0 0 1200 540" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="요청이 Pod까지 닿는 전체 흐름 — 1·2단계와 컨트롤 플레인이 위쪽 가로로, 3단계가 아래쪽에 두 컬럼(order/stores)으로 평행">
  ...
  <text x="310" y="247" text-anchor="middle" font-size="9" fill="#7b341e">노드 외벽 진입</text>
  ...
  <text x="460" y="216" text-anchor="middle" font-size="12" font-weight="700" fill="#7b341e">kube-proxy (1차)</text>
  ...
  <text x="1040" y="174" text-anchor="middle" font-size="11" font-weight="600" fill="#475569">API Server</text>
  <text x="1040" y="193" text-anchor="middle" font-size="8" fill="#94a3b8">Ingress·Service·Endpoints 정보</text>
  <rect x="940" y="232" width="200" height="50" rx="4" fill="#fff" stroke="#94a3b8" stroke-width="1.2"/>
  <text x="1040" y="251" text-anchor="middle" font-size="11" font-weight="600" fill="#475569">Endpoint Controller</text>
  <text x="1040" y="270" text-anchor="middle" font-size="8" fill="#94a3b8">Pod 변동 감시 → Endpoints 갱신</text>
  <line x1="1040" y1="232" x2="1040" y2="212" stroke="#94a3b8" stroke-width="1" stroke-dasharray="3,2" marker-end="url(#ov-s)"/>
  <text x="1056" y="225" font-size="9" font-style="italic" fill="#94a3b8">갱신</text>
  <line x1="860" y1="180" x2="938" y2="172" stroke="#94a3b8" stroke-width="1" stroke-dasharray="3,2" marker-end="url(#ov-s)"/>
  <line x1="938" y1="186" x2="860" y2="194" stroke="#94a3b8" stroke-width="1" stroke-dasharray="3,2" marker-end="url(#ov-s)"/>
  <text x="892" y="170" font-size="9" font-style="italic" fill="#94a3b8">watch</text>
  ...
  <text x="630" y="418" text-anchor="middle" font-size="11" font-weight="700" fill="#7b341e">kube-proxy (2차)</text>
  <text x="630" y="434" text-anchor="middle" font-size="9" fill="#7b341e">ClusterIP → Pod IP</text>
  <rect x="765" y="398" width="130" height="44" rx="6" fill="#fff" stroke="#ff7849" stroke-width="2"/>
  <text x="830" y="418" text-anchor="middle" font-size="11" font-weight="700" fill="#7b341e">kube-proxy (2차)</text>
  <text x="830" y="434" text-anchor="middle" font-size="9" fill="#7b341e">ClusterIP → Pod IP</text>
  ...
  <text x="600" y="497" text-anchor="middle" font-size="10" font-weight="700" fill="#7b341e">Pod A</text>
  <rect x="632" y="482" width="56" height="22" rx="11" fill="#fff4ed" stroke="#ff7849" stroke-width="1.6"/>
  <text x="660" y="497" text-anchor="middle" font-size="10" font-weight="700" fill="#7b341e">Pod A</text>
  ...
  <text x="800" y="497" text-anchor="middle" font-size="10" font-weight="700" fill="#7b341e">Pod B</text>
  <rect x="832" y="482" width="56" height="22" rx="11" fill="#fff4ed" stroke="#ff7849" stroke-width="1.6"/>
  <text x="860" y="497" text-anchor="middle" font-size="10" font-weight="700" fill="#7b341e">Pod B</text>
</svg>
</div>

*그림 5-21. 요청이 Pod까지 닿는 전체 흐름 - 세 단계가 일어나는 위치*

:::term-box
**kube-proxy**: 모든 노드(서버)에 항상 떠 있는 프로그램입니다. 노드로 들어온 요청의 받는 주소를 살아있는 Pod의 주소로 바꿔서 그쪽으로 흘려 보냅니다.
:::
...
*그림 5-22. 우체국 창구에서 우편의 외부 주소 위에 새 송장(바코드)이 덮여 붙어 다음 단계로 넘어갑니다*
...
*그림 5-23. NodePort와 kube-proxy(1차)가 외부 요청을 Ingress Controller로 전달*

### 5.3.2 2단계 분류 - 물류 센터가 바코드를 보고 어디로 보낼지 정한다

송장이 붙은 우편이 **물류 센터**에 도착합니다. 작업자가 송장의 **바코드**를 스캐너로 찍으면 시스템이 옆에 비치된 **분류 규칙**에 따라 어느 우체국으로 갈지 곧장 알려 줍니다. 분류 규칙은 우체국 정보가 바뀔 때마다 즉시 고쳐 둡니다.
...
*그림 5-24. 물류 센터가 송장의 바코드를 찍고 어느 우체국으로 보낼지 정합니다*
...
Ingress Controller는 요청의 URL 경로와 Host 헤더를 읽어 등록된 Ingress 규칙과 맞춥니다. *"이 요청은 `order-service`다."* 판단이 끝나면 분류 규칙에 적힌 `order-service`의 ClusterIP를 곧바로 꺼냅니다.
...
*그림 5-25. Ingress Controller가 URL을 읽어 Service를 선택, 미리 받아 둔 분류 규칙에서 ClusterIP를 꺼냄*
...
*그림 5-26. 서울 우체국에서 집배원이 배송 목록을 보고 우편함 B에 우편을 넣습니다*
...
*그림 5-27. ClusterIP가 kube-proxy(2차)를 통해 Pod IP로 변환되어 Pod에 응답. Endpoint Controller가 Pod IP 목록을 갱신*
...
종이 위에 길게 늘어놓은 컴포넌트들이 한 줄로 묶이자, 따로따로 떠 있던 이름들이 머릿속에서 제 위치를 찾았습니다.
