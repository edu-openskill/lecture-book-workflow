...
외부 손님이 대표번호(`NodePort`)로 전화를 걸면, **ARS**(`kube-proxy`)가 전화를 받아 콜센터(`Ingress Controller`)로 연결해 줍니다.
...
<svg viewBox="0 0 800 280" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="1단계 비유 — 외부 손님이 본사 대표번호로 전화, ARS가 콜센터로 연결">
  ...
  <text x="400" y="22" text-anchor="middle" font-size="13" font-weight="700" fill="#1f2937">외부 손님이 대표번호로 전화 → ARS → 콜센터</text>
  ...
  <text x="445" y="140" text-anchor="middle" font-size="9" font-weight="700" fill="#fff">ARS</text>
...
| ARS | **kube-proxy (1차)** | 노드 커널의 iptables 규칙으로 들어온 요청을 Ingress Controller Pod로 전달 |
...
콜센터 상담원(`Ingress Controller`)이 요청을 분석합니다. "주문 부서로 가야겠군"이라고 판단하면, 부서 명부(상담원이 미리 들고 있는 부서·내선 안내표)에서 해당 부서의 내선 번호(`ClusterIP`)를 찾아 전화를 돌려줍니다.
...
| 부서 명부 (이름 → 내선) | **Ingress Controller의 내부 명단** | API Server를 watch(변경 알림 구독)해서 받아 둔 Service 정보와 Pod IP 명단 |
| 부서 대표 내선 | **ClusterIP** | Service의 가상 진입 주소 |

Ingress Controller는 도착한 요청의 HTTP 본문을 읽습니다. URL 경로와 도메인 헤더를 등록된 Ingress 규칙과 대조해 "이 요청은 `order-service`로 보낸다"고 정합니다. 이름이 정해진 직후, Ingress Controller는 미리 받아 둔 내부 명단에서 곧바로 `order-service`의 ClusterIP를 꺼냅니다.
...
부서 내선으로 전화가 연결되면, **ARS**(`kube-proxy`)가 현재 응대가 가능한 직원(`Pod`)에게 최종적으로 전화를 연결합니다. 이때 부서 매니저(`Endpoint Controller`)는 누가 일하고 있는지(파드의 생사 여부)를 실시간으로 파악해 응대 직원 명단을 항상 최신으로 유지합니다.
...
<svg viewBox="0 0 800 340" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="3단계 비유 — ARS가 응대 직원 한 명에게 연결, 부서 매니저가 응대 직원 명단 갱신">
  ...
  <text x="400" y="22" text-anchor="middle" font-size="13" font-weight="700" fill="#1f2937">부서 대표 내선 → ARS → 응대 직원 응답</text>
  ...
  <text x="380" y="115" text-anchor="middle" font-size="12" font-weight="700" fill="#7b341e">ARS</text>
  <text x="380" y="138" text-anchor="middle" font-size="9" fill="#7b341e">ARS — 가용 인원 중 한 명에게 연결</text>
...
| 비유 | IT 용어 | 한 줄 설명 |
|:---:|:---|:---|
| ARS | **kube-proxy (2차)** | ClusterIP를 살아있는 Pod IP로 변환하는 L4 로드밸런서 |
| 응대 직원 명단 | **Pod IP 명단** | 현재 살아있는 Pod들의 IP 목록 (Endpoint Controller가 관리) |
| 부서 매니저 | **Endpoint Controller** | Pod 변동을 감지해 위 명단을 즉시 갱신하는 컨트롤 플레인 컴포넌트 |
| 응대 직원 | **Pod** | 실제 비즈니스 로직을 처리하는 앱 |

ClusterIP로 향한 요청은 같은 노드의 kube-proxy(2차)가 가로채서, Endpoint Controller가 관리하는 명단의 살아있는 Pod 중 하나의 IP로 바꿔 보냅니다. 요청이 백엔드 Pod에 닿으면 애플리케이션이 비즈니스 로직을 실행해 응답을 돌려보냅니다.
...
엄밀히 말하면 Endpoint Controller가 kube-proxy에게 직접 명단을 주는 게 아닙니다. Endpoint Controller가 명단을 갱신하면 그 결과는 API Server에 저장되고, 각 노드의 kube-proxy가 API Server를 watch(구독)해서 변경 알림을 받습니다. 즉 두 컴포넌트 사이에 API Server가 중간 매개체 역할을 합니다. 비유 그림에서는 이 매개를 생략하고 매니저와 ARS 사이의 양방향 화살표로 단순화했습니다.
:::
...
| **2. 결정** | Ingress Controller + API Server 명단 | URL 분석 및 서비스 결정 | URL 경로, Host 헤더 |
| **3. 응답** | kube-proxy + Pod | 실제 Pod IP로 변환 및 처리 | 실시간 IP 목록 |

L4가 빠른 대신 *어디로 갈지*만 보고, L7은 한 단계 더 들어가 *무슨 용건인지*까지 본다는 차이입니다. 콜센터로 옮겨 보면, 1단계와 3단계의 ARS는 *전화번호*만 보고 옮기는 일이고, 2단계의 상담원만 *고객의 말*을 듣고 부서를 정합니다.
