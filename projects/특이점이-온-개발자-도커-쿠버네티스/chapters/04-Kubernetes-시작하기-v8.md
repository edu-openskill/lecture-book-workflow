...
- **명령형**(Docker)과 **선언형**(Kubernetes)의 차이를 이해합니다
...
**선배**: "도커만으로는 운영까지 챙기기 어려워요. 그래서 쿠버네티스를 같이 알아야 해요. 컨테이너에 대한 관리는 쿠버네티스 담당이거든요."
...
*그림 4-2. 본사가 "가맹점 3개 유지"를 선언하면 시스템이 가맹점 개수를 자동으로 맞추는 구조*

쿠버네티스도 마찬가지입니다. 컨테이너 수나 상태 규칙 등 **원하는 상태**를 선언해 두면, 시스템이 자동으로 그 상태를 **유지**합니다.

### 4.1.3 Kubernetes 핵심 리소스 한눈에
...
### 4.1.4 Kubernetes - 컨트롤 플레인과 워커 노드
...
:::term-box
**RollingUpdate 전략**: Kubernetes Deployment의 기본 배포 전략입니다. `maxSurge`와 `maxUnavailable` 두 값으로 "몇 개를 더 실행할 수 있는지", "몇 개까지 사용 불가능해도 되는지"를 조정합니다.
:::
...
- **쿠버네티스는 프랜차이즈 본사 구조입니다.** 본사가 전국의 가맹점을 관리하는 모습과 같습니다. 컨트롤 플레인이 워커 노드 위의 Pod를 관리합니다.
- **Pod는 가맹점, 실행의 최소 단위입니다.** 쿠버네티스에서 컨테이너는 Pod라는 상자에 담겨 움직입니다.
