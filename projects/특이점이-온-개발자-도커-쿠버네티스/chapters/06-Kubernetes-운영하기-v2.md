...
챕터 4에서 같은 서버 네 대가 자동으로 살아나는 모습을 봤고, 챕터 5에서는 그 앞에 Service와 Ingress를 두어 IP가 흔들려도 끊기지 않는 진입점을 만들었습니다. 며칠 동안 만들고 지운 Pod와 Service만 한 화면 가득 쌓였습니다. 지나가던 팀장이 그 화면을 슬쩍 들여다봤습니다.

**팀장**: "구성 잘 잡았네요. 그럼 챕터 3에서 docker-compose로 짰던 그 통합 프로젝트, 이번엔 쿠버네티스 위에 올려볼래요?"

오픈이는 잠시 멈칫했습니다. Pod 하나, Service 하나는 깔끔하게 띄웠지만, 프론트엔드·백엔드·DB가 한 덩어리로 묶인 그 프로젝트를 그대로 옮기려고 보니 막상 손을 댈 자리가 보이지 않았습니다. docker-compose로 띄울 때 짚지 않고 지나갔던 것들이 한꺼번에 도드라졌습니다.

먼저 DB 비밀번호가 마음에 걸렸습니다. 챕터 3에서는 `docker-compose.yml`에 환경 변수로 그냥 박아 뒀는데, 로컬에서만 돌리던 거라 그래도 됐을 뿐이었습니다. 운영 환경 YAML에 같은 식으로 적으면 비밀번호가 Git 저장소에 그대로 올라갑니다. DB 주소나 접속 URL처럼 환경마다 달라지는 값도 마찬가지였습니다. 그렇다고 이미지에 박아 두자니 환경이 바뀔 때마다 이미지를 다시 빌드해야 한다는 점이 걸렸습니다.

데이터 쪽은 한 단계 더 무거운 문제였습니다. 챕터 4에서 Deployment가 죽은 Pod를 곧바로 살려 주는 것까지 본 다음이라, 같은 일이 DB Pod에서 벌어지면 회원 정보와 게시글이 통째로 사라진다는 게 더 또렷이 보였습니다. 코드는 다시 띄우면 그만이었지만, 그 안에 쌓아 둔 데이터는 새 Pod에 옮겨 갈 길이 없었습니다.

*'설정과 비밀번호, 데이터까지 두 매듭이 풀려야 챕터 3 프로젝트를 K8s에 그대로 옮길 수 있겠구나.'*
...
쿠버네티스에서 이 역할을 맡는 리소스는 두 가지입니다.
 ...
 - **Secret**: 비밀번호, 토큰, API 키처럼 민감한 정보를 저장할 때 사용합니다. ConfigMap과 구조는 비슷하지만, 값이 Base64로 인코딩되어 저장되며 접근 권한이 엄격하게 제한됩니다.
...
오픈이는 먼저 일반 설정을 관리하는 ConfigMap을 생성했습니다. ex13 폴더를 열어 키-값 두 개를 담은 ConfigMap YAML을 확인했습니다.
...
YAML을 다시 들여다보니 `envFrom` 아래에 ConfigMap 이름 한 줄만 적혀 있었습니다. 키를 하나씩 매핑해 줄 필요가 없었습니다. 컨테이너가 시작되는 순간 `configmap-conn`에 적힌 `conn_info`와 `conn_url`이 그대로 환경 변수로 들어옵니다.
...
Pod 내부의 환경 변수를 확인하자 `conn_info=localhost:80`과 같은 값이 나타났습니다. 이미지를 건드리지 않고도 설정값만 외부에서 주입하는 데 성공했습니다.
...
DB 비밀번호도 같은 방식으로 처리할 수 있을지 고민했습니다. 그런데 민감한 정보를 메뉴판처럼 공개된 곳에 적어둘 수는 없었습니다. 이를 위해 별도로 존재하는 리소스가 바로 Secret입니다.
...
새로운 Pod가 생성되었음에도 `c.txt` 파일이 보존되어 있습니다. Pod는 교체되었으나 파일의 실체는 PV에 존재하므로 데이터가 유지됩니다. PVC가 새로운 Pod에게도 동일한 창고를 연결해 준 결과입니다.
...
오픈이는 그동안 학습한 쿠버네티스의 기본 리소스들을 하나씩 되짚어 보았습니다. 컨테이너를 관리하는 Pod와 Deployment, 그 앞을 지키는 Service와 Ingress, 그리고 방금 배운 설정과 저장소 리소스들까지. 그런데 각각의 개념을 이해하는 것과, 이들을 하나로 엮어 실제 서비스를 구동하는 것은 다른 차원의 문제처럼 느껴졌습니다.
...
팀장의 말대로 이번 절은 단순한 실습의 연장이 아니었습니다. 그동안 파편화되어 있던 지식을 하나로 모아, 실제로 살아 움직이는 통합 서비스를 완성하는 마지막 관문이었습니다.
...
로그를 들여다보니 안에서 빌드 작업이 한창이었습니다. 잠시 기다리자 모든 Pod가 Running 상태로 바뀌었고, 오픈이는 외부 접근을 위한 통로를 열었습니다. 챕터 5에서처럼 별도 터미널에서 `minikube tunnel`을 실행하면 Ingress 입구가 `localhost`로 이어집니다.
...
처음부터 모든 걸 알고 시작할 수는 없습니다. 오픈이가 그랬듯, 문제를 만나면서 하나씩 알아 가는 과정입니다. 챕터 1에서 선배가 던졌던 "**환경이 달라서 그래요. Docker 한번 알아봐요.**" 한 마디가 여기까지 온 출발점이었습니다. 그 한 마디에서 시작해 컨테이너를 띄우고, 서비스를 엮고, 선언 한 줄로 운영 환경을 만드는 자리까지 왔습니다.

다음에 새 장애를 만나거나 새 리소스를 마주치면, 이 책에서 익힌 흐름을 그대로 꺼내 쓰면 됩니다. 컨테이너 한 칸에서 시작해 클러스터 전체를 손에 잡기까지의 거리는, 이미 한 번 걸어본 길이 됐습니다.
