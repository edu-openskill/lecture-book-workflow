...
문제는 비밀번호만이 아니었습니다. 통합 사이트의 DB Pod 한 대를 떠올리자 또 한 가지가 보였습니다. Deployment가 죽은 Pod를 곧장 새로 띄워 주는 모습을 며칠 전에 봤기 때문에, 같은 일이 DB Pod에서 벌어지면 어떻게 될지가 분명해졌습니다. 코드는 다시 띄우면 그만입니다. 그 안에 쌓아 둔 회원 정보와 게시글은 새 Pod로 따라오지 않습니다.
...
리눅스에서 환경 변수는 **프로세스가 시작될 때 한 번 꽂히는 값**입니다. 그래서 ConfigMap 쪽이 갱신되어도, 이미 떠 있는 Pod의 프로세스 안에는 처음 들어온 옛 값이 그대로 남아 있습니다. 새 값을 반영하려면 Pod를 한 번 새로 띄워 줘야 합니다.
...
*그림 6-7. rollout restart 로 Pod를 새로 띄우면 ConfigMap 새 값이 환경 변수로 반영됨*
