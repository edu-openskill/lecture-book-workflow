...
- 챕터 3의 통합 사이트를 K8s 위에 한 번에 올려 운영 환경에 가까운 시스템을 만듭니다
:::

월요일 저녁이었습니다. 며칠 전에 잡아 둔 진입점이 사내 데모용 클러스터에 안정적으로 떠 있었고, 통합 사이트의 프론트엔드·백엔드·DB도 그 뒤에서 정상적으로 동작하고 있었습니다. 이제 정리해서 운영 레포에 올려 두기만 하면 끝이었습니다.

`git add`로 올라갈 파일을 추리고, `git status`로 목록을 한 번 더 훑었습니다. 다음 순서가 `git push`였습니다.

손이 키보드 위에서 멈췄습니다. 화면 한쪽에 미리 열어 둔 운영용 deploy.yml의 환경 변수 영역이 눈에 들어왔습니다.

```yaml
env:
  - name: MYSQL_PASSWORD
    value: metacoding1234
```

*'잠깐. 이걸 그대로 푸시하면 비밀번호가 Git에 통째로 올라가는데.'*

챕터 3에서 MySQL 컨테이너를 띄우던 날, 빌드 도중 *민감한 정보는 ENV에 넣지 말라*는 경고가 한 줄로 지나간 적이 있었습니다. 그때는 학습용이라 그대로 뒀습니다. 학습 레포에 둔 그 한 줄이 지금은 운영 레포로 올라가기 직전이었습니다.

![운영 YAML에 적은 비밀번호가 Git에 그대로 올라간다](../assets/CH06/gemini/01_prologue-secret-leaks-to-git.png)

*그림 6-1. 운영 YAML에 적은 비밀번호가 Git 저장소에 그대로 올라간다*

이미지에 비밀번호를 박아 두면 환경이 바뀔 때마다 이미지를 다시 빌드해야 합니다. YAML로 옮겨 적자니 지금처럼 Git에 그대로 올라갑니다. DB 주소나 접속 URL처럼 환경마다 달라지는 값도 같은 처지였습니다.

문제는 비밀번호만이 아니었습니다. 통합 사이트의 DB Pod 한 대를 떠올리자 또 한 가지가 보였습니다. Deployment가 죽은 Pod를 곧장 새로 실행해 주는 모습을 며칠 전에 봤기 때문에, 같은 일이 DB Pod에서 벌어지면 어떻게 될지가 분명해졌습니다. 코드는 다시 실행하면 그만입니다. 그 안에 쌓아 둔 회원 정보와 게시글은 새 Pod로 따라오지 않습니다.

오픈이는 푸시 명령어를 잠시 내려놓고 노트에 두 단어를 적었습니다. **설정·비밀번호**, 그리고 **데이터**. 이 두 가지를 해결하지 못한 채 레포를 올리면 운영에 뜨자마자 문제가 터집니다.

*'두 가지부터 해결해야겠다.'*

다음 날 아침, 옆자리 선배에게 이 두 가지를 꺼내 봤습니다. 선배가 의자를 굴려 다가왔습니다.
...
설정값과 비밀번호를 따로 보관할 전용 공간이 필요했습니다. 쿠버네티스에는 이미 그 자리가 마련되어 있습니다. 가맹점 한쪽에 공용 메뉴판을 걸어 두고, 비밀 레시피는 같은 가맹점 안에 따로 두는 방식입니다. 메뉴를 바꿀 때마다 주방까지 새로 들일 필요는 없습니다. 메뉴판과 레시피만 갈아 끼우면 됩니다. 쿠버네티스에서도 같은 발상으로 두 자리가 나뉘어 있습니다.

이 일을 맡는 리소스는 두 가지입니다.
...
남은 건 비밀번호였습니다. 같은 방식으로 ConfigMap에 비밀번호를 적어 두면 그만일까 생각이 들었지만, 곧 고개를 저었습니다. 민감한 값을 메뉴판처럼 공개된 곳에 적어 둘 수는 없습니다. 비밀번호는 가맹점 안 비밀 레시피처럼 따로 보관해야 할 값이고, 쿠버네티스에서는 그 자리를 Secret이라는 이름으로 따로 두고 있습니다.
...
Pod에 주입하는 방식은 ConfigMap과 동일합니다. 앞서 작성해 둔 `deploy-ex03.yml`을 다시 열어 `envFrom` 아래에 `secretRef` 두 줄을 추가합니다. `secretRef`를 활성화하면 쿠버네티스가 실행 시점에 Base64 인코딩을 디코딩해 환경 변수로 넣어 줍니다.

```yaml [실습 4] ex13/deploy-ex03.yml. Secret 연결 활성화 (envFrom 부분)
envFrom:
  - configMapRef:
      name: configmap-conn
  - secretRef:
...
리눅스에서 환경 변수는 **프로세스가 시작될 때 한 번 꽂히는 값**입니다. 그래서 ConfigMap 쪽이 갱신되어도, 이미 떠 있는 Pod의 프로세스 안에는 처음 들어온 옛 값이 그대로 남아 있습니다. 새 값을 반영하려면 Pod를 한 번 새로 실행해 줘야 합니다.
...
```bash [터미널] Pod 재시작으로 새 ConfigMap 반영
kubectl rollout restart deployment nginx-config-secret   # Pod 재시작
```

`kubectl rollout restart`는 여러 Pod를 순차로 교체해 새 값을 반영하는 명령입니다. 재시작이 끝나고 다시 환경 변수를 찍어 보니 포트가 `90`으로 바뀌어 있었습니다. apply는 ConfigMap을 갱신할 뿐, 실제 Pod까지 반영되려면 재시작이 한 번 더 필요합니다.

설정과 비밀번호를 이미지 바깥으로 빼는 일은 끝났습니다. 노트에 적어 둔 두 단어 중 첫 번째 문제가 풀렸지만, 한 가지가 더 남아 있습니다. 비밀번호로 잠가 두는 DB의 데이터가 아직 그대로였습니다.

## 6.2 Volume - 데이터의 영속성 확보

### 6.2.1 Pod의 휘발성 문제

설정 분리는 끝났고, 남은 건 데이터였습니다. 챕터 4에서 본 그 자동 복구가 DB Pod에서는 다른 문제를 만듭니다. Pod는 기본적으로 **휘발성**이라, 안에서 만든 파일은 Pod 수명과 함께 사라집니다. 회원 가입과 게시글이 매일 쌓이는 DB가 이런 식으로 휘발되면 운영을 할 수가 없습니다.
...
Pod가 직접 저장 공간을 관리하려고 들면 인프라 세부 사항까지 같이 짊어져야 합니다. 어느 디스크에, 어떤 권한으로, 몇 기가짜리를 쓸 것인지 매번 따져야 합니다. Kubernetes는 그래서 저장소를 **창고**와 그 창고를 쓰겠다는 **신청서**로 분리해 두었습니다. 한 장 그림으로 보면 관계가 분명해집니다.
...
*그림 6-8. PV는 실제 저장 공간, PVC는 그 공간을 요청하는 신청서*

- **PV(PersistentVolume)**는 실제 저장 공간, 즉 **창고**입니다. 용량, 권한, 위치 같은 창고의 사양이 정의됩니다.
- **PVC(PersistentVolumeClaim)**는 창고를 쓰겠다고 작성하는 **신청서**입니다. "**1Gi짜리 읽기·쓰기 가능한 창고가 필요하다**"고 적어 두면, Kubernetes가 조건에 맞는 PV를 찾아 자동으로 PVC와 연결합니다.

Pod는 PV를 직접 건드리지 않고 PVC만 붙여 사용합니다. 실제 창고 위치는 PVC가 알아서 연결해 주기 때문에, Pod 입장에서는 "**용량이 맞는 저장 공간 하나**"만 보입니다.
...
*그림 6-9. PV와 PVC가 Bound 상태로 연결된 결과*
...
*그림 6-10. Pod가 새로 태어났는데도 c.txt가 그대로 남아 있는 모습*
...
*'Pod가 새로 만들어져도 창고는 그대로구나. 이제 데이터가 사라질 걱정이 없겠다.'*

회의실 화이트보드에 적어 둔 두 단어, **설정·비밀번호**와 **데이터** 위에 줄이 그어졌습니다.
...
다음 날 아침, 오픈이는 자리에 앉자마자 노트를 펼쳤습니다. 며칠 전 회의실에서 팀장이 의뢰한 챕터 3 그 사이트를 K8s 위에 올릴 차례였습니다. 준비는 끝났고, 도구는 쥐고 있었습니다. 이번 챕터에서는 그동안 따로따로 익힌 도구들이 한 사이트에서 한꺼번에 만납니다. 챕터 4·5에서 본 Pod·Deployment·Service·Ingress에, 이번 챕터에서 새로 더한 ConfigMap·Secret과 PV·PVC가 합쳐집니다.
...
오픈이가 펼쳐 본 구성도에는 서비스 네 개가 들어 있었습니다. 프론트엔드(Nginx), 백엔드(Spring Boot), DB(MySQL), 그리고 방문 횟수를 기록할 Redis. 챕터 3에서 본 세 덩어리에 Redis가 더해져, 운영 환경에 한층 더 가까운 구조입니다.
...
*그림 6-11. ex15 Kubernetes 웹사이트의 전체 구성*
...
그림을 보고 있자니 한 가지가 분명해졌습니다. Pod 사이의 모든 통신이 IP가 아닌 Service 이름으로 이루어진다는 사실이었습니다. `db-service`, `backend-service` 같은 이름만 정해 두면, 뒤에 어떤 Pod가 떠 있든 그 이름으로 Pod까지 연결됩니다.

*'IP가 어디로 옮겨 다니든 이름만 잡고 있으면 흐름이 안 끊기는구나.'*

### 6.3.2 폴더 구조와 진행 방식

오픈이가 ex15 폴더를 열어 보니 이미지를 만드는 영역과 쿠버네티스 설정(YAML) 영역이 깔끔하게 나뉘어 있었습니다. 이번 실습의 초점은 이미지 제작이 아니라 쿠버네티스 리소스들이 어떻게 한 덩어리로 맞물리는지 보는 데 있습니다.
...
오픈이는 `ex15/k8s/` 폴더를 열고 그 안의 리소스들을 하나씩 훑어 봤습니다. 서비스별로 폴더가 나뉘어 있고, 각 폴더에는 Deployment와 Service가 기본으로 들어 있습니다. 이번 절에서 오픈이가 보고 싶었던 건 이들이 한꺼번에 실행될 때 어떻게 서로 맞물려 돌아가는지였습니다.
...
한 회사 안에서도 팀마다 사무실을 따로 쓰듯, 쿠버네티스에서도 공간을 분리할 수 있습니다. Namespace는 하나의 클러스터를 논리적으로 구분해 주는 가상 공간입니다.
...
오픈이는 frontend 폴더부터 펼쳤습니다. 외부에서 들어오는 모든 요청이 처음 닿는 입구를 정의하는 폴더였습니다. 핵심은 단일 path로 모든 요청을 잡아 frontend-service로 넘기는 `frontend-ingress.yml`입니다.
...
| 파일 | 역할 |
|:---:|:---|
| `frontend-deploy.yml` | `metacoding/frontend:1` 이미지를 실행하는 Deployment. 정적 HTML과 `/api`를 backend-service로 프록시하는 Nginx 설정을 담은 컨테이너 한 개를 80번 포트로 올립니다 |
...
`volumeMounts`로 컨테이너의 `/var/lib/mysql`(MySQL이 데이터를 쓰는 표준 경로)에 PVC가 가리키는 PV를 끌어다 붙입니다. Pod가 사라져도 데이터는 PV에 남아, Deployment가 새 Pod를 실행해도 회원 정보와 게시글이 그대로 다시 보입니다.
...
| 파일 | 역할 |
|:---:|:---|
| `redis-deploy.yml` | `metacoding/redis:1` 이미지를 실행하는 Deployment. Redis 컨테이너 한 개를 6379 포트로 올립니다 |
| `redis-service.yml` | `redis-service` 이름으로 6379 포트를 노출하는 ClusterIP Service. 백엔드의 ConfigMap이 이 이름을 호스트 값으로 참조합니다 |

### 6.3.4 공통 준비

코드를 살펴봤으니 배포할 차례였습니다. 오픈이는 본격적으로 배포하기 전에 두 가지부터 챙겼습니다. Minikube와 Ingress Controller를 켜는 일, 그리고 네 가지 이미지를 빌드해 두는 일이었습니다.
...
*그림 6-13. Nginx Ingress Controller 애드온 설치*
...
*그림 6-14. Ingress를 거쳐 웹사이트가 화면에 응답*
...
**동료**: "어, 이거 며칠 전에 docker-compose로 띄웠던 그 사이트 아니에요? 이번엔 K8s 위에 있네요."

오픈이는 고개를 끄덕였습니다. 같은 사이트지만 자리가 달라졌습니다. docker-compose 한 파일 안에 묶여 있던 세 덩어리가, 이제는 클러스터 위에서 각자 Pod로 떠 있고 Service 이름으로 서로를 호출하고 있었습니다.
...
처음부터 모든 걸 알고 시작할 수는 없습니다. 챕터 1에서 선배가 던진 "**환경이 달라서 그래요. Docker 한번 알아봐요.**" 한 마디가 여기까지 온 출발점이었습니다.
