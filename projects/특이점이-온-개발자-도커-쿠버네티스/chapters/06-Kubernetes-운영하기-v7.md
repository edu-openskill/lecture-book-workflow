...
설정값과 비밀번호를 따로 보관할 전용 공간이 필요했습니다. 쿠버네티스에는 이미 그 자리가 마련되어 있었습니다. 가맹점 한쪽에 공용 메뉴판을 걸어 두고, 비밀 레시피는 같은 가맹점 안에 따로 두는 방식입니다. 메뉴를 바꿀 때마다 주방까지 새로 들일 필요는 없습니다. 메뉴판과 레시피만 갈아 끼우면 됩니다. 쿠버네티스에서도 같은 발상으로 두 자리가 나뉘어 있습니다.
