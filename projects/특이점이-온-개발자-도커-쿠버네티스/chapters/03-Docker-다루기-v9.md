...
- **Dockerfile**로 환경 구성을 자동화하는 프로비저닝을 익힙니다
- **NGINX**의 리버스 프록시·로드밸런싱·캐싱을 직접 설정해 봅니다
- **Redis**로 여러 서버가 공용 세션을 들여다보는 구조를 만듭니다
- **Docker Compose**로 프론트엔드·백엔드·DB 통합 사이트를 한 줄 명령으로 띄웁니다
:::
...
화요일 오전 열 시. 회의실에 사람이 모였습니다. 팀장이 화이트보드 앞에서 새 프로젝트의 윤곽을 잡기 시작했습니다. 사내에서 쓸 작은 통합 사이트 하나를 새로 만든다는 이야기였습니다. 화면을 그릴 프론트엔드, 그 뒤를 받쳐 줄 백엔드, 회원 정보를 보관할 DB가 함께 구성된 형태였습니다.
...
오픈이는 고개를 끄덕였습니다. 회의가 끝난 뒤 자리로 돌아오는 길은 막막했습니다. 컨테이너 하나를 실행하는 건 익혔지만, 셋을 한꺼번에 실행하는 일은 처음이었습니다. 프론트엔드에서 백엔드로, 백엔드에서 DB로 요청이 흘러야 했고, 셋이 동시에 살아 있어야 했습니다.

*'한 대씩은 띄워봤는데, 여러 대를 함께 구성하는 건 또 다른 이야기인데.'*

자리에 앉아 모니터를 노려봤습니다. 사무실 형광등이 키보드 위로 길게 떨어졌고, 옆자리에서 마우스 클릭 소리만 또각또각 울렸습니다. 무엇부터 손대야 할지 그림이 잘 잡히지 않았습니다. 퇴근 후 직접 부딪혀보면서 무엇이 막히는지부터 보기로 했습니다.

## 3.1 Dockerfile - 환경을 자동으로 만들기

### 3.1.1 프로비저닝

저녁 여덟 시. 식탁에 노트북을 펼쳤습니다. 지난주에 손으로 컨테이너에 들어가 패키지를 깔고 commit으로 이미지를 남기던 그 과정이 떠올랐습니다. 처음 한 번은 신기했지만, 이제는 프론트엔드·백엔드·DB까지 세 번을 똑같이 반복해야 했습니다.

*'매번 이걸 다시 해야 할까.'*

그러다 며칠 전 편의점에서 본 밀키트가 머릿속을 스쳤습니다. 포장을 뜯어 끓이기만 하면 매번 같은 요리가 나왔습니다.

*'도커도 비슷한 방법이 있지 않을까.'*

이런 방식이 도커에 이미 마련되어 있습니다. 환경 구성을 정의 파일 하나에 적어 두면 그 정의대로 환경이 자동으로 갖춰지는 작업을 **프로비저닝(Provisioning)** 이라고 부릅니다.
...
오픈이는 Dockerfile에서 가장 자주 쓰이는 지시어를 표로 정리했습니다.
...
표를 한 번 훑고 첫 실습에 들어갔습니다. Ubuntu 환경에 vim이 미리 깔려 있는 이미지를 만들어 보는 일입니다. 빈 폴더 하나를 만들고 그 안에 확장자 없이 `Dockerfile`이라는 이름으로 파일을 하나 만들었습니다.
...
빌드 로그가 한 줄씩 위로 올라갔습니다. 마지막 줄이 떨어지자 이미지가 완성되었습니다. 그 이미지로 컨테이너를 실행해 봤습니다. 들어가자마자 vim을 쳐 보니 별도의 설치 과정 없이 편집기가 그대로 떴습니다.
...
지난주에 손으로 다섯 번 친 과정이 **Dockerfile 하나**로 끝났습니다.

### 3.1.4 WORKDIR와 COPY

환경은 갖췄으니 이제 로컬 코드를 컨테이너 안으로 옮길 차례입니다. 여기에 쓰는 지시어가 **WORKDIR**와 **COPY**입니다.

오픈이는 빈 `index.html` 파일을 Dockerfile과 같은 폴더에 만들었습니다.

![](../assets/CH03/chap02-11.png)

*그림 3-6. 폴더 및 파일 구조*

Dockerfile에 두 줄을 추가했습니다. 작업 디렉토리를 `/app`으로 정하고, 로컬의 `index.html`을 그 안에 복사하는 설정입니다.
...
이렇게 하면 컨테이너 시작 시 작업 디렉토리는 `/app`이고, `index.html`은 그 안에 복사되어 있습니다. 다시 빌드하고 실행해 봤습니다.
...
문법 표 마지막 두 줄이 비슷해 보였습니다. 챕터 2에서 만진 CMD는 "컨테이너 시작 시 실행되는 기본 명령"이었습니다. ENTRYPOINT는 "반드시 실행되는 메인 프로세스". 둘 다 '실행'이 들어가서 차이가 한눈에 들어오지 않았습니다.

*'CMD만 써도 메인 프로세스가 됐는데, ENTRYPOINT가 따로 있는 이유가 뭘까.'*

두 지시어는 함께 정의될 때 역할이 갈립니다. ENTRYPOINT가 없을 때는 CMD가 **메인 프로세스**로 단독 실행되지만, ENTRYPOINT가 있으면 CMD는 그 **인자 자리**로 옮겨갑니다.

이 동작은 카페의 커피 머신을 떠올리면 가깝습니다. ENTRYPOINT는 **커피를 내린다는 동작 자체**이고, CMD는 **기본 메뉴 아메리카노**입니다. 손님이 라떼를 주문하면 기본값만 라떼로 바뀝니다.

:::note
**CMD와 ENTRYPOINT**

- **ENTRYPOINT**: 컨테이너 시작 시 반드시 실행되는 고정 명령. 외부 인자로 쉽게 덮어쓸 수 없습니다.
- **CMD (ENTRYPOINT와 함께 쓰일 때)**: ENTRYPOINT 뒤에 따라붙는 기본 인자. `docker run` 뒤에 인자를 직접 넘기면 그 값으로 교체됩니다.
:::
...
ENTRYPOINT의 `echo "컨테이너 실행"` 뒤에 CMD의 `/bin/bash`가 인자로 따라붙어 한 줄로 출력됐습니다. Dockerfile에 적은 CMD가 **ENTRYPOINT의 기본 인자로 사용된 결과**입니다.

*'명령어를 한 줄씩 손으로 치던 일을 Dockerfile이 대신하네. 이게 프로비저닝이구나.'*

Dockerfile로 프로비저닝 하는 방법까지 이해했습니다. 다음은 사용자가 어디로 들어오느냐였습니다.

## 3.2 NGINX - 요청을 앞에서 받아 나눠주기

### 3.2.1 NGINX를 서버 앞에 두는 이유

회의실에서 받은 사이트 구조에는 빠진 게 하나 있었습니다. 사용자가 들어오는 입구입니다. 컨테이너 셋이 각자 다른 포트를 쓰는 상태에서 그 번호를 사용자에게 그대로 노출할 수는 없습니다. 내부 주소가 외부로 그대로 드러납니다.

*'입구는 하나로 두고, 들어온 요청을 알맞은 컨테이너로 보낼 방법은 없을까.'*

떠오른 도구는 NGINX였습니다. NGINX는 웹 서버이자, 요청을 앞에서 받아 뒤쪽 서버로 전달하는 **리버스 프록시(Reverse Proxy)** 입니다.
...
NGINX를 앞단에 세우면 실제 서버의 IP나 내부 포트를 사용자에게 노출할 일이 없습니다. 나중에 백엔드 서버를 두 대, 세 대로 늘려도 NGINX가 알아서 요청을 골고루 나눠 주는 로드밸런싱 역할까지 합니다.
...
`location`에 지정된 경로 → `proxy_pass`에 설정된 upstream 그룹 → `upstream`에 지정된 서버 주소
...
다음 실습은 로드밸런싱입니다. NGINX는 `upstream` 블록 안에 서버 주소를 여러 개 적어 두면, 들어오는 요청을 등록된 순서대로 한 곳씩 돌아가며 보내 줍니다. 카드 딜러가 카드를 한 장씩 차례대로 나눠 주듯, 서버 한 곳에 한 번씩 돌아가는 방식입니다. 이 방식을 **라운드 로빈(Round Robin)** 이라고 부릅니다.
...
화면만으로는 어느 서버가 응답한 건지 알 길이 없었습니다. 두 컨테이너가 같은 이미지이기 때문입니다. 두 서버 양쪽으로 진짜 요청이 갈라져 들어갔는지 확인하려면 각자의 로그를 직접 들여다봐야 했습니다. `docker logs` 명령을 두 번 쳤습니다.
...
두 로그를 나란히 펼쳐 보니, 두 서버에 요청이 전달된 것을 확인할 수 있습니다. NGINX는 upstream에 서버가 둘 이상이면 별도 설정 없이 라운드 로빈으로 분배합니다.
...
*'단순한 HTML을 매번 백엔드까지 가서 만들어 주고 있다니. 사람이 늘면 이건 또 다른 부담이 되겠는데.'*

이 비효율을 풀어 주는 방법이 있었습니다. 자주 입는 옷을 옷장 깊숙이 넣지 않고 옷걸이에 따로 걸어 두면 다음 날 그 옷걸이에서 바로 꺼낼 수 있습니다. 자주 나가는 응답을 NGINX 앞단에 저장해 두고 같은 요청이 다시 들어오면 백엔드까지 가지 않고 그 자리에서 돌려주는 방식을 **캐싱(Caching)** 이라고 부릅니다.
...
새로고침을 한 번 더 눌렀습니다. 같은 자리에 찍히는 값이 이번에는 **HIT**로 바뀌어 있었습니다. 두 번째 요청은 백엔드까지 가지 않고, NGINX가 캐시에 저장해 둔 응답을 돌려준 결과입니다.
...
오픈이는 의자를 끌어당기며 화면을 같이 들여다봤습니다. 동료가 로그인 버튼을 눌렀습니다. 환영 화면이 한 번 떴고, 메뉴를 한 번 누르자 로그인 페이지로 다시 떨어졌습니다. 한 번 더 눌러도 마찬가지였습니다.

*'로그인은 됐는데 왜 다음 페이지에서 풀려 버리지.'*

원인은 로그인 기록이 처음 들어간 서버에만 남아 있어서였습니다. 사용자가 로그인하면 **세션**은 그 요청을 받은 서버에 저장됩니다. 서버 한 대일 땐 문제가 안 보이지만, 두 대가 되면 1번 서버에 저장된 세션을 2번 서버는 알 수 없습니다.

:::term-box
**세션(Session)**: 사용자가 로그인했을 때 서버가 메모리에 보관하는 임시 인증 기록입니다.
:::
...
<svg viewBox="0 0 720 270" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="세션 불일치 — 서버마다 따로 보관해서 생기는 문제">
  ...
  <text x="210" y="135" text-anchor="middle" font-size="13" font-weight="700" fill="#0f172a">NGINX (LB)</text>
  <text x="210" y="155" text-anchor="middle" font-size="11" fill="#6b7280">라운드 로빈</text>
...
<svg viewBox="0 0 720 280" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Redis로 세션 공유 — LB가 분배해도 같은 Redis를 조회">
  ...
  <text x="205" y="150" text-anchor="middle" font-size="13" font-weight="700" fill="#0f172a">NGINX (LB)</text>
  <text x="205" y="170" text-anchor="middle" font-size="11" fill="#6b7280">라운드 로빈</text>
...
1번 서버가 로그인 정보를 Redis에 저장하고, 다음 요청을 받은 2번 서버는 자기 메모리 대신 Redis를 조회합니다. 같은 사용자라는 사실이 확인되어 사용자는 한 번 로그인하면 끊기지 않습니다.
...
오픈이는 ex04 폴더를 열었습니다. Redis는 공식 이미지를 그대로 쓰고, 직접 만들 이미지는 API 서버 한 대뿐이었습니다.
...
`app.py`는 두 개의 경로를 들고 있는 서버입니다. `/save`로 들어오면 값을 Redis에 적어 두고, `/read`로 들어오면 그 값을 다시 꺼내 응답합니다.
...
사용자 정의 네트워크를 만든 뒤 세 컨테이너를 같은 네트워크에 묶어 차례로 실행합니다.
...
두 서버가 같은 Redis를 공유하는지 확인합니다. `localhost:5001/save`로 값을 저장한 뒤 `localhost:5002/read`로 같은 값을 조회합니다.
...
한 서버에 저장한 값이 다른 서버에서 그대로 조회됩니다. 두 서버가 같은 Redis를 본다는 신호입니다.

자리로 돌아온 동료에게 화면을 보여 줬습니다.

**오픈이**: "이제 한 번 로그인하면 다음 페이지에서도 풀리지 않아요. Redis라는 공용 저장소를 두니까 두 서버가 같은 세션을 조회해요."

**동료**: "아, 그래서 다시 로그인 화면이 안 뜨는 거네요."

각 서버가 따로 보관하던 세션을 Redis 한 곳에 모은 결과, 로그인이 더는 끊기지 않습니다.
...
오래 남아야 하는 데이터에는 따로 데이터베이스 서버가 필요합니다. 회원 정보 저장소로 MySQL을 선택하고, 컨테이너로 실행해 보겠습니다.

:::term-box
**MySQL**: 표 형태로 데이터를 보관하는 관계형 데이터베이스(RDBMS)입니다. 회원 정보·게시글처럼 영구적으로 남아야 하는 데이터를 저장할 때 주로 사용합니다. 도커 공식 이미지로 제공되어 컨테이너로 손쉽게 실행할 수 있습니다.
:::
...
Dockerfile에서 눈여겨볼 지시어는 두 가지입니다.
...
빌드 도중 환경 변수에 민감한 정보를 넣지 말라는 안내가 한 줄 뜹니다. 위 `:::note`에서 짚은 사항입니다. 이번에는 학습용으로 그대로 두고, 컨테이너가 정상적으로 떴는지 확인합니다. `docker ps`로 컨테이너 ID를 확인한 뒤 `docker exec`로 내부에 접속합니다.
...
컨테이너 안에서 MySQL 클라이언트를 실행합니다. 접속 정보는 Dockerfile에 적어 둔 값 그대로 입력합니다.
...
접속 후 DB 목록, 테이블, 그 안의 초기 데이터를 차례로 조회합니다.
...
수동 세팅이 피곤해서 Dockerfile을 손에 익혔습니다. 그런데 그 Dockerfile로 만든 이미지를 하나씩 실행하다 보니 결국 또 다른 반복 작업에 갇혀 있었습니다.
...
검색 끝에 마주친 도구가 Docker Compose입니다. 컨테이너마다 따로 명령을 치는 대신, 한 파일에 모든 컨테이너 정의를 적어 두고 한 번에 실행하는 도구입니다.
...
설정 파일의 뼈대는 단순합니다. 자주 쓰이는 옵션을 정리해 보겠습니다.
...
연습용으로는 앞에서 손에 익힌 ex01을 그대로 다시 짜는 게 가장 빨라 보였습니다. Dockerfile은 그대로 두고, 세 컨테이너를 묶을 `docker-compose.yml`만 새로 적었습니다.
...
빌드부터 실행까지 한 줄로 끝납니다. 브라우저에서 본 결과 화면은 ex01과 같지만, 거기까지 가는 명령어가 한 줄로 줄어듭니다.

*'화면 하나 만들려고 명령어 다섯 줄, 여섯 줄씩 치던 게 결국 명령 하나로 끝나네.'*
...
여기까지 컨테이너 한 대씩 실행하는 법과 Compose로 묶는 법을 따로따로 손에 익혔습니다. 이제 지금까지 익힌 것을 모아 프론트엔드·백엔드·DB 세 컨테이너로 구성된 통합 사이트를 만들어 보겠습니다. `docker compose up` 한 줄로 셋이 같이 올라가는 게 목표입니다.
...
ex07 폴더를 열었습니다. 백엔드, DB, 프론트엔드가 각자 하위 폴더에 들어 있고, 그 위에 `docker-compose.yml`이 있었습니다.
...
- **Spring Boot**: 자바로 만든 백엔드 프레임워크. API 서버를 빠르게 실행할 때 자주 쓰입니다.
...
backend 폴더를 펼치자 눈에 띄는 점이 있었습니다. Spring Boot 서버라고 적혀 있는데도 폴더 안에 자바 소스 파일이 하나도 없습니다. 대신 `entrypoint.sh`라는 셸 스크립트가 하나 들어 있습니다.

이 스크립트는 컨테이너가 뜨는 그 순간 깃허브에서 소스를 받아 와서, 그 자리에서 빌드하고 실행하도록 짜여 있습니다. 한 번 적어 두면 누가 어디서 컨테이너를 실행해도 같은 모양의 백엔드가 떨어집니다.
...
백엔드 서버 자체는 단순합니다. `/api/users` 요청이 오면 DB에서 회원 목록을 꺼내 JSON으로 돌려주는 컨트롤러 한 개입니다.

*'이미지 안에 소스 가져오는 절차를 같이 묶어 두니, 환경에 상관없이 코드 한 곳만 잡혀 있으면 같은 백엔드가 그대로 뜨네.'*

### 3.6.3 DB - 3.4와 동일한 MySQL 컨테이너

DB는 앞서 만진 MySQL 구성을 그대로 가져옵니다. 이미지도 `init.sql`도 손댈 데가 없습니다.
...
핵심은 백엔드 주소를 IP가 아니라 `backend:8080`처럼 서비스 이름으로 적는다는 점입니다. Compose가 자동으로 만드는 네트워크에서 서비스 이름이 그대로 호스트명으로 통합니다.

### 3.6.5 docker-compose.yml

마지막으로 셋을 묶는 설정 파일을 적습니다.
...
엔터 한 번에 빌드 로그가 위로 줄줄이 올라갔습니다. 백엔드는 첫 실행이라 깃허브에서 소스를 받고 Gradle 빌드 시간이 더 걸립니다. 빌드가 끝나길 기다렸다가 브라우저에 `localhost:80`을 입력했습니다.

![](../assets/CH03/chap02-58.png)

*그림 3-37. 사용자 목록 조회 성공*

화면에 회원 목록이 떴습니다. NGINX가 받아 백엔드로 넘기고, 백엔드가 DB에서 꺼낸 데이터가 그대로 올라온 결과입니다.

다음 날 회의 시간이 다가왔습니다. 회의실에서 노트북을 펼치고 빔 프로젝터에 연결했습니다. 팀장과 동료가 앉았습니다.

**오픈이**: "한 번에 셋이 같이 올라오는지 보시면 돼요."

엔터를 눌렀습니다. 빔에 컨테이너 세 개의 빌드 로그가 줄줄이 올라갔습니다. 실행이 완료되자 브라우저에 회원 목록이 조회되었습니다.

**팀장**: "환경 구성은 깔끔하게 됐네요. 그런데 운영 서버에 올린 다음 새벽에 컨테이너가 한 대라도 죽으면 누가 살리죠?"

회의실을 나오는 길은 가볍지만은 않았습니다. 팀장의 마지막 물음에 마땅한 답이 떠오르지 않았기 때문입니다. 도커로 여기까지 온 길은 결국 컨테이너를 실행하는 단계까지였습니다. 한 번에 같이 실행할 수는 있게 됐지만, 그 사이에 무엇이 죽고 살아나는지를 챙겨 줄 도구는 없었습니다.

*'새벽에 컨테이너가 죽으면, 누가 알아서 살려 줄 수는 없을까.'*
...
- **Redis는 여러 서버가 함께 사용하는 메모리 저장소입니다.** 서버가 여러 대로 늘어도 세션을 외부 저장소에 두면 로그인 상태가 흩어지지 않습니다.
...
자동 복구와 무중단 배포, 다중 서버 관리, 설정 분리까지 운영에서 마주칠 이 숙제들을 한 묶음으로 받쳐 주는 도구가 다음 챕터의 주인공 **쿠버네티스(Kubernetes)** 입니다.
:::
