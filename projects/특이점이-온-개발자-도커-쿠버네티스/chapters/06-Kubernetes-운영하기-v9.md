...
*그림 6-5. 환경 변수 목록에서 확인한 Secret의 값*
...
*그림 6-6. ConfigMap 적용 직후 기존 Pod의 환경 변수 조회 결과*

ConfigMap은 갱신됐지만, 이미 실행 중인 Pod의 환경 변수는 여전히 80이었습니다.
...
*그림 6-7. apply 만 한 경우 - ConfigMap 변경이 기존 Pod에는 반영되지 않음*
...
*그림 6-8. rollout restart 로 Pod를 새로 실행하면 ConfigMap 새 값이 환경 변수로 반영됨*
...
*그림 6-9. Pod 재시작 후 환경 변수에 새 포트 90이 반영된 모습*

재시작 후 환경 변수가 적용된 것을 확인할 수 있습니다.

:::tip
**apply했는데 반영되는 경우 vs 안 되는 경우**

ConfigMap이나 Secret의 설정 값만 변경하면 기존 Pod에는 새 값이 반영되지 않습니다. 반면, Deployment의 Pod 템플릿(template)을 변경하면 곧바로 적용됩니다. 이때 새 값을 반영하기 위해 기존 Pod가 교체되므로, Pod의 이름이 변경됩니다.
:::
...
*그림 6-10. PV는 실제 저장 공간, PVC는 그 공간을 요청하는 신청서*
...
컨테이너가 보는 마운트 경로는 `/data`이고, Minikube 내부의 실제 저장 경로는 `/mnt/data`입니다. 컨테이너가 `/data`에 파일을 쓰면 PVC와 PV를 거쳐 최종적으로 Minikube의 `/mnt/data`에 저장됩니다.
...
*그림 6-11. PV와 PVC가 Bound 상태로 연결된 결과*
...
*그림 6-12. Pod가 새로 태어났는데도 c.txt가 그대로 남아 있는 모습*
...
*그림 6-13. ex15 Kubernetes 웹사이트의 전체 구성*
...
*그림 6-14. 같은 클러스터 안에서 Namespace가 리소스를 층처럼 분리*
...
*그림 6-15. Nginx Ingress 컨트롤러 애드온 설치*
...
프론트엔드와 데이터베이스 Pod는 빠르게 Running 상태로 바뀝니다. 백엔드 Pod만 Gradle 빌드와 의존성 설치 때문에 시간이 소요됩니다.
...
*그림 6-16. Ingress를 거쳐 웹사이트가 화면에 응답*
...
- **설정과 비밀번호는 이미지 외부에 둡니다.** ConfigMap에는 환경마다 달라지는 일반 설정값을, Secret에는 비밀번호·토큰 같은 민감 정보를 두고 `envFrom`으로 한꺼번에 주입합니다.
...
- **데이터는 PV·PVC로 Pod 외부에 두어 보존합니다.** Pod는 휘발성이라 안에서 만든 파일은 함께 사라지지만, PVC가 가리키는 PV에 데이터를 두면 Pod가 새로 태어나도 그대로 남습니다.
- **이름 하나로 한 시스템이 됩니다.** Service 이름만 맞으면 그 뒤 Pod까지 자동으로 연결됩니다. 여러 리소스가 한꺼번에 올라가도 IP를 알 필요 없이 이름만으로 흐름이 이어집니다.
:::

### 마치며

이 책은 로컬 환경과 운영 서버의 차이에서 오는 답답함에서 출발했습니다. 코드를 실행 환경과 함께 컨테이너에 담아 어디서든 똑같이 동작하게 만들고, 늘어난 컨테이너들을 쿠버네티스 클러스터 위에 올려 스스로 유지되도록 구성하는 과정까지 차근차근 살펴보았습니다.

이제 책을 덮고 현업으로 돌아가면, 여기서 다룬 예제보다 훨씬 거대하고 복잡한 시스템을 마주하게 됩니다. 수많은 Pod와 낯선 설정 파일들이 얽혀 있는 모습에 다시금 막막함이 앞설 수도 있습니다.
...
앞으로 마주할 문제들 앞에서, 이 책에서 함께 그려 본 밑그림이 단단한 기준점이 되어 주기를 기대합니다.

### 에필로그

오픈이는 화면을 새로고침했습니다. 방문 횟수가 오르고, 사이트는 문제없이 응답합니다. 프론트엔드와 백엔드, 데이터베이스와 Redis가 클러스터 위에서 서로 이름으로 연결되어 통신합니다. 겉보기엔 하나의 웹사이트지만, 안에서는 네 개의 서비스가 저마다 따로 돌아갑니다.

처음에는 서비스 하나를 올리는 것조차 버거웠지만, 이제는 여러 서비스를 클러스터에 배포하고 전체 시스템이 어떻게 맞물려 돌아가는지 한눈에 들어옵니다.

오픈이의 도커와 쿠버네티스 학습은 여기까지입니다.
