...
**선배**: "테스트 환경에서는 괜찮지만, 실제 운영 환경에 배포할 때는 두 가지를 주의해야 해요. 먼저 Dockerfile에 비밀번호 같은 설정값을 직접 적지 말고, **이미지 밖으로 분리**해서 따로 관리해야 해요.<br>그리고 Pod는 언제든 종료될 수 있어서 안에 쌓인 데이터도 함께 날아가니, 유지해야 할 데이터는 **Pod 외부의 저장소**에 보관해야 해요. 설정과 데이터 모두 컨테이너 밖으로 안전하게 빼 두는 게 핵심이죠."

:::goal
**이번 챕터가 끝나면**

- 설정값과 비밀번호를 이미지 바깥으로 분리합니다
- Pod가 죽어도 데이터가 사라지지 않게 영속화합니다
- 리소스를 논리적으로 구분하는 방법을 익힙니다
- 앞서 만든 통합 사이트를 쿠버네티스 위에 올려 운영 환경에 가까운 시스템을 만듭니다
:::
...
그래서 쿠버네티스는 컨테이너 이미지와 환경 변수를 분리합니다. 일반 설정값은 **ConfigMap**에, 민감한 정보는 **Secret**에 담아 외부에 따로 보관하는 방식입니다.
...
코드를 살펴봤으니 배포할 차례입니다. 본격적으로 배포하기 전에 Minikube와 Ingress 컨트롤러를 켜고, 네 가지 이미지를 빌드해 둡니다.
...
*그림 6-14. Nginx Ingress 컨트롤러 애드온 설치*
