...
*그림 6-12. Pod가 새로 생성됐는데도 c.txt가 그대로 남아 있는 모습*
...
*그림 6-14. 같은 클러스터 안에서 Namespace가 리소스를 논리적으로 분리*
...
며칠 후, 오픈이는 지금까지 배운 내용을 자신의 운영 환경에도 그대로 적용했습니다. 예전에는 배포한 날이면 혹시 몰라 자리를 뜨지 못했습니다. 컨테이너가 멈추면 개발자가 직접 살려내야 했습니다.

오늘은 다릅니다. 컨테이너 하나가 멈춰도 클러스터가 곧바로 새 컨테이너를 생성하고, 사이트는 멈추지 않습니다. 사이트를 지키는 일은 이제 시스템에 맡겨 두면 됩니다.

오픈이는 노트북을 덮고 가벼운 마음으로 사무실을 나섰습니다.
