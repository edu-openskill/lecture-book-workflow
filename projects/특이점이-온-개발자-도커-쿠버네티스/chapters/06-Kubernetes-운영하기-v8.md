...
며칠 뒤였습니다. 이미지를 배포하기 전, 마지막으로 한 번 더 코드 리뷰를 하고 있었습니다. 리뷰 중 Dockerfile의 환경 변수 코드가 눈에 띄었습니다.
...
새 Pod 안에서도 `c.txt`가 그대로 보였습니다. Pod는 사라졌다 다시 떴지만, PV 안 파일은 그대로였습니다.
...
backend 폴더에서 가장 손이 많이 간 매니페스트는 `backend-deploy.yml`입니다. 앞에서 따로따로 다룬 `replicas`와 ConfigMap·Secret이 한 매니페스트에서 만납니다.
...
db 폴더의 `db-deploy.yml`은 앞서 단일 Pod에 붙여 본 PVC를 Deployment 한가운데로 끌어들인 형태입니다.
...
로그를 들여다보니 안에서 Gradle 빌드가 한창이었습니다. 잠시 기다리자 모든 Pod가 Running 상태로 바뀌었고, 오픈이는 외부 접근을 위한 통로를 열었습니다. 앞에서처럼 별도 터미널에서 `minikube tunnel`을 실행하면 Ingress 입구가 `localhost`로 이어집니다.
...
오픈이는 고개를 끄덕였습니다. 같은 사이트지만 운영 환경이 달라졌습니다. docker-compose 한 파일 안에 묶여 있던 세 덩어리가, 이제는 클러스터 위에서 각자 Pod로 떠 있고 Service 이름으로 서로를 호출하고 있었습니다.
