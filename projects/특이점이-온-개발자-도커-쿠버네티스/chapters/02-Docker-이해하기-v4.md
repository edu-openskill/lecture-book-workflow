...
토요일 아침. 어제의 그 빨간 글씨 석 줄이 아직 머릿속에 남아 있었습니다. 런타임 버전, 시스템 패키지, 설정 경로. 어제 지하철에서 적어 둔 노트 한 장은 책상 위에 그대로 펼쳐져 있었고, '컨테이너'와 'Docker'와 'Kubernetes' 세 단어 옆에는 화살표 하나가 그어져 있었습니다.

오픈이는 커피 한 잔을 들고 노트북 앞에 앉았습니다. 오늘의 목표는 한 줄이었습니다.

**Docker가 무엇이고 어떻게 돌아가는지, 개념과 기본 사용법을 손에 익힌다.**

오늘 안에 다 마칠 수 있을지는 자신이 없었지만 어디까지 갈 수 있는지는 보고 싶었습니다. 마우스를 설치 버튼 위로 가져갔다가 멈췄습니다.

*'명령어부터 치면 그냥 외우게 될 텐데. 원리부터 보자.'*

뒤에 나올 명령어들을 기계적으로 입력하지 않으려면 이 도구가 환경을 어떤 방식으로 다루는지부터 잡아 둬야 했습니다.

## 2.1 가상화 — 가상머신과 컨테이너

### 2.1.1 주방 네 개 대신 주방 하나

오픈이는 컨테이너의 원리부터 정리하기로 했습니다. 자료를 한참 읽다가, 비유 하나를 떠올리면 머리에 남는다는 평소 습관대로 일상의 장면을 끌어와 봤습니다. 머리에 떠오른 건 요리사 여러 명이 한 주방에 들어가는 그림이었습니다.
...
비유는 머리에 잘 남았지만 비유만 들고 있다가는 나중에 진짜 용어와 헷갈릴 게 분명했습니다. 오픈이는 화면을 새 페이지로 넘기고 주방 그림을 IT 구조로 다시 그려 봤습니다.
...
구조의 핵심은 **호스트 OS의 커널 하나를 모든 컨테이너가 공유**한다는 점입니다. VM처럼 OS를 통째로 따로 올리지 않으니 가볍고 빠르게 뜹니다. Docker 엔진이 그 커널에 "**새 격리 공간을 만들어 달라**"고 요청하면, 실제로 프로세스를 격리하는 일은 **커널**이 합니다. 그래서 다른 컨테이너가 다른 앱과 다른 라이브러리를 가져도 충돌하지 않고 한쪽이 런타임을 업그레이드해도 옆 컨테이너는 영향을 받지 않습니다.
...
컨테이너가 무엇인지는 잡혔습니다. 그런데 이 컨테이너가 **어떻게** 만들어지는지는 아직 그림이 흐릿했습니다. 명령 한 줄을 친 그 순간 무엇이 어떤 순서로 움직이는지를 한 번은 그려 둬야, 나중에 명령어를 칠 때 손이 따라가도 머릿속이 같이 따라갈 것 같았습니다.
...
여기서 중요한 점이 있습니다. 컨테이너를 실제로 만드는 건 Docker 엔진이 아니라 커널입니다. 커널이 주방장이라면, Docker 엔진은 홀의 프런트 직원에 가깝습니다. 새 주문이 들어오면 프런트가 주방장에게 그대로 넘깁니다. 실제로 새 요리사(프로세스)를 불러 작업을 시작시키는 건 주방장이 합니다.
...
원리를 알았으니 이제 Docker를 설치하고 직접 띄워 보겠습니다.
...
가장 먼저 오픈이는 Docker Hub에서 이미지를 내려받았습니다.
...
컨테이너 안에서 앱이 돌아도 외부에서 접속할 수 없으면 쓸모가 없습니다. 격리된 프로세스라는 개념은 익혔지만, 이 컨테이너가 바깥과 어떻게 연결되는지를 모르면 안에서 도는 앱에 접근할 수 없습니다. 여기서 컨테이너가 어떻게 통신하는지 큰 그림을 잡아보겠습니다.

### 2.5.1 네트워크 한눈에 보기

오픈이는 방금 띄운 nginx가 어떻게 묶여 있는지부터 그려보고 싶었습니다. 격리된 공간이라고 해놓고 실제로는 무엇과 무엇이 연결돼 있는지를 알아야, 다음 명령을 칠 때 손이 덜 막힐 것 같았습니다.

자료를 훑어보니 컨테이너 네트워크는 크게 세 갈래로 정리되어 있습니다. **컨테이너끼리 연결하는 방법**, **외부 요청이 컨테이너로 들어오는 길**, **이름으로 컨테이너를 찾는 방법**입니다.
...
docker0는 도커가 호스트에 기본으로 만들어 두는 **가상 스위치**입니다. 이 위에서 동작하는 네트워크를 **기본 네트워크**라고 부릅니다. 공유기가 집안 기기를 한 네트워크로 묶어주듯, docker0도 같은 호스트의 컨테이너들을 이 기본 네트워크로 묶어둡니다.
...
새 컨테이너가 뜰 때마다 이름과 IP가 자동으로 Docker DNS에 등록됩니다. 그다음 `app` 컨테이너가 `db`라는 이름으로 요청을 보내면 Docker DNS가 먼저 받아 `db`를 `172.17.0.3`으로 변환해 알려주고 app은 그 IP로 다시 요청을 보냅니다. 컨테이너가 다시 떠서 IP가 바뀌어도 이름은 그대로라 코드를 수정할 필요가 없습니다.
...
안으로 들어왔지만 당장 무엇부터 해야 할지 막막했습니다. 명령어를 모르면 실습 도중에 매번 검색을 하게 되고, 그 사이에 흐름이 끊깁니다. 오픈이는 본격적으로 손을 움직이기 전에 앞으로 자주 쓸 리눅스 명령어부터 한 표로 정리해뒀습니다.
...
기본 명령어를 손에 익힌 오픈이는, 앞서 도커 이미지로 받아서 실행했던 nginx를 이번엔 직접 설치해 보기로 했습니다. 이미지로 받을 땐 한 줄로 끝났던 과정을 손으로 한 단계씩 밟아보면, 앞서 배운 이미지가 무엇을 담고 있었는지가 거꾸로 보입니다.
...
설치 로그가 마무리되며 Nginx 설치가 끝났습니다. 오픈이는 설정이 제대로 반영됐는지 확인하기 위해 현재 활성화된 포트를 점검해 보기로 했습니다. `net-tools` 패키지를 추가로 설치한 뒤, 명령어를 입력해 Nginx가 네트워크상에서 정상적으로 대기 중인지 확인했습니다.
...
그러고 보니 아까 깔았던 nginx의 기본 페이지는 어디에 놓여 있을까 궁금해졌습니다. 오픈이는 `find`로 `index.html` 파일의 위치를 찾아봤습니다.
...
오픈이는 같은 상황을 한 번 더 만들어 비교해보기로 했습니다. 이름을 붙여 띄운 뒤 `exit`로 나와봤습니다.
...
오픈이는 `exit` 말고 다른 방식이 있는지 찾아봤습니다. `CTRL+P → CTRL+Q`를 쓰면 컨테이너는 그대로 두고 쉘만 빠져나올 수 있습니다. 같은 방식으로 한 번 더 띄우고 이 조합으로 빠져나왔습니다.
...
실행 중인 프로세스 목록에 ubuntu가 없었습니다. 컨테이너가 실행되자마자 바로 종료되었기 때문입니다. 반면 이전에 실습했던 nginx는 똑같은 -d 옵션으로도 잘 돌아갔습니다.

오픈이는 그 원인을 찾다 **메인 프로세스**의 개념을 알게 되었습니다. 도커 컨테이너는 시작과 동시에 실행되는 메인 프로세스가 종료되면 컨테이너도 함께 종료됩니다. 이 관점에서 두 이미지를 비교해 보니 차이가 명확했습니다.
...
ubuntu를 백그라운드로 살리려면 `-dit`(`-d` 백그라운드, `-i` 사용자와 상호작용, `-t` 터미널 화면)을 쓰거나 메인 프로세스를 다른 걸로 바꿔야 합니다.
...
오픈이는 백그라운드로 띄운 컨테이너에 다시 들어가는 방법이 궁금해졌습니다. 자료를 살펴보니 두 갈래가 있었습니다. 메인 프로세스에 직접 연결하는 `attach`와, 새 프로세스를 하나 더 띄우는 `exec`입니다. 결과는 비슷해 보이지만 내부에서 일어나는 일이 다르다고 해서, 두 방법을 직접 비교해봤습니다.
...
오픈이는 먼저 ubuntu를 백그라운드로 띄우고, 그 컨테이너에 `docker attach`로 접속했습니다.
...
ubuntu의 입력 터미널이 나타났습니다. `attach`는 **메인 프로세스**에 직접 연결되므로, 여러 터미널에서 붙어도 모두 같은 화면을 공유합니다. 다만 이 상태에서 잘못 빠져나와 메인 프로세스를 종료하면 컨테이너 전체가 꺼집니다. 오픈이는 `CTRL+P → CTRL+Q`로 안전하게 빠져나왔습니다.
...
여기까지 공부하고 나니 컨테이너의 생명 주기는 더는 헷갈리지 않았습니다. 오픈이는 의자에서 일어나 잠깐 어깨를 풀고, 다시 노트북 앞에 앉으면서 한 가지가 마음에 걸리는 걸 느꼈습니다. 앞서 Ubuntu 컨테이너 안에 패키지를 깔고 파일도 몇 개 만들어 두었습니다. 컨테이너를 내리는 순간 그 작업이 다 사라진다면 다음에 똑같은 걸 또 깔아야 합니다.
