...
각 단계마다 비유로 먼저 풀고, 곧이어 같은 단계의 IT 구조를 짝지어 봅니다.
...
| 본사 자동 안내 | **kube-proxy(1차)** | 노드 커널의 iptables 규칙을 따라 들어온 요청을 Ingress Controller로 자동 연결하는 라우터입니다 |
| 본사 콜센터 | **Ingress Controller** | 다음 단계에서 URL을 읽는 컨트롤러 Pod입니다 |

브라우저에서 `http://localhost/order` 같은 주소로 요청을 보내면, 노드에 뚫린 NodePort(또는 클라우드의 LoadBalancer)를 통해 클러스터 안으로 들어옵니다. 5.2.3에서 띄워 둔 `minikube tunnel` 이 바로 이 입구를 호스트와 이어 주는 통로입니다. 그 직후 같은 노드의 kube-proxy가 미리 등록해 둔 iptables 항목을 따라 요청을 Ingress Controller로 전달합니다.
...
<svg viewBox="0 0 800 280" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="1단계 IT — 외부 요청이 NodePort + kube-proxy(1차)를 거쳐 Ingress Controller에 도달">
  ...
  <text x="167" y="150" text-anchor="middle" font-size="10" font-family="monospace" font-weight="700" fill="#7b341e">localhost:9000</text>
  ...
  <text x="280" y="170" text-anchor="middle" font-size="11" font-family="monospace" fill="#7b341e">:9000</text>
...
이 모든 게 가능한 건 **Endpoint Controller**가 뒤에서 받쳐 주기 때문입니다. Service의 selector(라벨)에 매칭되는 Pod가 새로 뜨거나 죽는 즉시 감지해 Pod IP 목록을 갱신하므로, kube-proxy(2차)가 항상 살아있는 Pod에만 연결됩니다. 만약 이 갱신이 늦어지면 죽은 Pod IP로 트래픽이 흘러가 요청이 실패할 수 있습니다.
