...
잠깐 당황했지만, 이 명령은 터미널을 계속 붙잡고 있어야 연결이 유지되는 방식이라는 걸 깨달았습니다. 생성된 URL을 복사해 브라우저에 입력하자, 드디어 기다리던 NGINX 화면이 나타났습니다.
...
가맹점이 늘어나면 고객이 직통 번호를 일일이 외우기 어렵습니다. 본사가 외부 손님 전화를 받는 **콜센터**를 두면, 손님은 대표번호 한 곳으로 전화해도 적절한 가맹점으로 알아서 연결됩니다.

Ingress가 이 본사 콜센터 역할을 합니다. 고객이 `http://도메인/order`로 접속하면 주문 Service로, `http://도메인/stores`로 접속하면 매장 Service로 나눠 보냅니다.
...
|:---------:|:-----|:-----|:---------------|
| `Ingress 리소스` | 어떤 도메인과 URL 경로의 요청을 어떤 Service로 보낼지 정의한 규칙 (YAML) | 본사 콜센터의 연결 규칙 | `선언` |
| `Ingress Controller` | 실제로 외부 요청을 받아 처리하는 소프트웨어 | 연결 규칙을 실행하는 본사 콜센터 | `집행` |

*'아, 규칙만 적어 둔다고 알아서 굴러가는 게 아니구나. 그 규칙을 읽고 실제로 실행해 주는 역할이 따로 필요한 거네.'*

이제 오픈이가 실습으로 풀어 갈 순서가 머릿속에 그려졌습니다. 아직 없는 **본사 콜센터(Ingress Controller)** 부터 실행하고, 그 콜센터가 연결해 줄 백엔드 두 개를 띄운 다음, 라우팅 규칙을 YAML로 적어 길을 안내하고, 마지막으로 브라우저로 두 경로가 각자 다른 가맹점에 닿는지 확인하는 흐름이었습니다.
...
*그림 5-13. minikube에서 ingress 애드온 활성화*
...
*그림 5-14. Ingress Controller가 Running 상태*

:::note
**운영 환경에서의 Ingress Controller 실행**

미니큐브 addon은 학습용 명령어입니다. 실제 운영에서는 `ingress-nginx` 같은 컨트롤러를 직접 설치하거나, 클라우드 환경에서는 클라우드가 제공하는 컨트롤러를 그대로 쓰면 됩니다. 어느 쪽이든 클러스터 안에 컨트롤러 Pod가 떠서 외부 요청을 갈라 주는 모습은 똑같습니다.
:::

#### 두 서비스 준비

`/order`와 `/stores`로 갈라 받을 백엔드 두 개를 띄웁니다. 둘은 같은 이미지를 쓰고 응답 문구만 다릅니다. 이미지는 `hashicorp/http-echo` 로, 받은 요청에 `-text` 옵션의 문자열을 그대로 응답합니다.
...
*그림 5-15. Ingress 리소스 등록 확인 — ex12-ingress*
...
*그림 5-16. minikube tunnel 실행으로 외부 접근 경로 확보*
...
본사 콜센터 안에서 손님 요청이 어떻게 처리되는지 흐름을 따라가 봅니다. 흐름은 크게 세 단계로 나뉩니다.

1. **진입** — 외부 손님 전화가 본사 콜센터로 이어지는 단계
2. **부서 결정** — 콜센터 상담원이 어느 부서로 보낼지 결정하는 단계
3. **직원 응답** — 부서 안 자동 분배가 응대 직원에게 연결되어 응답하는 단계

각 단계마다 비유로 먼저 풀고, 곧이어 같은 단계의 IT 구조를 짝지어 봅니다. 흐름 안에서 **kube-proxy** 가 두 번 등장합니다. 같은 프로그램이 노드 안에서 두 시점(NodePort 진입 직후·ClusterIP 처리 시점)에 같은 일(iptables DNAT)을 합니다.
...
<svg viewBox="0 0 1080 600" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="요청이 Pod까지 닿는 전체 흐름 — 세 단계가 클러스터 안에서 일어나는 위치">
  ...
  <text x="850" y="283" text-anchor="middle" font-size="11" font-weight="700" fill="#7b341e">return-service</text>
...
**1단계 — 진입: 손님이 본사 대표번호로 전화, 자동 안내가 콜센터로 연결**

외부 손님이 본사 **대표번호**로 전화를 겁니다. **자동 안내**가 곧장 본사 **콜센터**로 연결해 줍니다.
...
| 비유 | IT 용어 | 한 줄 설명 |
|:---:|:---|:---|
| 외부 손님 | **외부 호스트** | 클러스터 바깥에서 요청을 보내는 사용자입니다 |
| 본사 대표번호 | **NodePort** | 외부 요청이 클러스터로 진입하는 공개 포트입니다 |
| 본사 자동 안내 | **kube-proxy(1차)** | 노드 커널의 iptables 규칙으로 들어온 요청을 NodePort Service의 backend Pod(여기서는 Ingress Controller Pod)로 전달합니다 |
| 본사 콜센터 | **Ingress Controller** | 다음 단계에서 URL을 읽는 컨트롤러 Pod입니다 |

브라우저에서 `http://localhost/order` 로 요청을 보냅니다. 요청은 노드에 뚫린 NodePort(클라우드라면 LoadBalancer)를 통해 클러스터 안으로 들어옵니다. 5.2.3에서 띄워 둔 `minikube tunnel` 이 이 입구를 호스트와 이어 주는 통로입니다. 그 직후 같은 노드의 kube-proxy가 iptables 규칙으로 요청을 NodePort Service의 backend Pod인 Ingress Controller로 DNAT합니다.
...
**2단계 — 부서 결정: 콜센터 상담원이 부서 명부 보고 부서 대표 내선 알아냄**

콜센터에 도착한 손님 전화를 **상담원**이 받습니다. 손님 용건을 듣고 "**주문 관련이네, 주문 부서로 보내야겠다**"라고 판단합니다. 사내 **부서 명부**(부서 이름 → 대표 내선)를 펼쳐 주문 부서의 대표 내선(예: 1234번)을 확인한 뒤, 그 내선으로 전화를 돌려줍니다.
...
| 비유 | IT 용어 | 한 줄 설명 |
|:---:|:---|:---|
| 콜센터 상담원 | **Ingress Controller** | URL 경로와 도메인 헤더로 어느 백엔드 Service에 보낼지 정합니다 |
| 부서 명부 (이름 → 내선) | **클러스터 DNS (CoreDNS)** | Service 이름을 ClusterIP로 풀어 줍니다 |
| 부서 대표 내선 | **ClusterIP** | Service의 가상 진입 IP입니다 |
...
`nginx-ingress` 같은 실제 컨트롤러는 DNS 조회 대신 **API Server를 watch** 해서 Pod IP 목록을 직접 들고 있다가 곧바로 보내는 경우가 많습니다. DNS 조회는 일반 Pod끼리 Service 이름으로 부를 때 더 자주 쓰입니다.
:::
...
<svg viewBox="0 0 800 260" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="2단계 IT — Ingress Controller가 DNS 조회로 결정된 Service의 ClusterIP로 전송">
  ...
  <text x="677" y="148" text-anchor="middle" font-size="10" font-weight="600" fill="#94a3b8">return-service (10.96.0.21)</text>
...
**3단계 — 직원 응답: 부서 자동 분배가 응대 직원 한 명에게 연결, 부서 매니저가 응대 직원 명단 갱신**

부서 대표 내선으로 전화가 들어오면, 부서 안 **자동 전화 분배**가 받아 **응대 가능한 직원** 한 명에게 자동으로 연결합니다. 직원이 응답합니다.

이 분배가 정확하려면 누군가가 "**지금 응대 가능한 직원이 누구인지**" 명단을 늘 챙겨야 합니다. 그 역할이 **부서 매니저**입니다. 매니저가 팀 **근태 관리**(출퇴근·휴가)로 가용 인력 명단을 정리해, 자동 분배가 살아있는 직원에게만 전화를 돌리도록 받쳐 줍니다.
...
*그림 5-24. 부서 매니저가 응대 직원 명단을 갱신, 자동 분배가 명단 보고 응대 가능한 직원에게 전화 연결*

이 비유를 IT로 옮기면 네 가지가 짝지어집니다.

| 비유 | IT 용어 | 한 줄 설명 |
|:---:|:---|:---|
| 부서 자동 전화 분배 | **kube-proxy(2차)** | ClusterIP를 살아있는 Pod IP로 변환하는 L4 로드밸런서입니다 |
| 응대 직원 명단 | **Pod IP 목록 (EndpointSlice)** | Service에 묶인 살아있는 백엔드 Pod의 IP 목록입니다 |
| 부서 매니저 | **Endpoint Controller** | Pod 변동을 감지해 위 명단을 즉시 갱신하는 컨트롤 플레인 컴포넌트입니다 |
| 응대 직원 | **Pod** | 비즈니스 로직을 실행하는 백엔드 인스턴스입니다 |

ClusterIP로 향한 요청은 같은 노드의 kube-proxy(2차)가 가로채서, **Pod IP 목록(EndpointSlice)** 에서 살아있는 Pod 중 하나의 IP로 바꿔 보냅니다. 요청이 백엔드 Pod에 닿으면 애플리케이션이 비즈니스 로직을 실행해 응답을 돌려보냅니다.

이 모든 게 가능한 건 **Endpoint Controller**가 뒤에서 받쳐 주기 때문입니다. Service의 selector에 매칭되는 Pod가 새로 뜨거나 죽으면 즉시 감지해 Pod IP 목록을 갱신합니다. 그래야 kube-proxy(2차)가 항상 살아있는 Pod에만 트래픽을 보낼 수 있습니다. 갱신이 늦어지면 죽은 Pod IP로 트래픽이 흘러가 요청이 실패할 수 있습니다.
...
*그림 5-25. ClusterIP → kube-proxy(2차)가 Pod IP로 변환 → Pod 응답. Endpoint Controller가 Pod IP 목록 갱신*

:::note
**EC와 kube-proxy의 정확한 통신 — API Server가 매개**

엄밀히 말하면 Endpoint Controller가 kube-proxy에게 직접 명단을 주는 게 아닙니다. EC가 EndpointSlice를 갱신하면 그 결과는 **API Server에 저장**되고, 각 노드의 kube-proxy가 API Server를 **watch(구독)** 해서 변경 알림을 받습니다. 즉 두 컴포넌트 사이에 **API Server가 중간 매개체** 역할을 합니다. 비유 그림에서는 이 매개를 생략하고 매니저 ↔ 분배기 양방향 화살표로 단순화했습니다.
:::

세 단계를 한 표로 정리하면 다음과 같습니다.

| 단계 | 일하는 곳 | 진입점/프로그램 | 하는 일 | 의사결정 기준 |
|:-:|:--|:--|:--|:--|
| 1 진입 | 클러스터 입구 + 노드 | NodePort + kube-proxy(1차) | 외부 요청을 받아 Ingress Controller로 전달 | 노드 IP·포트, ClusterIP, Pod IP 목록 |
| 2 부서 결정 | Ingress Controller + DNS | Ingress Controller, 클러스터 DNS | URL 읽고 백엔드 Service 결정, 이름을 ClusterIP로 변환 | URL 경로, Host 헤더 → 이름 → ClusterIP |
| 3 직원 응답 | 백엔드 노드 + Pod | kube-proxy(2차) + Pod, Endpoint Controller | ClusterIP를 살아있는 Pod IP로 변환, Pod가 응답 | Service ClusterIP, Pod IP 목록 |

같은 "전달"처럼 보이지만 단계별로 보는 정보의 깊이가 다릅니다. NodePort와 kube-proxy는 봉투 겉면(IP·Port)만 보고 옮기고, Ingress Controller는 봉투를 열어 안의 내용(URL·Host)까지 읽고 어디로 보낼지 결정합니다.

:::note
**L4와 L7**

네트워크에서는 IP·Port 같은 봉투 겉면 정보를 다루는 계층을 **L4(전송 계층)** , URL 경로·헤더 같은 봉투 안쪽 정보를 다루는 계층을 **L7(애플리케이션 계층)** 이라고 부릅니다. NodePort와 kube-proxy는 L4 정보로 일하고, 앞서 본 DNS·Endpoint Controller도 L4 영역에 머무릅니다. 세 단계 중 HTTP 본문(L7)까지 들여다보는 프로그램은 Ingress Controller 하나뿐입니다.
:::
...
- **Ingress는 본사 콜센터 역할을 합니다.** 숫자(IP/Port)만 보는 Service와 달리, Ingress는 도메인과 URL 경로를 읽고 적절한 Service로 연결하는 라우팅을 담당합니다. 라우팅 규칙을 정의하는 **리소스(YAML)** 와 앱을 실제로 구동하는 **컨트롤러(S/W)** 가 한 팀으로 움직이며, Minikube에서는 `minikube addons enable ingress`로 컨트롤러를 먼저 실행해 두어야 동작합니다.
