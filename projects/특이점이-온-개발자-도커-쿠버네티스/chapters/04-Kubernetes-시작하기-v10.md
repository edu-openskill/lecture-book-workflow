...
*그림 4-4. 명령 단계 — 개발자의 명령이 Kube API Server에 도착하는 길*

**2. 판단 — 컨트롤 플레인의 구성 요소들이 협의해 결론을 좁힌다**
...
*그림 4-5. 판단 단계 — Kube API Server를 중심으로 구성 요소들이 협의하는 모습*

세 구성 요소는 Kube API Server를 통해 서로 상태를 주고받으며 **이 명령을 어떻게 처리할지 결정합니다**.

**3. 지시 — 컨트롤 플레인이 워커 노드 kubelet에 지시한다**
...
*그림 4-7. Kubernetes 핵심 리소스의 전체 구조*
...
다만 노드가 하나뿐입니다. 그 노드 안에 컨트롤 플레인과 워커 노드가 모두 들어 있습니다. 프랜차이즈로 치면 본점밖에 없는 구조입니다.
...
*그림 4-8. Minikube는 단일 노드 안에 컨트롤 플레인과 워커 노드 기능이 함께 들어간 구조*

노드가 하나라 구조가 가볍고 리소스도 적게 먹습니다. Minikube에서 작성한 설정 파일은 실제 운영 환경에서도 그대로 사용할 수 있습니다.

:::term-box
**Minikube**: Mini + Kubernetes의 합성어로, 개인 PC에서 쿠버네티스를 실습하기 위한 표준 도구입니다. 가벼운 가상 환경을 사용해 클러스터를 흉내 냅니다.
:::

*'우선 노트북에 설치해 두고 직접 만져 봐야겠다.'*

오픈이는 한 번 기지개를 켠 뒤 새 터미널 창을 열었습니다.

### 4.2.2 설치와 시작

먼저 Minikube를 사용중인 OS에 맞게 설치합니다.
...
Minikube가 설치되면 `minikube` 명령을 쓸 수 있습니다. 다음 한 줄로 노트북 안에 클러스터를 시작합니다.
...
*그림 4-9. minikube start 실행 결과*

*'쿠버네티스 테스트 준비가 끝났네. minikube start 한 줄에 클러스터가 다 떴어.'*
...
도커가 컨테이너 단위로 관리한다면, 쿠버네티스는 **하나 이상의 컨테이너를 한 상자로 묶은 Pod(파드)** 단위로 관리합니다.
...
*그림 4-10. Pod - 컨테이너들이 네트워크를 공유하는 단위*
...
클러스터에 명령을 내릴 때 사용하는 도구는 **kubectl**입니다. "이런 상태를 만들어 달라"고 쿠버네티스에 요청하는 명령줄 도구입니다.
...
*그림 4-11. kubectl run으로 Pod 생성*

Pod가 생성됐습니다. `docker run`이랑 같은 명령형 방식 그대로입니다.

*'그런데 쿠버네티스는 선언형이라고 했는데, 이건 그냥 도커식 명령이잖아.'*
...
선언형은 **YAML 파일**에 원하는 상태를 적어 두는 방식입니다. 한 줄 명령 대신 파일에 정의해 둔 뒤, 그 파일을 클러스터에 적용합니다.
...
*그림 4-12. kubectl apply로 Pod 생성*

### 4.3.4 Pod 조회

Pod 두 개를 생성했으니 이제 클러스터 상태를 조회해 보겠습니다.
...
*그림 4-13. Pod 목록 조회 결과*

만약 Pod가 어느 노드에 배치되었는지, 혹은 생성 과정에서 어떤 일이 있었는지 상세히 알고 싶다면 **kubectl describe** 명령어를 사용합니다.
...
노트북에 두 개의 Pod가 나란히 떴습니다. 그런데 회의실에서 들었던 팀장의 물음이 다시 마음에 걸렸습니다.

*'근데 선언형이면 죽어도 알아서 다시 살아나는 건가?'*

## 4.4 Deployment - 자동 복구·스케일링·무중단 배포

### 4.4.1 Pod 하나를 직접 만들면 생기는 문제

방금 만든 Pod로 작은 실험을 해 보겠습니다. 운영 중에 누군가의 실수로 Pod가 삭제되거나, 프로그램 오류로 종료된다면 어떤 일이 벌어질까요.
...
*그림 4-15. Pod 삭제 후 목록을 조회하면 hello-pod1이 사라져 있습니다*

hello-pod1은 목록에서 사라졌고, 시간이 지나도 다시 나타나지 않았습니다.

*'아, 그냥 선언만 해서는 자동 복구가 안 되네.'*

직접 만든 Pod는 일회성이라, 문제가 생겨 사라져도 자동으로 복구해 주는 책임자가 없습니다.

오픈이에게 필요한 건 **"가맹점 개수를 항상 일정하게 관리하라"** 는 명확한 지침이었습니다. 쿠버네티스에서 이 일을 담당하는 리소스가 **Deployment(디플로이먼트)** 입니다.
...
Deployment는 "**Pod를 몇 개 유지할지, 문제가 생기면 어떻게 교체할지**"를 적어 두는 **매뉴얼**과 같습니다. 본사가 **운영 지침서**를 내려 주면 현장 운영팀이 그 지침대로 가맹점 개수를 맞추는 구조입니다.
...
<svg viewBox="0 0 760 270" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Deployment 구조: Deployment가 원하는 상태를 선언해 두고 여러 Pod를 띄워 관리한다">
  ...
  <text x="380" y="250" text-anchor="middle" font-size="11" font-style="italic" fill="#475569">Pod가 사라지면 Deployment가 같은 모양의 Pod를 다시 띄워 원래대로 맞춥니다.</text>
</svg>
</div>

*그림 4-16. Deployment의 구조 — Pod를 띄우고 원하는 상태를 계속 유지*
...
여기서 가장 중요한 개념은 **selector** 와 **labels** 의 연결입니다. Deployment의 selector에 **app: nginx** 라고 적어 두면, 쿠버네티스는 해당 라벨을 가진 Pod들만 골라 관리합니다.
...
*그림 4-17. selector가 지정한 라벨(app: nginx)을 가진 Pod만 골라 관리, 다른 라벨은 건드리지 않음*
...
*그림 4-18. Deployment가 만든 Pod가 뜬 모습*
...
*그림 4-19. Pod를 다 지워도 Deployment가 자동으로 새 Pod를 띄웁니다*
...
:::term-box
**ReplicaSet**: 선언한 개수만큼 Pod를 항상 유지하는 리소스입니다. 쿠버네티스에 별도로 존재하는 리소스이지만 롤링 업데이트·롤백 같은 배포 기능이 없어서, 실무에서는 Deployment를 통해 관리합니다.
:::
...
<svg viewBox="0 0 760 320" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="ReplicaSet 자가 복구: Pod 하나가 종료되면 새 Pod를 자동으로 띄워 개수를 맞춘다">
  ...
  <text x="170" y="124" text-anchor="middle" font-size="13" font-weight="700" fill="#fff">Deployment</text>
  <text x="170" y="160" text-anchor="middle" font-size="12" fill="#3730a3">Pod 2개 유지</text>
  <text x="170" y="185" text-anchor="middle" font-size="12" fill="#3730a3">↻ 비교·유지</text>
  <text x="170" y="211" text-anchor="middle" font-size="11" font-style="italic" fill="#64748b">(매뉴얼 역할)</text>
  ...
  <text x="380" y="305" text-anchor="middle" font-size="11" font-style="italic" fill="#475569">Pod 2가 종료되면 Deployment가 즉시 Pod 3을 띄워 다시 2개를 유지합니다.</text>
</svg>
</div>

*그림 4-20. Pod 하나가 종료되면 새 Pod를 자동으로 띄워 개수를 맞춤*
...
*그림 4-21. replicas 설정대로 Pod 4개가 생성*

### 4.4.4 롤링 업데이트 - 끊김 없는 버전 교체

운영에서는 서비스를 유지하면서 새 버전으로 교체하는 일이 자주 일어납니다. 4개 Pod를 한 대씩 손으로 끄고 새 버전으로 띄우는 작업을 매번 사람이 처리하긴 번거롭습니다. 쿠버네티스는 이 과정을 자동화하는 방식, **롤링 업데이트(Rolling Update)** 를 기본 제공합니다.
...
`deploy-ex02.yml`의 `strategy` 블록의 속성은 다음과 같습니다.

- **maxSurge**: 업데이트 중 정원(replicas)보다 잠시 증가되는 Pod의 수 (추가 투입 인원)
- **maxUnavailable**: 업데이트 중 **작동 불능** 상태가 되어도 허용되는 Pod의 수 (잠시 문 닫아도 되는 가게 수)

예를 들어 replicas: 4, maxSurge: 4, maxUnavailable: 0이라면, 기존 4개는 그대로 둔 상태에서 새 버전 4개를 한꺼번에 더 만듭니다. 새 Pod가 Ready 상태가 되는 대로 기존 Pod를 차례로 종료시킵니다. **maxSurge를 줄이면** 한 번에 띄울 새 Pod 수를 제한해 천천히 교체하고, **maxUnavailable을 늘리면** 기존 Pod를 줄이며 빠르게 교체하는 등 두 값을 조정해 원하는 배포 방식을 설정할 수 있습니다.
...
세상에 완벽한 배포는 없습니다. 만약 새 버전에 문제가 보인다면, `kubectl rollout` 명령어로 이전 버전으로 되돌릴 수 있습니다.
...
다만 외부에서 이 Pod에 들어올 방법은 아직 없습니다. 다음 챕터에서는 Service와 Ingress로 외부와 연결되는 진입점을 만들어 보겠습니다.
:::
