...
며칠이 지났습니다. 컨테이너 한 대를 띄우고 내리는 일은 더 이상 어색하지 않았습니다. 자투리 시간마다 docker 명령어를 손에 익히면서, 어제까지만 해도 낯설던 단어가 키보드 위에서 자연스럽게 흘러나왔습니다. 노트 한 귀퉁이에는 마지막 줄로 적어 둔 한 문장이 그대로 남아 있었습니다.

*'그런데 실제 서비스는 컨테이너 한 대로 안 끝날 텐데.'*

화요일 오전 열 시. 회의실에 사람들이 모였습니다. 팀장이 화이트보드 앞에서 새 프로젝트의 윤곽을 그리기 시작했습니다. 사내에서 쓸 작은 통합 사이트 하나를 새로 만든다는 이야기였습니다. 회원 정보가 들어가야 하니 DB가 한 자리, 화면을 그릴 프론트엔드가 한 자리, 그 사이를 받쳐 줄 백엔드가 한 자리. 화이트보드 위에 네모 세 개가 나란히 그려졌습니다.

팀장이 펜을 내려놓고 오픈이 쪽을 봤습니다.

**팀장**: "환경 구성은 Docker로 가요. 요즘 공부했잖아요."

오픈이는 고개를 끄덕였습니다. 회의가 끝나고 자리로 돌아오는 길에야 머리가 복잡해졌습니다. 컨테이너 한 대를 띄우는 것까지는 손에 익었습니다. 그런데 화이트보드의 네모 세 개를 떠올려 보니, 한 대씩 따로 띄우고 끝날 일이 아니었습니다. 프론트엔드에서 백엔드로, 백엔드에서 DB로 요청이 흘러야 했습니다. 셋이 서로의 주소를 알아야 했고, 같은 시점에 같이 살아 있어야 했습니다.
...
명령어 한 줄을 잘못 치면 처음부터 다시 가야 했습니다. 패키지 이름을 한 글자만 빠뜨려도 그랬습니다. 손가락 하나로 두 시간을 날린 적도 있었습니다. 그러다 머릿속에 갑자기 다른 그림이 떠올랐습니다. 며칠 전 편의점에서 본 밀키트였습니다. 비닐을 뜯으면 손질된 재료와 양념이 한 봉지에 들어 있어서, 봉지를 열고 끓이기만 하면 끝나는 그 상자였습니다.

*'도커도 저런 식으로 미리 준비해 둘 수 없을까. 누군가 한 번만 적어 두면, 다음에는 그 종이만 들고 있어도 같은 환경이 만들어지는 식으로.'*

도커에 이미 그런 방식이 마련되어 있었습니다. 환경 구성을 정의 파일 하나에 적어 두고 자동으로 만들어 내는 작업을 **프로비저닝(Provisioning)** 이라고 부릅니다. 밀키트의 그 봉지처럼, 한 번만 잘 적어 두면 그 다음부터는 봉지를 뜯듯 환경이 차려집니다.
...
컨테이너를 삭제하더라도 이미지는 그대로 남아 있습니다. 같은 환경이 필요할 때마다 언제든 다시 띄울 수 있습니다. 지난주에 commit 명령어를 일일이 쳐서 만들었던 그 자리는 이제 Dockerfile이 채웁니다.
...
패키지가 미리 깔린 이미지를 만드는 것까지는 풀렸습니다. 그런데 회의실의 통합 사이트를 떠올리면 다음 질문이 바로 따라왔습니다. 로컬에서 작업한 소스 코드는 어떻게 컨테이너 안으로 옮겨야 할까요. 화이트보드의 네모 세 개 모두 자기 자리에 코드를 들고 있어야 했습니다. 이때 쓰이는 지시어가 **WORKDIR**와 **COPY**입니다.
...
문법 표를 다시 들여다보던 시선이 표 아래쪽 두 줄에서 멈췄습니다. CMD는 "**실행할 기본 명령어**", ENTRYPOINT는 "**반드시 실행되는 메인 프로세스**". 둘 다 '실행'이라는 단어가 들어 있어서 차이가 한눈에 들어오지 않았습니다.

*'기본 명령어와 반드시 실행되는 메인 프로세스. 어쨌든 둘 다 실행한다는 말로 들리는데, 무엇이 다른 걸까.'*

두 지시어는 성격이 조금 다릅니다. 카페의 커피 머신 한 대를 떠올리면 쉽습니다. 머신이 하는 본질적인 일은 '커피를 내린다'입니다. 손님이 따로 주문하지 않으면 그냥 '아메리카노'가 나옵니다. 손님이 라떼를 주문하면 머신은 라떼를 내리고, 에스프레소를 주문하면 에스프레소를 내립니다. 메뉴는 바뀌지만 '커피를 내린다'는 동작 자체는 그대로입니다. **ENTRYPOINT**가 그 '커피를 내린다'에 해당하고, **CMD**가 따로 주문이 없을 때 기본으로 나가는 '아메리카노'에 해당합니다.
...
이미지를 자동으로 만드는 준비는 끝났습니다. 이제 회의실에서 받은 통합 사이트의 첫 칸을 채울 차례였습니다.

## 3.2 NGINX — 요청을 앞에서 받아 나눠주기

### 3.2.1 NGINX를 서버 앞에 두는 이유

다음 날 점심시간이 끝나고 자리에 앉아 회의실에서 받은 그림을 메모장에 옮겨 그렸습니다. 프론트엔드와 백엔드, DB까지 화이트보드에서 본 그대로였습니다. 그런데 한 군데가 자꾸 마음에 걸렸습니다. 사용자가 어떻게 들어오는지였습니다. 컨테이너 셋이 각자 포트를 들고 있다고 해서, 사용자에게 그 포트 번호를 그대로 알려 줄 수는 없었습니다. 외부로 내부 주소가 그대로 흘러나가는 구조가 됩니다.

*'사내 통합 사이트인데, 내부 구조를 사용자에게 다 보여주면 안 되는데. 입구 하나만 두고 그 뒤에서 적당히 나눠 줄 방법은 없을까.'*

펜 끝으로 종이를 톡톡 두드리다가 머릿속에 들어온 단어가 있었습니다. 지난주 도커 명령어를 익히면서 `nginx` 이미지를 한 번 띄워 봤습니다. 그때는 '웹 서버구나' 정도로 흘려 들었는데, 자료를 다시 펼쳐 보니 NGINX는 웹 서버이면서 동시에 요청을 중간에서 받아 뒤로 넘겨 주는 **리버스 프록시(Reverse Proxy)** 역할도 한다고 적혀 있었습니다. 사용자의 요청을 NGINX가 대신 받고, 뒤쪽 서버의 응답을 다시 사용자에게 돌려주는 구조였습니다.
...
NGINX의 동작은 전국 택배가 모이는 대형 분류 센터를 떠올리면 가까웠습니다. 전국에서 들어온 택배가 한 곳에 쌓이고, 그 자리에서 주소지에 따라 지역 물류 센터로 갈라져 나갑니다. NGINX의 설정도 비슷한 흐름이었습니다.
...
방금 작성한 `nginx.conf`를 다시 들여다보던 시선이 한 줄에서 멈췄습니다. `host.docker.internal:8000`이라는 주소였습니다. lb 컨테이너 안에서 app1 컨테이너를 부르려고 적은 자리인데, 도중에 호스트 PC를 한 번 거치는 모양이 영 깔끔하지 않게 느껴졌습니다.
...
<svg viewBox="0 0 720 360" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="lb 컨테이너에서 호스트 PC를 거쳐 app1 컨테이너로 가는 우회 경로">
  ...
  <text x="360" y="24" text-anchor="middle" font-size="13" font-weight="700" fill="#1f2937">기본 docker0 — 컨테이너 이름으로는 서로 못 부른다 (IP는 가능)</text>
...
같은 호스트 위에 있는 두 컨테이너인데, 옆집 문을 두드리려고 일부러 일층까지 내려갔다가 다시 올라오는 길을 거치고 있었습니다.
...
다음 실습은 같은 NGINX의 다른 얼굴이었습니다. 같은 서비스를 두 대 이상에 올려 두고, 들어오는 요청을 양쪽으로 나눠 보내는 일이었습니다.

NGINX는 `upstream` 블록 안에 `server` 줄을 한 줄 더 추가하면, 들어오는 요청을 차례대로 두 서버에 번갈아 보내 줍니다. 카드 딜러가 카드를 한 장씩 양쪽으로 나눠 주듯 한 명에게 한 번씩 돌아가는 방식이었습니다. 이 방식을 **라운드 로빈(Round Robin)** 이라고 부릅니다.
...
로드밸런싱 로그를 위에서 아래로 훑어 내려가던 시선이 한 군데에서 멈췄습니다. 양쪽 컨테이너 모두 똑같은 HTML을 매번 새로 만들어 응답하고 있었습니다. 같은 한 줄짜리 페이지를 두 대가 번갈아 가며 다시 그리고 있는 모양이었습니다.
...
이미지가 떴다고 해서 끝난 게 아니었습니다. 정말 두 번째 요청이 백엔드까지 가지 않고 NGINX 안에서 끝난 건지 확인해야 했습니다. 개발자 도구(F12)의 Network 탭을 열고, 브라우저 자체 캐시가 끼어들지 못하도록 **Disable cache** 를 체크한 뒤 응답 헤더를 들여다봤습니다. 첫 요청에서는 `X-Cache-Status` 값이 예상대로 **MISS**로 찍혀 있었습니다.
...
세션 문제가 풀린 자리에서 잠깐 손을 멈췄습니다. Redis는 메모리 위에서 도는 도구라서, 컨테이너를 한 번 내렸다가 다시 띄우면 그 안의 값이 모두 사라집니다.
...
DB까지 컨테이너로 잡아 두고 나서 터미널의 히스토리를 위로 올려 봤습니다. 지난 며칠 사이에 빌드와 실행 명령이 꽤 길게 늘어서 있었습니다. 같은 패턴이 거듭 나오고 있었습니다. `docker build`, 그 다음에 `docker run`, 같은 네트워크에 묶어 주는 옵션, 포트 매핑. 컨테이너가 한 대씩 늘어날 때마다 이 패턴이 한 묶음씩 더 붙었습니다.

*'프론트엔드, 백엔드, DB까지 셋이 같이 도는데 이걸 매번 한 줄씩 친다고? 한 번에 묶어 주는 방법이 있겠지.'*
...
수동 세팅이 피곤해서 Dockerfile을 손에 익혔습니다. 그런데 그 Dockerfile로 만든 이미지를 한 줄씩 띄우다 보니 결국 또 다른 수동 작업에 갇혀 있었습니다.
...
검색 끝에 마주친 도구가 Docker Compose였습니다. 화면에 펼쳐진 설명을 읽어 보니, 여러 악기가 모인 합주 자리를 떠올리게 했습니다. 연주자마다 자기 악보를 따로 들고 있는 게 아니라, 모든 파트가 한 장의 총악보에 적혀 있고 지휘자의 손짓 한 번에 다 함께 첫 음을 냅니다.
...
여기까지 컨테이너 한 대씩 띄우는 법, 그리고 Compose로 묶는 법을 따로따로 손에 익혔습니다. 이제 그 조각을 모아 회의실에서 받았던 그 통합 사이트의 한 세트를 만들어 볼 차례였습니다. 프론트엔드 자리에 NGINX, 백엔드 자리에 Spring Boot, DB 자리에 MySQL. 목표는 단순했습니다. `docker compose up` 한 줄로 셋이 같이 올라가는 모습을 만드는 것이었습니다.
...
세 컨테이너가 어떤 식으로 손을 맞잡는지부터 짚었습니다. 각자 받쳐 주는 자리가 있었습니다.
...
*'며칠 전부터 한 대씩 떼어 놓고 만지던 셋이, 이제 한 그림 안으로 들어오네.'*
...
화면에 회원 목록이 떴습니다. NGINX가 받아 백엔드로 넘기고, 백엔드가 MySQL에서 꺼낸 데이터가 그 자리에 그대로 올라온 결과였습니다. 며칠 전 회의실에서 화이트보드에 그렸던 네모 세 개가 한 화면 안에 같이 들어와 있었습니다.
...
회의실을 나오는 길에 어깨가 한결 가벼웠습니다. 며칠 전 같은 자리에서 받은 의뢰가 한 줄짜리 명령으로 뜨는 그림으로 끝맺어진 참이었습니다.
