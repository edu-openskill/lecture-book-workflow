...
**동료**: "오픈 씨, 어제 테스트 서버에 올렸다는 서비스에 접속이 안 되는데, 어떻게 들어가요?"

오픈이는 당황했습니다. 로그상으로는 분명 정상 실행 중이었지만, 정작 브라우저로 접속해 본 적은 없었기 때문입니다. 서둘러 Pod 목록에서 IP를 확인해 브라우저에 입력해 보았지만, 페이지는 열리지 않았습니다.
...
**선배**: "외부 요청이 실제 Pod까지 도달하려면 두 가지 역할이 필요해요. 들어온 요청을 도메인이나 경로에 따라 알맞은 곳으로 보내 주는 **Ingress**, 그리고 수시로 바뀌는 Pod를 묶어 고정된 진입점을 제공하는 **Service**죠. 이 두 가지를 한번 알아봐요."
...
YAML을 살펴보던 오픈이는 `type: NodePort`를 보고 궁금증이 생겼습니다.

*'NodePort 말고 다른 type도 있는 거 같은데. 어떤 역할을 하는 걸까.'*

Service는 접근 범위에 따라 ClusterIP, NodePort, LoadBalancer로 나뉩니다.
...
**LoadBalancer**는 외부 요청을 하나로 묶어 연결하는 **대표 번호**입니다. Service를 LoadBalancer로 지정하면, AWS·GCP 같은 클라우드 플랫폼이 **공인 IP**를 발급합니다. **그 IP로 들어온 모든 요청을 받아, 뒤에 있는 여러 노드로 나눠 보냅니다.** 실제 운영에서 가장 흔히 씁니다.
...
앞서 만든 Service YAML을 보면, 포트에는 `port`, `targetPort`, `nodePort` 세 종류가 있습니다. 각 포트는 외부에서 들어온 요청을 Pod까지 단계별로 전달합니다.

외부 사용자가 노드의 `nodePort`로 접속하면, 그 요청이 `port`를 가진 Service를 거쳐 Pod의 `targetPort`로 전달됩니다.
...
*그림 5-7. NodePort에서 Service를 거쳐 Pod 컨테이너까지 이어지는 포트 흐름*
...
`ex11/service-ex01.yml`을 클러스터에 적용한 뒤, 외부에서 실제로 접속되는지 확인합니다.
...
브라우저를 열고 NodePort로 설정한 30080 포트(`localhost:30080`)로 접속합니다. 그런데 연결할 수 없다는 화면이 떴습니다.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>LoadBalancer·Ingress에 외부 IP 부여. LoadBalancer·Ingress 접근에 사용</td>
    </tr>
  </tbody>
</table>
...
다음으로 Pod가 사라지고 IP가 바뀌어도 Service가 새 Pod로 자동 연결되는지 직접 확인합니다. Ctrl+C로 터미널을 종료하고 Pod를 전부 지웁니다.
...
새 URL로 접속해도 NGINX 페이지가 그대로 나옵니다. Pod IP가 모두 새로 바뀌었지만 Service는 새 Pod에 이미 연결되어 있습니다.

*'Pod IP가 다 바뀌었는데도 Service는 새 Pod와 연결돼 있네. 외부에서는 Service만 바라보고 있으면 되는구나.'*

## 5.2 Ingress - 도메인 라우팅

Service는 Pod 그룹마다 고정 주소를 제공합니다. 기능이 늘어 Service가 많아지면 사용자가 접근해야 할 주소도 그만큼 늘어납니다. 하지만 Service는 IP와 포트만으로 요청을 전달할 뿐, 같은 도메인 안에서 **URL 경로에 따라 요청을 나눠 보내지는 못합니다**. 이 한계를 극복하고 URL 경로 기반의 라우팅을 담당하는 리소스가 Ingress입니다.
...
<svg viewBox="0 0 760 360" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Ingress Controller가 Ingress 리소스(YAML)에서 규칙을 받아 외부 요청을 두 Service로 분기하는 구조">
  ...
  <text x="200" y="120" font-size="11" font-family="monospace" fill="#0f172a">/order   → order-service</text>
  <text x="200" y="140" font-size="11" font-family="monospace" fill="#0f172a">/stores → stores-service</text>
...
YAML로 작성한 Ingress 리소스를 등록하면 API Server를 거쳐 상태 저장소인 **etcd**에 보관됩니다. Ingress 컨트롤러는 API Server로부터 라우팅 규칙을 받아 적용합니다.
:::
...
이제 들어온 요청을 받아 처리할 주문 Service와 매장 Service를 만들 차례입니다. 둘 다 **ClusterIP** 타입이라 클러스터 바깥에서는 직접 접근할 수 없어, 외부 요청을 받아 알맞은 곳으로 보낼 **Ingress 규칙**이 필요합니다.
...
클러스터를 호스트와 연결하기 위해 별도 터미널에서 `minikube tunnel`을 실행합니다.
...
<svg viewBox="0 0 800 305" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="우편 위 송장에 바코드(스캐너로 읽음)와 받는 곳 주소(눈으로 읽음)가 함께 있고, 물류 센터는 바코드까지, 집배원은 받는 곳 주소만 읽음">
  ...
  <text x="400" y="22" text-anchor="middle" font-size="13" font-weight="700" fill="#1f2937">우편 송장에 두 가지 정보가 함께 있습니다</text>
  <g transform="translate(180, 56)">
    ...
    <g transform="translate(20, 90)">
      ...
      <text x="16" y="24" font-size="10" font-weight="700" fill="#dc2626">송장 바코드</text>
      <text x="386" y="24" text-anchor="end" font-size="8" font-style="italic" fill="#1f2937">스캐너로 읽음</text>
      ...
      <text x="200" y="62" text-anchor="middle" font-size="9" font-family="monospace" fill="#dc2626">1234-5678-90</text>
      <rect x="8" y="72" width="384" height="56" rx="2" fill="#fffaf0" stroke="#cbd5e1" stroke-width="0.8"/>
      <text x="16" y="88" font-size="10" font-weight="700" fill="#475569">받는 곳 주소</text>
      <text x="386" y="88" text-anchor="end" font-size="8" font-style="italic" fill="#1f2937">사람이 눈으로 읽음</text>
      <text x="16" y="106" font-size="11" font-family="monospace" font-weight="700" fill="#0f172a">서울시 강남구 테헤란로 123</text>
      <text x="16" y="122" font-size="11" font-family="monospace" font-weight="700" fill="#0f172a">○○빌딩 5층  김영진</text>
    </g>
  </g>
  <g transform="translate(20, 110)">
    <rect x="0" y="0" width="150" height="60" rx="6" fill="#fff" stroke="#ff7849" stroke-width="2"/>
    <text x="75" y="22" text-anchor="middle" font-size="13" font-weight="700" fill="#7b341e">물류 센터</text>
  ...
  <g transform="translate(630, 158)">
    ...
    <text x="75" y="22" text-anchor="middle" font-size="13" font-weight="700" fill="#0f172a">집배원</text>
    <text x="75" y="40" text-anchor="middle" font-size="9" fill="#475569">받는 곳 주소만</text>
    <text x="75" y="54" text-anchor="middle" font-size="8" fill="#1f2937">눈으로 읽고 분류</text>
  </g>
  <line x1="630" y1="200" x2="582" y2="230" stroke="#475569" stroke-width="2.2" marker-end="url(#lbl-slate)"/>
</svg>
</div>

*그림 5-20. 우편 위 송장에 바코드와 받는 곳 주소가 함께 있고, 물류 센터는 바코드까지, 집배원은 받는 곳 주소만 읽습니다*
...
<svg viewBox="0 0 800 270" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="실제 패킷의 두 정보 — HTTP 페이로드(Host·URL)와 TCP/IP 헤더(IP·포트). Ingress는 HTTP 페이로드까지, Service는 TCP/IP 헤더만 읽음">
  ...
  <g transform="translate(220, 50)">
    ...
    <g transform="translate(14, 16)">
      ...
      <text x="318" y="18" text-anchor="end" font-size="8" font-style="italic" fill="#1f2937">L7 · HTTP 서버가 파싱</text>
    ...
    <g transform="translate(14, 112)">
      ...
      <text x="318" y="18" text-anchor="end" font-size="8" font-style="italic" fill="#1f2937">L4 · iptables가 즉시 읽음</text>
      ...
      <text x="14" y="71" font-size="9" font-family="monospace" fill="#1f2937">...프로토콜·TTL 등</text>
    </g>
  </g>
  ...
  <g transform="translate(630, 168)">
    ...
    <text x="75" y="54" text-anchor="middle" font-size="8" fill="#1f2937">iptables 규칙 매칭</text>
  </g>
  <line x1="630" y1="198" x2="568" y2="194" stroke="#475569" stroke-width="2.2" marker-end="url(#it-slate)"/>
</svg>
</div>

*그림 5-21. 실제 패킷에 HTTP 페이로드와 TCP/IP 헤더가 함께 들어 있고, Ingress는 페이로드까지, Service는 헤더만 읽습니다*

패킷 속 정보는 우편 송장처럼 역할에 따라 다른 종류의 데이터들이 섞여 있습니다. 이 정보들은 각자의 역할에 따라 **전송 계층**(Layer 4)과 **응용 계층**(Layer 7)에서 각각 처리됩니다.

| 구분 | 전송 계층 (L4) | 응용 계층 (L7) |
|:---:|:---|:---|
| **역할** | 데이터를 어느 컴퓨터의 어느 프로세스로 보낼지 안내 | 패킷 내부의 메시지·콘텐츠를 해석 |
| **보는 정보** | 패킷 겉면(헤더)의 받는 IP·포트 | 패킷 내부의 URL·Host |
| **담당 리소스** | **Service** | **Ingress** |
| **동작** | 지정된 IP·포트로 들어온 요청을 백엔드 Pod로 분산·전달 | 세부 경로(`/order`, `/stores`)에 따라 알맞은 Service로 전달 |

## 5.3 브라우저에서 Pod까지의 경로

이전 챕터에서는 개발자의 명령으로 Pod를 띄우고 관리하는 흐름을 다뤘습니다. 이번에는 사용자가 브라우저에서 요청했을 때, Pod에 도달하는 흐름을 알아보겠습니다.
...
<svg viewBox="0 0 1200 540" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="요청이 Pod까지 도달하는 전체 흐름 — 1·2단계와 컨트롤 플레인이 위쪽 가로로, 3단계가 아래쪽에 두 컬럼(order/stores)으로 평행">
  ...
  <text x="85" y="226" text-anchor="middle" font-size="17" font-weight="700" fill="#0f172a">Host</text>
  <rect x="200" y="70" width="980" height="460" rx="10" fill="none" stroke="#475569" stroke-width="1.4" stroke-dasharray="6,4"/>
  <text x="290" y="92" text-anchor="middle" font-size="14" font-style="italic" fill="#475569">쿠버네티스 가상세계 (클러스터)</text>
  ...
  <text x="310" y="247" text-anchor="middle" font-size="12" fill="#7b341e">각 노드의 외부 포트</text>
  ...
  <text x="460" y="233" text-anchor="middle" font-size="12" fill="#7b341e">iptables DNAT</text>
  ...
  <text x="730" y="196" text-anchor="middle" font-size="12" fill="#7b341e">URL·Host로 Service 결정</text>
  ...
  <text x="565" y="265" text-anchor="middle" font-size="12" font-weight="600" fill="#7b341e">ClusterIP</text>
  ...
  <text x="895" y="265" text-anchor="middle" font-size="12" font-weight="600" fill="#7b341e">ClusterIP</text>
  ...
  <text x="1010" y="223" font-size="12" font-style="italic" fill="#1f2937" text-anchor="end">갱신</text>
  ...
  <text x="630" y="434" text-anchor="middle" font-size="12" fill="#7b341e">iptables DNAT</text>
  ...
  <text x="830" y="434" text-anchor="middle" font-size="12" fill="#7b341e">iptables DNAT</text>
  ...
  <text x="600" y="497" text-anchor="middle" font-size="13" font-weight="700" fill="#7b341e">Pod A1</text>
  <rect x="632" y="482" width="56" height="22" rx="11" fill="#fff4ed" stroke="#ff7849" stroke-width="1.6"/>
  <text x="660" y="497" text-anchor="middle" font-size="13" font-weight="700" fill="#7b341e">Pod A2</text>
  ...
  <text x="800" y="497" text-anchor="middle" font-size="13" font-weight="700" fill="#7b341e">Pod B1</text>
  <rect x="832" y="482" width="56" height="22" rx="11" fill="#fff4ed" stroke="#ff7849" stroke-width="1.6"/>
  <text x="860" y="497" text-anchor="middle" font-size="13" font-weight="700" fill="#7b341e">Pod B2</text>
  <line x1="937" y1="285" x2="893" y2="398" stroke="#94a3b8" stroke-width="1" stroke-dasharray="3,2" marker-end="url(#ov-s)"/>
</svg>
</div>

*그림 5-22. 요청이 Pod까지 도달하는 전체 흐름 - 세 단계가 일어나는 위치*
...
<div class="svg-figure">
<svg viewBox="0 0 800 280" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="1단계 비유 — 창구 직원이 받은 우편을 다음 거점인 물류 센터로 보냄">
  <defs>
    <marker id="m1n-a" markerWidth="9" markerHeight="9" refX="8" refY="3" orient="auto"><path d="M0,0 L0,6 L8,3 z" fill="#1f2937"/></marker>
  </defs>
  <text x="400" y="22" text-anchor="middle" font-size="15" font-weight="700" fill="#1f2937">창구 직원이 우편을 물류 센터로 보냅니다</text>
  <!-- 좌측: 외부 주소 우편 -->
  <g transform="translate(20, 110)">
    <text x="65" y="-10" text-anchor="middle" font-size="12" font-weight="700" fill="#475569">우편</text>
    ...
    <text x="8" y="80" text-anchor="middle" font-size="10" fill="#94a3b8">발신자</text>
    <g transform="translate(28, 12)">
      ...
      <text x="50" y="46" text-anchor="middle" font-size="8" fill="#475569">받는 곳 주소</text>
      <text x="50" y="55" text-anchor="middle" font-size="9" font-weight="700" fill="#0f172a">서울시 강남구 ○○동</text>
    </g>
  </g>
  ...
  <g transform="translate(238, 60)">
    ...
    <text x="160" y="2" text-anchor="middle" font-size="12" font-weight="700" fill="#7b341e">중앙 우체국</text>
    <text x="160" y="44" text-anchor="middle" font-size="12" font-style="italic" font-weight="700" fill="#7b341e">받은 우편을 물류 센터로</text>
    <rect x="0" y="50" width="44" height="80" fill="#fff" stroke="#7b341e" stroke-width="1.4"/>
    <rect x="15" y="78" width="14" height="52" fill="#7b341e"/>
    <text x="22" y="43" text-anchor="middle" font-size="11" font-weight="700" fill="#7b341e">입구</text>
    <path d="M 44 90 Q 70 105 100 125" stroke="#1f2937" stroke-width="1.2" marker-end="url(#m1n-a)" fill="none" stroke-dasharray="3,2"/>
    ...
    <text x="200" y="80" text-anchor="middle" font-size="12" font-weight="700" fill="#7b341e">창구 카운터</text>
    <g transform="translate(125, 116)">
      ...
      <rect x="14" y="32" width="82" height="22" rx="2" fill="#fff" stroke="#475569" stroke-width="1"/>
      <text x="55" y="42" text-anchor="middle" font-size="8" fill="#475569">받는 곳 주소</text>
      <text x="55" y="51" text-anchor="middle" font-size="9" font-weight="700" fill="#0f172a">서울시 강남구 ○○동</text>
    </g>
    ...
    <text x="265" y="140" text-anchor="middle" font-size="10" fill="#475569">창구 직원</text>
  </g>
  <line x1="566" y1="148" x2="628" y2="148" stroke="#1f2937" stroke-width="1.4" marker-end="url(#m1n-a)"/>
  <!-- 우측: 물류 센터 (우편 도착) -->
  <g transform="translate(636, 95)">
    <text x="80" y="-14" text-anchor="middle" font-size="12" font-weight="700" fill="#475569">물류 센터</text>
    <rect x="20" y="6" width="120" height="86" rx="3" fill="#fff" stroke="#475569" stroke-width="1.8"/>
    <path d="M 16 6 L 144 6 L 130 -6 L 30 -6 Z" fill="#475569"/>
    <rect x="32" y="20" width="40" height="72" fill="#fff" stroke="#475569" stroke-width="1.2"/>
    <line x1="32" y1="32" x2="72" y2="32" stroke="#475569" stroke-width="0.7"/>
    <line x1="32" y1="44" x2="72" y2="44" stroke="#475569" stroke-width="0.7"/>
    <line x1="32" y1="56" x2="72" y2="56" stroke="#475569" stroke-width="0.7"/>
    <line x1="32" y1="68" x2="72" y2="68" stroke="#475569" stroke-width="0.7"/>
    <line x1="32" y1="80" x2="72" y2="80" stroke="#475569" stroke-width="0.7"/>
    <g transform="translate(84, 40)">
      <rect x="0" y="0" width="50" height="34" rx="2" fill="#fef3c7" stroke="#d97706" stroke-width="1.4"/>
      <line x1="0" y1="0" x2="25" y2="17" stroke="#d97706" stroke-width="1"/>
      <line x1="50" y1="0" x2="25" y2="17" stroke="#d97706" stroke-width="1"/>
      <rect x="8" y="18" width="34" height="13" rx="1" fill="#fff" stroke="#475569" stroke-width="0.8"/>
      <text x="25" y="27" text-anchor="middle" font-size="7" font-weight="700" fill="#0f172a">서울 강남</text>
    </g>
  </g>
</svg>
</div>

*그림 5-23. 창구 직원이 받은 우편을 다음 거점인 물류 센터로 보냅니다*

여기서 우편은 패킷에 해당합니다. 비유를 실제와 비교해 정리하면 다음과 같습니다.
...
<text x="235" y="151" text-anchor="middle" font-size="11" font-family="monospace" font-weight="700" fill="#7b341e">:30080</text>
...
<text x="420" y="89" text-anchor="middle" font-size="13" font-weight="700" fill="#7b341e">kube-proxy</text>
<text x="420" y="104" text-anchor="middle" font-size="10" font-style="italic" fill="#1f2937">iptables NAT 규칙</text>
<rect x="305" y="118" width="90" height="44" rx="3" fill="#fff" stroke="#475569" stroke-width="1.2"/>
<text x="350" y="132" text-anchor="middle" font-size="10" fill="#475569">Node 포트</text>
<line x1="312" y1="137" x2="388" y2="137" stroke="#cbd5e1" stroke-width="0.6"/>
<text x="350" y="154" text-anchor="middle" font-size="12" font-family="monospace" font-weight="700" fill="#0f172a">:30080</text>
<line x1="395" y1="140" x2="445" y2="140" stroke="#1f2937" stroke-width="1.4" marker-end="url(#s21-red)"/>
<text x="420" y="131" text-anchor="middle" font-size="11" font-style="italic" font-weight="700" fill="#dc2626">주소 변환</text>
<rect x="445" y="118" width="85" height="44" rx="3" fill="#fff" stroke="#dc2626" stroke-width="2"/>
<text x="488" y="132" text-anchor="middle" font-size="10" fill="#dc2626">Ingress IP</text>
...
<text x="608" y="143" font-size="10" font-weight="600" fill="#7b341e">Ingress IP:</text>
<text x="752" y="143" text-anchor="end" font-size="12" font-family="monospace" font-weight="700" fill="#dc2626">10.244.0.5:80</text>
<line x1="608" y1="150" x2="752" y2="150" stroke="#fed7aa" stroke-width="0.6"/>
<text x="608" y="165" font-size="10" font-weight="600" fill="#7b341e">요청:</text>
<text x="752" y="165" text-anchor="end" font-size="12" font-family="monospace" font-weight="700" fill="#0f172a">/order</text>
</g>
</svg>
</div>
...
<svg viewBox="0 0 800 280" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="2단계 비유 — 송장 붙은 우편이 물류 센터에서 바코드 스캔으로 분류 규칙에 따라 갈 우체국이 결정됨">
  ...
  <text x="400" y="22" text-anchor="middle" font-size="15" font-weight="700" fill="#1f2937">송장 바코드를 찍으면 분류 규칙대로 갈 우체국이 정해집니다</text>
  <!-- 좌측: 입력 (송장 붙은 우편 — 1단계 출력과 동일) -->
  <g transform="translate(20, 97)">
    <text x="65" y="-10" text-anchor="middle" font-size="12" font-weight="700" fill="#475569">송장 붙은 우편</text>
    ...
    <rect x="22" y="44" width="82" height="22" rx="2" fill="#fff" stroke="#475569" stroke-width="1"/>
    <text x="63" y="54" text-anchor="middle" font-size="8" fill="#475569">받는 곳 주소</text>
    <text x="63" y="63" text-anchor="middle" font-size="9" font-weight="700" fill="#0f172a">서울시 강남구 ○○동</text>
    <g transform="translate(53, 50) rotate(-5)">
      <rect x="0" y="0" width="48" height="30" rx="2" fill="#fff" stroke="#dc2626" stroke-width="2"/>
      <text x="24" y="10" text-anchor="middle" font-size="9" font-weight="700" fill="#dc2626">송장</text>
  ...
  <text x="200" y="131" text-anchor="middle" font-size="11" font-style="italic" fill="#7b341e">물류 센터로</text>
  <!-- 가운데: 물류 센터 (작업대 + 사전 대조) -->
  <g transform="translate(238, 60)">
    <rect x="0" y="20" width="320" height="180" rx="3" fill="#fff" stroke="#7b341e" stroke-width="1.8"/>
    <path d="M -3 20 L 323 20 L 323 8 L -3 8 Z" fill="#7b341e"/>
    <text x="160" y="2" text-anchor="middle" font-size="12" font-weight="700" fill="#7b341e">물류 센터</text>
    <text x="160" y="42" text-anchor="middle" font-size="13" font-style="italic" font-weight="700" fill="#dc2626">바코드 스캔 → 분류 규칙 → 결정!</text>
    <!-- 좌: 작업대 위 송장 (바코드 강조) -->
    <text x="60" y="68" text-anchor="middle" font-size="11" font-weight="700" fill="#7b341e">송장 바코드</text>
    <g transform="translate(20, 78)">
      <rect x="0" y="0" width="80" height="50" rx="2" fill="#fff" stroke="#dc2626" stroke-width="2.2"/>
      <text x="40" y="13" text-anchor="middle" font-size="10" font-weight="700" fill="#dc2626">송장</text>
    ...
    <!-- 스캔 화살표 (송장 → 규칙) -->
    <line x1="105" y1="105" x2="155" y2="105" stroke="#1f2937" stroke-width="1.2" stroke-dasharray="3,2" marker-end="url(#m2n-a)"/>
    <text x="130" y="97" text-anchor="middle" font-size="11" font-style="italic" fill="#7b341e">스캔</text>
    <!-- 우: 분류 규칙 (펼쳐진 책) -->
    <g transform="translate(160, 70)">
      ...
      <text x="70" y="-6" text-anchor="middle" font-size="11" font-weight="700" fill="#7b341e">분류 규칙</text>
      <text x="6" y="18" font-size="9" fill="#94a3b8">바코드</text>
      <text x="76" y="18" font-size="9" fill="#94a3b8">→ 우체국</text>
      ...
      <text x="8" y="36" font-size="9" font-weight="700" fill="#dc2626">|||| ||||</text>
      <text x="76" y="36" font-size="9" font-weight="700" fill="#dc2626">▶ 서울</text>
      <text x="8" y="50" font-size="9" fill="#94a3b8">|||| ||||</text>
      <text x="76" y="50" font-size="9" fill="#94a3b8">  부산</text>
      <text x="8" y="64" font-size="9" fill="#94a3b8">|||| ||||</text>
      <text x="76" y="64" font-size="9" fill="#94a3b8">  대구</text>
      <text x="8" y="78" font-size="9" fill="#94a3b8">|||| ||||</text>
      <text x="76" y="78" font-size="9" fill="#94a3b8">  광주</text>
      <text x="8" y="92" font-size="9" fill="#94a3b8">  ...</text>
    </g>
  </g>
  <line x1="566" y1="140" x2="628" y2="140" stroke="#1f2937" stroke-width="1.4" marker-end="url(#m2n-a)"/>
  <text x="597" y="131" text-anchor="middle" font-size="11" font-style="italic" fill="#7b341e">서울로</text>
  <!-- 우측: 출력 (서울 우체국 결정, 나머지는 흐림) -->
  <g transform="translate(635, 118)">
    <text x="65" y="-10" text-anchor="middle" font-size="12" font-weight="700" fill="#7b341e">결정된 우체국</text>
    <circle cx="65" cy="22" r="22" fill="#fff4ed" stroke="#ff7849" stroke-width="2.6"/>
    <text x="65" y="26" text-anchor="middle" font-size="13" font-weight="700" fill="#7b341e">서울</text>
    <circle cx="20" cy="80" r="11" fill="#f8fafc" stroke="#cbd5e1" stroke-width="1"/>
    <text x="20" y="83" text-anchor="middle" font-size="9" fill="#94a3b8">부산</text>
    <circle cx="65" cy="98" r="11" fill="#f8fafc" stroke="#cbd5e1" stroke-width="1"/>
    <text x="65" y="101" text-anchor="middle" font-size="9" fill="#94a3b8">대구</text>
    <circle cx="110" cy="80" r="11" fill="#f8fafc" stroke="#cbd5e1" stroke-width="1"/>
    <text x="110" y="83" text-anchor="middle" font-size="9" fill="#94a3b8">광주</text>
  </g>
</svg>
</div>

*그림 5-25. 물류 센터가 송장의 바코드를 찍고 어느 우체국으로 보낼지 정합니다*

비유를 실제와 비교해 정리하면 다음과 같습니다.

| 비유 | 실제 (IT 용어) | 비유 흐름 (우편의 동선) | 실제 역할 (패킷의 처리) |
|:---:|:---|:---|:---|
| 물류 센터 | **Ingress Controller** | 우편이 어디로 갈지 정해 주는 시설 | 요청의 도메인과 경로를 읽고 보낼 Service를 결정하는 주체 |
| 바코드 | **URL·Host 헤더** | 물류 센터에서 분기를 위한 바코드화된 주소 | 요청 안에 들어 있는 도메인과 경로 정보 |
| 분류 규칙 | **Ingress 규칙** | 바코드에서 주소를 읽기 위한 규칙 | 어느 URL이 어느 Service로 갈지 미리 등록해 둔 규칙 |
| 결정된 우체국 | **Service (ClusterIP)** | 물류 센터에서 우편이 갈 우체국 | 같은 라벨을 가진 Pod 그룹을 묶은 서비스 내부 주소 |

Ingress 컨트롤러는 요청의 URL 경로와 Host 헤더를 등록된 Ingress 규칙과 대조합니다. 그리고 일치하는 Service의 ClusterIP로 전달합니다.
...
<text x="400" y="18" text-anchor="middle" font-size="15" font-weight="700" fill="#1f2937">요청을 읽고 Ingress 규칙과 대조해 어느 Service로 보낼지 결정합니다</text>
<rect x="60" y="40" width="680" height="230" rx="4" fill="#fff" stroke="#475569" stroke-width="1.4" stroke-dasharray="6,4"/>
<text x="400" y="36" text-anchor="middle" font-size="13" font-style="italic" fill="#475569">Kubernetes 클러스터</text>
<rect x="270" y="50" width="240" height="35" rx="3" fill="#f8fafc" stroke="#94a3b8" stroke-width="1.2"/>
<text x="390" y="65" text-anchor="middle" font-size="13" font-weight="700" fill="#475569">API Server</text>
<text x="390" y="78" text-anchor="middle" font-size="10" font-style="italic" fill="#1f2937">Ingress 규칙 보관</text>
<line x1="390" y1="88" x2="390" y2="113" stroke="#94a3b8" stroke-width="1.2" stroke-dasharray="3,2" marker-end="url(#s23-gray)"/>
<text x="140" y="128" text-anchor="middle" font-size="11" font-weight="700" fill="#475569">도착한 패킷</text>
<rect x="75" y="143" width="130" height="68" rx="3" fill="#fff" stroke="#475569" stroke-width="1.4"/>
<text x="83" y="159" font-size="10" font-weight="600" fill="#7b341e">Ingress IP:</text>
<text x="83" y="174" font-size="11" font-family="monospace" font-weight="700" fill="#dc2626">10.244.0.5:80</text>
<line x1="83" y1="180" x2="197" y2="180" stroke="#fed7aa" stroke-width="0.6"/>
<text x="83" y="195" font-size="10" font-weight="600" fill="#7b341e">요청:</text>
<text x="197" y="195" text-anchor="end" font-size="11" font-family="monospace" font-weight="700" fill="#0f172a">/order</text>
...
<text x="390" y="130" text-anchor="middle" font-size="14" font-weight="700" fill="#7b341e">Ingress Controller</text>
<text x="390" y="143" text-anchor="middle" font-size="10" font-style="italic" fill="#1f2937">요청 읽고 규칙 대조</text>
<rect x="285" y="152" width="210" height="85" rx="2" fill="#fff" stroke="#dc2626" stroke-width="1.4"/>
<text x="390" y="166" text-anchor="middle" font-size="11" font-weight="700" fill="#dc2626">Ingress 규칙</text>
<line x1="295" y1="170" x2="485" y2="170" stroke="#fed7aa" stroke-width="0.6"/>
<text x="295" y="187" font-size="11" font-family="monospace" fill="#475569">매칭:</text>
<text x="345" y="187" font-size="12" font-family="monospace" font-weight="700" fill="#0f172a">/order</text>
<text x="295" y="207" font-size="11" font-family="monospace" fill="#475569">결정:</text>
<text x="345" y="207" font-size="12" font-family="monospace" font-weight="700" fill="#dc2626">order-service</text>
<line x1="512" y1="168" x2="565" y2="168" stroke="#1f2937" stroke-width="1.4" marker-end="url(#s23-arrow)"/>
<text x="538" y="159" text-anchor="middle" font-size="11" font-style="italic" font-weight="700" fill="#7b341e">결정</text>
<text x="650" y="128" text-anchor="middle" font-size="11" font-weight="700" fill="#7b341e">결정된 Service</text>
<rect x="570" y="143" width="160" height="46" rx="23" fill="#fff4ed" stroke="#ff7849" stroke-width="2.4"/>
<text x="650" y="161" text-anchor="middle" font-size="13" font-weight="700" fill="#dc2626">order-service</text>
<text x="650" y="178" text-anchor="middle" font-size="11" font-family="monospace" fill="#7b341e">10.96.0.20</text>
<rect x="575" y="195" width="150" height="20" rx="10" fill="#f8fafc" stroke="#cbd5e1" stroke-width="1"/>
<text x="650" y="209" text-anchor="middle" font-size="11" fill="#94a3b8">stores-service</text>
<rect x="575" y="219" width="150" height="20" rx="10" fill="#f8fafc" stroke="#cbd5e1" stroke-width="1"/>
<text x="650" y="233" text-anchor="middle" font-size="11" fill="#94a3b8">payment-service</text>
</svg>
</div>
...
<svg viewBox="0 0 800 280" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="3단계 비유 — 서울 우체국에 도착한 우편이 받는 주소대로 동네 우편함 B에 들어감">
  ...
  <text x="400" y="22" text-anchor="middle" font-size="15" font-weight="700" fill="#1f2937">집배원이 받는 주소 목록과 우편함을 매칭해 우편함 B에 넣습니다</text>
  <!-- 좌측: 입력 (서울 우체국에서 막 나온 우편) -->
  <g transform="translate(20, 94)">
    <text x="68" y="-10" text-anchor="middle" font-size="12" font-weight="700" fill="#475569">서울 우체국 도착</text>
    ...
    <text x="28" y="17" text-anchor="middle" font-size="9" font-weight="700" fill="#7b341e">서울</text>
    <rect x="22" y="38" width="11" height="22" fill="#7b341e"/>
    <g transform="translate(60, 22)">
      <rect x="0" y="0" width="100" height="48" rx="2" fill="#fef3c7" stroke="#d97706" stroke-width="1.6"/>
      <line x1="0" y1="0" x2="50" y2="24" stroke="#d97706" stroke-width="1.2"/>
      <line x1="100" y1="0" x2="50" y2="24" stroke="#d97706" stroke-width="1.2"/>
      <rect x="9" y="22" width="82" height="20" rx="2" fill="#fff" stroke="#475569" stroke-width="1"/>
      <text x="50" y="30" text-anchor="middle" font-size="8" fill="#475569">받는 곳 주소</text>
      <text x="50" y="39" text-anchor="middle" font-size="9" font-weight="700" fill="#0f172a">서울시 강남구 ○○동</text>
    </g>
  </g>
  ...
  <text x="255" y="208" text-anchor="middle" font-size="11" font-weight="700" fill="#475569">집배원</text>
  <!-- 배송 목록 (집배원 옆) -->
  <rect x="285" y="90" width="115" height="110" rx="2" fill="#fffaf0" stroke="#7b341e" stroke-width="1.4"/>
  <text x="342" y="83" text-anchor="middle" font-size="11" font-weight="700" fill="#7b341e">배송 목록</text>
  <line x1="291" y1="108" x2="394" y2="108" stroke="#94a3b8" stroke-width="0.4"/>
  <text x="298" y="125" font-size="11" fill="#94a3b8">  A: 강남구 △△동</text>
  <rect x="291" y="130" width="103" height="18" fill="#fff4ed" stroke="#ff7849" stroke-width="1.4"/>
  <text x="298" y="144" font-size="12" font-weight="700" fill="#dc2626">▶ B: 강남구 ○○동</text>
  <text x="298" y="165" font-size="11" fill="#94a3b8">  C: 강남구 ××동</text>
  <!-- 매칭 화살표 -->
  <line x1="405" y1="140" x2="475" y2="140" stroke="#1f2937" stroke-width="1.2" stroke-dasharray="3,2" marker-end="url(#m3n-a)"/>
  <text x="440" y="131" text-anchor="middle" font-size="11" font-style="italic" fill="#7b341e">매칭</text>
  <!-- 우편함 3개 (A, B 강조, C) -->
  <g transform="translate(490, 110)">
    ...
    <text x="45" y="108" text-anchor="middle" font-size="13" font-weight="700" fill="#94a3b8">우편함 A</text>
  </g>
  <g transform="translate(600, 110)">
    ...
    <text x="45" y="108" text-anchor="middle" font-size="14" font-weight="700" fill="#dc2626">우편함 B</text>
  </g>
  <g transform="translate(710, 110)">
    ...
    <rect x="80" y="40" width="3" height="18" fill="#cbd5e1"/>
    <rect x="83" y="40" width="8" height="10" fill="#cbd5e1"/>
    <text x="45" y="108" text-anchor="middle" font-size="13" font-weight="700" fill="#94a3b8">우편함 C</text>
  </g>
  <!-- 우편 (우편함 B 슬롯 위로 배송) -->
  ...
  <text x="700" y="148" text-anchor="middle" font-size="11" font-style="italic" font-weight="700" fill="#dc2626">배송!</text>
</svg>
</div>

*그림 5-27. 서울 우체국에서 집배원이 배송 목록을 보고 우편함 B에 우편을 넣습니다*
...
<text x="400" y="18" text-anchor="middle" font-size="15" font-weight="700" fill="#1f2937">kube-proxy가 ClusterIP를 살아있는 Pod IP로 변환해 결정된 Pod로 전달합니다</text>
<rect x="98" y="40" width="604" height="230" rx="4" fill="#fff" stroke="#475569" stroke-width="1.4" stroke-dasharray="6,4"/>
<text x="400" y="36" text-anchor="middle" font-size="13" font-style="italic" fill="#475569">Kubernetes 클러스터</text>
<rect x="270" y="50" width="240" height="35" rx="3" fill="#f8fafc" stroke="#94a3b8" stroke-width="1.2"/>
<text x="390" y="65" text-anchor="middle" font-size="13" font-weight="700" fill="#475569">API Server</text>
<text x="390" y="78" text-anchor="middle" font-size="10" font-style="italic" fill="#1f2937">Pod 목록(EndpointSlice) 보관</text>
<line x1="390" y1="88" x2="390" y2="110" stroke="#94a3b8" stroke-width="1.2" stroke-dasharray="3,2" marker-end="url(#s25-gray)"/>
<g transform="translate(0, 45)">
<text x="169" y="100" text-anchor="middle" font-size="11" font-weight="700" fill="#475569">도착한 패킷</text>
<rect x="112" y="115" width="115" height="50" rx="3" fill="#fff" stroke="#475569" stroke-width="1.4"/>
<text x="120" y="131" font-size="10" font-weight="600" fill="#7b341e">ClusterIP:</text>
<text x="219" y="131" text-anchor="end" font-size="11" font-family="monospace" font-weight="700" fill="#dc2626">10.96.0.20</text>
<line x1="120" y1="136" x2="219" y2="136" stroke="#fed7aa" stroke-width="0.6"/>
<text x="120" y="151" font-size="10" font-weight="600" fill="#7b341e">요청:</text>
<text x="219" y="151" text-anchor="end" font-size="11" font-family="monospace" font-weight="700" fill="#0f172a">/order</text>
...
<text x="382" y="88" text-anchor="middle" font-size="13" font-weight="700" fill="#7b341e">kube-proxy</text>
<text x="382" y="103" text-anchor="middle" font-size="10" font-style="italic" fill="#1f2937">iptables DNAT</text>
<rect x="277" y="115" width="210" height="85" rx="2" fill="#fff" stroke="#dc2626" stroke-width="1.4"/>
<text x="382" y="129" text-anchor="middle" font-size="11" font-weight="700" fill="#dc2626">Pod 목록 (살아있는 Pod IP)</text>
<line x1="287" y1="133" x2="477" y2="133" stroke="#fed7aa" stroke-width="0.6"/>
<text x="297" y="150" font-size="12" font-family="monospace" fill="#94a3b8">10.244.0.6</text>
<rect x="287" y="156" width="190" height="20" fill="#fff4ed" stroke="#ff7849" stroke-width="1.4"/>
<text x="297" y="170" font-size="12" font-family="monospace" font-weight="700" fill="#dc2626">▶ 10.244.0.7</text>
<text x="297" y="190" font-size="12" font-family="monospace" fill="#94a3b8">10.244.0.8</text>
<line x1="504" y1="140" x2="527" y2="140" stroke="#1f2937" stroke-width="1.4" marker-end="url(#s25-arrow)"/>
<text x="515" y="131" text-anchor="middle" font-size="11" font-style="italic" font-weight="700" fill="#7b341e">전달</text>
<text x="612" y="100" text-anchor="middle" font-size="11" font-weight="700" fill="#7b341e">결정된 Pod</text>
<rect x="532" y="115" width="160" height="46" rx="23" fill="#fff4ed" stroke="#ff7849" stroke-width="2.4"/>
<text x="612" y="133" text-anchor="middle" font-size="13" font-weight="700" fill="#dc2626">Pod B</text>
<text x="612" y="150" text-anchor="middle" font-size="11" font-family="monospace" fill="#7b341e">10.244.0.7</text>
<rect x="537" y="167" width="150" height="20" rx="10" fill="#f8fafc" stroke="#cbd5e1" stroke-width="1"/>
<text x="612" y="181" text-anchor="middle" font-size="11" fill="#94a3b8">Pod A</text>
<rect x="537" y="191" width="150" height="20" rx="10" fill="#f8fafc" stroke="#cbd5e1" stroke-width="1"/>
<text x="612" y="205" text-anchor="middle" font-size="11" fill="#94a3b8">Pod C</text>
</g>
</svg>
</div>
