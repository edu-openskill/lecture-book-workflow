...
`ex11` 폴더를 열고 이전 챕터에서 적용한 `deploy-ex02.yml`을 다시 적용합니다.
...
**LoadBalancer**는 외부 요청을 한곳에서 받아 연결하는 **대표 번호**입니다. Service를 LoadBalancer로 지정하면, AWS·GCP 같은 클라우드 플랫폼이 **공인 IP**를 발급합니다. **그 IP로 들어온 모든 요청을 받아, 뒤에 있는 여러 노드로 나눠 보냅니다.** 실제 운영에서 가장 흔히 사용합니다.
...
외부 사용자가 노드의 `nodePort`로 접속하면, Service가 이 요청을 받아 최종 목적지인 Pod의 `targetPort`로 연결해 줍니다.
...
이제 실습을 해 보겠습니다. `ex11/service-ex01.yml`을 클러스터에 적용한 뒤, 외부에서 실제로 접속되는지 확인합니다.
