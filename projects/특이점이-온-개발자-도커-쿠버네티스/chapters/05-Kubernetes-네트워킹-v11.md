...
<svg viewBox="0 0 1200 540" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="요청이 Pod까지 도달하는 전체 흐름 — 1·2단계와 컨트롤 플레인이 위쪽 가로로, 3단계가 아래쪽에 두 컬럼(order/stores)으로 평행">
  ...
  <text x="730" y="196" text-anchor="middle" font-size="14" fill="#7b341e">URL·Host로 Service 결정</text>
...
YAML로 작성한 Ingress 리소스를 클러스터에 등록하면 API Server를 거쳐 상태 저장소인 **etcd**에 보관됩니다. Ingress 컨트롤러는 API Server로부터 라우팅 규칙을 받아 적용합니다.
:::
...
<svg viewBox="0 0 780 260" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="1단계 손그림 — 우편이 우체국(NodePort·kube-proxy)을 거쳐 물류 센터(Ingress 컨트롤러)로">
  ...
  <text x="305" y="205" text-anchor="middle" font-size="9" font-family="monospace" fill="#7b341e">:30080</text>
  ...
  <text x="640" y="243" text-anchor="middle" font-size="10" fill="#7b341e">URL·Host로 분기</text>
</svg>
</div>
...
| **kube-proxy** | 패킷의 목적지를 Ingress 컨트롤러의 IP로 바꿈 |

### 5.3.2 Ingress가 URL·Host로 Service 분기하기

우편이 **물류 센터**에 도착합니다. 물류 센터에서는 바코드를 스캐너로 찍어 등록된 **분류 규칙**에 따라 어느 지역 우체국으로 갈지 정합니다.

Ingress 컨트롤러도 **Ingress 리소스**에 적힌 규칙으로 요청을 보낼 곳을 정합니다. 요청의 **URL 경로와 Host**를 그 규칙과 비교해 맞는 **Service**를 고릅니다.
...
<svg viewBox="0 0 760 300" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="2단계 손그림 — 물류 센터(Ingress 컨트롤러)가 송장을 보고 /order는 order-service, /stores는 stores-service 우체국으로 분기">
  ...
  <line x1="180" y1="175" x2="276" y2="175" stroke="#1f2937" stroke-width="1.6" marker-end="url(#il2-a)"/>
  <path d="M272 110 L488 110 L380 82 Z" fill="#fde4d3" stroke="#7b341e" stroke-width="1.6"/>
  <rect x="278" y="110" width="204" height="130" fill="#fff" stroke="#7b341e" stroke-width="1.8"/>
  <rect x="306" y="116" width="148" height="20" fill="#7b341e"/>
  <text x="380" y="131" text-anchor="middle" font-size="11" font-weight="700" fill="#fff">물류 센터</text>
  <rect x="294" y="146" width="172" height="60" rx="3" fill="#fff4ed" stroke="#ff7849" stroke-width="1.4"/>
  <text x="380" y="163" text-anchor="middle" font-size="10" font-weight="700" fill="#7b341e">Ingress 규칙</text>
  <text x="304" y="183" font-size="11" font-family="monospace" fill="#0f172a">/order  → order</text>
  <text x="304" y="199" font-size="11" font-family="monospace" fill="#0f172a">/stores → stores</text>
  <text x="380" y="228" text-anchor="middle" font-size="12" font-weight="700" fill="#7b341e">Ingress 컨트롤러</text>
  <path d="M482 156 Q512 118 538 104" fill="none" stroke="#1f2937" stroke-width="1.6" marker-end="url(#il2-a)"/>
  <text x="514" y="110" text-anchor="middle" font-size="11" font-family="monospace" font-weight="700" fill="#7b341e">/order</text>
  <path d="M482 194 Q512 232 538 246" fill="none" stroke="#1f2937" stroke-width="1.6" marker-end="url(#il2-a)"/>
  <text x="514" y="256" text-anchor="middle" font-size="11" font-family="monospace" font-weight="700" fill="#7b341e">/stores</text>
...
| 컴포넌트 | 이 단계에서 하는 일 |
|---|---|
| **Ingress 컨트롤러** | 요청을 받아 URL 경로·Host에 맞는 Service로 전달 |
| **Ingress 규칙** | 어떤 경로·Host를 어느 Service로 보낼지 적어 둔 설정 |
