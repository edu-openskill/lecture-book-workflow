...
<div class="svg-figure">
<svg viewBox="0 0 920 260" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Ingress가 도메인과 경로를 읽어 적절한 Service로 요청을 분기하고, 각 Service가 점선으로 묶인 Pod 묶음을 가리키는 구조">
  <defs>
  ...
  <text x="180" y="61" font-size="11" font-weight="700" fill="#7b341e">Kubernetes 클러스터</text>
  ...
  <text x="135" y="127" text-anchor="middle" font-size="10" font-style="italic" fill="#6b7280">/order, /stores</text>
  <rect x="180" y="105" width="160" height="60" rx="8" fill="#fff4ed" stroke="#ff7849" stroke-width="1.8"/>
  <text x="260" y="130" text-anchor="middle" font-size="14" font-weight="700" fill="#7b341e">Ingress</text>
  <text x="260" y="150" text-anchor="middle" font-size="10" fill="#7b341e">경로 읽음</text>
  <line x1="340" y1="120" x2="398" y2="80" stroke="#475569" stroke-width="1.6" marker-end="url(#ir-p)"/>
  <text x="368" y="82" text-anchor="middle" font-size="11" font-family="monospace" font-weight="700" fill="#0f172a">/order</text>
  <line x1="340" y1="150" x2="398" y2="190" stroke="#475569" stroke-width="1.6" marker-end="url(#ir-p)"/>
  <text x="368" y="195" text-anchor="middle" font-size="11" font-family="monospace" font-weight="700" fill="#0f172a">/stores</text>
  <rect x="400" y="60" width="120" height="40" rx="8" fill="#fff" stroke="#475569" stroke-width="1.6"/>
  <text x="460" y="85" text-anchor="middle" font-size="12" font-weight="700" fill="#0f172a">order-service</text>
  <rect x="400" y="170" width="120" height="40" rx="8" fill="#fff" stroke="#475569" stroke-width="1.6"/>
  <text x="460" y="195" text-anchor="middle" font-size="12" font-weight="700" fill="#0f172a">stores-service</text>
  ...
  <text x="655" y="85" text-anchor="middle" font-size="11" font-weight="700" fill="#0f172a">Pod</text>
  <rect x="720" y="62" width="110" height="36" rx="6" fill="#fff" stroke="#475569" stroke-width="1.6"/>
  <text x="775" y="85" text-anchor="middle" font-size="11" font-weight="700" fill="#0f172a">Pod</text>
  ...
  <text x="655" y="195" text-anchor="middle" font-size="11" font-weight="700" fill="#0f172a">Pod</text>
  <rect x="720" y="172" width="110" height="36" rx="6" fill="#fff" stroke="#475569" stroke-width="1.6"/>
  <text x="775" y="195" text-anchor="middle" font-size="11" font-weight="700" fill="#0f172a">Pod</text>
</svg>
</div>
...
Ingress 리소스에는 어떤 경로를 어느 Service로 보낼지 **라우팅 규칙을 적어 둡니다**. 외부 요청이 들어오면 Ingress 컨트롤러가 **이 규칙대로 알맞은 Service로 전달합니다**.
...
실제 클러스터에서는 NGINX·Traefik 같은 Ingress Controller를 Helm으로 직접 설치하고, 그 Service를 `type: LoadBalancer`로 두어 클라우드의 외부 IP에 연결합니다. 미니큐브 addon은 이 두 단계를 한 명령어로 묶어 줍니다.
:::

이제 들어온 요청을 받아 처리할 Service를 만들 차례입니다. 주문 쪽과 매장 쪽을 각각 **ClusterIP Service**로 만듭니다. 두 Service 모두 ClusterIP 타입이라 클러스터 바깥에서는 직접 접근할 수 없어, 외부 요청을 받아 이 둘로 갈라 보낼 **Ingress 규칙**이 필요합니다.
...
도메인은 하나(`localhost`)인데 경로마다 다른 응답이 돌아왔습니다. Ingress가 경로를 보고 알맞은 **Service**로 요청을 넘기고, Service가 다시 그 뒤의 **Pod**로 요청을 넘겨 응답이 만들어진 결과입니다.

그렇다면 이 하나의 요청 안에서 Ingress와 Service는 각각 **무엇을 보고** 자신들의 역할을 수행하는 걸까요?

### 5.2.6 두 리소스의 계층 분담

두 리소스가 같은 트래픽에서 보는 정보가 다른 모습은 우편 봉투의 송장과 닮았습니다.

**송장** 한 장에는 두 가지 정보가 함께 적혀 있습니다. 물류 센터가 어느 지역으로 보낼지 정할 때 찍는 **송장 바코드**, 그리고 그 지역 우체국에서 **집배원**이 직접 눈으로 확인하는 **받는 곳 주소**입니다.
...
패킷 속 정보는 우편 송장처럼 역할에 따라 저마다 다른 종류의 데이터들이 섞여 있습니다. 이 정보들은 각자의 역할에 따라 전송 계층(Layer 4)과 응용 계층(Layer 7)에서 각각 처리됩니다.

| 구분 | 전송 계층 (L4) | 응용 계층 (L7) |
|:---:|:---|:---|
| **역할** | 데이터를 어느 컴퓨터의 어느 프로세스로 보낼지 안내 | 패킷 내부의 메시지·콘텐츠를 해석 |
| **보는 정보** | 패킷 겉면의 받는 IP·포트 | 패킷 내부의 URL·Host |
| **담당 리소스** | **Service** | **Ingress** |
| **동작** | 지정된 IP·포트로 들어온 요청을 백엔드 Pod로 분산·전달 | 세부 경로(`/order`, `/stores`)에 따라 알맞은 Service로 전달 |

## 5.3 브라우저에서 Pod까지의 경로

이전 챕터에서는 개발자의 명령으로 Pod를 띄우고 관리하는 흐름을 다뤘습니다. **Service**와 **Ingress**까지 살펴봤으니, 이제 사용자 요청이 Pod에 도달하는 흐름을 정리해 보겠습니다.

사용자가 브라우저에서 요청했을 때 Pod까지 도달하는 흐름을 한 장에 그리면 다음과 같습니다.
...
<svg viewBox="0 0 1200 540" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="요청이 Pod까지 도달하는 전체 흐름 — 1·2단계와 컨트롤 플레인이 위쪽 가로로, 3단계가 아래쪽에 두 컬럼(order/stores)으로 평행">
  ...
  <text x="1040" y="142" text-anchor="middle" font-size="10" font-style="italic" fill="#1f2937">컨트롤 플레인 (백그라운드)</text>
  ...
  <text x="1040" y="193" text-anchor="middle" font-size="8" fill="#1f2937">Pod 변동 감시 → Pod 목록 갱신</text>
  ...
  <text x="1040" y="270" text-anchor="middle" font-size="8" fill="#1f2937">Ingress·Service·EndpointSlice 정보</text>
  <line x1="1025" y1="205" x2="1025" y2="230" stroke="#94a3b8" stroke-width="1" stroke-dasharray="3,2" marker-end="url(#ov-s)"/>
  <text x="1010" y="223" font-size="9" font-style="italic" fill="#1f2937" text-anchor="end">갱신</text>
  <line x1="1055" y1="230" x2="1055" y2="205" stroke="#94a3b8" stroke-width="1" stroke-dasharray="3,2" marker-end="url(#ov-s)"/>
  <text x="1070" y="223" font-size="9" font-style="italic" fill="#1f2937">push</text>
  <line x1="938" y1="252" x2="860" y2="218" stroke="#94a3b8" stroke-width="1" stroke-dasharray="3,2" marker-end="url(#ov-s)"/>
  <text x="905" y="230" font-size="9" font-style="italic" fill="#1f2937">push</text>
  ...
  <text x="920" y="345" font-size="9" font-style="italic" fill="#1f2937">push</text>
</svg>
</div>
...
발신자가 우편을 들고 **중앙 우체국** 입구로 들어갑니다. 우편 봉투에는 받는 곳 주소가 적혀 있습니다. **창구 직원**이 우편 위에 새 **송장 바코드**를 붙입니다. 송장 바코드에는 우편이 **다음으로 갈 물류 센터의 라우팅 코드**가 들어 있습니다.

<div class="svg-figure">
<svg viewBox="0 0 800 280" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="1단계 비유 — 우편 위에 새 송장 바코드가 붙어 다음 단계로 흘러 나감">
  <defs>
    <marker id="m1n-a" markerWidth="9" markerHeight="9" refX="8" refY="3" orient="auto"><path d="M0,0 L0,6 L8,3 z" fill="#1f2937"/></marker>
  </defs>
  <text x="400" y="22" text-anchor="middle" font-size="13" font-weight="700" fill="#1f2937">우편 위에 새 송장 바코드가 붙어 다음 단계로 갑니다</text>
  <!-- 좌측: 외부 주소 우편 -->
  <g transform="translate(20, 110)">
    <text x="65" y="-10" text-anchor="middle" font-size="10" font-weight="700" fill="#475569">우편</text>
    ...
    <g transform="translate(28, 12)">
      ...
      <text x="50" y="46" text-anchor="middle" font-size="6" fill="#475569">받는 곳 주소</text>
      <text x="50" y="55" text-anchor="middle" font-size="7" font-weight="700" fill="#0f172a">서울시 강남구 ○○동</text>
    </g>
  </g>
  ...
  <g transform="translate(238, 60)">
    ...
    <text x="160" y="42" text-anchor="middle" font-size="11" font-style="italic" font-weight="700" fill="#dc2626">우편 위에 새 송장 바코드 붙임</text>
    <rect x="0" y="50" width="44" height="80" fill="#fff" stroke="#7b341e" stroke-width="1.4"/>
    <rect x="15" y="78" width="14" height="52" fill="#7b341e"/>
    <text x="22" y="43" text-anchor="middle" font-size="9" font-weight="700" fill="#7b341e">입구</text>
    ...
    <g transform="translate(125, 116)">
      ...
      <text x="55" y="42" text-anchor="middle" font-size="6" fill="#475569">받는 곳 주소</text>
      <text x="55" y="51" text-anchor="middle" font-size="7" font-weight="700" fill="#0f172a">서울시 강남구 ○○동</text>
    ...
    <text x="265" y="140" text-anchor="middle" font-size="8" fill="#475569">창구 직원</text>
  </g>
  ...
  <g transform="translate(640, 110)">
    ...
    <rect x="25" y="44" width="82" height="22" rx="2" fill="#fff" stroke="#475569" stroke-width="1"/>
    <text x="66" y="54" text-anchor="middle" font-size="6" fill="#475569">받는 곳 주소</text>
    <text x="66" y="63" text-anchor="middle" font-size="7" font-weight="700" fill="#0f172a">서울시 강남구 ○○동</text>
...
*그림 5-23. 중앙 우체국 창구에서 우편 위에 새 송장 바코드가 붙어 다음 단계로 넘어갑니다*

여기서 패킷은 우편에 해당합니다. 비유를 실제와 비교해 정리하면 다음과 같습니다.

| 비유 | 실제 (IT 용어) | 비유 흐름 (우편의 동선) | 실제 역할 (패킷의 처리) |
|:---:|:---|:---|:---|
| 발신자 | **외부 호스트** | 우편을 보내는 출발점 | 브라우저가 요청을 보내는 클러스터 외부 |
| 우체국 입구 | **NodePort** | 우편물이 안으로 들어오는 첫 번째 문 | 외부 요청이 노드의 공개 포트로 진입하는 지점 |
| 창구 직원 | **kube-proxy (1차)** | 우편물 위에 새 송장 바코드를 덧붙이는 주체 | 들어온 패킷의 목적지를 미리 정해진 규칙대로 다시 적는 주체 |
| 새 송장 부착 | **NAT 주소 변환** | 물류 센터에 보내기 위한 과정 | 패킷의 목적지 주소를 Ingress Controller로 다시 적는 과정 |

브라우저가 보낸 요청은 노드의 NodePort로 들어옵니다. 같은 노드의 kube-proxy가 미리 깔아 둔 **iptables 규칙**에 따라 패킷의 목적지 IP를 **Ingress 컨트롤러의 IP로 변환합니다**.
...
<text x="420" y="104" text-anchor="middle" font-size="8" font-style="italic" fill="#1f2937">iptables NAT 규칙</text>
...
*그림 5-24. NodePort와 kube-proxy(1차)가 외부 요청을 Ingress Controller로 전달*

:::term-box
**kube-proxy**: 모든 노드에 떠 있는 변환기입니다. 들어온 패킷의 목적지 IP를 iptables 규칙에 따라 다음 단계의 IP로 바꿉니다. NodePort로 진입한 직후에는 Ingress 컨트롤러의 IP로, Service의 ClusterIP에 도착한 직후에는 살아있는 Pod의 IP로 변환합니다.
:::

### 5.3.2 Ingress가 URL·Host로 Service 고르기

송장이 붙은 우편이 **물류 센터**에 도착합니다. 작업자가 송장의 **바코드**를 스캐너로 찍으면 시스템이 미리 등록된 **분류 규칙**에 따라 어느 우체국으로 갈지 곧장 알려 줍니다.
...
<svg viewBox="0 0 800 280" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="2단계 비유 — 송장 붙은 우편이 물류 센터에서 바코드 스캔으로 분류 규칙에 따라 갈 우체국이 결정됨">
  ...
  <g transform="translate(20, 97)">
    ...
    <text x="63" y="54" text-anchor="middle" font-size="6" fill="#475569">받는 곳 주소</text>
    <text x="63" y="63" text-anchor="middle" font-size="7" font-weight="700" fill="#0f172a">서울시 강남구 ○○동</text>
  ...
  <g transform="translate(238, 60)">
    ...
    <text x="130" y="97" text-anchor="middle" font-size="9" font-style="italic" fill="#7b341e">스캔</text>
    <!-- 우: 분류 규칙 (펼쳐진 책) -->
...
<text x="390" y="78" text-anchor="middle" font-size="8" font-style="italic" fill="#1f2937">Ingress 규칙 보관</text>
<line x1="390" y1="88" x2="390" y2="113" stroke="#94a3b8" stroke-width="1.2" stroke-dasharray="3,2" marker-end="url(#s23-gray)"/>
<text x="405" y="103" font-size="8" font-style="italic" fill="#1f2937">push</text>
...
<text x="390" y="143" text-anchor="middle" font-size="8" font-style="italic" fill="#1f2937">요청 읽고 규칙 대조</text>
...
<svg viewBox="0 0 800 280" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="3단계 비유 — 서울 우체국에 도착한 우편이 받는 주소대로 동네 우편함 B에 들어감">
  ...
  <g transform="translate(20, 94)">
    ...
    <g transform="translate(60, 22)">
      ...
      <text x="50" y="30" text-anchor="middle" font-size="6" fill="#475569">받는 곳 주소</text>
      <text x="50" y="39" text-anchor="middle" font-size="7" font-weight="700" fill="#0f172a">서울시 강남구 ○○동</text>
    </g>
  </g>
  ...
  <text x="298" y="125" font-size="9" fill="#94a3b8">  A: 강남구 △△동</text>
  <rect x="291" y="130" width="103" height="18" fill="#fff4ed" stroke="#ff7849" stroke-width="1.4"/>
  <text x="298" y="144" font-size="10" font-weight="700" fill="#dc2626">▶ B: 강남구 ○○동</text>
  <text x="298" y="165" font-size="9" fill="#94a3b8">  C: 강남구 ××동</text>
  <!-- 매칭 화살표 -->
...
| 집배원 | **kube-proxy (2차)** | 우편을 받아 어느 우편함에 넣을지 정하는 배달원 | 받은 패킷을 살아있는 Pod로 보내는 주체 |
| 배송 목록 | **EndpointSlice** | 지금 받을 수 있는 우편함을 적은 명단 | 현재 살아있는 Pod의 IP 명단 |
| 우편함 | **Pod** | 우편의 최종 도착지 | 요청을 실제로 처리하는 앱 |

Service로 온 요청은 같은 노드의 kube-proxy(2차)에 들어옵니다. kube-proxy는 미리 받아 둔 **EndpointSlice** 명단에 따라 패킷의 목적지 IP를 **살아있는 Pod의 IP로 변환합니다**. 변환된 IP로 패킷이 해당 **Pod**로 전달되고, Pod가 요청을 처리합니다.
...
<rect x="270" y="50" width="240" height="35" rx="3" fill="#f8fafc" stroke="#94a3b8" stroke-width="1.2"/>
<text x="390" y="65" text-anchor="middle" font-size="11" font-weight="700" fill="#475569">API Server</text>
<text x="390" y="78" text-anchor="middle" font-size="8" font-style="italic" fill="#1f2937">Pod 목록(EndpointSlice) 보관</text>
<line x1="390" y1="88" x2="390" y2="110" stroke="#94a3b8" stroke-width="1.2" stroke-dasharray="3,2" marker-end="url(#s25-gray)"/>
<text x="405" y="102" font-size="8" font-style="italic" fill="#1f2937">push</text>
...
<text x="382" y="103" text-anchor="middle" font-size="8" font-style="italic" fill="#1f2937">iptables DNAT</text>
...
*그림 5-28. kube-proxy(2차)가 미리 받아 둔 Pod 목록에서 살아있는 Pod IP를 골라 변환해 Pod로 전달*
