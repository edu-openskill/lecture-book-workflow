...
- 브라우저에서 Pod까지 패킷이 가는 3단계 흐름을 그려 보겠습니다
:::
...
*그림 5-7. 노드 안에 NodePort, Service, Pod가 위치하고 외부 사용자가 NodePort로 진입해 Service를 거쳐 Pod 안 컨테이너에 도달하는 구조*
...
먼저 `http://localhost/order`를 열자 "주문 접수 완료"가 떴습니다.
...
이어서 `http://localhost/stores`를 열자 "매장 선택"이 떴습니다.
...
오픈이는 이전 챕터에서 개발자의 명령으로 Pod를 띄우고 관리하는 흐름을 익혔습니다. 이제 **Service**와 **Ingress**까지 살펴봤으니, 사용자가 요청을 보냈을 때 Pod에 도달하는 흐름을 정리해 보기로 했습니다.
