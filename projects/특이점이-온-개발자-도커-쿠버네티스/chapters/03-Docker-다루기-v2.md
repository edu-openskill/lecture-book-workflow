...
로드밸런싱으로 부하는 나눴지만, 오픈이는 또 다른 장벽에 부딪혔습니다. 로그인을 하고 페이지를 이동했는데 갑자기 인증 실패가 떴습니다.
...
RUN pip install flask && pip install redis    # Flask + Redis 클라이언트 설치
...
오픈이는 먼저 세 컨테이너가 어떻게 맞물려 도는지 살폈습니다. 요청 한 번이 들어오면 다음 순서로 움직입니다.

1. **브라우저**: localhost:80으로 접속해 화면(HTML)을 요청합니다.
2. **NGINX**: 화면을 응답하고, 내부 JS가 보낸 /api/users 요청을 backend로 전달합니다.
3. **Spring**: db:3306 주소로 MySQL에 접속해 회원 데이터를 조회합니다.
4. 응답이 Spring → NGINX → 브라우저로 돌아가 화면에 표시됩니다.

*'하나씩 떼어서 연습했던 것들을 이제 하나의 서비스로 합쳐보는 과정이네.'*

오픈이는 ex07 폴더의 파일을 살폈습니다. 백엔드, DB, 프론트엔드가 각자 폴더에 담겨 있고, 이를 docker-compose.yml이 감쌌습니다.
...
백엔드 설정을 보던 오픈이의 눈에 특이한 부분이 들어왔습니다. 소스 코드를 미리 빌드해 넣어두지 않고, 컨테이너가 뜨는 순간 깃허브에서 코드를 가져와 그 자리에서 빌드하도록 짜여 있었습니다. 이 초기 작업이 `entrypoint.sh`에 담겨 있습니다.

:::note
**Dockerfile vs entrypoint.sh**

- **Dockerfile** : 이미지 **빌드** 단계. 한 번만 실행되며, 이미지 안에 어떤 파일·도구를 넣을지 정의합니다.
- **entrypoint.sh** : 컨테이너 **시작** 단계. 컨테이너가 뜰 때마다 실행되며, 소스 다운로드·빌드·서버 기동처럼 켜지자마자 해야 할 일을 적어둡니다.
:::
...
빌드되어 실행되는 백엔드 서버 안에는 `/api/users`로 요청이 오면 DB에서 사용자 목록을 조회해 JSON으로 돌려주는 컨트롤러가 들어 있습니다.

**UserController.java** (참고)
...
이 컨트롤러가 데이터를 가져오는 DB는 3.4에서 만든 MySQL 컨테이너와 같은 구성을 그대로 사용합니다(같은 이미지·init.sql). 그래서 이번 종합 실습에서는 DB 부분을 따로 다시 다루지 않습니다.
...
### 3.6.3 Frontend: 정적 페이지 + API 프록시

오픈이는 이어서 프론트엔드 NGINX 설정을 살폈습니다. `/`로 들어오는 요청에는 정적 페이지(index.html)를 응답하고, `/api/`로 들어오는 요청은 백엔드 컨테이너로 넘기는 구조입니다.
...
오픈이는 프로젝트가 있는 ex07로 이동한 후 서비스를 실행했습니다.
...
오픈이는 화면에 뜬 사용자 목록을 한참 들여다봤습니다. 컨테이너 여러 개가 한 줄짜리 명령으로 맞물려 돌아간다는 사실이, 손에 익은 도구처럼 자연스럽게 와닿았습니다.
