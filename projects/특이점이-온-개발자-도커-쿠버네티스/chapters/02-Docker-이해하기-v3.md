...
다음 날 저녁, 오픈이는 퇴근하고 곧장 집으로 돌아왔습니다. 식탁에 노트북을 펼치고 어제 적어둔 메모를 꺼냈습니다. **표준 상자**, **컨테이너**, 그리고 어제 자기 전에 머릿속에 남겨둔 한 문장이 노트 끝에 적혀 있었습니다.

*'그래서 이 표준 상자가 내 컴퓨터 안에서는 도대체 어떻게 만들어지는 거지.'*

오늘 저녁의 목표는 분명했습니다.

**Docker가 무엇이고 어떻게 돌아가는지, 개념과 기본 사용법을 손에 익힌다.**

선배가 던진 한마디로 시작된 일이었습니다. 저녁 한 번에 다 끝나리라 생각하지는 않았지만, 어디까지 갈 수 있는지는 보고 싶었습니다. 오픈이는 키보드 위로 손을 가져갔다가 멈췄습니다.

*'명령어를 치기 전에 원리부터 보자.'*

Docker가 환경을 어떻게 다루는지부터 이해해야 뒤에 나올 명령어가 기계적으로 외워지지 않을 것 같았습니다.

## 2.1 가상화 - 가상머신과 컨테이너

### 2.1.1 주방 네 개 대신 주방 하나

오픈이는 컨테이너가 어떤 원리로 돌아가는지부터 파고들었습니다. 자료를 훑다 보니 머릿속에 한 장면이 떠올랐습니다. 주방에서 요리사 여러 명이 함께 일하는 모습이었습니다.
...
비유만 머리에 두면 나중에 실제 용어와 헷갈립니다. 오픈이는 메모장에 주방 그림을 IT 구조로 다시 옮겨 봤습니다.
...
컨테이너의 정체는 잡혔지만, **어떻게 만들어지는지**까지는 아직 그림이 그려지지 않았습니다. 한 줄의 명령 뒤에서 무슨 일이 도는지를 먼저 보고 싶었습니다. 그래야 나중에 직접 명령을 칠 때 의미를 알고 칠 수 있을 것 같았습니다.
...
오픈이는 메모장에 "**엔진=프런트, 커널=주방장**"이라고 한 줄 적었습니다. 적고 나니 컨테이너의 정체와 만들어지는 과정이 머릿속에서 자리를 잡았습니다.
...
컨테이너 안에서 앱이 돌아도 외부에서 접속할 수 없으면 쓸모가 없습니다. 격리된 프로세스라는 개념은 익혔지만, 이 격리된 것이 바깥과 어떻게 연결되는지를 모르면 컨테이너 내부에 접근할 수 없습니다. 여기서 컨테이너가 어떻게 통신하는지 큰 그림을 잡아보겠습니다.
...
여기까지 보고 나니 시계가 자정을 넘기고 있었습니다. 오픈이는 노트북을 닫지 않은 채 의자를 뒤로 빼고 한 번 기지개를 켰습니다. 컨테이너 안쪽이 궁금해진 건 그때였습니다. 컨테이너 안은 작은 리눅스 환경과 같아서, 내부를 살펴보려면 가장 먼저 기본 리눅스 명령어부터 알아둬야 했습니다.
...
설치 로그가 마무리되며 Nginx 설치가 끝났습니다. 오픈이는 설정이 제대로 반영되었는지 확인하기 위해 현재 활성화된 포트를 점검해 보기로 했습니다. `net-tools` 패키지를 추가로 설치한 뒤, 명령어를 입력해 Nginx가 네트워크상에서 정상적으로 대기 중인지 확인했습니다.
...
오픈이는 `exit` 말고 다른 방식도 있는지 찾아봤습니다. `CTRL+P → CTRL+Q`를 쓰면 컨테이너는 그대로 두고 쉘만 빠져나올 수 있습니다. 같은 방식으로 한 번 더 띄우고 이 조합으로 빠져나왔습니다.
...
그림처럼 `attach`는 컨테이너 안에서 이미 돌고 있는 메인 프로세스를 연결합니다. 같은 프로세스를 공유하기 때문에 여기서 잘못 빠져나와 프로세스를 종료하면 컨테이너 전체가 같이 꺼집니다.
...
여기까지 오니 컨테이너의 생명 주기는 더는 헷갈리지 않았습니다. 그런데 한 가지 생각이 오픈이를 붙잡았습니다. 앞서 Ubuntu 컨테이너에 패키지를 설치하고 파일을 추가했는데, 컨테이너를 내리는 순간 지금까지 했던 작업이 사라지는 문제였습니다.
...
방금 전에 만들었던 `b.txt`가 새 컨테이너 안에서도 그대로 보였습니다. 컨테이너를 내려도 데이터는 사라지지 않았습니다. 처음 던졌던 걱정이 풀렸습니다.
...
오픈이는 메모장에 한 줄 적었습니다. **컨테이너 한 대, 띄우고 들여다보고 기록까지.** 어제 낮까지만 해도 처음 들어보는 단어였는데, 오늘 밤에는 손끝으로 직접 다루고 있었습니다.

노트북을 닫았습니다. 식탁 위엔 식어버린 커피 잔이 남아 있었습니다. 잔을 들어 입에 가져다 대는데 문득 한 생각이 머리를 스쳤습니다.

*'그런데 실제 서비스는 컨테이너 한 대로 안 끝날 텐데.'*

웹 서비스 하나를 떠올려도 화면을 그리는 프론트엔드, 일을 처리하는 백엔드, 데이터를 담아두는 DB가 필요합니다. 그게 다 컨테이너 한 대로 될 일은 아니었습니다. 다음 챕터에서는 한 번에 컨테이너 한 대가 아니라, 프론트엔드·백엔드·DB가 함께 뜨는 실제 서비스 구성을 다룹니다. **Dockerfile**로 환경을 자동으로 만들고 **Docker Compose**로 여러 컨테이너를 한 번에 묶어 올려 보겠습니다.
