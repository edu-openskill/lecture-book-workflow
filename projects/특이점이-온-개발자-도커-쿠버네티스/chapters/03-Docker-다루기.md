...
이 문제를 해결하는 도구가 챕터 2에서 본 **사용자 정의 네트워크(User-defined Network)** 입니다. 같은 네트워크에 묶인 컨테이너끼리는 도커 내부 DNS가 이름을 IP로 자동 변환해 주므로, `host.docker.internal` 같은 우회 없이 컨테이너 이름을 그대로 쓸 수 있습니다.
