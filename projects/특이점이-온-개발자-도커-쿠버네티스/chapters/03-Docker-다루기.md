...
오픈이는 챕터 2에서 CMD을 사용해 메인 프로세스를 변경해봤습니다. 그런데 문법 표를 다시 보니 ENTRYPOINT라는 지시어도 눈에 띄었습니다.
...
오픈이는 NGINX를 전국 택배 분류 센터의 중앙 허브라고 상상했습니다. 들어오는 수많은 택배(요청)를 주소지에 따라 각 지역 물류 센터로 정확하게 전달하는 과정에 비유하면 NGINX의 역할을 쉽게 이해할 수 있습니다.
 ...
 - **upstream (지역 물류 센터 정의)** : 특정 지역을 담당할 **배송 센터(서버 그룹)** 를 묶어 이름을 붙이는 작업입니다. **"서울 센터", "부산 센터"** 처럼 물건을 넘겨줄 목적지를 미리 등록해 두는 것입니다.
 - **location (분류 게이트)** : 택배의 **주소지(URL)를 확인**하고 분류하는 게이트입니다. **"주소가 '서울','부산' 등 무엇으로 시작하는가?"** 를 확인하여 최종 행선지를 결정합니다.
 - **proxy_pass (물류 상차)** : 분류된 택배를 **지정된 물류 센터로 이동**시키는 지시어입니다. **"이 물품은 서울 센터로 보내"** 라는 최종 명령입니다.
...
nginx.conf를 작성하던 오픈이의 눈에 계속 걸리는 지점이 하나 있었습니다. 바로 **host.docker.internal:8000** 이라는 주소입니다. 구조를 가만히 보니 lb 컨테이너가 app1 컨테이너를 직접 부르는 게 아니라, 호스트 PC를 한 번 거쳐서 돌아가는 방식이었습니다.
...
이유는 앞서 챕터 2에서 다뤘던 네트워크의 특성 때문입니다. 컨테이너를 명령어로 각각 따로 실행하면 이들은 도커의 기본 네트워크에 자동으로 묶입니다. 같은 네트워크에 있으니 IP로 직접 호출하면 될 것 같지만, IP는 컨테이너를 재시작할 때마다 새로 부여되어 nginx.conf에 고정값으로 적어둘 수 없습니다. 'app1' 같은 이름으로 부르는 기능도 기본 네트워크에는 없습니다. 결국 항상 같은 위치에 있는 호스트 PC만이 안정적으로 가리킬 수 있는 주소였고, 그래서 호스트를 의미하는 host.docker.internal을 거치는 우회 경로를 선택한 것입니다.
...
<div class="terminal-log">
  ...
  <div class="tl-body">
    <div><span class="tl-key">$</span> <span class="tl-str">docker logs ff8e235c79a2</span></div>
    ...
    <div>...</div>
    <div>172.17.0.1 - - [25/Nov/2025:15:11:23 +0000] "GET / HTTP/1.0" 200 273 "-" "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/142.0.0.0 Safari/537.36" "172.17.0.1"</div>
    <div>172.17.0.1 - - [25/Nov/2025:15:11:52 +0000] "GET / HTTP/1.0" 200 273 "-" "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/142.0.0.0 Safari/537.36" "172.17.0.1"</div>
  </div>
</div>
...
<div class="terminal-log">
  ...
  <div class="tl-body">
    <div><span class="tl-key">$</span> <span class="tl-str">docker logs 22f65c73f3c5</span></div>
    ...
    <div>...</div>
    <div>172.17.0.1 - - [25/Nov/2025:15:11:33 +0000] "GET / HTTP/1.0" 200 273 "-" "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/142.0.0.0 Safari/537.36" "172.17.0.1"</div>
    <div>172.17.0.1 - - [25/Nov/2025:15:12:08 +0000] "GET / HTTP/1.0" 200 273 "-" "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/142.0.0.0 Safari/537.36" "172.17.0.1"</div>
  </div>
</div>
...
<div class="svg-figure">
<svg viewBox="0 0 720 200" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="첫 번째 요청 — 캐시 MISS">
  <defs>
    <marker id="rr316" markerWidth="10" markerHeight="10" refX="8" refY="3" orient="auto"><path d="M0,0 L0,6 L8,3 z" fill="#475569"/></marker>
  </defs>
  <text x="360" y="22" text-anchor="middle" font-size="13" font-weight="700" fill="#1f2937">첫 번째 요청 (MISS) — 캐시 비어있음, 백엔드까지 다녀옴</text>
  <rect x="20" y="80" width="120" height="60" rx="8" fill="#fff" stroke="#475569" stroke-width="1.8"/>
  <text x="80" y="105" text-anchor="middle" font-size="13" font-weight="700" fill="#0f172a">클라이언트</text>
  <text x="80" y="125" text-anchor="middle" font-size="11" fill="#6b7280">처음 접속</text>
  <line x1="140" y1="110" x2="240" y2="110" stroke="#475569" stroke-width="1.6" marker-end="url(#rr316)"/>
  <text x="190" y="100" text-anchor="middle" font-size="11" font-weight="600" font-family="monospace" fill="#0f172a">GET /image.png</text>
  <rect x="240" y="60" width="240" height="100" rx="8" fill="#fff" stroke="#475569" stroke-width="1.8"/>
  ...
  <line x1="480" y1="110" x2="580" y2="110" stroke="#475569" stroke-width="1.6" marker-end="url(#rr316)"/>
  <text x="530" y="100" text-anchor="middle" font-size="11" font-weight="600" fill="#0f172a">요청 전달</text>
  <rect x="580" y="80" width="120" height="60" rx="8" fill="#fff" stroke="#475569" stroke-width="1.8"/>
  <text x="640" y="105" text-anchor="middle" font-size="13" font-weight="700" fill="#0f172a">백엔드 서버</text>
  <text x="640" y="125" text-anchor="middle" font-size="11" fill="#6b7280">실제 응답 생성</text>
  <text x="360" y="183" text-anchor="middle" font-size="10" fill="#6b7280">받아온 응답을 캐시에 저장한 뒤 클라이언트에 돌려줍니다</text>
</svg>
</div>
...
<div class="svg-figure">
<svg viewBox="0 0 720 200" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="두 번째 요청 — 캐시 HIT">
  <defs>
    <marker id="rr317" markerWidth="10" markerHeight="10" refX="8" refY="3" orient="auto"><path d="M0,0 L0,6 L8,3 z" fill="#475569"/></marker>
  ...
  <rect x="20" y="80" width="120" height="60" rx="8" fill="#fff" stroke="#475569" stroke-width="1.8"/>
  <text x="80" y="105" text-anchor="middle" font-size="13" font-weight="700" fill="#0f172a">클라이언트</text>
  <text x="80" y="125" text-anchor="middle" font-size="11" fill="#6b7280">두 번째 접속</text>
  <line x1="140" y1="110" x2="240" y2="110" stroke="#475569" stroke-width="1.6" marker-end="url(#rr317)"/>
  <text x="190" y="100" text-anchor="middle" font-size="11" font-weight="600" font-family="monospace" fill="#0f172a">GET /image.png</text>
  <rect x="240" y="60" width="240" height="100" rx="8" fill="#fff" stroke="#475569" stroke-width="1.8"/>
  <text x="360" y="83" text-anchor="middle" font-size="13" font-weight="700" fill="#0f172a">NGINX</text>
  ...
  <line x1="480" y1="110" x2="580" y2="110" stroke="#cbd5e1" stroke-width="1.6" stroke-dasharray="5,4" marker-end="url(#rr317m)"/>
  <text x="530" y="100" text-anchor="middle" font-size="11" font-weight="600" fill="#94a3b8">백엔드 가지 않음</text>
  <rect x="580" y="80" width="120" height="60" rx="8" fill="#fff" stroke="#cbd5e1" stroke-width="1.5" stroke-dasharray="4,3"/>
  <text x="640" y="105" text-anchor="middle" font-size="13" font-weight="700" fill="#94a3b8">백엔드 서버</text>
  <text x="640" y="125" text-anchor="middle" font-size="11" fill="#cbd5e1">호출 없음</text>
  <text x="360" y="183" text-anchor="middle" font-size="10" fill="#6b7280">캐시 히트로 백엔드 부하 감소 + 응답 시간 단축</text>
</svg>
</div>
...
로드밸런싱으로 부하를 나누는 것까지는 성공했지만, 오픈이는 설계를 검토하다가 또 다른 장벽에 부딪혔습니다. 단순히 서버를 복제해서 늘리는 것만으로는 해결되지 않는 '상태 유지'의 문제입니다.
...
<svg viewBox="0 0 720 230" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="세션 불일치 — 서버마다 따로 보관해서 생기는 문제">
  ...
  <text x="620" y="190" text-anchor="middle" font-size="11" fill="#dc2626">세션 없음 → 로그인 풀림</text>
</svg>
</div>
...
<svg viewBox="0 0 720 280" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Redis로 세션 공유">
  ...
  <line x1="420" y1="85" x2="540" y2="135" stroke="#475569" stroke-width="1.6" marker-end="url(#rr322)"/>
  <text x="478" y="100" text-anchor="middle" font-size="11" font-weight="600" fill="#0f172a">조회/저장</text>
  <line x1="420" y1="200" x2="540" y2="155" stroke="#475569" stroke-width="1.6" marker-end="url(#rr322)"/>
  <text x="478" y="195" text-anchor="middle" font-size="11" font-weight="600" fill="#0f172a">조회/저장</text>
...
<div class="terminal-log">
  ...
  <div class="tl-body">
    <div><span class="tl-key">$</span> <span class="tl-str">docker build -t db ./db</span></div>
