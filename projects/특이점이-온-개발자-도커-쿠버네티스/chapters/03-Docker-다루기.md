...
이유는 앞서 챕터 2에서 다뤘던 네트워크의 특성 때문입니다. 세 컨테이너를 **docker run** 명령어로 각각 따로 실행했기 때문에, 이들은 도커의 기본 네트워크에 속합니다. 이 기본 네트워크 안에서는 컨테이너끼리 서로의 이름(app1, app2 등)을 불러서 통신할 수 있는 기능이 없습니다. 그래서 어쩔 수 없이 "lb → 호스트 포트 → app1"이라는 우회 경로를 선택한 것입니다.
