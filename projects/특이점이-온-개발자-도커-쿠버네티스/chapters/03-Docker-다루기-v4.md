...
며칠 동안 오픈이는 자투리 시간마다 docker의 원리와 명령어를 손에 익혔습니다. 컨테이너 하나를 띄우고 내리는 일은 이제 어색하지 않았습니다. 그러던 어느 날 팀 회의에서 새 프로젝트 이야기가 나왔습니다. 사내용 웹 서비스인데, 프론트엔드와 백엔드, 그리고 데이터를 담을 DB까지 필요한 구성이었습니다. 팀장이 오픈이를 보며 말했습니다.

**팀장**: "환경 구성은 Docker로 해볼래요. 이번에 공부했잖아요."
...
컨테이너를 삭제하더라도 이미지는 그대로 남아 있습니다. 덕분에 똑같은 환경이 필요할 때마다 언제든 다시 띄울 수 있습니다. 지난번 수동으로 commit 명령어를 쳐서 만들었던 자리는, 이제 Dockerfile이 대신 채웁니다.
...
앞서 정리해둔 문법 표를 다시 들여다보던 오픈이의 시선이 표 아래쪽 두 줄에서 멈췄습니다. CMD는 "**실행할 기본 명령어**", ENTRYPOINT는 "**반드시 실행되는 메인 프로세스**". 둘 다 '실행'이라는 단어가 들어가 있었습니다.
...
이번에는 NGINX의 로드밸런싱을 실습해보기로 했습니다.

NGINX는 upstream 블록 안에 server 줄을 추가하기만 하면 들어오는 요청을 순차적으로 서버에게 전달합니다. 마치 카드 딜러가 한 장씩 돌아가며 카드를 나눠주는 것과 비슷합니다. 이러한 방식을 **라운드 로빈(Round Robin)** 이라고 부릅니다.
...
실습을 마친 오픈이는 nginx.conf의 패턴이 늘 같다는 걸 알아챘습니다. **어떤 요청(location)을** **어디로 보낼지(proxy_pass)**, 그리고 **어떤 옵션을 끼울지** 정하는 흐름이 그것입니다.

## 3.3 Redis — 서버 여러 대가 공유하는 세션 저장소

### 3.3.1 서버 여러 대면 생기는 세션 문제

NGINX로 부하 문제는 풀었지만, 오픈이는 곧 다른 장벽에 부딪혔습니다. 로그인을 하고 페이지를 이동했는데 갑자기 인증 실패가 떴습니다.
...
각 서버가 따로 상태를 들고 있을 때 생긴 문제가, 외부 저장소 하나로 한꺼번에 풀린 셈입니다.
...
꼭 남아야 하는 데이터는 별도 데이터베이스 서버에 두어야 했습니다. 오픈이는 프로젝트의 사용자 데이터를 담을 DB로 MySQL을 선택하고, 이를 컨테이너로 띄워보기로 했습니다.
...
이 고민을 해결해 주는 도구가 바로 Docker Compose입니다. 여러 악기가 모여 연주할 때, 각자의 악보를 따로 들고 있는 게 아니라 **악보** 한 장에 모든 파트를 적어두고 지휘자의 손짓 한 번에 연주를 시작하는 것과 같습니다.
...
지금까지 컨테이너 하나씩 띄우는 법, 그리고 컴포즈로 묶는 법을 익혔습니다. 이번에는 그 조각을 모아 프론트엔드(NGINX), 백엔드(Spring Boot), DB(MySQL)가 함께 도는 한 세트를 만들어 보겠습니다. 목표는 단순합니다. `docker compose up` 한 줄로 전부 띄우는 것입니다.
...
이 스크립트는 컨테이너가 뜨는 순간 깃허브에서 소스를 받아와 그 자리에서 빌드하고 실행하도록 짜여 있었습니다. 한 번 적어두면 누가 어디서 컨테이너를 띄워도 같은 백엔드가 자동으로 올라옵니다.
...
백엔드가 소스를 가져와 빌드하는 과정이 있어 첫 실행 시에는 시간이 다소 걸렸습니다. 

![](../assets/CH03/chap02-58.png)

*그림 3-37. 사용자 목록 조회 성공*

빌드가 완료된 후 브라우저에서 localhost:80에 접속하자, DB에서 가져온 사용자 목록이 화면에 나타났습니다.

며칠 전만 해도 프론트엔드·백엔드·DB를 함께 돌릴 방법이 막막해 노트북을 펼치고 부딪치던 오픈이가, 이제는 yml 한 장만 있으면 그 셋을 한 번에 띄울 수 있습니다.

다음 날 팀장 앞에서 `docker compose up` 한 줄로 세 컨테이너가 동시에 올라가는 모습을 시연했습니다.
...
프로젝트를 돌릴 환경은 갖춰졌습니다. 그런데 한 가지 생각이 머리를 떠나지 않았습니다.

*'지금은 내가 수동으로 관리하지만, 만약 새벽 2시에 컨테이너가 죽으면 누가 다시 살려주지?'*

자동 복구, 무중단 배포, 다중 서버 관리, 설정 분리. 실제 운영에서 마주칠 이런 과제들을 한 번에 풀어주는 자동화 시스템이 다음 챕터의 주인공 **쿠버네티스(Kubernetes)** 입니다.
