...
토요일 아침이었습니다. 어제 봤던 에러 로그가 다시 떠올랐습니다. 책상 위에는 어젯밤 영상을 보며 적어 둔 노트가 펼쳐져 있었고, '컨테이너', '도커', '쿠버네티스' 세 단어가 적혀 있었습니다.

오픈이는 커피 한 잔을 들고 노트북 앞에 앉았습니다. 오늘의 목표는 하나였습니다.
...
Docker와 컨테이너를 이해하려면 먼저 가상화를 알아야 합니다. 주방을 예로 들어 보겠습니다.
...
두 번째 방식은 냉장고와 가스레인지 같은 공용 설비는 함께 쓰고, 칼·도마·조리대처럼 개인이 쓸 도구만 각자 챙기는 방법입니다. 장비 수는 그대로인데 공간은 **네 명이 동시에 쓸 수 있습니다**.

![](../assets/CH02/chap01-7.png)

*그림 2-3. 공용 설비는 공유하고 개별 공간만 쪼개는 두 번째 방식*

이렇게 하나의 주방을 여러 명이 사용하는 것처럼, 하나의 컴퓨터를 여러 대처럼 나눠 쓰는 IT 기술을 **가상화(Virtualization)** 라고 부릅니다.

:::term-box
**가상화(Virtualization)**: 하나의 물리 서버를 논리적으로 여러 대처럼 나누어 쓰는 기술입니다. 자원을 효율적으로 쓰면서 서로 다른 환경을 안전하게 격리하는 것이 목적입니다.
:::

가상화에도 두 가지 방식이 있습니다. 환경을 통째로 복제하는 쪽이 **하이퍼바이저 가상화**, 공유하는 쪽이 **컨테이너 가상화**입니다. **Docker는 두 번째 방식(컨테이너 가상화)의 도구입니다.**

:::note
**하이퍼바이저 가상화 vs 컨테이너 가상화**

**하이퍼바이저 가상화(Hypervisor Virtualization)** 는 호스트 OS 위에 **또 다른 OS를 통째로 올리는** 방식이고, 결과물은 **가상 머신(VM)** 입니다. 격리는 완전하지만 OS 전체가 새로 떠야 해서 무겁고 기동이 느립니다.

반면 **컨테이너 가상화(Container Virtualization)** 는 **호스트 OS의 커널을 공유**하면서 파일시스템·네트워크·프로세스 공간만 따로 두는 방식이고, 결과물은 **컨테이너**입니다. 커널을 공유하므로 가볍고 빠르게 뜹니다.
:::
...
컨테이너 가상화 구조는 크게 세 계층입니다. 맨 아래는 **하드웨어 계층**, 그 위는 호스트 OS와 Docker 엔진이 있는 **OS 계층**, 맨 위는 컨테이너들이 놓인 **애플리케이션 계층**입니다. 컨테이너마다 애플리케이션과 라이브러리가 독립적으로 담깁니다.

이 구조에서 주방장 역할을 하는 것이 호스트 OS의 핵심, 곧 **커널**입니다. 주방장이 요리사들의 동선과 역할을 조율하듯, 컨테이너마다 독립된 프로세스와 격리된 메모리 공간을 할당하고 관리하는 일은 모두 커널이 합니다.
...
사용자가 컨테이너 실행 명령을 내리면, **Docker 엔진**은 가장 먼저 로컬에 해당 이미지가 있는지 확인하고, 없으면 Docker Hub에서 자동으로 내려받습니다. 이미지가 준비되면 Docker 엔진은 그 이미지를 바탕으로 격리된 프로세스를 만들어 달라고 호스트 OS의 **커널**에 요청합니다. 요청을 받은 커널은 격리된 공간을 만든 뒤 그 안에서 새 프로세스를 띄웁니다. 그 공간에서 실행되는 프로세스가 곧 **컨테이너**입니다.
...
<div class="svg-figure">
<svg viewBox="0 0 620 292" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="이미지 상자 안에 베이스 OS부터 애플리케이션 코드까지 레이어가 아래에서 위로 쌓여 있는 구조">
  <text x="310" y="28" text-anchor="middle" font-size="14" font-weight="700" fill="#1f2937">이미지 = 레이어의 묶음</text>
  <text x="310" y="74" text-anchor="middle" font-size="12" font-weight="700" fill="#334155">이미지</text>
  <rect x="168" y="84" width="284" height="192" rx="8" fill="#fff" stroke="#475569" stroke-width="2"/>
  <rect x="186" y="96" width="248" height="38" rx="4" fill="#fff4ed" stroke="#ff7849" stroke-width="1.7"/>
  <text x="310" y="120" text-anchor="middle" font-size="12" font-weight="700" fill="#7b341e">애플리케이션 코드</text>
  <rect x="186" y="140" width="248" height="38" rx="4" fill="#f1f5f9" stroke="#475569" stroke-width="1.5"/>
  <text x="310" y="164" text-anchor="middle" font-size="12" font-weight="700" fill="#334155">런타임</text>
  <rect x="186" y="184" width="248" height="38" rx="4" fill="#f1f5f9" stroke="#475569" stroke-width="1.5"/>
  <text x="310" y="208" text-anchor="middle" font-size="12" font-weight="700" fill="#334155">시스템 라이브러리</text>
  <rect x="186" y="228" width="248" height="38" rx="4" fill="#e2e8f0" stroke="#475569" stroke-width="1.7"/>
  <text x="310" y="252" text-anchor="middle" font-size="12" font-weight="700" fill="#334155">베이스 OS</text>
</svg>
</div>

*그림 2-8. 베이스부터 한 겹씩 쌓아 하나의 이미지로 묶입니다*

맨 아래 레이어는 어느 앱이든 공통으로 쓰는 OS나 런타임처럼 **거의 바뀌지 않는** 부분이고, 위쪽 레이어는 그 앱에서만 쓰는 **자주 바뀌는** 코드입니다. 그래서 코드만 바뀐 새 버전을 만들 때도 아래 레이어는 그대로 재사용하고, 바뀐 위쪽 레이어만 새로 만듭니다.
...
엔터를 누르자 로그가 화면에 올라온 뒤, 프롬프트가 돌아오지 않았습니다. 새 명령을 입력해도 아무 반응이 없었습니다.

*'뭐가 잘못된 건가.'*

이는 컨테이너가 **포그라운드** 상태로 터미널을 점유하고 있기 때문입니다. 통화 중일 때는 다른 전화를 받을 수 없는 것과 같습니다.

우선 터미널에 `CTRL + C`를 입력해 빠져나왔습니다.
...
컨테이너를 실제 운영 환경에서 활용하기 위해서는 외부와 데이터를 주고받는 네트워크 연결 과정을 반드시 이해해야 합니다.
...
컨테이너 네트워크는 세 가지로 정리됩니다. **컨테이너끼리 연결하는 방법**, **외부 요청이 컨테이너로 들어오는 길**, **이름으로 컨테이너를 찾는 방법**입니다.
...
컨테이너가 실행되는 순간 컨테이너의 격리된 네트워크 공간(**Namespace**)이 케이블(**veth pair**)을 통해 docker0에 연결됩니다. 그러면 docker0가 빈 IP를 컨테이너에 할당합니다.
...
`docker run`으로 띄운 컨테이너는 docker0가 만드는 **기본 네트워크**에 자동으로 들어갑니다. 같은 네트워크 안의 컨테이너끼리는 IP만 알면 통신할 수 있지만, **외부에서는 그 IP로 바로 닿을 수 없습니다**. 두 가지 이유가 있습니다.

- Windows·macOS는 Docker Desktop 안 **가상 리눅스**에서 컨테이너가 돌기 때문에, 호스트 터미널에서 그 IP로 직접 접근할 수 없습니다.
- 컨테이너 IP는 **외부에 공개된 주소가 아니고, 컨테이너가 뜰 때마다 바뀝니다**.
...
`docker run -p 8080:80`을 실행하면 Docker는 **iptables**에 '호스트 8080 → 컨테이너 80' 규칙을 추가합니다. 이후 브라우저에서 8080 포트로 요청이 오면, iptables가 이 규칙대로 도착지를 바꿔 **docker0**를 거쳐 80 포트를 가진 컨테이너로 전달합니다.
...
새 컨테이너가 뜰 때마다 이름과 IP가 자동으로 **Docker DNS**에 등록됩니다. 그 다음 `app` 컨테이너가 `db`라는 이름으로 요청을 보내면 Docker DNS가 먼저 받아 **`db`를 `172.17.0.3`으로 변환**해 알려주고 app은 그 IP로 다시 요청을 보냅니다. 컨테이너가 다시 떠서 **IP가 바뀌어도 이름은 그대로**라 코드를 수정할 필요가 없습니다.
...
<svg viewBox="0 0 700 240" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Docker DNS가 이름을 IP로 변환하는 흐름">
  ...
  <text x="100" y="120" text-anchor="middle" font-size="11" font-family="monospace" fill="#6b7280">172.17.0.2</text>
  ...
  <text x="600" y="120" text-anchor="middle" font-size="11" font-family="monospace" fill="#6b7280">172.17.0.3</text>
  ...
  <text x="225" y="133" text-anchor="middle" font-size="11" fill="#7b341e"><tspan font-weight="700">2.</tspan> 172.17.0.3</text>
  ...
  <text x="350" y="198" text-anchor="middle" font-size="11" fill="#0f172a"><tspan font-weight="700">3.</tspan> 172.17.0.3 으로 요청</text>
</svg>
</div>
...
리눅스를 연습하려면 리눅스 환경이 필요합니다. **Ubuntu**는 가장 널리 쓰이는 배포판 중 하나라 자료가 풍부하고, Docker Hub에 공식 이미지가 올라와 있어서 바로 받아 실행할 수 있습니다. 오픈이는 포트포워딩을 사용해 ubuntu 컨테이너 내부로 들어갔습니다.
...
오픈이가 정리한 자주 쓰는 리눅스 명령어는 다음과 같습니다.
...
오픈이는 네트워크 확인 도구인 `net-tools`를 추가로 설치하고 Nginx가 어느 포트에서 대기 중인지 확인했습니다.
...
서버 설치를 마쳤더라도 운영하다 보면 설정 파일을 수정해야 할 일이 생기기 마련입니다. 리눅스에서 파일을 편집하려면 전용 편집기가 필요한데, **Vim**을 선택해 설치합니다.
...
컨테이너가 **언제 살아 있고 언제 꺼지는지** 아직 헷갈렸습니다. 이 규칙부터 정리해보기로 했습니다.

## 2.6 포그라운드와 백그라운드

### 2.6.1 exit vs detach

오픈이는 같은 상황을 한 번 더 만들어 비교해 보기로 했습니다. 이름을 붙여 띄운 뒤 `exit`로 나와 봤습니다.
...
`exit` 말고 다른 방식이 있는지 찾아봤습니다. `CTRL+P → CTRL+Q`를 쓰면 컨테이너는 그대로 두고 쉘만 빠져나올 수 있습니다. 같은 방식으로 한 번 더 실행하고 이 조합으로 빠져나왔습니다.
...
오픈이는 챕터 2.3.3에서 봤던 `-d` 옵션을 사용해 ubuntu를 백그라운드로 실행해 보기로 했습니다.
...
실행 목록에 ubuntu가 없었습니다. **메인 프로세스**가 종료되면 컨테이너도 함께 종료되기 때문입니다. nginx는 똑같은 `-d`로 잘 돌아갔는데 ubuntu만 안 된 이유도 여기에 있었습니다.

먼저 nginx는 **'웹 서버로서 요청을 대기하는 것'** 이 목적입니다. 그래서 메인 프로세스인 **nginx**는 요청이 올 때까지 계속 깨어 있는 데몬으로 동작합니다. 백그라운드에서도 '대기'라는 할 일이 명확히 있기 때문에 프로세스가 죽지 않고 컨테이너를 계속 살려둡니다.

:::term-box
**데몬(Daemon)**: 백그라운드에서 요청을 기다리며 계속 실행되는 프로그램입니다. nginx나 데이터베이스처럼 '서버' 역할을 수행하며, 요청을 기다리는 것 자체가 일이라 터미널이 없어도 종료되지 않습니다.
:::

반면 ubuntu는 **'사용자에게 명령어를 입력받는 것'** 이 목적입니다. 그래서 메인 프로세스인 **bash**는 사용자의 입력을 받는 터미널이 있어야 합니다. `-d` 옵션을 줘서 터미널이 연결되지 않으면, bash는 더 이상 할 일이 없다고 판단해 종료됩니다.

:::term-box
**bash(셸)**: 사용자의 명령어를 운영체제에 전달하는 창구입니다. 입력받을 터미널이 없으면 할 일이 사라져 프로세스가 종료됩니다.
:::

ubuntu를 백그라운드로 실행하려면 `-dit`을 쓰거나 메인 프로세스를 다른 걸로 바꿔야 합니다.

### 2.6.3 CMD로 메인 프로세스 바꾸기

이번에는 ubuntu의 메인 프로세스를 유지하기 위해 메인 프로세스를 바꾸는 테스트를 해 보기로 했습니다. bash 대신 **sleep 1000** 을 메인으로 지정했습니다.
...
오픈이는 먼저 ubuntu를 백그라운드로 실행하고, 그 컨테이너에 `docker attach <컨테이너ID>`로 접속했습니다.
...
ubuntu의 입력 터미널이 나타났습니다. 여러 터미널에서 붙어도 같은 화면을 공유하지만, 잘못 빠져나오면 메인 프로세스가 종료되어 컨테이너 전체가 꺼집니다. 오픈이는 `CTRL+P → CTRL+Q`로 안전하게 빠져나왔습니다.

이번에는 같은 컨테이너에 `docker exec -it <컨테이너ID> bash`로 다시 들어가봤습니다.
