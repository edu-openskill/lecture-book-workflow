...
이제 남은 건 직접 알아보는 일이었습니다. 오늘의 목표는 하나였습니다.
...
- **macOS Apple Silicon(M1/M2/M3 등)**: 일부 AMD64 이미지 호환을 위해 **Rosetta 2** 설치를 권장합니다. 터미널에서 `softwareupdate --install-rosetta` 한 줄로 설치할 수 있습니다.
:::
...
이번 챕터는 공개 이미지(`nginx` 등)로 실습합니다. 클론한 예제 코드는 챕터 3부터 사용합니다.
::::
...
컨테이너 가상화 구조는 크게 세 계층으로 나뉩니다. 맨 아래는 **하드웨어 계층**, 그 위는 호스트 OS와 Docker 엔진이 있는 **OS 계층**, 맨 위는 독립된 컨테이너들이 놓인 **애플리케이션 계층**입니다. 컨테이너마다 실행에 필요한 애플리케이션과 라이브러리가 독립적으로 담깁니다.

이 구조에서 **Docker 엔진**은 주문을 받는 매니저 역할을 합니다. 우리가 커널을 직접 제어하는 대신 Docker 엔진에게 명령을 내리면, 엔진이 그 명령을 커널에 전달합니다.

명령을 전달받아 실제로 격리 공간을 만드는 **주방장**은 호스트 OS의 핵심인 **커널**입니다. 주방장이 주방을 총괄하듯, 커널은 컨테이너마다 독립된 프로세스와 격리된 메모리 공간을 할당하고 관리합니다.
...
*그림 2-5. 커널은 OS의 핵심으로, 앱과 하드웨어 사이에서 시스템 자원을 관리합니다*
...
*그림 2-6. 컨테이너 실행 명령이 커널의 격리 기능을 타고 컨테이너가 되는 흐름*

사용자가 컨테이너 실행 명령을 내리면, **Docker 엔진**은 가장 먼저 로컬에 해당 이미지가 있는지 확인하고, 없으면 Docker Hub에서 자동으로 내려받습니다. 이미지가 준비되면 Docker 엔진은 그 이미지를 바탕으로 격리된 프로세스를 만들어 달라고 호스트 OS의 **커널**에 요청합니다. 요청을 받은 커널은 격리된 공간을 만든 뒤 그 안에서 새 프로세스를 띄웁니다. 격리된 공간에서 실행되는 프로세스가 곧 **컨테이너**입니다.

*'이미지로 컨테이너를 만드는데, 정작 그 이미지가 뭐지.'*

## 2.2 이미지 - 컨테이너의 설계도

### 2.2.1 이미지란?

**이미지(Image)** 는 컨테이너가 무엇을 실행할지를 담은 **원본**입니다. 컨테이너는 격리된 **빈 공간**일 뿐이고, 그 안에서 무엇이 어떻게 동작할지는 모두 이미지에 들어 있습니다.
...
*그림 2-7. 하나의 이미지로 여러 컨테이너를 찍어내는 구조*

이미지는 **붕어빵 틀**과 같습니다. 틀(이미지) 하나로 붕어빵(컨테이너)을 얼마든지 찍어낼 수 있고, 붕어빵을 만든다고 해서 틀 자체가 변하지는 않습니다. 같은 이미지로 만든 컨테이너는 어떤 환경에서 실행하든 완전히 동일하게 작동합니다. 이 덕분에 **내 PC에서 돌아가던 환경을 서버에 그대로 재현하는 일**이 가능해집니다.
...
*그림 2-8. 이미지를 실행하면 레이어 묶음 위에서 프로그램이 도는 컨테이너가 됩니다*
...
이번에는 컨테이너 ID가 한 줄 찍히고 곧바로 프롬프트가 돌아왔습니다. 방금 프롬프트가 돌아오지 않던 것과 달리, `-d`로 띄우면 컨테이너는 백그라운드에서 돌고 터미널은 그대로 쓸 수 있습니다.
...
*그림 2-14. 컨테이너 통신 한눈에 보기 - 외부 진입과 컨테이너끼리 IP 통신*
...
*그림 2-15. 주방 모니터·전용 통신 케이블·주문 전광판이 컨테이너 네트워크의 비유*
...
컨테이너가 실행되는 순간 컨테이너의 격리된 네트워크 공간(**Namespace**)이 케이블(**veth pair**)을 통해 docker0에 연결됩니다. 그러면 Docker가 docker0의 네트워크에서 빈 IP 하나를 컨테이너에 배정합니다.

이제 A 컨테이너가 B의 IP로 호출하면 데이터는 **A → docker0 → B** 순서로 흐릅니다.

<div class="svg-figure">
<svg viewBox="0 0 760 260" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="컨테이너 네트워크 격리 구조: 호스트 안에 두 개의 Namespace와 docker0">
  <defs>
  ...
  <!-- Namespace A (호스트 안 좌측) -->
  <rect x="40" y="85" width="180" height="150" rx="8" fill="#fff" stroke="#9ca3af" stroke-width="1.4" stroke-dasharray="5,3"/>
  <text x="55" y="103" font-size="11" font-weight="600" fill="#6b7280">Namespace A</text>
  <rect x="55" y="115" width="150" height="105" rx="6" fill="#fff" stroke="#475569" stroke-width="1.6"/>
  <text x="130" y="155" text-anchor="middle" font-size="13" font-weight="700" fill="#0f172a">컨테이너 A</text>
  <text x="130" y="185" text-anchor="middle" font-size="11" font-family="monospace" fill="#6b7280">172.17.0.2</text>
  ...
  <!-- Namespace B (호스트 안 우측) -->
  <rect x="540" y="85" width="180" height="150" rx="8" fill="#fff" stroke="#9ca3af" stroke-width="1.4" stroke-dasharray="5,3"/>
  <text x="555" y="103" font-size="11" font-weight="600" fill="#6b7280">Namespace B</text>
  <rect x="555" y="115" width="150" height="105" rx="6" fill="#fff" stroke="#475569" stroke-width="1.6"/>
  <text x="630" y="155" text-anchor="middle" font-size="13" font-weight="700" fill="#0f172a">컨테이너 B</text>
  <text x="630" y="185" text-anchor="middle" font-size="11" font-family="monospace" fill="#6b7280">172.17.0.3</text>

  <!-- veth pair: Namespace A ↔ docker0 -->
...
- Windows·macOS에서는 컨테이너가 Docker Desktop 속 가상 리눅스 안에서 돌기 때문에, 외부는 물론 호스트에서도 컨테이너 IP로 바로 접근하기 어렵습니다.
- 게다가 컨테이너 IP는 **새로 띄울 때마다 바뀌어**, 외부에 고정 주소로 알려 줄 수도 없습니다.
...
*그림 2-17. 입구 키오스크가 손님의 주문을 각 식당으로 넘겨주는 비유*
...
*그림 2-18. 호스트 포트가 컨테이너 포트로 연결되는 구조*

*'아, 외부 IP가 따로 있는 게 아니구나. 내 컴퓨터 포트와 컨테이너 포트를 포트포워딩으로 연결해 줘야 외부 요청이 안으로 들어올 수 있는 거네.'*
...
*그림 2-19. 안내 지도가 이름을 위치로 바꿔주는 비유*
...
새 컨테이너가 뜰 때마다 이름과 IP가 자동으로 **Docker DNS**에 등록됩니다. 그 다음 `app` 컨테이너가 `db`라는 이름으로 요청을 보내면 Docker DNS가 먼저 받아 **`db`를 `172.18.0.3`으로 변환**해 알려주고 app은 그 IP로 다시 요청을 보냅니다. 컨테이너가 다시 떠서 IP가 바뀌어도 **같은 이름으로 통신할 수 있습니다**.
...
*그림 2-20. Docker DNS가 이름을 IP로 변환*

다만 내장 DNS는 **사용자 정의 네트워크(User-defined Network)** 에서만 동작합니다. 기본 네트워크(docker0)에서는 이름으로 찾을 수 없기 때문에, 직접 네트워크를 만들어 컨테이너들을 묶어주어야 합니다. 구체적인 생성 방법은 다음 챕터에서 다룹니다.

컨테이너 통신을 살펴봤으니, 이제 컨테이너 내부를 들여다볼 차례입니다. 컨테이너 안은 작은 리눅스 환경과 같아서, 가장 먼저 기본 리눅스 명령어부터 알아보겠습니다.

## 2.5 컨테이너 내부 - 격리된 작은 리눅스

리눅스를 연습하려면 리눅스 환경이 필요합니다. **Ubuntu**는 가장 널리 쓰이는 배포판 중 하나라 자료가 풍부하고, Docker Hub에 공식 이미지가 올라와 있어서 바로 받아 실행할 수 있습니다. 포트포워딩을 사용해 ubuntu 컨테이너 내부로 들어가 보겠습니다.

```bash [터미널] ubuntu 컨테이너 안으로 들어가기
# -i 입력 받기, -t 터미널 화면, -p 80:80 포트포워딩
...
*그림 2-21. Ubuntu 컨테이너 안으로 들어간 상태*
...
*그림 2-24. nginx 환영 페이지 응답*

익숙한 nginx 환영 페이지가 떴습니다. 컨테이너를 실행할 때 `-p 80:80`으로 포트를 열어 둔 덕에, 컨테이너 안의 nginx와 브라우저가 연결됐기 때문입니다.
...
컨테이너 안에서 작업을 마치고 빠져나올 때는 `exit`를 칩니다. 그렇게 호스트로 돌아온 뒤 `docker ps`로 실행 중인 컨테이너를 확인했더니, 방금까지 들어가 있던 ubuntu가 목록에 없었습니다.

*'안에서 나오기만 했는데 컨테이너가 왜 사라졌지.'*

방금처럼 컨테이너가 언제 멈추고 언제 계속 도는지 이해하려면, 그 중심에서 도는 **메인 프로세스**를 알아야 합니다.
...
*그림 2-27. 컨테이너 안에서 도는 중심 프로그램이 메인 프로세스입니다*

### 2.6.2 메인 프로세스에서 빠져나오기 - exit와 detach
...
*그림 2-30. ubuntu는 -d만으로는 바로 종료*

*'nginx는 백그라운드에서 실행이 됐는데, ubuntu는 안 되는 건가?'*

ubuntu는 백그라운드로 실행되자마자 종료되어 실행 목록에 보이지 않습니다. nginx와 ubuntu의 메인 프로세스가 다르기 때문입니다.
...
*그림 2-31. 같은 PID 1 자리에 이미지마다 다른 프로그램이 메인 프로세스로 들어갑니다*
...
**nginx와의 차이**: 반면 nginx는 사용자가 터미널로 연결하지 않아도 백그라운드에서 계속 실행되었습니다. 웹 서버인 nginx의 목적은 사용자의 입력을 받는 것이 아니라 외부에서 들어오는 **웹 요청**을 기다리는 것이기 때문입니다. 터미널이 없어도 **요청 대기**라는 명확한 할 일이 끊기지 않고 유지되므로 종료되지 않고 계속 떠 있게 됩니다.
:::

결국 컨테이너가 백그라운드에서 계속 살아 있으려면, **메인 프로세스에 끊기지 않는 할 일이 있어야 합니다.** 그 할 일을 만드는 방법은 두 가지입니다. **메인 프로세스가 입력을 기다리도록 터미널을 함께 열어 두거나, 메인 프로세스 자체를 할 일이 끊기지 않는 다른 명령으로 바꾸면 됩니다.**

### 2.6.4 메인 프로세스 바꾸기 - CMD

이번에는 메인 프로세스를 직접 바꿔 보겠습니다. ubuntu의 기본 메인 프로세스인 bash 대신, 1000초 동안 멈춰 있는 **sleep 1000** 을 지정합니다.
...
*그림 2-32. CMD를 sleep으로 바꾸면 유지*

이제 ubuntu가 목록에 남아 있습니다. 메인 프로세스가 bash에서, 터미널이 없어도 1000초 동안 할 일이 있는 sleep으로 바뀌었기 때문입니다. 이렇게 **기본 메인 프로세스를 다른 명령으로 덮어쓰는 설정을 CMD**라고 합니다.

:::term-box
**CMD**: 컨테이너가 시작될 때 실행되는 **메인 프로세스**를 정의하는 설정입니다. `docker run <이미지> <명령>` 형태로 명령을 직접 적어 주면, 이미지에 정해진 기본 메인 프로세스 대신 그 명령이 메인 프로세스가 됩니다.
:::
...
*그림 2-33. attach는 이미 떠 있는 PID 1 프로세스에 터미널을 직접 연결하는 방식*
...
*그림 2-35. exec는 컨테이너 안에 새 bash 프로세스를 띄우고 내 터미널에 연결하는 방식*

새 프로세스에 연결되므로 exit해도 메인 프로세스는 그대로입니다. `docker exec -it <컨테이너ID> bash`로 접근합니다.
...
목록에 bash가 둘 있습니다. **PID 1**은 앞서 본 메인 프로세스이고, **PID 12**가 방금 exec로 새로 실행한 프로세스입니다. 두 프로세스는 같은 컨테이너의 파일과 네트워크를 공유하지만 셸 세션은 서로 독립이라, 한쪽에서 만든 변수는 다른 쪽에서 보이지 않습니다.
...
*그림 2-38. Tomcat 404 에러 화면*

*'연결은 된 거 같은데 왜 404 에러가 뜨지?'*

이유는 Docker Hub에서 내려받은 Tomcat 이미지의 `webapps/` 폴더가 비어 있어, 기본 응답 페이지인 `webapps/ROOT/index.html`이 없기 때문입니다.

이 경로에 index.html을 직접 만들어 브라우저에서 페이지가 응답되도록 해 보겠습니다.
...
*그림 2-41. 로컬 이미지 목록에 새 이미지 추가 확인*
...
*그림 2-42. docker push 실행 결과*

업로드가 완료된 뒤 Docker Hub의 Repositories 탭을 열면 방금 올린 이미지를 확인할 수 있습니다.

![](../assets/CH02/chap01-70.png)

*그림 2-43. Docker Hub 저장소 확인*

## 2.8 마운트로 데이터 보존

방금 한 commit은 그 시점의 컨테이너를 이미지로 만들 뿐, 데이터를 계속 보관하는 저장소는 아닙니다. 컨테이너가 실행되며 쌓는 데이터는 이미지에 남지 않아, 데이터베이스처럼 데이터를 계속 쌓는 컨테이너가 삭제되면 그 데이터도 **통째로 날아갑니다**.
