...
컨테이너에서 빠져나올 때는 `exit`입니다. 그런데 `exit`를 치고 `docker ps`를 쳐 보니, 방금까지 돌던 ubuntu 컨테이너가 목록에서 사라져 있었습니다.

*'어? 안에서 나왔을 뿐인데 왜 꺼지지.'*

컨테이너가 **언제 살아 있고 언제 꺼지는지**는 헷갈리기 쉽습니다. 이 규칙부터 정리해 보겠습니다.

## 2.6 컨테이너 생명주기

### 2.6.1 exit vs detach

같은 상황을 한 번 더 만들어 비교합니다. 이름을 붙여 띄운 뒤 `exit`로 나옵니다.
...
*그림 2-27. exit로 빠져나오면 컨테이너가 종료*

dead는 목록에 없습니다.

`exit` 말고 다른 방식도 있습니다. `CTRL+P → CTRL+Q`를 쓰면 컨테이너는 그대로 두고 쉘만 빠져나올 수 있습니다. 같은 방식으로 한 번 더 실행한 뒤 이 조합으로 빠져나옵니다.
...
*그림 2-28. CTRL+P → CTRL+Q로는 컨테이너가 유지*

이번에는 alive가 살아 있습니다. 같은 `docker run`인데 나오는 방식에 따라 결과가 갈립니다.
...
챕터 2.3.3에서 본 `-d` 옵션으로 ubuntu를 백그라운드로 실행합니다.
...
*그림 2-29. ubuntu는 -d만으로는 바로 종료*

*'어 왜 실행 목록에 없지?'*

실행 목록에 ubuntu가 없는 이유는 메인 프로세스가 종료되면 컨테이너도 함께 종료되기 때문입니다.

:::note
**nginx가 살아 있는 이유**: 웹 서버인 nginx는 백그라운드에서 요청을 기다리며 계속 실행되는 데몬(Daemon) 프로그램입니다. '요청 대기'라는 명확한 할 일이 있으므로 프로세스가 죽지 않고 컨테이너를 살려둡니다.

**ubuntu가 바로 종료된 이유**: ubuntu 이미지의 메인 프로세스인 bash(셸)는 사용자의 명령어를 입력받는 창구입니다. `-d` 옵션으로 입력받을 터미널이 연결되지 않으면, bash는 할 일이 없다고 판단해 그 즉시 종료됩니다.
:::

결국 ubuntu처럼 터미널 입력이 필요한 컨테이너를 백그라운드로 띄우려면, 터미널을 열어 두는 `-dit`로 실행하거나 메인 프로세스를 계속 켜져 있는 다른 명령으로 바꿔야 합니다.

### 2.6.3 CMD로 메인 프로세스 바꾸기

이번에는 ubuntu의 메인 프로세스를 바꿔 컨테이너가 유지되는지 확인합니다. bash 대신 **sleep 1000** 을 메인으로 지정합니다.
...
이번엔 ubuntu가 목록에 남아 있습니다. sleep은 터미널이 없어도 1000초 동안 할 일이 있는 프로세스이기 때문입니다. 이렇게 기본 실행 명령을 덮어쓰는 것이 **CMD**입니다.

:::term-box
**CMD**: 컨테이너가 시작될 때 실행되는 **기본 프로세스**를 정의하는 설정입니다. `docker run <이미지> <CMD옵션>` 형태로 직접 주면 이미지의 메인 프로세스 대신 CMD에 넣은 설정으로 덮어씁니다.
:::

### 2.6.4 백그라운드 접속 - attach vs exec

백그라운드로 실행한 컨테이너에 다시 들어가는 방법은 두 가지입니다. **메인 프로세스에 직접 연결하는 attach**와 **새 프로세스를 하나 더 만드는 exec**, 결과는 비슷해 보이지만 내부 동작이 다릅니다.

그림처럼 `attach`는 컨테이너 안에서 돌고 있는 메인 프로세스에 그대로 연결합니다.
...
*그림 2-31. attach는 이미 떠 있는 PID 1 프로세스에 터미널을 직접 꽂는 방식*

먼저 ubuntu를 백그라운드로 실행하고, 그 컨테이너에 `docker attach <컨테이너ID>`로 접속합니다.
...
ubuntu의 입력 터미널이 나타났습니다. 여러 터미널에서 붙어도 같은 화면을 공유하지만, 잘못 빠져나오면 메인 프로세스가 종료되어 컨테이너 전체가 꺼집니다. `CTRL+P → CTRL+Q`로 안전하게 빠져나옵니다.

이번에는 같은 컨테이너에 `docker exec -it <컨테이너ID> bash`로 다시 들어갑니다.
...
그림처럼 `exec`는 메인 프로세스에 직접 연결하는 대신, 컨테이너 안에 **별도의 bash 프로세스를 새로 하나 띄우고** 거기에 내 터미널을 연결합니다. 메인 프로세스와 분리되어 있어 이쪽에서 `exit`로 나와도 메인 프로세스는 종료되지 않습니다.
...
`exec`가 정말 새 프로세스를 만드는지 눈으로 확인하기 위해 호스트에서 터미널을 하나 더 열고 컨테이너 내부의 프로세스 목록을 찍어 봅니다.
...
**PID 1**이 컨테이너의 메인 프로세스이고, **PID 12**가 방금 `exec`로 만들어진 프로세스입니다. 두 프로세스는 같은 컨테이너의 파일과 네트워크를 공유합니다. 하지만 셸 세션은 독립이라 한쪽에서 만든 변수는 다른 쪽에서 보이지 않습니다. 

*'attach는 메인 프로세스에 연결되고, exec는 새 프로세스를 띄우는구나. 앞으로는 exec만 쓰자.'*
...
오픈이는 이 경로에 index.html을 직접 만들어 보기로 했습니다.

```bash [터미널] Tomcat 컨테이너 접속과 설정 수정
docker exec -it 5fcd bash              # 실행 중인 컨테이너 안으로 들어가기
...
vim 패키지로 index.html 생성 및 저장을 마치고 **localhost:8080** 으로 다시 접속했더니 방금 만든 페이지가 응답되었습니다.
...
업로드가 완료된 뒤 Docker Hub의 Repositories 탭을 열어 보니 방금 올린 이미지를 확인할 수 있습니다.
...
방금 commit으로 만든 이미지에 담기는 건 컨테이너의 초기 상태뿐입니다.

컨테이너가 실행된 뒤 새로 쌓이는 데이터는 **이미지에 저장되지 않습니다**. 데이터베이스처럼 실시간으로 데이터를 쌓는 컨테이너가 삭제되면, 그동안 저장한 데이터도 **통째로 날아갑니다**.

*'컨테이너를 내려도 데이터는 남아 있어야 하는데.'*

결국 데이터를 잃지 않으려면 컨테이너 외부에 보관해야 합니다. 이를 위해 컨테이너와 외부 저장소를 연결해 주는 기능이 바로 **마운트**입니다.

Docker의 마운트는 두 가지 방식이 있습니다.

### 2.8.1 바인드 마운트: 호스트 폴더와 직접 연결

먼저 해 본 방식은 **호스트 PC의 특정 폴더**를 **컨테이너 내부의 경로**와 연결하는 **바인드 마운트(Bind Mount)** 입니다. USB를 컴퓨터에 꽂으면 그 안의 파일을 그대로 볼 수 있듯, 컨테이너도 호스트 PC의 폴더를 꽂아서 쓰는 방식입니다.
...
오픈이는 호스트의 `/c/app/bind` 경로에 폴더를 하나 만들고, 컨테이너 안의 경로와 직접 연결했습니다.
...
같은 파일이 **양쪽에** 있었습니다. 호스트에서 소스 코드를 고치면 컨테이너에 바로 반영되니까, **개발할 때 쓰기 딱 맞는 방식**입니다.
...
오픈이는 `metacoding-volume`이라는 이름으로 볼륨을 하나 만들면서 ubuntu 컨테이너를 실행했습니다. **존재하지 않는 볼륨 이름을 주면 Docker가 자동으로 만들어 줍니다**.
...
컨테이너 안에 들어가자 `/app/volume` 폴더가 자동으로 만들어져 있었습니다. 오픈이는 이 폴더에 빈 파일을 하나 만들고, 컨테이너를 빠져나와 볼륨이 그대로 남아 있는지 확인했습니다.
...
컨테이너는 사라졌지만 볼륨은 목록에 그대로 남아 있었습니다. 이어서 같은 볼륨을 연결해 새 컨테이너를 다시 실행했습니다.
...
방금 전에 만들었던 `b.txt`가 새 컨테이너 안에서도 그대로 보였습니다. **컨테이너를 내려도 데이터는 사라지지 않았습니다**. 처음 던졌던 걱정 한 줄이 풀렸습니다.
