...
게다가 'Pod는 새로 실행될 때마다 IP가 매번 바뀐다'는 점도 마음에 걸렸습니다. 오늘 이 네 개의 IP 중 하나를 골라 두어도, 그 주소가 내일도 같은 Pod를 가리킨다는 보장이 없었습니다.
...
<svg viewBox="0 0 760 260" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Service의 고정 주소가 여러 Pod에 요청을 분배하는 구조">
  <defs>
    <marker id="sv-p" markerWidth="10" markerHeight="10" refX="8" refY="3" orient="auto"><path d="M0,0 L0,6 L8,3 z" fill="#475569"/></marker>
  </defs>
  <text x="380" y="22" text-anchor="middle" font-size="13" font-weight="700" fill="#1f2937">Service의 고정 주소가 여러 Pod에 요청을 분배하는 구조</text>
  <rect x="60" y="100" width="180" height="80" rx="8" fill="#fff4ed" stroke="#ff7849" stroke-width="1.8"/>
  <text x="150" y="135" text-anchor="middle" font-size="14" font-weight="700" fill="#7b341e">Service</text>
  <text x="150" y="158" text-anchor="middle" font-size="11" fill="#7b341e">고정 접근 주소</text>
  <line x1="240" y1="130" x2="500" y2="65" stroke="#475569" stroke-width="1.8" marker-end="url(#sv-p)"/>
  <line x1="240" y1="140" x2="500" y2="140" stroke="#475569" stroke-width="1.8" marker-end="url(#sv-p)"/>
  <line x1="240" y1="150" x2="500" y2="215" stroke="#475569" stroke-width="1.8" marker-end="url(#sv-p)"/>
  <text x="370" y="135" text-anchor="middle" font-size="11" fill="#0f172a" font-weight="600">요청 분배</text>
...
:::term-box
**Service**: Pod 앞단의 고정 접근점입니다. Pod가 삭제되고 다시 생성되어 IP가 바뀌어도 Service 주소는 바뀌지 않습니다. 뒤에 여러 Pod가 연결되어 있으면 요청을 골고루 나누어 주는 로드밸런싱 기능을 수행합니다.
:::
...
*그림 5-2. ClusterIP — 외부 요청은 닿지 못하고 내부 Pod끼리만 통신*
...
*그림 5-3. NodePort — 노드의 특정 포트로 외부 접근 허용*
...
*그림 5-4. LoadBalancer — 클라우드가 공인 IP를 발급하고 여러 노드에 분산*
...
*그림 5-6. minikube service URL 생성*
...
*그림 5-9. Ingress가 도메인과 경로를 읽어 요청을 적절한 Service로 연결하는 구조*
...
*그림 5-10. Ingress 리소스(YAML, 선언)와 Ingress Controller(Pod, 집행) — 선언과 집행의 분리*
...
<div class="svg-figure">
<svg viewBox="0 0 760 200" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Ingress 실습 4단계 흐름 — Controller 실행, 백엔드 서비스, 규칙 문서, 브라우저 접속">
  <defs>
    <marker id="if-p" markerWidth="10" markerHeight="10" refX="8" refY="3" orient="auto"><path d="M0,0 L0,6 L8,3 z" fill="#475569"/></marker>
  </defs>
  <text x="380" y="22" text-anchor="middle" font-size="13" font-weight="700" fill="#1f2937">Ingress 실습 4단계</text>
  <rect x="20" y="60" width="160" height="100" rx="8" fill="#fff" stroke="#ff7849" stroke-width="1.8"/>
  <circle cx="40" cy="80" r="11" fill="#ff7849"/>
  <text x="40" y="84" text-anchor="middle" font-size="11" font-weight="700" fill="#fff">1</text>
  <text x="100" y="105" text-anchor="middle" font-size="12" font-weight="700" fill="#7b341e">Controller 실행</text>
  <text x="100" y="125" text-anchor="middle" font-size="10" font-family="monospace" fill="#475569">minikube addons</text>
  <text x="100" y="140" text-anchor="middle" font-size="10" font-family="monospace" fill="#475569">enable ingress</text>
  <line x1="180" y1="110" x2="200" y2="110" stroke="#475569" stroke-width="1.6" marker-end="url(#if-p)"/>
  <rect x="200" y="60" width="160" height="100" rx="8" fill="#fff" stroke="#475569" stroke-width="1.6"/>
  <circle cx="220" cy="80" r="11" fill="#475569"/>
  <text x="220" y="84" text-anchor="middle" font-size="11" font-weight="700" fill="#fff">2</text>
  <text x="280" y="105" text-anchor="middle" font-size="12" font-weight="700" fill="#0f172a">백엔드 서비스</text>
  <text x="280" y="125" text-anchor="middle" font-size="10" fill="#6b7280">order-service</text>
  <text x="280" y="140" text-anchor="middle" font-size="10" fill="#6b7280">stores-service</text>
  <line x1="360" y1="110" x2="380" y2="110" stroke="#475569" stroke-width="1.6" marker-end="url(#if-p)"/>
  <rect x="380" y="60" width="160" height="100" rx="8" fill="#fff" stroke="#475569" stroke-width="1.6"/>
  <circle cx="400" cy="80" r="11" fill="#475569"/>
  <text x="400" y="84" text-anchor="middle" font-size="11" font-weight="700" fill="#fff">3</text>
  <text x="460" y="105" text-anchor="middle" font-size="12" font-weight="700" fill="#0f172a">규칙 문서</text>
  <text x="460" y="125" text-anchor="middle" font-size="10" fill="#6b7280">Ingress YAML</text>
  <text x="460" y="140" text-anchor="middle" font-size="10" fill="#6b7280">경로 → Service</text>
  <line x1="540" y1="110" x2="560" y2="110" stroke="#475569" stroke-width="1.6" marker-end="url(#if-p)"/>
  <rect x="560" y="60" width="160" height="100" rx="8" fill="#fff" stroke="#475569" stroke-width="1.6"/>
  <circle cx="580" cy="80" r="11" fill="#475569"/>
  <text x="580" y="84" text-anchor="middle" font-size="11" font-weight="700" fill="#fff">4</text>
  <text x="640" y="105" text-anchor="middle" font-size="12" font-weight="700" fill="#0f172a">브라우저 접속</text>
  <text x="640" y="125" text-anchor="middle" font-size="10" font-family="monospace" fill="#475569">/order</text>
  <text x="640" y="140" text-anchor="middle" font-size="10" font-family="monospace" fill="#475569">/stores</text>
</svg>
</div>

*그림 5-11. Ingress 실습 4단계*
...
*그림 5-12. minikube에서 ingress 애드온 활성화*
...
*그림 5-13. Ingress Controller Pod가 Running 상태*
...
두 서비스는 모두 **ClusterIP** 타입이라 클러스터 바깥에서는 직접 부를 수 없는 내부 전용 창구입니다. 바깥 요청을 이 둘로 갈라 보낼 공식 앱, 즉 Ingress 규칙이 필요했습니다.
...
*그림 5-14. Ingress 리소스 등록 확인 — ex12-ingress*
...
*그림 5-15. minikube tunnel 실행으로 외부 접근 경로 확보*
...
*그림 5-18. Endpoint Controller가 Pod 변동을 감시해 Pod IP 목록을 갱신합니다*
...
*그림 5-19. 패킷은 커널이 직접 처리하고, kube-proxy는 옆에서 iptables 항목만 등록·갱신합니다*
...
*그림 5-20. DNS가 Service 이름을 ClusterIP로 변환합니다*
...
<div class="svg-figure">
<svg viewBox="0 0 1080 600" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="STEP 1 — 외부 요청이 NodePort 입구를 통해 클러스터에 진입하는 단계">
  <defs>
  ...
  <rect x="40" y="14" width="68" height="22" rx="11" fill="#ff7849"/>
  <text x="74" y="29" text-anchor="middle" font-size="11" font-weight="700" fill="#fff">STEP 1</text>
  <text x="120" y="30" font-size="13" font-weight="700" fill="#1f2937">외부 요청이 NodePort 입구로 클러스터에 진입</text>
  ...
  <line x1="150" y1="170" x2="232" y2="170" stroke="#ff7849" stroke-width="2.2" marker-end="url(#fl1-a)"/>
  <text x="190" y="160" text-anchor="middle" font-size="10" font-family="monospace" font-weight="700" fill="#7b341e">localhost:9000</text>
  ...
  <rect x="252" y="142" width="100" height="70" rx="6" fill="#fff" stroke="#ff7849" stroke-width="2.2"/>
  ...
  <line x1="370" y1="170" x2="490" y2="170" stroke="#cbd5e1" stroke-width="1.6" marker-end="url(#fl1-d)"/>
  <rect x="490" y="150" width="100" height="40" rx="6" fill="#fff" stroke="#cbd5e1" stroke-width="1.4"/>
  <text x="540" y="174" text-anchor="middle" font-size="11" font-weight="600" fill="#94a3b8">Kube Proxy</text>
  <line x1="590" y1="170" x2="640" y2="170" stroke="#cbd5e1" stroke-width="1.4" marker-end="url(#fl1-d)"/>
  <rect x="640" y="150" width="180" height="40" rx="6" fill="#fff" stroke="#cbd5e1" stroke-width="1.4"/>
  <text x="730" y="174" text-anchor="middle" font-size="11" font-weight="600" fill="#94a3b8">Ingress Controller Pod</text>
  ...
  <line x1="730" y1="190" x2="600" y2="245" stroke="#cbd5e1" stroke-width="1.4" marker-end="url(#fl1-d)"/>
  <line x1="730" y1="190" x2="850" y2="245" stroke="#cbd5e1" stroke-width="1.4" marker-end="url(#fl1-d)"/>
  <rect x="540" y="262" width="120" height="32" rx="16" fill="#fff" stroke="#cbd5e1" stroke-width="1.4"/>
  <text x="600" y="283" text-anchor="middle" font-size="11" font-weight="600" fill="#94a3b8">Service A</text>
  <rect x="790" y="262" width="120" height="32" rx="16" fill="#fff" stroke="#cbd5e1" stroke-width="1.4"/>
  <text x="850" y="283" text-anchor="middle" font-size="11" font-weight="600" fill="#94a3b8">Service B</text>
  <line x1="600" y1="310" x2="600" y2="335" stroke="#cbd5e1" stroke-width="1.4" marker-end="url(#fl1-d)"/>
  <line x1="850" y1="310" x2="850" y2="335" stroke="#cbd5e1" stroke-width="1.4" marker-end="url(#fl1-d)"/>
  ...
  <rect x="490" y="365" width="220" height="55" rx="6" fill="#fff" stroke="#cbd5e1" stroke-width="1.4"/>
  <text x="505" y="382" font-size="10" font-weight="700" fill="#94a3b8">Node 1</text>
  <text x="695" y="382" text-anchor="end" font-size="9" font-family="monospace" fill="#94a3b8">port :9000</text>
  <rect x="555" y="390" width="100" height="22" rx="4" fill="#fff" stroke="#cbd5e1" stroke-width="1"/>
  <text x="605" y="405" text-anchor="middle" font-size="10" font-weight="600" fill="#94a3b8">Kube Proxy</text>
  <rect x="740" y="365" width="220" height="55" rx="6" fill="#fff" stroke="#cbd5e1" stroke-width="1.4"/>
  <text x="755" y="382" font-size="10" font-weight="700" fill="#94a3b8">Node 2</text>
  <text x="945" y="382" text-anchor="end" font-size="9" font-family="monospace" fill="#94a3b8">port :9000</text>
  <rect x="805" y="390" width="100" height="22" rx="4" fill="#fff" stroke="#cbd5e1" stroke-width="1"/>
  <text x="855" y="405" text-anchor="middle" font-size="10" font-weight="600" fill="#94a3b8">Kube Proxy</text>
  <line x1="605" y1="412" x2="585" y2="450" stroke="#cbd5e1" stroke-width="1" stroke-dasharray="3,2" marker-end="url(#fl1-d)"/>
  <line x1="605" y1="412" x2="635" y2="450" stroke="#cbd5e1" stroke-width="1" stroke-dasharray="3,2" marker-end="url(#fl1-d)"/>
  <line x1="855" y1="412" x2="835" y2="450" stroke="#cbd5e1" stroke-width="1" stroke-dasharray="3,2" marker-end="url(#fl1-d)"/>
  <line x1="855" y1="412" x2="885" y2="450" stroke="#cbd5e1" stroke-width="1" stroke-dasharray="3,2" marker-end="url(#fl1-d)"/>
  <rect x="510" y="453" width="200" height="40" rx="6" fill="none" stroke="#cbd5e1" stroke-width="1" stroke-dasharray="4,3"/>
  <rect x="525" y="461" width="65" height="24" rx="12" fill="#fff" stroke="#cbd5e1" stroke-width="1"/>
  <text x="557" y="477" text-anchor="middle" font-size="10" font-weight="600" fill="#94a3b8">Pod A</text>
  <rect x="615" y="461" width="65" height="24" rx="12" fill="#fff" stroke="#cbd5e1" stroke-width="1"/>
  <text x="647" y="477" text-anchor="middle" font-size="10" font-weight="600" fill="#94a3b8">Pod A</text>
  <rect x="760" y="453" width="200" height="40" rx="6" fill="none" stroke="#cbd5e1" stroke-width="1" stroke-dasharray="4,3"/>
  <rect x="775" y="461" width="65" height="24" rx="12" fill="#fff" stroke="#cbd5e1" stroke-width="1"/>
  <text x="807" y="477" text-anchor="middle" font-size="10" font-weight="600" fill="#94a3b8">Pod A</text>
  <rect x="865" y="461" width="65" height="24" rx="12" fill="#fff" stroke="#cbd5e1" stroke-width="1"/>
  <text x="897" y="477" text-anchor="middle" font-size="10" font-weight="600" fill="#94a3b8">Pod B</text>
...
*그림 5-21. 외부 요청이 NodePort(또는 LoadBalancer)로 진입. 실습에선 `minikube tunnel` 이 이 입구 역할*
...
<div class="svg-figure">
<svg viewBox="0 0 1080 600" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="STEP 2 — kube-proxy(1차)가 NodePort로 들어온 요청을 Ingress Controller Pod로 전달">
  <defs>
  ...
  <rect x="40" y="14" width="68" height="22" rx="11" fill="#ff7849"/>
  <text x="74" y="29" text-anchor="middle" font-size="11" font-weight="700" fill="#fff">STEP 2</text>
  <text x="120" y="30" font-size="13" font-weight="700" fill="#1f2937">kube-proxy가 Ingress Controller Pod로 전달</text>
  <text x="20" y="120" font-size="10" font-weight="600" fill="#475569">로컬호스트</text>
  <rect x="20" y="130" width="130" height="100" rx="6" fill="#fff" stroke="#475569" stroke-width="1.6"/>
  <text x="85" y="186" text-anchor="middle" font-size="14" font-weight="700" fill="#0f172a">Host</text>
  <line x1="150" y1="170" x2="232" y2="170" stroke="#cbd5e1" stroke-width="1.4" marker-end="url(#fl2-d)"/>
  <text x="190" y="160" text-anchor="middle" font-size="10" font-family="monospace" fill="#94a3b8">localhost:9000</text>
  <rect x="200" y="80" width="860" height="500" rx="10" fill="none" stroke="#475569" stroke-width="1.4" stroke-dasharray="6,4"/>
  <text x="630" y="100" text-anchor="middle" font-size="11" font-style="italic" fill="#475569">쿠버네티스 가상세계 (클러스터)</text>
  <rect x="235" y="110" width="135" height="135" rx="10" fill="#fff4ed" stroke="#ff7849" stroke-width="1.6"/>
  <text x="302" y="128" text-anchor="middle" font-size="10" font-weight="700" fill="#7b341e">클러스터 입구</text>
  <rect x="252" y="142" width="100" height="70" rx="6" fill="#fff" stroke="#cbd5e1" stroke-width="1.4"/>
  <text x="302" y="170" text-anchor="middle" font-size="14" font-weight="700" fill="#94a3b8">NodePort</text>
  <text x="302" y="190" text-anchor="middle" font-size="12" font-family="monospace" fill="#94a3b8">:9000</text>
  <text x="302" y="234" text-anchor="middle" font-size="9" fill="#7b341e">NodePort Service</text>
  <line x1="370" y1="170" x2="490" y2="170" stroke="#ff7849" stroke-width="2.2" marker-end="url(#fl2-a)"/>
  <rect x="490" y="150" width="100" height="40" rx="6" fill="#fff4ed" stroke="#ff7849" stroke-width="2"/>
  <text x="540" y="174" text-anchor="middle" font-size="11" font-weight="700" fill="#7b341e">Kube Proxy</text>
  <line x1="590" y1="170" x2="640" y2="170" stroke="#ff7849" stroke-width="2.2" marker-end="url(#fl2-a)"/>
  <rect x="640" y="150" width="180" height="40" rx="6" fill="#fff4ed" stroke="#ff7849" stroke-width="2"/>
  <text x="730" y="174" text-anchor="middle" font-size="11" font-weight="700" fill="#7b341e">Ingress Controller Pod</text>
  <rect x="420" y="245" width="600" height="65" rx="32" fill="#f5faff" stroke="#bfdbfe" stroke-width="1"/>
  <text x="1005" y="260" text-anchor="end" font-size="10" font-weight="700" fill="#94a3b8">L7</text>
  <line x1="730" y1="190" x2="600" y2="245" stroke="#cbd5e1" stroke-width="1.4" marker-end="url(#fl2-d)"/>
  <line x1="730" y1="190" x2="850" y2="245" stroke="#cbd5e1" stroke-width="1.4" marker-end="url(#fl2-d)"/>
  <rect x="540" y="262" width="120" height="32" rx="16" fill="#fff" stroke="#cbd5e1" stroke-width="1.4"/>
  <text x="600" y="283" text-anchor="middle" font-size="11" font-weight="600" fill="#94a3b8">Service A</text>
  <rect x="790" y="262" width="120" height="32" rx="16" fill="#fff" stroke="#cbd5e1" stroke-width="1.4"/>
  <text x="850" y="283" text-anchor="middle" font-size="11" font-weight="600" fill="#94a3b8">Service B</text>
  <line x1="600" y1="310" x2="600" y2="335" stroke="#cbd5e1" stroke-width="1.4" marker-end="url(#fl2-d)"/>
  <line x1="850" y1="310" x2="850" y2="335" stroke="#cbd5e1" stroke-width="1.4" marker-end="url(#fl2-d)"/>
  <rect x="420" y="335" width="600" height="100" rx="6" fill="#f8fafc" stroke="#cbd5e1" stroke-width="1"/>
  <text x="1005" y="350" text-anchor="end" font-size="10" font-weight="700" fill="#94a3b8">L4</text>
  <rect x="490" y="365" width="220" height="55" rx="6" fill="#fff" stroke="#cbd5e1" stroke-width="1.4"/>
  <text x="505" y="382" font-size="10" font-weight="700" fill="#94a3b8">Node 1</text>
  ...
  <rect x="805" y="390" width="100" height="22" rx="4" fill="#fff" stroke="#cbd5e1" stroke-width="1"/>
  <text x="855" y="405" text-anchor="middle" font-size="10" font-weight="600" fill="#94a3b8">Kube Proxy</text>
  <line x1="605" y1="412" x2="585" y2="450" stroke="#cbd5e1" stroke-width="1" stroke-dasharray="3,2" marker-end="url(#fl2-d)"/>
  <line x1="605" y1="412" x2="635" y2="450" stroke="#cbd5e1" stroke-width="1" stroke-dasharray="3,2" marker-end="url(#fl2-d)"/>
  <line x1="855" y1="412" x2="835" y2="450" stroke="#cbd5e1" stroke-width="1" stroke-dasharray="3,2" marker-end="url(#fl2-d)"/>
  <line x1="855" y1="412" x2="885" y2="450" stroke="#cbd5e1" stroke-width="1" stroke-dasharray="3,2" marker-end="url(#fl2-d)"/>
  <rect x="510" y="453" width="200" height="40" rx="6" fill="none" stroke="#cbd5e1" stroke-width="1" stroke-dasharray="4,3"/>
  <rect x="525" y="461" width="65" height="24" rx="12" fill="#fff" stroke="#cbd5e1" stroke-width="1"/>
  <text x="557" y="477" text-anchor="middle" font-size="10" font-weight="600" fill="#94a3b8">Pod A</text>
  <rect x="615" y="461" width="65" height="24" rx="12" fill="#fff" stroke="#cbd5e1" stroke-width="1"/>
...
*그림 5-22. iptables 규칙을 따라 Ingress Controller Pod로 전달*
...
<div class="svg-figure">
<svg viewBox="0 0 1080 600" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="STEP 3 — Ingress Controller가 URL을 읽고 Service를 선택, DNS가 이름을 ClusterIP로 변환">
  <defs>
  ...
  <rect x="40" y="14" width="68" height="22" rx="11" fill="#ff7849"/>
  <text x="74" y="29" text-anchor="middle" font-size="11" font-weight="700" fill="#fff">STEP 3</text>
  <text x="120" y="30" font-size="13" font-weight="700" fill="#1f2937">Controller가 URL 읽고 Service 선택 + DNS가 이름→ClusterIP</text>
  <text x="20" y="120" font-size="10" font-weight="600" fill="#475569">로컬호스트</text>
  <rect x="20" y="130" width="130" height="100" rx="6" fill="#fff" stroke="#475569" stroke-width="1.6"/>
  <text x="85" y="186" text-anchor="middle" font-size="14" font-weight="700" fill="#0f172a">Host</text>
  <line x1="150" y1="170" x2="232" y2="170" stroke="#cbd5e1" stroke-width="1.4" marker-end="url(#fl3-d)"/>
  <text x="190" y="160" text-anchor="middle" font-size="10" font-family="monospace" fill="#94a3b8">localhost:9000</text>
  <rect x="200" y="80" width="860" height="500" rx="10" fill="none" stroke="#475569" stroke-width="1.4" stroke-dasharray="6,4"/>
  <text x="630" y="100" text-anchor="middle" font-size="11" font-style="italic" fill="#475569">쿠버네티스 가상세계 (클러스터)</text>
  <rect x="235" y="110" width="135" height="135" rx="10" fill="#fff4ed" stroke="#ff7849" stroke-width="1.6"/>
  ...
  <text x="302" y="190" text-anchor="middle" font-size="12" font-family="monospace" fill="#94a3b8">:9000</text>
  <text x="302" y="234" text-anchor="middle" font-size="9" fill="#7b341e">NodePort Service</text>
  <line x1="370" y1="170" x2="490" y2="170" stroke="#cbd5e1" stroke-width="1.4" marker-end="url(#fl3-d)"/>
  <rect x="490" y="150" width="100" height="40" rx="6" fill="#fff" stroke="#cbd5e1" stroke-width="1.4"/>
  <text x="540" y="174" text-anchor="middle" font-size="11" font-weight="600" fill="#94a3b8">Kube Proxy</text>
  <line x1="590" y1="170" x2="640" y2="170" stroke="#cbd5e1" stroke-width="1.4" marker-end="url(#fl3-d)"/>
  <rect x="640" y="150" width="180" height="40" rx="6" fill="#fff" stroke="#cbd5e1" stroke-width="1.4"/>
  <text x="730" y="174" text-anchor="middle" font-size="11" font-weight="600" fill="#94a3b8">Ingress Controller Pod</text>
  <rect x="900" y="135" width="140" height="80" rx="8" fill="#fff4ed" stroke="#ff7849" stroke-width="2"/>
  ...
  <line x1="822" y1="160" x2="898" y2="155" stroke="#475569" stroke-width="1.4" stroke-dasharray="4,3" marker-end="url(#fl3-s)"/>
  <text x="860" y="148" text-anchor="middle" font-size="9" font-style="italic" font-family="monospace" fill="#475569">"order-service?"</text>
  <line x1="898" y1="195" x2="822" y2="190" stroke="#ff7849" stroke-width="1.4" stroke-dasharray="4,3" marker-end="url(#fl3-a)"/>
  <text x="860" y="208" text-anchor="middle" font-size="9" font-style="italic" font-family="monospace" fill="#7b341e">"10.96.0.20"</text>
  <rect x="420" y="245" width="600" height="65" rx="32" fill="#f5faff" stroke="#bfdbfe" stroke-width="1"/>
  <text x="1005" y="260" text-anchor="end" font-size="10" font-weight="700" fill="#94a3b8">L7</text>
  <line x1="730" y1="190" x2="600" y2="245" stroke="#ff7849" stroke-width="2.2" marker-end="url(#fl3-a)"/>
  <line x1="730" y1="190" x2="850" y2="245" stroke="#ff7849" stroke-width="2.2" marker-end="url(#fl3-a)"/>
  <rect x="540" y="262" width="120" height="32" rx="16" fill="#fff4ed" stroke="#ff7849" stroke-width="2"/>
  <text x="600" y="283" text-anchor="middle" font-size="11" font-weight="700" fill="#7b341e">Service A</text>
  <rect x="790" y="262" width="120" height="32" rx="16" fill="#fff4ed" stroke="#ff7849" stroke-width="2"/>
  <text x="850" y="283" text-anchor="middle" font-size="11" font-weight="700" fill="#7b341e">Service B</text>
  <line x1="600" y1="310" x2="600" y2="335" stroke="#cbd5e1" stroke-width="1.4" marker-end="url(#fl3-d)"/>
  <line x1="850" y1="310" x2="850" y2="335" stroke="#cbd5e1" stroke-width="1.4" marker-end="url(#fl3-d)"/>
  <rect x="420" y="335" width="600" height="100" rx="6" fill="#f8fafc" stroke="#cbd5e1" stroke-width="1"/>
  <text x="1005" y="350" text-anchor="end" font-size="10" font-weight="700" fill="#94a3b8">L4</text>
  <rect x="490" y="365" width="220" height="55" rx="6" fill="#fff" stroke="#cbd5e1" stroke-width="1.4"/>
  <text x="505" y="382" font-size="10" font-weight="700" fill="#94a3b8">Node 1</text>
  ...
  <rect x="805" y="390" width="100" height="22" rx="4" fill="#fff" stroke="#cbd5e1" stroke-width="1"/>
  <text x="855" y="405" text-anchor="middle" font-size="10" font-weight="600" fill="#94a3b8">Kube Proxy</text>
  <line x1="605" y1="412" x2="585" y2="450" stroke="#cbd5e1" stroke-width="1" stroke-dasharray="3,2" marker-end="url(#fl3-d)"/>
  <line x1="605" y1="412" x2="635" y2="450" stroke="#cbd5e1" stroke-width="1" stroke-dasharray="3,2" marker-end="url(#fl3-d)"/>
  <line x1="855" y1="412" x2="835" y2="450" stroke="#cbd5e1" stroke-width="1" stroke-dasharray="3,2" marker-end="url(#fl3-d)"/>
  <line x1="855" y1="412" x2="885" y2="450" stroke="#cbd5e1" stroke-width="1" stroke-dasharray="3,2" marker-end="url(#fl3-d)"/>
  <rect x="510" y="453" width="200" height="40" rx="6" fill="none" stroke="#cbd5e1" stroke-width="1" stroke-dasharray="4,3"/>
  <rect x="525" y="461" width="65" height="24" rx="12" fill="#fff" stroke="#cbd5e1" stroke-width="1"/>
  <text x="557" y="477" text-anchor="middle" font-size="10" font-weight="600" fill="#94a3b8">Pod A</text>
  <rect x="615" y="461" width="65" height="24" rx="12" fill="#fff" stroke="#cbd5e1" stroke-width="1"/>
...
*그림 5-23. Ingress Controller가 URL을 읽고 Service를 선택, 그 직후 DNS가 이름을 ClusterIP로 변환*
...
<div class="svg-figure">
<svg viewBox="0 0 1080 600" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="STEP 4 — kube-proxy(2차)가 ClusterIP를 Pod IP로 변환 (L4 로드밸런싱)">
  <defs>
  ...
  <rect x="40" y="14" width="68" height="22" rx="11" fill="#ff7849"/>
  <text x="74" y="29" text-anchor="middle" font-size="11" font-weight="700" fill="#fff">STEP 4</text>
  <text x="120" y="30" font-size="13" font-weight="700" fill="#1f2937">kube-proxy(2차)가 ClusterIP를 Pod IP로 변환</text>
  <text x="20" y="120" font-size="10" font-weight="600" fill="#475569">로컬호스트</text>
  <rect x="20" y="130" width="130" height="100" rx="6" fill="#fff" stroke="#475569" stroke-width="1.6"/>
  <text x="85" y="186" text-anchor="middle" font-size="14" font-weight="700" fill="#0f172a">Host</text>
  <line x1="150" y1="170" x2="232" y2="170" stroke="#cbd5e1" stroke-width="1.4" marker-end="url(#fl4-d)"/>
  <text x="190" y="160" text-anchor="middle" font-size="10" font-family="monospace" fill="#94a3b8">localhost:9000</text>
  <rect x="200" y="80" width="860" height="500" rx="10" fill="none" stroke="#475569" stroke-width="1.4" stroke-dasharray="6,4"/>
  <text x="630" y="100" text-anchor="middle" font-size="11" font-style="italic" fill="#475569">쿠버네티스 가상세계 (클러스터)</text>
  <rect x="235" y="110" width="135" height="135" rx="10" fill="#fff4ed" stroke="#ff7849" stroke-width="1.6"/>
  ...
  <text x="302" y="190" text-anchor="middle" font-size="12" font-family="monospace" fill="#94a3b8">:9000</text>
  <text x="302" y="234" text-anchor="middle" font-size="9" fill="#7b341e">NodePort Service</text>
  <line x1="370" y1="170" x2="490" y2="170" stroke="#cbd5e1" stroke-width="1.4" marker-end="url(#fl4-d)"/>
  <rect x="490" y="150" width="100" height="40" rx="6" fill="#fff" stroke="#cbd5e1" stroke-width="1.4"/>
  <text x="540" y="174" text-anchor="middle" font-size="11" font-weight="600" fill="#94a3b8">Kube Proxy</text>
  <line x1="590" y1="170" x2="640" y2="170" stroke="#cbd5e1" stroke-width="1.4" marker-end="url(#fl4-d)"/>
  <rect x="640" y="150" width="180" height="40" rx="6" fill="#fff" stroke="#cbd5e1" stroke-width="1.4"/>
  <text x="730" y="174" text-anchor="middle" font-size="11" font-weight="600" fill="#94a3b8">Ingress Controller Pod</text>
  <rect x="420" y="245" width="600" height="65" rx="32" fill="#f5faff" stroke="#bfdbfe" stroke-width="1"/>
  <text x="1005" y="260" text-anchor="end" font-size="10" font-weight="700" fill="#94a3b8">L7</text>
  <line x1="730" y1="190" x2="600" y2="245" stroke="#cbd5e1" stroke-width="1.4" marker-end="url(#fl4-d)"/>
  <line x1="730" y1="190" x2="850" y2="245" stroke="#cbd5e1" stroke-width="1.4" marker-end="url(#fl4-d)"/>
  <rect x="540" y="262" width="120" height="32" rx="16" fill="#fff" stroke="#cbd5e1" stroke-width="1.4"/>
  <text x="600" y="283" text-anchor="middle" font-size="11" font-weight="600" fill="#94a3b8">Service A</text>
  <rect x="790" y="262" width="120" height="32" rx="16" fill="#fff" stroke="#cbd5e1" stroke-width="1.4"/>
  <text x="850" y="283" text-anchor="middle" font-size="11" font-weight="600" fill="#94a3b8">Service B</text>
  <line x1="600" y1="310" x2="600" y2="335" stroke="#ff7849" stroke-width="2.2" marker-end="url(#fl4-a)"/>
  <line x1="850" y1="310" x2="850" y2="335" stroke="#ff7849" stroke-width="2.2" marker-end="url(#fl4-a)"/>
  <rect x="420" y="335" width="600" height="100" rx="6" fill="#f8fafc" stroke="#cbd5e1" stroke-width="1"/>
  <text x="1005" y="350" text-anchor="end" font-size="10" font-weight="700" fill="#94a3b8">L4</text>
  <rect x="490" y="365" width="220" height="55" rx="6" fill="#fff4ed" stroke="#ff7849" stroke-width="2"/>
  ...
  <rect x="555" y="390" width="100" height="22" rx="4" fill="#fff4ed" stroke="#ff7849" stroke-width="1.6"/>
  <text x="605" y="405" text-anchor="middle" font-size="10" font-weight="700" fill="#7b341e">Kube Proxy</text>
  <rect x="740" y="365" width="220" height="55" rx="6" fill="#fff4ed" stroke="#ff7849" stroke-width="2"/>
  ...
  <rect x="805" y="390" width="100" height="22" rx="4" fill="#fff4ed" stroke="#ff7849" stroke-width="1.6"/>
  <text x="855" y="405" text-anchor="middle" font-size="10" font-weight="700" fill="#7b341e">Kube Proxy</text>
  <line x1="605" y1="412" x2="585" y2="450" stroke="#cbd5e1" stroke-width="1" stroke-dasharray="3,2" marker-end="url(#fl4-d)"/>
  <line x1="605" y1="412" x2="635" y2="450" stroke="#cbd5e1" stroke-width="1" stroke-dasharray="3,2" marker-end="url(#fl4-d)"/>
  <line x1="855" y1="412" x2="835" y2="450" stroke="#cbd5e1" stroke-width="1" stroke-dasharray="3,2" marker-end="url(#fl4-d)"/>
  <line x1="855" y1="412" x2="885" y2="450" stroke="#cbd5e1" stroke-width="1" stroke-dasharray="3,2" marker-end="url(#fl4-d)"/>
  <rect x="510" y="453" width="200" height="40" rx="6" fill="none" stroke="#cbd5e1" stroke-width="1" stroke-dasharray="4,3"/>
  <rect x="525" y="461" width="65" height="24" rx="12" fill="#fff" stroke="#cbd5e1" stroke-width="1"/>
  <text x="557" y="477" text-anchor="middle" font-size="10" font-weight="600" fill="#94a3b8">Pod A</text>
  <rect x="615" y="461" width="65" height="24" rx="12" fill="#fff" stroke="#cbd5e1" stroke-width="1"/>
...
*그림 5-24. kube-proxy(2차)가 ClusterIP를 Pod IP로 변환 (L4 로드밸런싱)*
...
<div class="svg-figure">
<svg viewBox="0 0 1080 600" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="STEP 5 — 요청이 Pod에 도달, Endpoint Controller는 백그라운드에서 Pod IP 목록 유지">
  <defs>
  ...
  <rect x="40" y="14" width="68" height="22" rx="11" fill="#ff7849"/>
  <text x="74" y="29" text-anchor="middle" font-size="11" font-weight="700" fill="#fff">STEP 5</text>
  <text x="120" y="30" font-size="13" font-weight="700" fill="#1f2937">Pod 도달 + Endpoint Controller가 Pod IP 목록을 유지</text>
  <text x="20" y="120" font-size="10" font-weight="600" fill="#475569">로컬호스트</text>
  <rect x="20" y="130" width="130" height="100" rx="6" fill="#fff" stroke="#475569" stroke-width="1.6"/>
  <text x="85" y="186" text-anchor="middle" font-size="14" font-weight="700" fill="#0f172a">Host</text>
  <line x1="150" y1="170" x2="232" y2="170" stroke="#cbd5e1" stroke-width="1.4" marker-end="url(#fl5-d)"/>
  <text x="190" y="160" text-anchor="middle" font-size="10" font-family="monospace" fill="#94a3b8">localhost:9000</text>
  <rect x="200" y="80" width="860" height="500" rx="10" fill="none" stroke="#475569" stroke-width="1.4" stroke-dasharray="6,4"/>
  <text x="630" y="100" text-anchor="middle" font-size="11" font-style="italic" fill="#475569">쿠버네티스 가상세계 (클러스터)</text>
  <rect x="235" y="110" width="135" height="135" rx="10" fill="#fff4ed" stroke="#ff7849" stroke-width="1.6"/>
  ...
  <text x="302" y="190" text-anchor="middle" font-size="12" font-family="monospace" fill="#94a3b8">:9000</text>
  <text x="302" y="234" text-anchor="middle" font-size="9" fill="#7b341e">NodePort Service</text>
  <line x1="370" y1="170" x2="490" y2="170" stroke="#cbd5e1" stroke-width="1.4" marker-end="url(#fl5-d)"/>
  <rect x="490" y="150" width="100" height="40" rx="6" fill="#fff" stroke="#cbd5e1" stroke-width="1.4"/>
  <text x="540" y="174" text-anchor="middle" font-size="11" font-weight="600" fill="#94a3b8">Kube Proxy</text>
  <line x1="590" y1="170" x2="640" y2="170" stroke="#cbd5e1" stroke-width="1.4" marker-end="url(#fl5-d)"/>
  <rect x="640" y="150" width="180" height="40" rx="6" fill="#fff" stroke="#cbd5e1" stroke-width="1.4"/>
  <text x="730" y="174" text-anchor="middle" font-size="11" font-weight="600" fill="#94a3b8">Ingress Controller Pod</text>
  <rect x="420" y="245" width="600" height="65" rx="32" fill="#f5faff" stroke="#bfdbfe" stroke-width="1"/>
  <text x="1005" y="260" text-anchor="end" font-size="10" font-weight="700" fill="#94a3b8">L7</text>
  <line x1="730" y1="190" x2="600" y2="245" stroke="#cbd5e1" stroke-width="1.4" marker-end="url(#fl5-d)"/>
  <line x1="730" y1="190" x2="850" y2="245" stroke="#cbd5e1" stroke-width="1.4" marker-end="url(#fl5-d)"/>
  <rect x="540" y="262" width="120" height="32" rx="16" fill="#fff" stroke="#cbd5e1" stroke-width="1.4"/>
  <text x="600" y="283" text-anchor="middle" font-size="11" font-weight="600" fill="#94a3b8">Service A</text>
  <rect x="790" y="262" width="120" height="32" rx="16" fill="#fff" stroke="#cbd5e1" stroke-width="1.4"/>
  <text x="850" y="283" text-anchor="middle" font-size="11" font-weight="600" fill="#94a3b8">Service B</text>
  <line x1="600" y1="310" x2="600" y2="335" stroke="#cbd5e1" stroke-width="1.4" marker-end="url(#fl5-d)"/>
  <line x1="850" y1="310" x2="850" y2="335" stroke="#cbd5e1" stroke-width="1.4" marker-end="url(#fl5-d)"/>
  <rect x="420" y="335" width="600" height="100" rx="6" fill="#f8fafc" stroke="#cbd5e1" stroke-width="1"/>
  <text x="1005" y="350" text-anchor="end" font-size="10" font-weight="700" fill="#94a3b8">L4</text>
  <rect x="490" y="365" width="220" height="55" rx="6" fill="#fff" stroke="#cbd5e1" stroke-width="1.4"/>
  <text x="505" y="382" font-size="10" font-weight="700" fill="#94a3b8">Node 1</text>
  ...
  <rect x="805" y="390" width="100" height="22" rx="4" fill="#fff" stroke="#cbd5e1" stroke-width="1"/>
  <text x="855" y="405" text-anchor="middle" font-size="10" font-weight="600" fill="#94a3b8">Kube Proxy</text>
  <rect x="225" y="375" width="120" height="55" rx="6" fill="#fff4ed" stroke="#ff7849" stroke-width="2"/>
  ...
  <line x1="345" y1="397" x2="555" y2="402" stroke="#475569" stroke-width="1.2" stroke-dasharray="4,3" marker-end="url(#fl5-s)"/>
  <text x="430" y="392" text-anchor="middle" font-size="9" font-style="italic" fill="#475569">Pod IP 목록 참조</text>
  <line x1="345" y1="420" x2="555" y2="473" stroke="#475569" stroke-width="1.2" stroke-dasharray="4,3" marker-end="url(#fl5-s)"/>
  <text x="430" y="466" text-anchor="middle" font-size="9" font-style="italic" fill="#475569">Pod 감시</text>
  <line x1="605" y1="412" x2="585" y2="450" stroke="#ff7849" stroke-width="2.2" stroke-dasharray="3,2" marker-end="url(#fl5-a)"/>
  <line x1="605" y1="412" x2="635" y2="450" stroke="#ff7849" stroke-width="2.2" stroke-dasharray="3,2" marker-end="url(#fl5-a)"/>
  <line x1="855" y1="412" x2="835" y2="450" stroke="#ff7849" stroke-width="2.2" stroke-dasharray="3,2" marker-end="url(#fl5-a)"/>
  <line x1="855" y1="412" x2="885" y2="450" stroke="#ff7849" stroke-width="2.2" stroke-dasharray="3,2" marker-end="url(#fl5-a)"/>
...
*그림 5-25. Pod 도달. Endpoint Controller는 다섯 단계 내내 백그라운드에서 Pod IP 목록을 최신으로 유지*
...
선배가 챕터 3 마지막에 던졌던 한마디 — "**이 흐름 기억해 둬, 쿠버네티스에서 똑같은 게 더 큰 스케일로 나와**" — 가 이 표 한 장에 그대로 들어와 있었습니다.
