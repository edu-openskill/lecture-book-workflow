...
외부에서 들어온 요청은 노드의 nodePort에 들어와 Service의 port를 거치고, 마지막으로 Pod 컨테이너의 targetPort에 도달합니다.

<div class="svg-figure">
<svg viewBox="0 0 760 210" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="외부 요청이 nodePort, port, targetPort 순으로 변환되며 Pod 안 컨테이너까지 도달하는 포트 흐름">
  <defs>
    <marker id="pf-arr" markerWidth="10" markerHeight="10" refX="8" refY="3" orient="auto"><path d="M0,0 L0,6 L8,3 z" fill="#475569"/></marker>
  </defs>
  <text x="380" y="22" text-anchor="middle" font-size="13" font-weight="700" fill="#1f2937">포트 흐름 — 외부에서 Pod 안 컨테이너까지</text>
  <rect x="20" y="80" width="110" height="60" rx="8" fill="#fff" stroke="#475569" stroke-width="1.6"/>
  <text x="75" y="108" text-anchor="middle" font-size="12" font-weight="700" fill="#0f172a">외부 사용자</text>
  <text x="75" y="125" text-anchor="middle" font-size="10" fill="#6b7280">브라우저</text>
  <line x1="130" y1="110" x2="195" y2="110" stroke="#475569" stroke-width="1.8" marker-end="url(#pf-arr)"/>
  <text x="162" y="103" text-anchor="middle" font-size="10" font-family="monospace" font-weight="700" fill="#7b341e">nodePort</text>
  <text x="162" y="128" text-anchor="middle" font-size="10" font-family="monospace" fill="#7b341e">:30080</text>
  <rect x="200" y="80" width="110" height="60" rx="8" fill="#fff" stroke="#475569" stroke-width="1.6"/>
  <text x="255" y="108" text-anchor="middle" font-size="12" font-weight="700" fill="#0f172a">노드</text>
  <text x="255" y="125" text-anchor="middle" font-size="10" fill="#6b7280">워커 서버</text>
  <line x1="310" y1="110" x2="375" y2="110" stroke="#475569" stroke-width="1.8" marker-end="url(#pf-arr)"/>
  <text x="342" y="103" text-anchor="middle" font-size="10" font-family="monospace" font-weight="700" fill="#7b341e">port</text>
  <text x="342" y="128" text-anchor="middle" font-size="10" font-family="monospace" fill="#7b341e">:80</text>
  <rect x="380" y="80" width="110" height="60" rx="8" fill="#fff4ed" stroke="#ff7849" stroke-width="1.8"/>
  <text x="435" y="108" text-anchor="middle" font-size="12" font-weight="700" fill="#7b341e">Service</text>
  <text x="435" y="125" text-anchor="middle" font-size="10" fill="#7b341e">ClusterIP</text>
  <line x1="490" y1="110" x2="555" y2="110" stroke="#475569" stroke-width="1.8" marker-end="url(#pf-arr)"/>
  <text x="522" y="103" text-anchor="middle" font-size="10" font-family="monospace" font-weight="700" fill="#7b341e">targetPort</text>
  <text x="522" y="128" text-anchor="middle" font-size="10" font-family="monospace" fill="#7b341e">:80</text>
  <rect x="560" y="80" width="180" height="60" rx="8" fill="#fff" stroke="#475569" stroke-width="1.6"/>
  <text x="650" y="108" text-anchor="middle" font-size="12" font-weight="700" fill="#0f172a">Pod 안 컨테이너</text>
  <text x="650" y="125" text-anchor="middle" font-size="10" fill="#6b7280">nginx</text>
  <text x="380" y="180" text-anchor="middle" font-size="10" font-style="italic" fill="#6b7280">nodePort 단계는 Type=NodePort일 때만. ClusterIP 타입은 노드 단계 없이 Service 직접 진입</text>
</svg>
</div>

*그림 5-6. 포트 흐름 — 외부 요청이 nodePort → port → targetPort 순으로 변환되며 Pod까지 도달*

각 포트의 역할과 누구의 포트인지 정리하면 다음과 같습니다.
...
도커 때 같은 문제를 NGINX로 풀었던 게 떠올랐습니다. 앞단에 NGINX 한 대를 두고 `/app1`은 1번 서버로, `/app2`는 2번 서버로 보냈습니다. 한 도메인 안에서 경로로 트래픽을 갈라 준 것입니다.

쿠버네티스에서도 같은 구조가 필요했습니다. 주문 서비스(`/order`)와 매장 서비스(`/stores`)를 한 도메인 아래에 두고, 경로에 따라 다른 백엔드로 보내야 합니다.

*'Service에는 selector랑 port뿐이고, URL 경로 옵션이 없잖아. 둘로 나눠 만들면 사용자가 :30080이랑 :30081을 따로 외워야 하고, 한 도메인 안에서 /order, /stores로 갈라내는 길도 없네.'*
...
다섯 단계는 위 그림에서 ①~⑤ 위치를 차례로 짚어 가는 흐름입니다. 5.2의 비유를 빌리면, 고객이 본사 **공식 앱**(Ingress)으로 주문을 넣을 때 앱 내부에서 일어나는 일을 줌인해서 보는 셈입니다. 한 단계씩 비유로 먼저 풀고, 곧이어 같은 단계의 IT 구조를 봅니다.
