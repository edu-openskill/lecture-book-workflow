...
NGINX를 앞단에 세우면 실제 서버의 IP나 내부 포트를 사용자에게 노출할 일이 없습니다. 나중에 백엔드 서버를 두 대, 세 대로 늘려도 NGINX가 요청을 골고루 나눠 주는 로드밸런싱 역할까지 합니다.
...
로그를 살피던 오픈이의 눈에 한 가지가 들어왔습니다. 양쪽 컨테이너 모두 똑같은 HTML을 매번 서버에서 응답하고 있었습니다.
