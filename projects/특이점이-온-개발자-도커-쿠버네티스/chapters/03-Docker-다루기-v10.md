...
오픈이는 고개를 끄덕였습니다. 회의실을 나서는 발걸음이 가볍지 않았습니다. 컨테이너 하나를 실행하는 건 익혔지만, 여러 개를 한꺼번에 관리해 본 적은 없었습니다.

*'한 대씩은 띄워봤는데, 여러 대를 함께 구성하는 건 또 다른 이야기인데.'*

책상 앞에 앉아 모니터를 마주했습니다. 무엇부터 손대야 할지 그림이 잘 잡히지 않았습니다. 퇴근 후 직접 부딪혀보면서 무엇이 막히는지부터 보기로 했습니다.
...
저녁 여덟 시. 식탁에 노트북을 펼쳤습니다. 지난주에 손으로 컨테이너에 들어가 패키지를 깔고 commit으로 이미지를 남기던 그 과정이 떠올랐습니다. 처음 한 번은 신기했지만, 이제는 컨테이너를 만들 때마다 똑같은 작업을 처음부터 다시 반복해야 합니다.

그러다 며칠 전 편의점에서 본 밀키트가 머릿속을 스쳤습니다. 포장을 뜯어 끓이기만 하면 매번 같은 요리가 나왔습니다.

*'도커도 밀키트처럼 한 번 설정해 두고 그대로 가져다 쓰는 방법이 있지 않을까.'*
...
첫 실습으로 Ubuntu 환경에 vim이 미리 깔려 있는 이미지를 만들어 보겠습니다. 확장자 없이 `Dockerfile`이라는 이름의 파일을 만들고, 그 안에 다음 내용을 작성합니다.
...
파일을 저장하고, Dockerfile이 있는 폴더에서 터미널을 열어 빌드 명령을 입력합니다.
...
빌드 로그가 한 줄씩 올라가고, 마지막 줄과 함께 이미지가 완성됩니다. 이 이미지로 컨테이너를 실행해 vim을 입력하면, 별도의 설치 과정 없이 편집기가 그대로 뜹니다.
...
빈 `index.html` 파일을 Dockerfile과 같은 폴더에 만듭니다.

![](../assets/CH03/chap02-11.png)

*그림 3-6. 폴더 및 파일 구조*

Dockerfile에 두 줄을 추가합니다. 작업 디렉토리를 `/app`으로 정하고, 로컬의 `index.html`을 그 안에 복사하는 설정입니다.
...
CMD와 ENTRYPOINT는 둘 다 컨테이너가 시작될 때 무엇을 실행할지 정하는 지시어입니다. CMD는 **기본으로 실행할 명령어**를, ENTRYPOINT는 **반드시 실행되는 메인 프로세스**를 지정합니다.

이 둘의 관계는 커피 머신을 떠올리면 가깝습니다. ENTRYPOINT는 **커피를 내린다는 동작 자체**라, 어떤 메뉴를 선택하든 반드시 실행됩니다. 반면 CMD는 따로 옵션을 선택하지 않으면 **기본 메뉴인 아메리카노**가 나오고, 라떼를 선택하면 라떼가 나옵니다.

이 동작을 직접 확인해 보겠습니다. Dockerfile에 ENTRYPOINT 한 줄을 추가합니다. echo로 짧은 메시지를 찍는 단순한 구성입니다.
...
ENTRYPOINT의 `echo "컨테이너 실행"` 뒤에 CMD의 `/bin/bash`가 인자로 붙어 한 줄로 출력됩니다. 두 지시어가 함께 있으면 ENTRYPOINT가 **메인 프로세스로 실행**되고, CMD의 값은 **실행되지 않은 채 그 인자(Argument)로 전달**되기 때문입니다.
...
*'들어온 요청을 알맞은 컨테이너로 연결하려면 어떻게 해야 할까.'*

### 3.2.1 NGINX의 역할

이 역할을 맡는 도구가 **NGINX**입니다. NGINX는 들어온 요청을 받아, 그 요청을 처리할 알맞은 컨테이너로 전달합니다. 이처럼 요청을 받아 알맞은 컨테이너로 연결하는 역할을 **리버스 프록시(Reverse Proxy)** 라고 부릅니다.
...
*그림 3-9. 클라이언트에게는 NGINX 하나로 보이는 서버 영역*
...
<svg viewBox="0 0 720 300" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="택배 분류 센터에 빗댄 NGINX 설정 — 주소 확인 후 지정 센터로 분류">
  ...
  <text x="378" y="114" text-anchor="middle" font-size="9" fill="#6b7280">지정 센터로 전달</text>
...
*그림 3-10. 주소를 확인해(location) 지정 센터로 보내는(proxy_pass) NGINX 설정의 흐름*
...
*그림 3-12. /app1 경로로 접속한 결과*

![](../assets/CH03/chap02-22.png)

*그림 3-13. /app2 경로로 접속한 결과*
...
*그림 3-16. /app1 경로로 접속한 결과 (라운드 로빈)*

두 컨테이너가 같은 이미지라, 화면만으로는 어느 서버가 응답했는지 알 수 없습니다. 요청이 두 서버로 제대로 나뉘는지 확인하려면 각 서버의 로그를 직접 봐야 합니다. `docker logs` 명령을 두 번 실행합니다.
...
앞에서 본 두 로그를 다시 보면, 두 컨테이너가 요청을 번갈아 나눠 받고 있습니다. 그런데 어느 쪽이든 정적 페이지를 매번 다시 응답하고 있습니다.

*'단순한 HTML을 매번 백엔드까지 가서 가져와야 하다니. 사용자가 늘면 서버에 부담이 되겠는데.'*

이 비효율을 풀어 주는 방법이 있었습니다. 자주 나가는 응답을 NGINX 앞단에 저장해 두면, 같은 요청이 다시 들어왔을 때 백엔드까지 가지 않고 바로 돌려줄 수 있습니다. 이런 방식을 **캐싱(Caching)** 이라고 부릅니다.
...
*그림 3-19. 첫 번째 요청 (MISS) — 캐시가 비어있어 백엔드까지 요청 후 응답을 캐시에 저장*
...
*그림 3-20. 두 번째 요청 (HIT) — 캐시에 저장된 응답을 바로 반환, 백엔드 접근 없음*
...
*그림 3-21. 캐싱 실습 — 이미지 응답*

이제 캐싱이 됐는지 확인해 보겠습니다. **개발자 도구(F12, 브라우저 디버그 창)** 의 Network 탭을 열고, 브라우저 자체 캐시가 끼어들지 못하도록 **Disable cache**를 체크합니다. 그런 다음 **응답 헤더**를 확인합니다.

![](../assets/CH03/chap02-33.png)

*그림 3-22. X-Cache-Status: MISS 확인*

첫 요청의 `X-Cache-Status`는 예상대로 **MISS**로 찍혀 있습니다.

브라우저를 한 번 더 새로고침합니다.

![](../assets/CH03/chap02-34.png)

*그림 3-23. X-Cache-Status: HIT 확인*

같은 자리에 찍히는 값이 이번에는 **HIT**로 바뀌어 있었습니다. 두 번째 요청은 백엔드까지 가지 않고, NGINX가 캐시에 저장해 둔 응답을 돌려준 결과입니다.
...
*그림 3-24. 세션 불일치 — 1번 서버에 저장된 세션이 2번 서버엔 없어 인증 실패*
...
*그림 3-25. Redis로 해결 — 세션을 공용 저장소에 보관하여 어느 서버에서든 조회 가능*
...
ex04 폴더를 엽니다. Redis는 공식 이미지를 그대로 쓰고, 직접 만들 이미지는 API 서버 한 대뿐입니다.
...
*그림 3-26. api1에서 데이터 저장*

![](../assets/CH03/chap02-41.png)

*그림 3-27. api2에서 같은 데이터 조회*
...
*그림 3-34. Docker Compose 방식 — 한 번에 생성 및 연결*
...
*그림 3-37. docker compose up 명령으로 프론트엔드·백엔드·DB가 동시에 뜨는 모습*
