...
- **Docker Compose**로 프론트엔드·백엔드·DB를 한꺼번에 띄웁니다
:::
...
터미널 프롬프트가 처음부터 `/app`을 가리키고 있었고, 그 안에 `index.html`이 들어 있었습니다. 컨테이너로 들어가서 파일을 직접 옮길 일이 사라졌습니다. `exit`을 입력해 컨테이너를 닫았습니다.
...
NGINX의 동작은 전국 택배가 모이는 대형 분류 센터를 떠올리면 가깝습니다. 전국에서 들어온 택배가 한 곳에 쌓이고, 그곳에서 주소지에 따라 지역 물류 센터로 나뉘어 나갑니다. NGINX의 설정도 비슷한 흐름입니다.

- **upstream (서버 그룹 정의)** : 특정 지역을 담당할 **배송 센터(서버 그룹)** 를 묶어 이름을 붙이는 작업입니다. **"서울 센터", "부산 센터"** 처럼 물건을 넘겨줄 목적지를 미리 등록해 둡니다.
...
이 비효율을 풀어 주는 방법이 있었습니다. 자주 입는 옷을 옷장 깊숙이 넣지 않고 옷걸이에 따로 걸어 두면 다음 날 그 옷걸이에서 바로 꺼낼 수 있습니다. 자주 나가는 응답을 NGINX 앞단에 저장해 두고 같은 요청이 다시 들어오면 백엔드까지 가지 않고 NGINX가 바로 돌려주는 방식을 **캐싱(Caching)** 이라고 부릅니다.
...
ex05 폴더 안에는 DB 이미지를 만드는 `db` 폴더가 들어 있습니다.
...
컨테이너가 정상적으로 떴는지 한 번 더 확인합니다. `docker ps`로 컨테이너 ID를 확인한 뒤 `docker exec`로 내부에 접속합니다.
...
실습을 끝낸 오픈이는 터미널 히스토리를 위로 올렸습니다. 같은 모양의 `docker run`이 컨테이너 수만큼 줄지어 있었습니다. 옵션 조합만 매번 조금씩 달랐습니다.

*'같은 명령어를 매번 칠 게 아니라, 어디 적어 두고 한 번에 부르면 안 되나.'*
...
이런 수고를 명령어 하나로 묶어 처리하는 도구가 **Docker Compose**입니다.
...
│   ├── Dockerfile       # nginx 이미지 + nginx.conf 복사
│   └── nginx.conf       # 로드밸런싱 + 경로 라우팅 설정
├── docker-compose.yml   # app1·app2·lb를 묶어 실행하는 Compose 설정
└── README.md            # 실습 안내
```
...
*'전에는 빌드와 실행을 따로 쳤는데, 이제는 Compose 명령이면 끝나네.'*
...
지금까지의 내용을 모아 프론트엔드·백엔드·DB를 한꺼번에 띄워 보겠습니다. `docker compose up` 명령으로 모든 서비스가 함께 실행됩니다.
...
backend 폴더에는 자바 소스 파일이 보이지 않습니다. 대신 Dockerfile과 `entrypoint.sh` 셸 스크립트가 들어 있습니다.

두 파일이 컨테이너 흐름에서 맡는 단계는 다음과 같이 다릅니다.
...
*그림 3-36. docker compose up 명령으로 프론트엔드·백엔드·DB가 동시에 뜨는 모습*
...
화면에 회원 목록이 떴습니다. NGINX가 받은 요청을 백엔드로 넘기고, 백엔드가 DB에서 꺼낸 회원 데이터가 화면까지 도달한 결과입니다.

다음 날 회의 시간이 됐습니다. 회의실에서 노트북을 펼치고 빔 프로젝터에 연결했습니다. 팀장과 동료가 앉았습니다.
...
회의실을 나오는 길은 가볍지만은 않았습니다. 팀장의 마지막 물음에 마땅한 답이 떠오르지 않았기 때문입니다. 도커로 여기까지 온 길은 결국 컨테이너를 실행하는 단계까지였습니다. 한 번에 함께 실행할 수는 있게 됐지만, 그 사이에 무엇이 죽고 살아나는지를 다룰 도구는 없었습니다.
...
- **Redis는 여러 서버가 공유하는 메모리 저장소입니다.** 서버 대수가 늘어도 세션을 외부 저장소에 두면 로그인 상태가 흩어지지 않습니다.
- **사용자 정의 네트워크는 컨테이너끼리 이름으로 통신할 수 있게 합니다.** `host.docker.internal`처럼 호스트를 우회할 필요가 없어집니다.
- **Docker Compose는 여러 컨테이너를 한 파일로 관리하는 설계도입니다.** Compose 명령으로 빌드·네트워크·의존 관계가 자동으로 묶입니다.

운영에는 자동 복구, 무중단 배포, 다중 서버 관리, 설정 분리 같은 숙제가 기다립니다. 이런 일을 한꺼번에 다루는 도구가 다음 챕터의 주인공 **쿠버네티스(Kubernetes)** 입니다.
:::
