...
퇴근 후 저녁. 오픈이는 식탁에 노트북을 열었습니다. 지난주에 컨테이너의 환경을 직접 세팅하던 과정이 떠올랐습니다. 처음은 신기했지만, 이제는 여러 컨테이너를 실행해야 하기 때문에 일일이 환경을 세팅할 수 없습니다.

그러다 저녁으로 먹은 밀키트를 떠올렸습니다. 밀키트는 이미 손질된 재료를 조리만 하면 완성된 요리가 나옵니다.
...
로그를 살피던 오픈이의 눈에 한 가지가 걸렸습니다. 양쪽 컨테이너 모두 똑같은 HTML을 매번 서버에서 응답하고 있었습니다.
...
원인은 로그인 기록이 처음 요청을 처리한 서버에만 남기 때문입니다. 사용자가 로그인하면 해당 **인증 정보(세션)** 는 요청을 받은 특정 서버의 메모리에 저장됩니다. 서버가 한 대일 때는 문제가 없지만, 서버가 두 대 이상이 되면 **라운드 로빈** 방식으로 요청이 번갈아 분산됩니다. 이 과정에서 세션이 없는 서버로 요청이 전달될 때마다 **로그인이 풀립니다.**
...
로그인을 처리한 1번 서버는 세션을 Redis에 저장하고, 다음 요청을 받은 2번 서버는 내부 메모리 대신 **Redis에서 그 세션을 조회합니다.** 두 서버가 같은 세션을 공유하므로, 사용자는 한 번만 로그인하면 인증이 끊기지 않습니다.
...
옆자리 동료를 불러 화면을 보여 줬습니다.

**오픈이**: "이제 아무리 새로고침해도 로그인이 풀리지 않아요. Redis라는 공용 저장소에 세션을 두니까, 어느 서버로 요청이 가도 같은 세션을 가져와요."

**동료**: "오, 이제 새로고침해도 계속 멀쩡하네요."

각 서버가 따로 보관하던 세션을 Redis 한 곳에 모은 결과, 로그인이 더는 끊기지 않습니다.

## 3.4 MySQL - 영구 데이터 저장하기

그런데 Redis는 메모리 위에서 동작하기 때문에, 컨테이너를 내렸다가 다시 실행하면 그 안의 값이 모두 사라집니다.

*'세션처럼 잠깐 들고 있다 사라져도 되는 데이터는 괜찮은데, 회원 정보나 게시글 같은 건 어디에 두지. 컨테이너를 재시작해도 그대로 남아 있어야 할 텐데.'*

오래 남아야 하는 데이터는 메모리가 아니라 데이터베이스에 보관해야 합니다. 회원 정보를 저장할 데이터베이스를 컨테이너로 실행해 보겠습니다.
...
퇴근길, 통합 사이트 구축이 자꾸 마음에 걸렸습니다. 프론트엔드·백엔드·DB를 컨테이너로 함께 돌려야 하는데, 지금처럼 하나씩 빌드하고 실행하면 컨테이너 수만큼 똑같은 명령어를 반복해야 합니다. 여러 컨테이너를 하나로 관리할 방법이 필요합니다.
...
이러한 번거로움을 명령어 한 줄로 해결해 주는 도구가 바로 **Docker Compose**입니다.
...
Docker Compose는 여러 컨테이너의 설정을 **하나의 YAML 파일(.yml)** 에 모아 관리합니다. 이 파일에 컨테이너 간의 네트워크 연결이나 실행 옵션을 미리 정의해 두면, **단 하나의 명령으로 전체 컨테이너를 동시에 실행하거나 중지**할 수 있습니다.
...
*'매번 따로 만들던 컨테이너들을 이제는 하나의 덩어리로 묶어 관리할 수 있겠구나.'*
...
브라우저의 요청을 프론트엔드의 NGINX가 받아 화면을 응답하고, `/api`로 들어오는 요청은 백엔드로 전달합니다. 요청을 넘겨받은 백엔드는 MySQL에서 회원 데이터를 조회하여 다시 브라우저로 응답합니다.
...
그 주 금요일 오전 열 시. 회의실에서 노트북을 펼치고 빔 프로젝터에 연결했습니다. 팀장과 동료가 앉았습니다.
...
- **Dockerfile은 필요한 설정을 담아 둔 밀키트입니다.** 베이스 이미지·설치할 패키지·실행 명령을 적어두면 어디서든 동일한 환경을 자동으로 재현합니다.
...
운영에는 자동 복구, 무중단 배포, 다중 서버 관리, 설정 분리 같은 숙제가 남아 있습니다.
:::
