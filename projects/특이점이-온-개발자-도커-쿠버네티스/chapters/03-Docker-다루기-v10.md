...
![화이트보드에 그려진 프론트엔드·백엔드·DB 구성](../assets/CH03/gemini/01_prologue-meeting-room-task.png)
...
그림 3-16과 3-17을 보면 들어온 요청이 빠짐없이 백엔드 서버까지 전달됩니다. 같은 요청이 다시 들어와도 그때마다 백엔드를 거쳐 같은 응답을 받아 와서, 사용자가 늘수록 이 왕복이 부담이 됩니다.
...
`init.sql`에 적어 둔 초기 데이터가 테이블 안에 그대로 들어와 있습니다. 프로젝트의 회원 정보 저장소가 준비됐습니다.
...
*그림 3-35. docker compose up 명령으로 프론트엔드·백엔드·DB가 동시에 뜨는 모습*
