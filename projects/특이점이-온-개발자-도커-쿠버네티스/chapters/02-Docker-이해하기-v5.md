...
먼저 컨테이너의 원리부터 정리하기로 했습니다. 자료를 한참 읽다가, 비유 하나를 떠올리면 머리에 남는다는 평소 습관대로 일상의 장면을 끌어와 봤습니다. 머리에 떠오른 그림은 한 주방에 요리사 여러 명이 들어가는 장면이었습니다.
...
비유는 머리에 잘 남았습니다. 다만 비유만 들고 있다가는 나중에 진짜 용어와 헷갈릴 게 분명했습니다. 그래서 화면을 새 페이지로 넘기고 주방 그림을 IT 구조로 다시 그려 봤습니다.

맨 아래에 **하드웨어**, 그 위에 **호스트 OS**, 그 위에 **Docker 엔진**이 있습니다. Docker 엔진 위에 **컨테이너**가 나란히 떠 있고, 각 컨테이너 안에는 애플리케이션(App)과 라이브러리(Library)가 같이 들어갑니다.
...
오픈이는 노트 한쪽에 "**엔진=프런트, 커널=주방장**"이라고 한 줄 적었습니다. 적고 나니 컨테이너의 정체와 만들어지는 과정이 머릿속에서 한 번에 정리됐습니다.
...
여기까지 오픈이는 잠시 의자에 등을 기대고 컵에 식어 있던 커피를 한 모금 마셨습니다. Docker Hub에서 이미지를 받고 컨테이너를 띄우는 데까지는 명령 두 줄이면 충분했습니다. 자료에서 본 것보다 시작은 단순했습니다. 다만 단순함만으로 끝낼 수는 없었습니다. 컨테이너 안에서 nginx가 돌고 있다는 건 알겠는데, 그 안의 페이지를 어떻게 브라우저로 끌고 와야 하는지가 다음 질문이었습니다.

## 2.5 컨테이너 통신

컨테이너 안에서 앱이 돌아도 외부에서 접속할 수 없으면 쓸모가 없습니다. 격리된 프로세스라는 개념은 익혔지만, 이 컨테이너가 바깥과 어떻게 연결되는지를 모르면 안에서 도는 앱에 접근할 수 없습니다. 그래서 컨테이너가 어떻게 통신하는지 큰 그림을 잡아 보겠습니다.

### 2.5.1 네트워크 한눈에 보기

오픈이는 방금 띄운 nginx가 어떻게 묶여 있는지부터 그려 보고 싶었습니다. 격리된 공간이라고 해 놓고 실제로는 무엇과 무엇이 연결돼 있는지를 알아야 다음 명령을 칠 때 손이 덜 막힐 것 같았습니다.

자료를 훑어보니 컨테이너 네트워크는 크게 세 갈래로 정리되어 있었습니다. **컨테이너끼리 연결하는 방법**, **외부 요청이 컨테이너로 들어오는 길**, **이름으로 컨테이너를 찾는 방법**입니다.

세 갈래를 한 번에 잡으려면 비유 하나가 필요했습니다. 식당이 여러 개 모여 있는 구조라면 좋겠는데, 떠올린 곳이 **푸드코트**였습니다.
...
다음으로 알아본 건 컨테이너끼리 IP가 아닌 이름으로 부르는 방법이었습니다.

### 2.5.4 이름으로 찾기 - 홀의 안내 지도

컨테이너끼리 통신하려면 결국 상대 IP를 알아야 합니다. 그런데 컨테이너가 둘, 셋 늘어나면 IP를 일일이 외우고 적어 두는 게 영 번거롭습니다.
...
*그림 2-17. Docker DNS가 이름을 IP로 변환 (사용자 정의 네트워크 한정)*
...
여기까지 정리하니 컨테이너가 어떻게 묶이는지는 그림이 그려졌습니다. 이번엔 컨테이너 한 대 안으로 들어가 보고 싶어졌습니다. 격리된 공간이라고만 했는데, 그 안이 실제로 어떤 모양인지를 한 번 봐 둬야 다음에 새 컨테이너를 띄울 때 망설이지 않을 것 같았습니다.
...
안으로 들어왔지만 당장 무엇부터 해야 할지 막막했습니다. 명령어를 모르면 실습 도중에 매번 검색하게 되고, 그 사이에 흐름이 끊깁니다. 본격적으로 손을 움직이기 전에 앞으로 자주 쓸 리눅스 명령어부터 한 표로 정리해 뒀습니다.
...
기본 명령어를 손에 익힌 오픈이는, 앞서 도커 이미지로 받아서 실행했던 nginx를 이번엔 직접 설치해 보기로 했습니다. 이미지로 받을 땐 한 줄로 끝났던 과정을 손으로 한 단계씩 밟아 보면, 앞서 익힌 이미지가 무엇을 담고 있었는지가 거꾸로 보입니다.
...
오픈이는 감을 잡으려고 간단한 파일을 하나 만들어 봤습니다.
...
그러고 보니 아까 깔았던 nginx의 기본 페이지는 어디에 놓여 있을까 궁금해졌습니다. `find`로 `index.html` 파일의 위치를 찾아봤습니다.
...
ubuntu를 백그라운드로 살리려면 `-dit`(`-d` 백그라운드, `-i` 입력 받기, `-t` 터미널 화면)을 쓰거나 메인 프로세스를 다른 걸로 바꿔야 합니다.
...
오픈이는 백그라운드로 띄운 컨테이너에 다시 들어가는 방법이 궁금해졌습니다. 자료를 살펴보니 두 갈래가 있었습니다. 메인 프로세스에 직접 연결하는 `attach`와, 새 프로세스를 하나 더 띄우는 `exec`입니다. 결과는 비슷해 보이지만 내부에서 일어나는 일이 다르다고 해서, 두 방법을 직접 비교해 봤습니다.
...
여기까지 공부하고 나니 컨테이너의 생명 주기는 더는 헷갈리지 않았습니다. 오픈이는 의자에서 일어나 잠깐 어깨를 풀고, 다시 노트북 앞에 앉으면서 한 가지가 마음에 걸렸습니다. 앞서 Ubuntu 컨테이너 안에 패키지를 깔고 파일도 몇 개 만들어 두었습니다. 컨테이너를 내리는 순간 그 작업이 다 사라진다면 다음에 똑같은 걸 또 깔아야 합니다.
...
Docker Hub에 이미지를 올리고 한숨 돌리려던 오픈이의 머릿속에 한 가지 걱정이 스쳤습니다. 이미지에 담긴 것은 컨테이너의 초기 상태뿐이기 때문입니다.
...
오픈이는 노트북 옆에 펼쳐 둔 노트를 들여다봤습니다. 어제 지하철에서 적어 둔 단어 세 개 중 'Docker' 옆이 하루 사이에 빽빽해져 있었습니다. 격리, 이미지, 포트포워딩, CMD, 볼륨. 어제 두 시간을 흘려보냈던 그 에러 세 줄도 이제는 어디에 어떤 지점에서 막혔던 건지 짚을 수 있을 것 같았습니다.
