...
컨테이너가 무엇인지는 잡혔습니다. 그런데 이 컨테이너가 **어떻게** 만들어지는지는 아직 그림이 흐릿했습니다. 명령 한 줄을 친 그 순간 무엇이 어떤 순서로 움직이는지를 한 번은 그려 둬야, 나중에 명령어를 칠 때 머릿속이 손을 따라갈 것 같았습니다.
...
볼륨 마운트가 그 역할을 합니다. Docker 엔진이 자기 안에 별도 저장 공간을 두고, 사용자는 볼륨에 이름만 붙여 컨테이너와 연결합니다. 그래서 실제 저장 위치는 Docker가 맡아서 관리합니다.
...
오픈이는 하루 사이에 익힌 키워드를 정리했습니다. 격리, 이미지, 포트포워딩, CMD, 볼륨이 머릿속에 자리를 잡았습니다. 어제 두 시간을 흘려보냈던 그 에러도 이제는 어디에서 막혔던 건지 짚을 수 있을 것 같았습니다.
