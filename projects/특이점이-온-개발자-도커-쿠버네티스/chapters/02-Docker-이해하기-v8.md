...
토요일 아침이었습니다. 어제 빨간 글씨로 지나갔던 석 줄(런타임 버전, 시스템 패키지, 설정 경로)이 그대로 머릿속에 남아 있었습니다. 책상 위에는 지하철에서 적어 둔 노트가 펼쳐진 채였고, '컨테이너', 'Docker', 'Kubernetes' 세 단어 옆에는 화살표 하나가 그어져 있었습니다.

오픈이는 커피 한 잔을 들고 노트북 앞에 앉았습니다. 오늘의 목표는 한 줄이었습니다.

**Docker가 무엇이고 어떻게 돌아가는지, 개념과 기본 사용법을 손에 익힌다.**

오늘 안에 다 마칠 수 있을지는 자신이 없었습니다. 그래도 어디까지 갈 수 있는지는 보고 싶었습니다. 마우스를 설치 버튼 위로 가져갔다가 손을 멈췄습니다.

*'명령어부터 치면 그냥 외우게 될 텐데. 원리부터 보자.'*

뒤에 나올 명령어를 기계적으로 입력하지 않으려면 이 도구가 환경을 어떤 방식으로 다루는지부터 짚어 둬야 했습니다.

:::goal
**이번 챕터가 끝나면**

- 컨테이너가 호스트 OS 위에서 어떻게 격리되는지(커널·Namespace·docker0) 이해합니다
- **Docker CLI**로 이미지를 받고 컨테이너를 띄우고 안으로 들어가 봅니다
- 컨테이너 통신과 포트포워딩의 동작 흐름을 그림으로 그립니다
- `docker commit`과 마운트(`bind`·`volume`)로 데이터를 어떻게 보존하는지 익힙니다
:::
...
먼저 컨테이너의 원리부터 정리하기로 했습니다. 자료를 한참 읽다 떠오른 장면은 한 주방에 요리사 여러 명이 들어가는 모습이었습니다.
...
첫 번째 방식은 요리사마다 냉장고, 가스레인지, 조리대를 각자 한 세트씩 구매하는 방법입니다. 이렇게 하면 네 사람이 완전히 독립된 환경에서 요리하지만, 같은 설비를 네 번 갖춰야 하니 비용이 많이 듭니다.
...
비유로 컨테이너의 원리는 잡혔습니다. 같은 구조를 IT 용어로 옮기면 다음과 같습니다.

맨 아래에 **하드웨어**가 있고, 그 위에 **호스트 OS와 Docker 엔진**이 한 층(OS 계층)을 이룹니다. 그 위에 **컨테이너**가 나란히 떠 있고, 각 컨테이너 안에는 애플리케이션(App)과 라이브러리(Library)가 같이 들어갑니다.

주방 비유의 **공용 설비가 호스트 OS**, **각자의 조리대가 컨테이너**입니다.
...
이 구조에서 주방장에 해당하는 것이 OS의 핵심, 곧 **커널**입니다. 요리사들이 주방장의 명령에 따라 움직이고 어떤 위치에서 어떤 역할을 할지도 주방장이 정해 주듯, 컨테이너에 격리 공간을 만들어 주는 일은 커널이 합니다.
...
<svg viewBox="0 0 800 420" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="커널은 OS의 핵심으로, 사용자 앱과 하드웨어 사이에서 프로세스·파일시스템·네트워크 같은 시스템 자원을 관리하는 중재자">
  ...
  <text x="400" y="406" text-anchor="middle" font-size="11" font-weight="700" fill="#0f172a">메모리</text>
  ...
  <text x="570" y="406" text-anchor="middle" font-size="11" font-weight="700" fill="#0f172a">디스크</text>
</svg>
</div>

*그림 2-5. 커널은 OS의 핵심으로, 앱과 하드웨어 사이에서 시스템 자원을 관리합니다*

구조의 핵심은 호스트 OS의 커널 하나를 모든 컨테이너가 공유한다는 점입니다. Docker 엔진은 "새 격리 공간을 만들어 달라"는 요청만 보냅니다. 실제로 프로세스를 격리하는 일은 커널이 합니다. 그래서 다른 컨테이너가 다른 앱과 다른 라이브러리를 가져도 충돌하지 않고 한쪽이 런타임을 업그레이드해도 옆 컨테이너는 영향을 받지 않습니다.
...
컨테이너가 무엇인지는 잡혔습니다. 그런데 이 컨테이너가 **어떻게** 만들어지는지는 아직 명확하지 않았습니다. 명령 한 줄을 친 그 순간 무엇이 어떤 순서로 움직이는지를 한 번은 그려 둬야, 나중에 명령어를 칠 때 흐름이 더 명확할 것 같았습니다.
...
<svg viewBox="0 0 760 200" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="컨테이너 실행 명령이 Docker 엔진을 거쳐 커널에 전달되고, 커널이 격리된 프로세스를 만들어 컨테이너가 되는 흐름">
  ...
  <text x="390" y="92" text-anchor="middle" font-size="10" font-style="italic" fill="#b91c1c">격리 요청</text>
...
여기서 짚어 둘 점이 하나 있습니다. 컨테이너를 실제로 만드는 건 Docker 엔진이 아니라 커널입니다. 커널이 주방장이라면, Docker 엔진은 홀의 프런트 직원에 가깝습니다. 새 주문이 들어오면 프런트가 주방장에게 그대로 넘기고, 실제로 새 요리사(프로세스)를 불러 작업을 시작시키는 일은 주방장이 합니다.

엔진과 커널의 역할 분담이 정리되자 다음 의문이 따라왔습니다.

*'그럼 이 안에서 뭘 실행할지는 어디서 정해지지.'*

## 2.2 이미지 - 컨테이너의 설계도

컨테이너가 격리된 프로세스라는 건 알았습니다. 그런데 그 프로세스가 무엇을 실행할지를 결정하는 원본이 따로 있어야 합니다. 그 원본이 **이미지**입니다.
...
이미지는 붕어빵 틀에 가깝습니다. 틀(이미지) 하나로 붕어빵(컨테이너)을 여러 개 찍어낼 수 있고, 틀 자체는 변하지 않습니다. 같은 이미지에서 나온 컨테이너는 어디서 띄우든 똑같은 환경을 가집니다. 이 덕에 "내 PC와 서버 환경이 같다"가 가능합니다.

:::term-box
**이미지(Image)**: 컨테이너를 만드는 데 필요한 파일시스템과 실행 정보를 하나로 묶은 읽기 전용 패키지입니다. 애플리케이션 코드, 라이브러리, 설정, 실행 명령이 함께 들어 있어 어디서 띄우든 동일한 환경이 만들어집니다.
:::
...
이미지는 하나의 큰 파일이 아니라 여러 레이어가 층층이 쌓인 구조입니다. 아래쪽에는 OS 기본 라이브러리나 런타임처럼 잘 안 바뀌는 부분이 있고, 위쪽에는 애플리케이션 코드처럼 자주 바뀌는 부분이 있습니다. 한 번 받은 아래 레이어는 저장해 두고 다시 쓰기 때문에, 코드만 바뀐 새 버전이 나오면 맨 위 레이어만 새로 받으면 됩니다.
:::

원리를 짚었으니 이제 Docker를 설치하고 직접 실행해 보겠습니다.

## 2.3 Docker 설치 - 실습 환경 준비

Docker Desktop 설치는 공식 사이트(https://www.docker.com/products/docker-desktop/)에서 OS에 맞는 설치 파일을 받아 설치합니다.
...
가장 먼저 Docker Hub에서 이미지를 내려받았습니다.
...
진행 바가 줄줄이 올라가더니 프롬프트가 돌아왔습니다. 이미지가 로컬 저장소에 내려와 있을 뿐, 아직 실행된 건 아닙니다.

### 2.4.2 docker run: 컨테이너 띄우기

이어서 다운받은 이미지로 컨테이너를 실행해 봤습니다.
...
*그림 2-13. 컨테이너 통신 한눈에 보기 - 외부 진입과 컨테이너끼리 IP 통신*
...
*그림 2-14. 주방 모니터·전용 통신 케이블·주문 전광판이 컨테이너 네트워크의 비유*
...
*그림 2-16. 입구 키오스크가 손님의 주문을 각 식당으로 넘겨주는 비유*
...
*그림 2-18. 안내 지도가 이름을 위치로 바꿔주는 비유*
...
*그림 2-19. Docker DNS가 이름을 IP로 변환*
...
*그림 2-20. Ubuntu 컨테이너 안으로 들어간 상태*
...
*그림 2-23. nginx 환영 페이지 응답*

익숙한 nginx 환영 페이지가 떴습니다. 컨테이너를 실행할 때 `-p 80:80`으로 포트를 열어 둔 덕에, 컨테이너 안의 nginx와 브라우저가 연결됐기 때문입니다.

### 2.6.3 vim으로 파일 편집

서버 설치를 마쳤더라도 운영하다 보면 설정 파일을 수정해야 할 일이 생기기 마련입니다. 리눅스에서 파일을 편집하려면 전용 편집기가 필요한데, **Vim**을 선택해 설치했습니다.
...
오픈이는 챕터 2.4.3에서 봤던 `-d` 옵션을 사용해 ubuntu를 백그라운드로 실행해 보기로 했습니다.
...
*그림 2-30. attach는 이미 떠 있는 PID 1 프로세스에 터미널을 직접 꽂는 방식*
...
*그림 2-32. exec는 컨테이너 안에 새 bash 프로세스를 띄우고 내 터미널에 연결하는 방식*
...
`exec`가 정말 새 프로세스를 만드는지 눈으로 확인하기 위해 호스트에서 터미널을 하나 더 열고 컨테이너 내부의 프로세스 목록을 찍어봤습니다.
...
컨테이너 안에서 `a.txt`를 만들고, 호스트 쪽 폴더를 확인해 봤습니다.
...
*그림 2-43. 호스트 PC에서 같은 파일이 보이는 것을 확인*
...
*그림 2-44. Docker 엔진이 관리하는 내부 저장 공간에 데이터가 저장되는 구조*
