...
목요일 오후, 회의실 한쪽에서 팀장이 화이트보드 앞에 서 있었습니다. 챕터 5에서 IP가 흔들려도 끊기지 않는 진입점을 만들어 둔 며칠이 지난 참이었습니다. 보드 위에는 챕터 3에서 docker-compose로 짰던 통합 사이트의 골격이 그대로 그려져 있었습니다. 프론트엔드와 백엔드, 그리고 그 뒤의 DB 한 덩어리.

**팀장**: "구성 잘 잡았네요. 그럼 챕터 3에서 docker-compose로 짰던 그 통합 사이트, 이번엔 쿠버네티스 위에 올려볼래요?"

오픈이는 펜을 든 채 잠시 멈칫했습니다. Pod 하나, Service 하나는 깔끔하게 띄울 수 있습니다. 그런데 프론트엔드·백엔드·DB가 한 덩어리로 묶인 사이트를 그대로 옮기려고 하니, 막상 손을 댈 자리가 보이지 않았습니다. docker-compose로 띄울 때 짚지 않고 지나갔던 두 가지가 한꺼번에 도드라졌습니다.
...
데이터 쪽은 한 단계 더 무거운 문제였습니다. 챕터 4에서 Deployment가 죽은 Pod를 곧장 살려 주는 모습을 봤기 때문에, 같은 일이 DB Pod에서 벌어지면 어떻게 되는지가 또렷이 보였습니다. 코드는 다시 띄우면 그만입니다. 그 안에 쌓아 둔 회원 정보와 게시글은 새 Pod로 따라오지 않습니다.
...
설정값과 비밀번호를 따로 보관할 전용 공간이 필요했습니다. 쿠버네티스에는 이미 그 자리가 마련되어 있었습니다. 떠오른 그림은 프랜차이즈 매장이었습니다. 매장 한쪽에 공용 메뉴판을 걸어 두고, 비밀 레시피는 같은 매장 안에 따로 두는 방식입니다. 건물을 새로 짓지 않아도 메뉴판과 레시피만 교체하면 매장을 운영할 수 있습니다. 쿠버네티스에서도 같은 발상으로 두 자리가 나뉘어 있습니다.

이 역할을 맡는 리소스는 두 가지입니다.
...
data 영역에 키-값 쌍을 적는 것만으로 준비는 끝났습니다. 이어서 이 ConfigMap을 Pod가 가져다 쓰도록 Deployment에서 연결해 줍니다. `envFrom.configMapRef`를 쓰면 ConfigMap의 모든 값을 한꺼번에 환경 변수로 주입할 수 있습니다.
...
남은 건 비밀번호였습니다. 같은 방식으로 ConfigMap에 비밀번호를 적어 두면 그만일까 생각이 들었지만, 곧 고개를 저었습니다. 민감한 값을 메뉴판처럼 공개된 곳에 적어 둘 수는 없습니다. 비밀번호는 매장 안 비밀 레시피처럼 따로 보관해야 할 값이고, 쿠버네티스에서는 그 자리를 Secret이라는 이름으로 따로 두고 있습니다.
...
Pod에 주입하는 방식은 ConfigMap과 동일합니다. 앞서 작성해 둔 `deploy-ex03.yml`을 다시 열어 `envFrom` 아래의 `secretRef` 두 줄에 걸린 `#` 주석을 풀어 줍니다. `secretRef`를 활성화하면 쿠버네티스가 실행 시점에 Base64 인코딩을 디코딩해 환경 변수로 넣어 줍니다.
...
apply를 걸자 `configured` 메시지가 떴습니다. ConfigMap 자체는 갱신된 듯했는데, 막상 Pod 안의 환경 변수를 들여다보니 포트는 여전히 `80`이었습니다.
...
설정과 비밀번호를 이미지 바깥으로 빼는 일은 끝났습니다. 노트에 적어 둔 두 단어 중 첫 번째 매듭이 풀렸지만, 한 가지가 더 남아 있습니다. 비밀번호로 잠가 두는 DB의 데이터가 아직 그대로였습니다.

## 6.2 Volume - 데이터의 영속성 확보

### 6.2.1 Pod의 휘발성 문제

설정 분리는 끝났고, 남은 건 데이터였습니다. DB Pod도 결국 다른 Pod와 똑같이 클러스터 위에서 도는 한 칸이라는 사실을 다시 떠올리자, 챕터 4에서 Pod 하나를 일부러 지워 봤던 장면이 함께 떠올랐습니다. 그때 Deployment가 곧장 새 Pod를 띄워 자가 치유는 잘 됐습니다. 그런데 사라진 Pod 안에 있던 파일은 새 Pod 어디에도 없었습니다. Pod는 기본적으로 **휘발성**이라, 안에서 만든 파일은 Pod 수명과 함께 사라집니다. 회원 가입과 게시글이 매일 쌓이는 DB가 이런 식으로 휘발되면 운영을 할 수가 없습니다.
...
다음 날 아침, 오픈이는 자리에 앉자마자 노트를 펼쳤습니다. 며칠 전 회의실에서 팀장이 의뢰한 챕터 3 그 사이트를 K8s 위에 올릴 차례였습니다. 매듭은 풀렸고, 도구는 쥐고 있었습니다. 컨테이너를 관리하는 Pod와 Deployment, 그 앞을 지키는 Service와 Ingress, 그리고 방금 익힌 ConfigMap·Secret과 PV·PVC. 한 줄로 늘어놓고 보니 책상 위에 어지간한 도구는 다 깔려 있었습니다.

*'지금까지는 따로따로 띄워 본 건데, 이것들이 한꺼번에 올라가도 진짜 설계도처럼 맞물려 돌아갈까?'*

부분 실습은 잘 됐습니다. 그런데 전체 시스템이 한 덩어리로 살아 움직이는 모습은 아직 머릿속에 잘 그려지지 않았습니다. 화면을 응시하던 오픈이의 등 뒤로 팀장의 목소리가 들려왔습니다.
...
<div class="svg-figure">
<svg viewBox="0 0 740 470" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="ex15 — 외부 진입과 프론트엔드·백엔드·데이터 3계층, 요청·응답 평행 표시">
  <defs>
  ...
  <text x="370" y="22" text-anchor="middle" font-size="13" font-weight="700" fill="#1f2937">ex15 Kubernetes 웹사이트의 전체 구성</text>

  <rect x="75" y="34" width="130" height="36" rx="6" fill="#fff" stroke="#9ca3af" stroke-width="1.4"/>
  <text x="140" y="57" text-anchor="middle" font-size="11" font-weight="700" fill="#374151">브라우저</text>

  <line x1="136" y1="70" x2="136" y2="148" stroke="#475569" stroke-width="1.6" marker-end="url(#ex15-p)"/>
  <line x1="144" y1="148" x2="144" y2="70" stroke="#94a3b8" stroke-width="1.4" stroke-dasharray="4,3" marker-end="url(#ex15-pr)"/>

  <rect x="20" y="84" width="700" height="376" rx="10" fill="#fff" stroke="#475569" stroke-width="1.4" stroke-dasharray="6,4"/>
  <text x="40" y="102" font-size="10" font-weight="600" fill="#0f172a">Cluster (namespace: metacoding)</text>

  <rect x="50" y="124" width="180" height="306" rx="8" fill="#f8fafc" stroke="#94a3b8" stroke-width="1.2" stroke-dasharray="3,2"/>
  <text x="70" y="142" font-size="10" font-style="italic" fill="#475569">프론트엔드</text>

  <rect x="75" y="150" width="130" height="32" rx="6" fill="#fff" stroke="#475569" stroke-width="1.6"/>
  <text x="140" y="170" text-anchor="middle" font-size="11" font-weight="700" fill="#0f172a">Ingress</text>

  <line x1="136" y1="184" x2="136" y2="202" stroke="#475569" stroke-width="1.6" marker-end="url(#ex15-p)"/>
  <line x1="144" y1="202" x2="144" y2="184" stroke="#94a3b8" stroke-width="1.4" stroke-dasharray="4,3" marker-end="url(#ex15-pr)"/>

  <rect x="75" y="205" width="130" height="32" rx="14" fill="#fff" stroke="#475569" stroke-width="1.6"/>
  <text x="140" y="225" text-anchor="middle" font-size="11" font-weight="700" fill="#0f172a">frontend-service</text>

  <line x1="136" y1="239" x2="136" y2="257" stroke="#475569" stroke-width="1.6" marker-end="url(#ex15-p)"/>
  <line x1="144" y1="257" x2="144" y2="239" stroke="#94a3b8" stroke-width="1.4" stroke-dasharray="4,3" marker-end="url(#ex15-pr)"/>

  <rect x="75" y="260" width="130" height="40" rx="6" fill="#fff" stroke="#475569" stroke-width="1.6"/>
  <text x="140" y="285" text-anchor="middle" font-size="11" font-weight="700" fill="#0f172a">Frontend Pod</text>

  <line x1="230" y1="274" x2="258" y2="274" stroke="#475569" stroke-width="1.6" marker-end="url(#ex15-p)"/>
  <line x1="258" y1="282" x2="230" y2="282" stroke="#94a3b8" stroke-width="1.4" stroke-dasharray="4,3" marker-end="url(#ex15-pr)"/>
  <text x="244" y="266" text-anchor="middle" font-size="9" fill="#6b7280" font-style="italic">/api</text>

  <rect x="260" y="124" width="200" height="306" rx="8" fill="#f8fafc" stroke="#94a3b8" stroke-width="1.2" stroke-dasharray="3,2"/>
  <text x="280" y="142" font-size="10" font-style="italic" fill="#475569">백엔드</text>

  <rect x="290" y="150" width="140" height="32" rx="14" fill="#fff" stroke="#475569" stroke-width="1.6"/>
  <text x="360" y="170" text-anchor="middle" font-size="11" font-weight="700" fill="#0f172a">backend-service</text>

  <line x1="356" y1="184" x2="356" y2="202" stroke="#475569" stroke-width="1.6" marker-end="url(#ex15-p)"/>
  <line x1="364" y1="202" x2="364" y2="184" stroke="#94a3b8" stroke-width="1.4" stroke-dasharray="4,3" marker-end="url(#ex15-pr)"/>

  <rect x="280" y="205" width="160" height="40" rx="6" fill="#fff" stroke="#475569" stroke-width="1.6"/>
  <text x="360" y="230" text-anchor="middle" font-size="11" font-weight="700" fill="#0f172a">Backend Pod ×2</text>

  <rect x="285" y="290" width="75" height="28" rx="6" fill="#fff" stroke="#475569" stroke-width="1.4"/>
  <text x="322" y="308" text-anchor="middle" font-size="10" font-weight="700" fill="#0f172a">ConfigMap</text>

  <rect x="380" y="290" width="75" height="28" rx="6" fill="#fff" stroke="#475569" stroke-width="1.4"/>
  <text x="417" y="308" text-anchor="middle" font-size="10" font-weight="700" fill="#0f172a">Secret</text>

  <path d="M 322 290 Q 332 270, 345 248" fill="none" stroke="#475569" stroke-width="1.3" stroke-dasharray="4,3" marker-end="url(#ex15-p)"/>
  <path d="M 417 290 Q 405 270, 380 248" fill="none" stroke="#475569" stroke-width="1.3" stroke-dasharray="4,3" marker-end="url(#ex15-p)"/>
  <text x="370" y="278" text-anchor="middle" font-size="9" fill="#6b7280" font-style="italic">envFrom</text>

  <line x1="460" y1="274" x2="488" y2="274" stroke="#475569" stroke-width="1.6" marker-end="url(#ex15-p)"/>
  <line x1="488" y1="282" x2="460" y2="282" stroke="#94a3b8" stroke-width="1.4" stroke-dasharray="4,3" marker-end="url(#ex15-pr)"/>

  <rect x="490" y="124" width="210" height="306" rx="8" fill="#f8fafc" stroke="#94a3b8" stroke-width="1.2" stroke-dasharray="3,2"/>
  <text x="510" y="142" font-size="10" font-style="italic" fill="#475569">데이터</text>

  <rect x="510" y="150" width="80" height="32" rx="14" fill="#fff" stroke="#475569" stroke-width="1.6"/>
  <text x="550" y="170" text-anchor="middle" font-size="11" font-weight="700" fill="#0f172a">db-service</text>

  <line x1="590" y1="162" x2="608" y2="162" stroke="#475569" stroke-width="1.6" marker-end="url(#ex15-p)"/>
  <line x1="608" y1="170" x2="590" y2="170" stroke="#94a3b8" stroke-width="1.4" stroke-dasharray="4,3" marker-end="url(#ex15-pr)"/>

  <rect x="610" y="146" width="85" height="42" rx="6" fill="#fff4ed" stroke="#ff7849" stroke-width="1.6"/>
  <text x="652" y="164" text-anchor="middle" font-size="11" font-weight="700" fill="#7b341e">MySQL Pod</text>
  <text x="652" y="180" text-anchor="middle" font-size="9" fill="#7b341e">+ PV (영속)</text>

  <rect x="510" y="260" width="80" height="32" rx="14" fill="#fff" stroke="#475569" stroke-width="1.6"/>
  <text x="550" y="280" text-anchor="middle" font-size="11" font-weight="700" fill="#0f172a">redis-service</text>

  <line x1="590" y1="272" x2="608" y2="272" stroke="#475569" stroke-width="1.6" marker-end="url(#ex15-p)"/>
  <line x1="608" y1="280" x2="590" y2="280" stroke="#94a3b8" stroke-width="1.4" stroke-dasharray="4,3" marker-end="url(#ex15-pr)"/>

  <rect x="610" y="258" width="85" height="36" rx="6" fill="#fff4ed" stroke="#ff7849" stroke-width="1.6"/>
  <text x="652" y="280" text-anchor="middle" font-size="11" font-weight="700" fill="#7b341e">Redis Pod</text>
</svg>
</div>
...
*그림 6-11. 같은 클러스터 안에서 Namespace가 리소스를 층처럼 분리*

:::term-box
**Namespace**는 리소스를 논리적으로 구분하는 가상 공간입니다. 별도 지정이 없으면 모든 리소스는 **default** 네임스페이스에 소속됩니다.
:::

이번 실습에서는 `metacoding`이라는 전용 네임스페이스를 만들어 모든 리소스를 그 안에서 관리하기로 했습니다.
...
코드를 살펴봤으니 띄울 차례였습니다. 오픈이는 본격적으로 배포하기 전에 두 가지부터 챙겼습니다. Minikube와 Ingress Controller를 켜는 일, 그리고 네 가지 이미지를 빌드해 두는 일이었습니다.
...
*그림 6-12. Nginx Ingress Controller 애드온 설치*
...
![](../assets/CH06/chap03-ingress-result.png)
...
![](../assets/CH06/chap03-ingress-result2.png)
...
입사 3개월 차에 환경이 달라서 코드가 안 돌던 그 금요일이 한참 멀게 느껴졌습니다. 그날 30줄짜리 변경분 하나에 두 시간을 흘려보냈고, 지하철에서 다큐멘터리를 보며 컨테이너라는 단어를 처음 만났던 오픈이가, 그 컨테이너로 표준화된 환경을 만들고, 같은 컨테이너 여러 개를 한 줄 명령으로 묶고, 클러스터 위에 한 사이트를 올려 자가 치유까지 보는 자리에 와 있었습니다.

여기까지가 이 책에서 짚은 흐름입니다. 한 명의 개발자가 하나의 클러스터 위에 하나의 서비스를 올리는 자리. 그 너머의 운영은 이 책의 범위 밖이지만, 다음에 공부할 거리를 제목만 짚어 두면 덜 막막합니다.
