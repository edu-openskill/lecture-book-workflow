...
지난 챕터의 세미나가 있고 며칠이 지났습니다. 컨테이너를 띄우고 내리는 명령어가 손에 익을 무렵, 팀장이 자리로 와서 작은 회의실에 모이자고 했습니다. 회의실 화이트보드에는 이미 사각형 세 개가 그려져 있었습니다. 프론트엔드, 백엔드, DB. 사내에서 흩어져 있던 자료를 한 곳에서 찾아 쓰자는 통합 사이트 이야기였습니다.

**팀장**: "환경 구성은 Docker로 해 봐요. 지난번에 공부했잖아요."

오픈이는 고개를 끄덕였습니다. 하지만 자리로 돌아오는 길에 머릿속이 복잡해졌습니다. 컨테이너 하나를 띄우는 것까지는 해 봤는데, 사이트 한 개를 통째로 올리려면 그것만으로는 부족했습니다. 화면을 보여주는 서버, 데이터를 처리하는 서버, 그 데이터를 보관할 데이터베이스가 따로 떠서 서로 연결되어야 했습니다.

*'한 대씩은 띄워봤는데, 여러 대를 동시에 굴려야 한다고?'*

퇴근 후 노트북을 열었습니다. 프로젝트에 필요한 부분을 하나씩 도커로 직접 구성해 보며 부딪쳐 보기로 했습니다.

## 3.1 Dockerfile — 환경을 자동으로 만들기

### 3.1.1 프로비저닝

가장 먼저 떠오른 것은 지난번에 했던 수동 작업이었습니다. 컨테이너에 접속해 패키지를 업데이트하고, 편집기를 설치하고, 소스코드를 만든 뒤 이미지로 남겼던 일입니다. 처음 한 번은 신기하고 재미있었습니다. 그런데 똑같은 일을 두 번째로 반복하려니 벌써 손이 무거워졌습니다.

*'프로젝트 세팅할 때마다 매번 이걸 반복해야 할까?'*

패키지 이름을 한 글자만 잘못 쳐도 처음부터 다시였고, 설정을 살짝 고치고 싶어도 전 과정을 다시 돌려야 했습니다. 고심하던 오픈이는 문득 밀키트를 떠올렸습니다. 재료와 양념이 미리 손질되어 있어 봉지만 뜯으면 바로 조리할 수 있는 밀키트처럼, 도커 환경도 누군가 미리 준비해 줄 수 없을까 하는 생각이 들었습니다.

다행히 도커에는 그런 방식이 이미 마련되어 있습니다. 밀키트를 준비하듯 환경 구성을 정의 파일 하나에 담아 자동화하는 작업을 **프로비저닝(Provisioning)** 이라고 부릅니다.
...
컨테이너를 삭제하더라도 이미지는 그대로 남아 있습니다. 덕분에 똑같은 환경이 필요할 때마다 언제든 다시 띄울 수 있습니다. 지난번 수동으로 commit 명령어를 쳐서 만들었던 자리를, 이제 Dockerfile이 대신 채웁니다.

### 3.1.3 Dockerfile 기본 문법

오픈이는 먼저 Dockerfile에서 가장 자주 쓰는 지시어를 정리해 두었습니다. 레시피를 적기 위한 일종의 재료 목록입니다.
...
오픈이는 첫 실습으로 Ubuntu 환경에 vim이 미리 설치된 이미지를 직접 만들어 보기로 했습니다. 빈 폴더에 확장자 없이 `Dockerfile`이라는 이름으로 파일을 하나 만들고 아래 내용을 작성했습니다.
...
작성한 파일을 저장하고 터미널에서 빌드 명령을 실행했습니다. (터미널은 Dockerfile이 있는 폴더에서 실행해야 합니다.)
...
Dockerfile로 패키지 설치는 끝냈습니다. 그런데 한 가지 의문이 남았습니다. 로컬에서 작성한 소스 코드는 컨테이너 안으로 어떻게 옮겨야 할까요. 프로젝트를 실제로 구동하려면 이 과정이 반드시 필요합니다. 이때 사용하는 지시어가 바로 **WORKDIR**와 **COPY**입니다.

오픈이는 우선 Dockerfile과 같은 위치에 비어 있는 **index.html** 파일을 하나 만들었습니다.
...
이렇게 하면 WORKDIR 덕분에 컨테이너가 실행될 때 기본 경로가 /app이 되고, 그 안에 index.html이 미리 들어가 있게 됩니다. 오픈이는 다시 빌드하고 컨테이너를 실행했습니다.
...
터미널은 접속하자마자 /app 경로를 가리키고 있었고, 그 안에는 index.html이 들어 있었습니다. 직접 들어가서 파일을 옮기거나 환경을 설정할 필요가 없어졌습니다. 확인을 마친 오픈이는 `exit`을 입력하고 컨테이너에서 빠져나왔습니다.

### 3.1.5 CMD와 ENTRYPOINT

앞서 정리해 둔 문법 표를 다시 들여다보던 오픈이의 시선이 표 아래쪽 두 줄에서 멈췄습니다. CMD는 "**실행할 기본 명령어**", ENTRYPOINT는 "**반드시 실행되는 메인 프로세스**". 둘 다 '실행'이라는 단어가 들어가 있었습니다.

*'기본 명령어와 반드시 실행되는 메인 프로세스. 둘 다 어쨌든 실행한다는 말 같은데, 무슨 차이지?'*

두 지시어는 성격이 조금 다릅니다. 비유하자면 **ENTRYPOINT** 는 커피 머신의 **'커피를 내린다'** 는 본질이고, **CMD** 는 따로 주문이 없을 때 기본으로 내려주는 **'아메리카노'** 같은 존재입니다. 손님이 라떼나 에스프레소를 주문하면 기본 메뉴는 다른 음료로 **바뀔 수 있지만**, '커피를 내린다'는 동작 자체는 어떤 주문에서든 **그대로 유지됩니다**.
...
오픈이는 실제 동작을 확인하려고 Dockerfile에 ENTRYPOINT를 추가해 보았습니다. echo 명령어로 메시지를 출력하는 간단한 구성입니다.
...
ENTRYPOINT에 설정된 echo 프로세스에 의해 **컨테이너 실행 /bin/bash** 라는 문구가 찍히고 프로세스가 바로 종료되었습니다.
...
이런 방식은 실제 서비스를 배포할 때 유용합니다. ENTRYPOINT에는 서버 실행 명령어를 고정해 두고, CMD에는 상황에 따라 바뀔 수 있는 옵션이나 파일명만 적어 두면 이미지를 훨씬 유연하게 재활용할 수 있습니다.

*'매번 명령어로 환경을 만들어야 했는데, 이제는 Dockerfile만 빌드하면 자동으로 세팅되네. 이게 프로비저닝이구나.'*
...
오픈이는 화이트보드에 통합 사이트의 구조를 그려 보았습니다. 프론트엔드와 백엔드 컨테이너가 따로 떠 있는 모양은 좋았지만, 내부의 주소나 포트를 그대로 외부에 노출하는 방식은 보안상 위험해 보였습니다.

*'내부 인프라가 어떻게 구성되어 있든 사용자는 하나의 입구로만 들어오게 할 수 없을까? 복잡한 내부 구조는 감추면서 요청만 적절히 배분해 줄 단일 창구가 필요해.'*

자료를 찾던 오픈이의 눈에 들어온 것은 도커 명령어를 공부할 때 사용했던 NGINX였습니다.

NGINX는 웹 서버이면서, 요청을 중간에서 중계하는 **리버스 프록시(Reverse Proxy)** 이기도 합니다. 사용자의 요청을 NGINX가 대신 받아 뒤쪽 서버로 넘겨주고, 서버의 응답을 다시 받아 사용자에게 돌려줍니다.

:::term-box
**NGINX**: 가벼우면서도 빠른 오픈소스 웹 서버이자 리버스 프록시입니다. 정적 파일 응답뿐 아니라 요청을 뒤쪽 서버로 전달·분배하고, 자주 쓰는 응답을 캐싱하는 등 서비스 앞단의 트래픽 관리에 두루 사용됩니다.
:::
...
이렇게 하면 실제 서버의 IP 주소나 내부 포트를 외부에 드러내지 않아도 되어 보안에 유리합니다. 또한 나중에 서버를 여러 대로 늘렸을 때 요청을 골고루 분산해 주는 로드밸런싱 기능까지 제공합니다. 오픈이는 모든 요청이 거쳐 가는 단일 진입점으로 NGINX를 가장 앞단에 두기로 했습니다.
...
NGINX는 이 설정을 `nginx.conf` 파일에 작성하며, 다양한 옵션을 조합해 원하는 기능을 구현할 수 있습니다.
...
오픈이는 가장 먼저 URL 경로에 따라 서로 다른 서버로 요청을 보내는 실습을 시작했습니다. /app1로 접속하면 1번 서버로, /app2로 접속하면 2번 서버로 요청이 가도록 만드는 방식입니다. 서비스가 여러 컨테이너로 나뉘어 있을 때 NGINX가 앞단에서 길을 갈라 주는 구조입니다.
...
실습은 ex01 폴더에 있습니다. 오픈이는 먼저 폴더 구조를 확인했습니다. 세 개의 컨테이너(app1, app2, lb)가 각자의 Dockerfile을 통해 실행되는 구조입니다.
...
이번 실습의 핵심은 lb 폴더의 nginx.conf입니다. 오픈이는 경로에 따라 서버 그룹을 매칭해 주는 설정을 작성했습니다.
...
설정 파일이 준비됐으니, 이제 이 파일을 담은 컨테이너를 어떻게 만들지가 남았습니다. lb 폴더의 Dockerfile이 그 역할을 합니다.
...
공식 NGINX 이미지에 방금 작성한 nginx.conf만 덮어쓰면 라우터가 완성됩니다. app1·app2의 Dockerfile도 같은 패턴으로, NGINX 이미지에 각자의 index.html만 얹은 구조입니다.

세 폴더가 모두 준비되자 오픈이는 차례대로 빌드하고 실행했습니다.
...
URL 경로만 달라졌을 뿐인데 서로 다른 서버가 응답했습니다. location이 요청을 낚아채고, proxy_pass가 해당 upstream으로 넘겨준 결과입니다.
...
*'이제 내가 네트워크를 만들어서 묶으면 컨테이너 이름으로 통신이 가능해지겠네.'*
...
이번에는 NGINX의 로드밸런싱을 실습해 보기로 했습니다.

NGINX는 upstream 블록 안에 server 줄을 추가하기만 하면 들어오는 요청을 순차적으로 서버에 전달합니다. 카드 딜러가 한 장씩 돌아가며 카드를 나눠 주는 모습과 비슷합니다. 이러한 방식을 **라운드 로빈(Round Robin)** 이라고 부릅니다.
...
이번 실습은 ex02 폴더에 들어 있습니다. 이전 실습과 달라진 점은 nginx.conf의 upstream 안에 컨테이너 이름으로 서버 두 개가 들어 있다는 것입니다.
...
오픈이는 같은 이미지를 사용해 컨테이너를 두 번 띄우기로 했습니다. 사용자 정의 네트워크에 묶어 두면 호스트 포트를 노출하지 않아도 컨테이너끼리 이름으로 통신할 수 있습니다.
...
설정을 마치고 브라우저에서 localhost:80/app1에 접속해 보았습니다. 새로고침을 반복해도 화면에 보이는 HTML은 똑같았습니다.

![](../assets/CH03/chap02-21.png)

*그림 3-14. /app1 경로로 접속한 결과 (라운드 로빈)*

오픈이는 실제로 두 컨테이너에 요청이 나뉘어 들어가는지 확인하기 위해 `docker logs`로 각 컨테이너의 로그를 살폈습니다.
...
새로고침을 할 때마다 요청이 app1-1과 app1-2 컨테이너로 번갈아 들어오는 것이 눈으로 보였습니다. 설정 파일에 server 줄 하나를 추가했을 뿐인데, NGINX가 알아서 트래픽을 두 대의 서버로 나누어 보내 준 것입니다. 별도의 복잡한 세팅 없이도 기본적으로 라운드 로빈 방식이 적용된 결과였습니다.
...
로드밸런싱 테스트를 하던 오픈이는 앞서 본 그림 3-15, 3-16의 로그를 보다가 의문이 생겼습니다. 두 컨테이너에 번갈아 들어온 요청 모두 같은 응답을 백엔드에서 새로 만들어 주고 있었습니다.

*'단순한 HTML 페이지인데, 같은 응답을 매번 백엔드까지 가서 만들어야 하나? 단순 페이지도 응답을 하면 서버에 부하가 많겠는데...'*

이 비효율을 풀어 주는 방법이 있습니다. 자주 꺼내 입는 옷을 옷장 깊숙이 넣지 않고 옷걸이에 따로 걸어 두는 것처럼, 자주 요청되는 응답을 NGINX 앞단에 잠시 보관해 두면 다음번에 같은 요청이 들어와도 백엔드까지 가지 않고 NGINX가 바로 돌려줄 수 있습니다. 이 과정을 **캐싱(Caching)** 이라고 부릅니다.
...
이번 캐싱 실습은 ex03 폴더에 준비되어 있습니다. 이미지 파일을 응답으로 내려 주는 간단한 파이썬 기반 API 서버를 활용합니다.
...
오픈이는 터미널을 열고 실습 환경을 실행했습니다. ex02에서 띄워 둔 lb 컨테이너가 80 포트를 잡고 있으므로, 새 nginx-cache가 같은 포트를 쓸 수 있도록 먼저 정리하고 시작합니다.
...
브라우저에서 localhost:80/image.png를 요청해 보았습니다. 화면에는 이미지가 정상적으로 출력되었습니다.
...
실습을 마친 오픈이는 nginx.conf의 패턴이 늘 같다는 것을 알아챘습니다. **어떤 요청(location)을** **어디로 보낼지(proxy_pass)**, 그리고 **어떤 옵션을 끼울지** 정하는 흐름입니다.

## 3.3 Redis — 서버 여러 대가 공유하는 세션 저장소

### 3.3.1 서버 여러 대면 생기는 세션 문제

NGINX로 부하 문제는 풀었습니다. 그러던 중 옆자리 동료가 화면을 들고 자리로 왔습니다.

**동료**: "오픈 님, 이거 좀 보세요. 로그인했는데 다음 페이지 누르면 다시 로그인하라고 떠요. 한 번이 아니라 계속이에요."

오픈이는 동료 자리에 가서 직접 눌러 봤습니다. 로그인하고 페이지를 이동하자마자 인증 실패 메시지가 떴습니다.

*'로그인을 했는데 왜 인증 실패가 뜨지?'*

원인은 로그인 기록이 처음 접속한 서버의 메모리에만 들어 있었기 때문입니다. 서버는 사용자가 로그인하면 그 정보를 자기 메모리에 임시로 저장합니다. 이런 기록을 **세션(Session)** 이라고 부릅니다.

:::term-box
**세션(Session)**: 사용자가 로그인했을 때 서버가 생성하는 임시 기록입니다. 서버는 "이 사용자는 인증되었습니다"라는 정보를 자신의 메모리에 보관하고, 이후 요청이 올 때마다 이 기록을 대조해 로그인 상태를 유지합니다.
:::

서버가 한 대일 때는 문제가 없습니다. 하지만 두 대 이상이 되면 메모리가 서버마다 따로 있어 한 서버에서 만들어진 세션을 다른 서버는 알 수 없습니다.
...
### 3.3.2 Redis — 서버들의 공유 칠판

오픈이가 찾은 답은 간단했습니다. 세션을 각 서버의 메모리가 아니라 모든 서버가 같이 들여다볼 수 있는 외부 저장소에 두는 것입니다.

이는 개인 노트와 칠판의 차이와 같습니다. 개인 노트에 적은 내용은 본인만 볼 수 있습니다. 칠판에 적은 내용은 누구나 볼 수 있습니다. 이 칠판 역할을 하는 대표적인 도구가 바로 **Redis** 입니다.
...
1번 서버가 로그인을 처리한 뒤 세션 기록을 Redis에 저장한다고 하겠습니다. 그러면 2번 서버로 요청이 왔을 때 Redis를 참조해 로그인 정보를 즉시 확인할 수 있습니다.
...
app.py는 `/save` 경로로 값을 저장하고 `/read` 경로로 값을 꺼내는 간단한 서버입니다.

이 app.py를 띄울 컨테이너의 Dockerfile은 다음과 같습니다.
...
가벼운 파이썬 이미지 위에 flask와 redis 라이브러리만 설치해 app.py를 띄우는 구조입니다. Redis는 별도 빌드 없이 공식 이미지를 그대로 쓰므로, 직접 만드는 이미지는 이 api 하나뿐입니다.

준비가 끝나자 오픈이는 네트워크를 먼저 생성한 뒤, 세 컨테이너가 모두 같은 네트워크 안에서 돌아가도록 차례대로 실행했습니다.
...
결과는 성공이었습니다. api1에서 저장한 데이터가 api2에서도 그대로 나왔습니다.

각 서버가 따로 상태를 들고 있을 때 생긴 문제가, 외부 저장소 하나로 한꺼번에 풀렸습니다. 동료에게 다시 화면을 보여줬더니 고개를 끄덕였습니다.
...
Redis로 세션 문제는 풀었습니다. 하지만 오픈이는 실습을 마치고 잠시 멈췄습니다. Redis는 데이터를 메모리에 저장하기 때문에 컨테이너를 재시작하면 모든 값이 사라집니다.

*'세션처럼 잠깐 들고 있는 데이터는 괜찮지만, 회원 정보나 게시글처럼 꼭 남아야 하는 데이터는 어디에 저장해야 하지?'*

꼭 남아야 하는 데이터는 별도 데이터베이스 서버에 두어야 했습니다. 오픈이는 통합 사이트의 사용자 데이터를 담을 DB로 MySQL을 선택했고, 이를 컨테이너로 띄워 보기로 했습니다.
...
- **/docker-entrypoint-initdb.d** : MySQL 공식 이미지가 제공하는 특수 경로입니다. 여기에 init.sql 파일을 넣어두면 컨테이너가 처음 실행될 때 자동으로 SQL 문을 실행해 테이블과 초기 데이터를 만들어 줍니다.
- **환경 변수(ENV)** : 네 개의 환경 변수를 통해 계정과 DB 이름을 미리 설정합니다. 실제 접속 시 이 설정값을 사용하게 됩니다.

:::note
**Dockerfile에 비밀번호를 직접 적는 방식**

이번 실습에서는 과정을 단순하게 보여드리기 위해 비밀번호를 Dockerfile에 직접 적었습니다. 하지만 실무에서 이런 방식은 매우 위험합니다. 이미지를 빌드하는 순간 이미지를 가진 사람 누구나 비밀번호를 볼 수 있기 때문입니다. 그래서 실제 서비스를 운영할 때는 비밀번호 같은 민감한 정보를 이미지 내부에 기록하지 않고 외부에 따로 보관해 두었다가, 컨테이너가 실행되는 시점에만 살짝 건네주는 방식을 사용합니다. 이 방식은 쿠버네티스에서 사용해 보겠습니다.
:::
...
init.sql에 적어 두었던 초기 데이터가 테이블에 그대로 담겨 있었습니다.

DB까지 컨테이너로 준비를 마친 오픈이는 터미널의 히스토리를 위로 올려 보다가 멈췄습니다. 지금까지 띄운 컨테이너만 벌써 여러 개였습니다. 매번 빌드와 실행 명령어를 일일이 입력하는 과정이 번거롭게 느껴졌습니다.

*'프론트엔드, 백엔드, DB까지... 이 많은 컨테이너를 한 번에 관리할 수 있는 방법은 없을까?'*

오픈이는 흩어져 있는 컨테이너를 하나로 묶어 관리할 수 있는 도구를 찾아보기 시작했습니다.

## 3.5 Docker Compose — 여러 컨테이너를 한 번에

### 3.5.1 docker run 반복의 한계

실습을 하던 오픈이는 불만이 생겼습니다. 수동 설치가 피곤해서 Dockerfile을 배웠는데, 이제는 그 Dockerfile로 만든 이미지를 일일이 실행하느라 또 다른 수동 작업에 갇힌 기분이었습니다.

*'몇 개 안되는 Dockerfile을 하나씩 실행하는 것도 힘든데 실무에서는 이걸 어떻게 하나씩 하지?'*
...
오픈이는 옆자리 선배에게 짧게 물어봤습니다.

**오픈이**: "선배님, 컨테이너가 늘어날수록 build, run 명령이 줄줄이 길어지는데, 다들 이걸 그냥 외워서 치시는 거예요?"

**선배**: "한 파일에 다 적어 두고 한 번에 띄우는 도구가 있어요. Compose라고. 한번 찾아봐요."

오픈이는 자리로 돌아와 검색창에 Compose를 쳐 봤습니다. 화면에 뜬 설명이 마음에 들었습니다. 여러 악기가 모여 연주할 때, 각자의 악보를 따로 들고 있는 게 아니라 **악보** 한 장에 모든 파트를 적어 두고 지휘자의 손짓 한 번에 연주를 시작하는 방식입니다.
...
<svg viewBox="0 0 720 280" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Docker Compose 방식 — 한 번에 생성 및 연결">
  ...
  <text x="422" y="180" text-anchor="middle" font-size="11" font-weight="700" fill="#7b341e">애플리케이션</text>
  <text x="422" y="196" text-anchor="middle" font-size="11" fill="#7b341e">컨테이너</text>
  <g transform="translate(478, 100)" stroke="#ff7849" fill="none" stroke-linecap="round" stroke-linejoin="round"><path d="M 0 12 L 18 0 L 68 0 L 50 12 Z" stroke-width="1.7" fill="#fff"/><path d="M 50 12 L 68 0 L 68 40 L 50 52 Z" stroke-width="1.7" fill="#fff"/><rect x="0" y="12" width="50" height="40" stroke-width="1.7" fill="#fff"/><line x1="10" y1="14" x2="10" y2="50" stroke-width="1"/><line x1="20" y1="14" x2="20" y2="50" stroke-width="1"/><line x1="30" y1="14" x2="30" y2="50" stroke-width="1"/><line x1="40" y1="14" x2="40" y2="50" stroke-width="1"/></g>
  <text x="512" y="180" text-anchor="middle" font-size="11" font-weight="700" fill="#7b341e">데이터베이스</text>
  <text x="512" y="196" text-anchor="middle" font-size="11" fill="#7b341e">컨테이너</text>
  <g transform="translate(568, 100)" stroke="#ff7849" fill="none" stroke-linecap="round" stroke-linejoin="round"><path d="M 0 12 L 18 0 L 68 0 L 50 12 Z" stroke-width="1.7" fill="#fff"/><path d="M 50 12 L 68 0 L 68 40 L 50 52 Z" stroke-width="1.7" fill="#fff"/><rect x="0" y="12" width="50" height="40" stroke-width="1.7" fill="#fff"/><line x1="10" y1="14" x2="10" y2="50" stroke-width="1"/><line x1="20" y1="14" x2="20" y2="50" stroke-width="1"/><line x1="30" y1="14" x2="30" y2="50" stroke-width="1"/><line x1="40" y1="14" x2="40" y2="50" stroke-width="1"/></g>
  <text x="602" y="180" text-anchor="middle" font-size="11" font-weight="700" fill="#7b341e">API</text>
...
Compose가 해 주는 핵심 역할은 세 가지입니다.

- **순서**: 어떤 컨테이너(예: DB)가 먼저 준비되어야 하는지 순서를 지정할 수 있습니다.
- **네트워크**: 같은 Compose 파일에 정의된 컨테이너는 자동으로 하나의 네트워크에 묶입니다. 이제 **docker network create** 를 따로 할 필요가 없습니다.
...
설정 파일의 뼈대는 생각보다 단순합니다. 오픈이는 자주 쓰이는 옵션을 중심으로 구조를 익혔습니다.
...
오픈이는 이전 ex01 실습(경로 기반 라우팅)을 Compose로 다시 만들어 보기로 했습니다. 기존 Dockerfile은 그대로 두고, 세 컨테이너를 묶을 docker-compose.yml만 새로 작성했습니다.
...
Compose는 네트워크도 자동으로 만들어 줍니다. 그래서 lb/nginx.conf에서 백엔드 주소를 `host.docker.internal:포트` 대신 `app1:80`, `app2:80`처럼 서비스 이름으로 바로 부를 수 있습니다.

이제 터미널에서 프로젝트가 있는 ex06으로 이동한 후 명령어를 실행합니다.
...
빌드부터 실행까지 한 번에 끝났습니다. 브라우저 접속 결과는 이전과 똑같았습니다. 그런데 여러 번 치던 명령어가 단 한 줄로 줄어들었습니다.
...
지금까지 컨테이너 하나씩 띄우는 법, 그리고 컴포즈로 묶는 법을 익혔습니다. 이제 그 조각을 모아 며칠 전 회의실에서 의뢰받은 통합 사이트를 한 세트로 만들어 보겠습니다. 프론트엔드(NGINX), 백엔드(Spring Boot), DB(MySQL)가 함께 도는 한 묶음입니다. 목표는 단순합니다. `docker compose up` 한 줄로 전부 띄우는 것입니다.
...
오픈이는 먼저 세 컨테이너가 어떻게 맞물려 도는지 살폈습니다. 각자의 역할은 다음과 같습니다.
...
*'하나씩 떼어서 연습했던 것들을 이제 하나의 서비스로 합쳐 보는 과정이네.'*

오픈이는 ex07 폴더의 파일을 살폈습니다. 백엔드, DB, 프론트엔드가 각자 폴더에 담겨 있고, docker-compose.yml로 이 셋을 한 번에 띄우는 구성입니다.
...
backend 폴더를 열어 보던 오픈이의 눈에 특이한 부분이 들어왔습니다. Spring Boot 서버인데도 소스 코드가 폴더에 없었습니다. 대신 Dockerfile 옆에 `entrypoint.sh`라는 셸 스크립트가 한 장 들어 있었습니다.

이 스크립트는 컨테이너가 뜨는 순간 깃허브에서 소스를 받아와 그 자리에서 빌드하고 실행하도록 짜여 있었습니다. 한 번 적어 두면 누가 어디서 컨테이너를 띄워도 같은 백엔드가 자동으로 올라옵니다.
...
이 스크립트를 컨테이너가 뜰 때 자동으로 실행하도록 Dockerfile에 묶어 둡니다.
...
빌드되어 실행되는 백엔드 서버 자체는 단순합니다. `/api/users` 요청이 오면 DB에서 회원 목록을 꺼내 JSON으로 돌려주는 컨트롤러 한 개입니다.

*'이미지에 소스 가져오는 로직을 넣으니, 환경에 상관없이 코드만 있으면 바로 실행해 볼 수 있겠구나.'*

### 3.6.3 DB: 3.4와 동일한 MySQL 컨테이너

DB 컨테이너는 3.4에서 만든 MySQL 구성을 그대로 사용합니다. 이미지와 init.sql 모두 동일합니다.
...
오픈이는 이어서 프론트엔드를 살폈습니다. frontend 폴더의 두 파일은 다음 역할을 합니다.
...
핵심은 백엔드 주소를 IP가 아니라 `backend:8080`처럼 서비스 이름으로 부른다는 점입니다. Compose가 자동으로 만든 네트워크에서 서비스 이름이 그대로 호스트명으로 통합니다.
...
그리고 프로젝트가 있는 ex07로 이동한 후 서비스를 실행했습니다.
...
백엔드가 소스를 가져와 빌드하는 과정이 있어 첫 실행에는 시간이 다소 걸렸습니다.

![](../assets/CH03/chap02-58.png)

*그림 3-37. 사용자 목록 조회 성공*

빌드가 완료된 후 브라우저에서 localhost:80에 접속하자, 며칠 전 회의실 화이트보드에 그린 그 사이트가 화면에 떴습니다. DB에서 가져온 사용자 목록이 페이지에 줄지어 나타났습니다. 노트북을 펴고 부딪치며 시작했던 며칠 전과 달리, 이제는 yml 한 장만 있으면 그 셋을 한 번에 띄울 수 있습니다.

다음 날 팀장 앞에서 시연이 있었습니다. 키보드 위에서 `docker compose up` 한 줄을 친 순간, 작은 회의실 모니터에 세 컨테이너가 동시에 올라가는 로그가 흘렀고, 곧이어 통합 사이트의 첫 화면이 떴습니다. 팀장이 짧게 한마디 했습니다.
...
- **Redis는 서버들이 함께 들여다보는 칠판입니다.** 서버가 여러 대로 늘어도 세션을 외부 저장소에 두면 로그인 상태가 흩어지지 않습니다.
...
프로젝트를 돌릴 환경은 갖춰졌습니다. 시연도 무사히 끝났습니다. 그런데 자리로 돌아오는 길에 한 가지 생각이 머리를 떠나지 않았습니다.

*'지금은 내가 수동으로 관리하지만, 만약 새벽 2시에 컨테이너가 죽으면 누가 다시 살려주지?'*

자동 복구, 무중단 배포, 다중 서버 관리, 설정 분리. 실제 운영에서 마주칠 이런 과제를 한 번에 풀어주는 자동화 시스템이 다음 챕터의 주인공 **쿠버네티스(Kubernetes)** 입니다.
