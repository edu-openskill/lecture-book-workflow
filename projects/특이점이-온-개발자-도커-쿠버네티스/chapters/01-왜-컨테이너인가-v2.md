...
*'런타임 버전, 시스템 패키지, 설정 경로. 그것만 한 상자에 통째로 들어 있었다면.'*
...
IT에도 똑같은 문제가 있습니다. 애플리케이션이 돌아가려면 런타임, 라이브러리, 설정, 시스템 파일이 모두 필요합니다. 이 환경이 자리마다 조금씩 달라서 한쪽에서 실행되던 코드가 다른 쪽에서는 오류가 발생합니다.
...
:::note
**해운 컨테이너와 IT 컨테이너 — 이름이 같은 이유**

해운 컨테이너는 화물을 담아 배·기차·트럭 사이를 옮기는 표준 상자이고, IT 컨테이너는 애플리케이션과 실행 환경을 담아 PC·서버·클라우드 사이를 옮기는 표준 상자입니다. 안에 들어가는 내용물은 다르지만 "규격을 통일해 어디든 같은 모양으로 옮긴다"는 발상이 같아서 이름까지 그대로 빌려왔습니다.
:::
...
컨테이너 수는 빠르게 늘어납니다. 말콤 맥린의 시대에도 그랬습니다. 수백, 수천 개가 되자 "**어느 배에 뭘 실을지, 고장 나면 누가 대체할지**"를 사람이 다 챙길 수 없었습니다. 항만은 전체를 조율할 **관제 시스템**을 들여야 했습니다.
...
<div class="evolve-flow">
  ...
  <div class="ev-card">
    ...
    <div class="ev-add">스스로 살아나게 한다</div>
  </div>
  <div class="ev-arrow">→</div>
  <div class="ev-card">
...
- **Kubernetes는 항만 관제 센터입니다.** 수많은 컨테이너(배)를 자동으로 관리합니다. 사람이 "원하는 상태"만 선언하면 시스템이 그 상태를 유지합니다.

규격이 같은 상자라는 발상이 어디서 왔는지 짚었으니, 이제 그 상자가 한 컴퓨터 안에서 실제로 어떻게 만들어지는지 들여다볼 차례입니다.

다음 챕터에서는 Docker가 한 서버 안에서 여러 컨테이너를 서로 보이지 않게 쪼개는 원리, 그 컨테이너를 찍어내는 **이미지**의 정체, 컨테이너 하나를 띄우고 안으로 들어가 보는 첫 실습까지 이어집니다.
