...
Ingress 리소스에는 어떤 경로를 어느 Service로 보낼지 **라우팅 규칙을 적어 둡니다**. Ingress 컨트롤러는 **라우팅 규칙을 미리 읽어 두었다가**, 외부 요청이 들어오면 **규칙에 맞는 Service로 보냅니다**.

<div class="svg-figure">
<svg viewBox="0 0 760 360" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Ingress Controller가 Ingress 리소스(YAML)의 규칙을 참조해 외부 요청을 두 Service로 분기하는 구조">
  <defs>
  ...
  <text x="380" y="22" text-anchor="middle" font-size="13" font-weight="700" fill="#1f2937">Ingress Controller가 리소스의 규칙을 참조해 요청을 Service로 분기</text>
  ...
  <line x1="280" y1="200" x2="280" y2="158" stroke="#475569" stroke-width="1.4" stroke-dasharray="4,3" marker-end="url(#ic13-ref)"/>
  <text x="318" y="182" text-anchor="middle" font-size="10" font-style="italic" fill="#475569">규칙 참조</text>
...
마지막 줄에 `'ingress' addon is enabled`가 떴습니다. Ingress 컨트롤러가 실제로 떠 있는지 한 번 더 확인합니다.
...
`rules` 아래 두 개의 `path`가 있고, `/order`와 `/stores` 두 경로를 각각의 Service로 돌리는 두 줄짜리 라우팅 규칙입니다. Ingress Controller가 이 규칙을 읽고 실제 트래픽을 분류해 보냅니다.
...
같은 도메인인데 뒤에 붙는 경로 한 글자에 따라 응답이 갈라집니다. `localhost` 한 곳으로만 들어오는데도 누른 경로에 따라 각각의 Service로 정확히 갈라집니다. Ingress가 URL 경로를 읽고 알맞은 Service로 트래픽을 분기해 준 결과입니다.
...
### 5.2.5 Service와 Ingress가 보는 정보가 다른 이유
