...
<svg viewBox="0 0 760 300" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="LoadBalancer — 공인 IP가 각 노드의 NodePort로 직접 트래픽을 분산하고, Service는 Pod 집합을 라벨로 묶는 논리 리소스">
  ...
  <text x="200" y="225" text-anchor="middle" font-size="10" font-family="monospace" fill="#6b7280">myapp.com</text>
  <line x1="250" y1="77" x2="278" y2="120" stroke="#475569" stroke-width="1.4" marker-end="url(#lb-p)"/>
  <line x1="250" y1="135" x2="278" y2="140" stroke="#475569" stroke-width="1.4" marker-end="url(#lb-p)"/>
  <line x1="250" y1="193" x2="278" y2="165" stroke="#475569" stroke-width="1.4" marker-end="url(#lb-p)"/>
  <rect x="280" y="100" width="140" height="90" rx="10" fill="#fff4ed" stroke="#ff7849" stroke-width="1.8" stroke-dasharray="6,4"/>
  <text x="350" y="125" text-anchor="middle" font-size="11" font-weight="600" fill="#7b341e">Cloud Provider</text>
  <text x="350" y="148" text-anchor="middle" font-size="13" font-weight="700" fill="#7b341e">LoadBalancer</text>
  <text x="350" y="172" text-anchor="middle" font-size="10" font-family="monospace" fill="#7b341e">공인 IP</text>
  <rect x="450" y="40" width="170" height="250" rx="10" fill="#fff" stroke="#ff7849" stroke-width="1.6" stroke-dasharray="5,3"/>
  <text x="463" y="58" font-size="11" font-weight="700" fill="#7b341e">Kubernetes 클러스터</text>
  <line x1="420" y1="120" x2="458" y2="107" stroke="#475569" stroke-width="1.4" marker-end="url(#lb-p)"/>
  <line x1="420" y1="145" x2="458" y2="180" stroke="#475569" stroke-width="1.4" marker-end="url(#lb-p)"/>
  <line x1="420" y1="170" x2="458" y2="253" stroke="#475569" stroke-width="1.4" marker-end="url(#lb-p)"/>
  <rect x="460" y="75" width="150" height="65" rx="6" fill="#fff" stroke="#cbd5e1" stroke-width="0.8"/>
  <text x="475" y="93" font-size="10" font-weight="700" fill="#0f172a">Node 1</text>
  <rect x="510" y="95" width="80" height="32" rx="4" fill="#fff4ed" stroke="#ff7849" stroke-width="1.4"/>
  <text x="550" y="115" text-anchor="middle" font-size="11" font-weight="700" fill="#7b341e">Pod</text>
  <rect x="460" y="148" width="150" height="65" rx="6" fill="#fff" stroke="#cbd5e1" stroke-width="0.8"/>
  <text x="475" y="166" font-size="10" font-weight="700" fill="#0f172a">Node 2</text>
  <rect x="510" y="168" width="80" height="32" rx="4" fill="#fff4ed" stroke="#ff7849" stroke-width="1.4"/>
  <text x="550" y="188" text-anchor="middle" font-size="11" font-weight="700" fill="#7b341e">Pod</text>
  <rect x="460" y="221" width="150" height="65" rx="6" fill="#fff" stroke="#cbd5e1" stroke-width="0.8"/>
  <text x="475" y="239" font-size="10" font-weight="700" fill="#0f172a">Node 3</text>
  <rect x="510" y="241" width="80" height="32" rx="4" fill="#fff4ed" stroke="#ff7849" stroke-width="1.4"/>
  <text x="550" y="261" text-anchor="middle" font-size="11" font-weight="700" fill="#7b341e">Pod</text>
</svg>
</div>
...
앞서 만든 Service YAML에는 `port`, `targetPort`, `nodePort` 세 포트가 한 곳에 모여 있습니다. 각 포트는 외부에서 들어온 요청을 Pod 안 앱까지 단계별로 전달하기 위한 역할을 합니다.
...
| `targetPort` | 컨테이너 | 컨테이너 앱이 사용하는 포트와 일치해야 함 | 생략 시 `port`와 같은 값 |
...
번뜩, 어제 미니큐브를 처음 띄울 때 봤던 메시지가 떠올랐습니다. 미니큐브는 컴퓨터 안에서 가상 환경을 한 겹 두르고 돌아갑니다. 노트북이 사는 동네와 미니큐브가 사는 동네가 서로 다른 네트워크라, 30080을 미니큐브 쪽에 열어 두었어도 노트북 쪽에서는 그 포트가 보이지 않습니다. 도커에서 `-p`로 호스트와 컨테이너 사이에 길을 뚫었던 것처럼, 미니큐브와 노트북 사이에도 별도의 길이 필요합니다.

다행히 미니큐브는 이 둘을 잇는 통로를 만드는 명령어를 따로 마련해 두고 있습니다.
...
URL이 한 줄 떴고 커서는 그대로 멈춰 있습니다. 이 명령어는 통로를 유지하기 위해 터미널을 점유한 채로 실행 중입니다. 발급된 주소를 그대로 브라우저에 붙여 넣자, 익숙한 NGINX 환영 페이지가 모니터에 가득 찼습니다.
...
새 URL로 접속하니 NGINX 페이지가 그대로 떴습니다. Pod IP가 전부 바뀌었어도 Service는 새 Pod에 이미 연결되어 있습니다.
...
*'안에서는 Service로 묶었는데, 밖에서 들어올 진입점은 아직 임시구나.'*

Service를 하나 더 만들면 진입점도 또 하나 늘어납니다. 외부에서 보면 진입점이 두 개, 세 개로 늘어난다는 뜻입니다. 그렇다고 임시 포트를 사람마다 알려 줄 수도 없습니다.

*'도커 때 NGINX가 그 자리에 있었지.'*

도커에서는 NGINX가 외부 요청을 받아 같은 도메인 아래 경로별로 뒤쪽 서버로 분기해 줬습니다. 쿠버네티스에서도 그 자리가 필요합니다.

Service YAML을 처음부터 끝까지 다시 훑어봐도 경로별 라우팅 항목은 보이지 않았습니다. Service는 라벨로 Pod를 묶을 뿐, URL 경로를 보고 분기하는 기능은 없습니다.
...
Service는 Pod 묶음마다 고정 진입점을 만듭니다. Ingress는 그렇게 만들어진 Service 여러 개를 외부 도메인 하나로 묶어 줍니다. 외부 요청이 들어오면 URL 경로를 보고 알맞은 Service로 갈라집니다.
