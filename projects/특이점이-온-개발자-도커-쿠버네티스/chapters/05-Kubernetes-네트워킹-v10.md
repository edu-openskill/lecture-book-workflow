...
Ingress는 라우팅 규칙을 적은 **Ingress 리소스(YAML)** 와 그 규칙을 읽고 외부 트래픽을 처리하는 **Ingress 컨트롤러** 두 가지로 분리됩니다. Ingress 리소스만 만들고 Ingress 컨트롤러가 떠 있지 않으면 라우팅이 동작하지 않으므로, 두 가지가 함께 떠 있어야 합니다.
...
<svg viewBox="0 0 760 280" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Ingress Controller(Pod)가 Ingress 리소스(YAML)의 규칙을 참조해 외부 요청을 Service로 분기">
  ...
  <text x="380" y="22" text-anchor="middle" font-size="13" font-weight="700" fill="#1f2937">Ingress Controller가 리소스의 규칙을 참조해 외부 요청을 Service로 분기합니다</text>
