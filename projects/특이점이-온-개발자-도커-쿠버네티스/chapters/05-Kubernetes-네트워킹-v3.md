...
다음 날 아침, 사무실 자리에 앉은 오픈이는 어제 성공적으로 테스트했던 네 개의 Pod를 떠올렸습니다. 자동 복구는 물론이고 새 버전으로의 교체까지 직접 확인하며 쿠버네티스의 위력을 실감했습니다.

하지만 기쁨도 잠시, 머릿속에는 실질적인 의문이 하나 스쳤습니다.

*'그런데... 이걸 실제로 어떻게 사용해야 하지?'*

Pod 네 대를 띄워두긴 했지만, 정작 어느 주소로 요청을 보내야 할지 막막했습니다. 도커 실습 때 Nginx로 만들었던 단일 진입점이 떠올랐습니다. Pod IP는 재시작될 때마다 수시로 바뀔 텐데, 그 앞단에서 변치 않고 자리를 지켜줄 고정 진입점이 절실했습니다.
...
문서를 훑던 오픈이의 눈에 **Service**라는 항목이 들어왔습니다. Pod의 상태나 위치와 상관없이 변하지 않는 고정 진입점 역할을 하는 것, 그것이 바로 Service였습니다.

마치 가맹점의 점장이나 직원이 누구인지와 관계없이 고객이 **브랜드 대표 번호** 하나로 언제든 주문할 수 있는 것과 같습니다. 이처럼 Service는 쿠버네티스 환경에서 Pod들의 변하지 않는 대표 창구 역할을 수행합니다.
...
본격적인 실습을 위해 우선 대상이 되는 Pod들을 준비하겠습니다.
...
오픈이는 `ex11` 폴더를 열어, 어제 사용했던 `deploy-ex02.yml`을 다시 실행해 Pod 네 개를 복구했습니다.
...
Service가 Pod를 찾는 메커니즘은 Deployment와 동일한 **라벨(Label) 매칭** 방식입니다. IP 주소가 아니라 라벨을 기준으로 연결하기 때문에, Pod가 새로 생성되어 IP가 바뀌더라도 라벨만 일치하면 Service는 요청을 정확히 전달할 수 있습니다.
...
조금 더 파헤쳐 보니, Service에는 누구에게 공개할 것인가에 따라 크게 세 가지 타입을 선택할 수 있었습니다.
...
<svg viewBox="0 0 760 300" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="LoadBalancer — 공인 IP가 각 노드의 kube-proxy를 거쳐 Pod로 분산되며, Service는 클러스터 레벨 논리 리소스로 모든 노드에 걸쳐 존재">
  ...
  <rect x="335" y="70" width="65" height="210" rx="8" fill="#fff4ed" stroke="#ff7849" stroke-width="2"/>
  <text x="367" y="175" text-anchor="middle" font-size="11" font-weight="700" fill="#7b341e" transform="rotate(-90 367 175)">Service (논리)</text>

  <line x1="290" y1="120" x2="423" y2="116" stroke="#475569" stroke-width="1.4" marker-end="url(#lb-p)"/>
  <line x1="290" y1="145" x2="423" y2="189" stroke="#475569" stroke-width="1.4" marker-end="url(#lb-p)"/>
  <line x1="290" y1="170" x2="423" y2="262" stroke="#475569" stroke-width="1.4" marker-end="url(#lb-p)"/>
...
타입은 정리됐지만, `port`, `targetPort`, `nodePort`가 한데 적힌 이유는 여전히 궁금했습니다. 사실 이 셋은 각자 주인이 다른 포트입니다.

* `nodePort`는 노드(서버)의 포트입니다.
* `port`는 Service 자체의 포트입니다.
* `targetPort`는 실제 Pod의 포트입니다.

외부에서 Pod까지 이어지는 전체 라우팅 경로를 Service가 한 곳에서 선언하기 위해 이 셋을 함께 정의하는 것입니다.
...
미니큐브는 컴퓨터 내부의 가상 환경(VM 또는 컨테이너)에서 돌아갑니다. 즉, 미니큐브라는 가상 세계와 우리 PC라는 현실 세계가 별도의 네트워크로 분리되어 있습니다. 그래서 도커 컨테이너를 포트포워딩으로 호스트와 연결했던 것처럼, 미니큐브 환경과 우리 PC를 잇는 별도의 통로가 필요하기 때문입니다.
:::

오픈이는 이 중 `minikube service --url` 명령어를 사용해 접속 주소를 받아오기로 했습니다. NodePort로 열어둔 서비스이니, 접근할 수 있는 주소만 하나 받아오면 브라우저에서 바로 확인할 수 있습니다.
...
확인을 마친 오픈이는 통로를 닫고 마지막 테스트를 진행했습니다. **'Pod가 죽어도 Service가 고정 진입점 역할을 유지하는가'** 를 확인하기 위해 실행 중인 Pod를 모두 삭제해 본 것입니다.
...
잠시 후 새로 생성된 주소로 접속해 보니, Pod들이 완전히 교체되었음에도 서비스는 변함없이 Nginx 페이지를 보여주었습니다. Service가 고정 진입점 역할을 충실히 수행하고 있음을 확인한 순간이었습니다.
...
도커 때 같은 문제를 Nginx로 풀었던 게 떠올랐습니다. 앞단에 Nginx 한 대를 두고 `/app1`은 1번 서버로, `/app2`는 2번 서버로 보냈습니다. 쿠버네티스에서도 같은 구조가 필요했지만, Service 설정에는 보이지 않았습니다.

*'그냥 Service에서 경로별로 라우팅을 하고 싶은데, 마땅한 설정이 없네.'*
...
이제 테스트를 위해 `/order`와 `/stores` 요청을 처리할 백엔드 Pod와 서비스를 준비하겠습니다. 둘은 같은 이미지를 쓰고 응답 문구만 다릅니다. 이미지는 `hashicorp/http-echo`로, 받은 요청에 `-text` 옵션의 문자열을 그대로 응답합니다.
...
두 서비스는 모두 ClusterIP 타입이라 클러스터 바깥에서는 직접 접근할 수 없습니다. 바깥 요청을 이 둘로 나눠 보낼 Ingress 규칙이 필요합니다.
...
`rules` 아래 두 개의 `path`가 있고, 각 경로가 서로 다른 Service를 가리킵니다. 규칙을 문서로 선언해 두면, 앞서 실행한 컨트롤러 Pod가 그 규칙을 읽고 실제 요청을 갈라 줍니다.
...
동일한 도메인이지만 뒤에 붙는 경로에 따라 서로 다른 서비스에 성공적으로 도달하는 것을 확인했습니다. Ingress 규칙대로 컨트롤러가 경로를 읽어 적절한 Service로 요청을 보낸 결과입니다.

겉으로 보이는 결과는 매끄러웠지만, 그 사이에서 누가 무엇을 하고 있는지는 아직 손에 잡히지 않았습니다.

## 5.3 브라우저에서 Pod까지의 경로

요청이 실제 Pod에 닿기까지, 쿠버네티스 내부에서는 어떤 일들이 벌어질까요? 오픈이는 이 복잡한 과정을 크게 3단계로 정리해 보았습니다.
...
<div class="svg-figure">
<svg viewBox="0 0 800 280" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="1단계 비유 — 외부 손님이 본사 대표번호로 전화, 자동 안내가 콜센터로 연결">
  <defs>
    <marker id="m1n-a" markerWidth="10" markerHeight="10" refX="8" refY="3" orient="auto"><path d="M0,0 L0,6 L8,3 z" fill="#ff7849"/></marker>
  </defs>
  <text x="400" y="22" text-anchor="middle" font-size="13" font-weight="700" fill="#1f2937">외부 손님이 대표번호로 전화 → 자동 안내 → 콜센터</text>
  <g transform="translate(40, 110)">
    <circle cx="20" cy="0" r="16" fill="#fff" stroke="#475569" stroke-width="1.6"/>
    <path d="M 0 22 Q 0 50 20 50 Q 40 50 40 22 L 40 90 L 0 90 Z" fill="#fff" stroke="#475569" stroke-width="1.6"/>
    <rect x="38" y="20" width="22" height="14" rx="2" fill="#1565c0" stroke="#0f3a78" stroke-width="1"/>
    <path d="M 41 22 Q 41 18 49 18 Q 57 18 57 22" fill="none" stroke="#fff" stroke-width="1.4"/>
  </g>
  <text x="60" y="220" text-anchor="middle" font-size="12" font-weight="700" fill="#0f172a">외부 손님</text>
  <line x1="115" y1="155" x2="170" y2="155" stroke="#ff7849" stroke-width="2.4" marker-end="url(#m1n-a)"/>
  <text x="142" y="145" text-anchor="middle" font-size="9" font-style="italic" fill="#7b341e">전화 걸기</text>
  <rect x="160" y="60" width="620" height="200" rx="10" fill="#fff" stroke="#7b341e" stroke-width="1.4" stroke-dasharray="6,4"/>
  <text x="470" y="78" text-anchor="middle" font-size="11" font-style="italic" fill="#7b341e">본사</text>
  <rect x="180" y="120" width="130" height="70" rx="6" fill="#fff4ed" stroke="#7b341e" stroke-width="1.8"/>
  <text x="245" y="142" text-anchor="middle" font-size="14" fill="#7b341e">☎</text>
  <text x="245" y="162" text-anchor="middle" font-size="10" font-weight="700" fill="#7b341e">본사 대표번호</text>
  <text x="245" y="180" text-anchor="middle" font-size="9" font-family="monospace" fill="#7b341e">02-000-0000</text>
  <line x1="310" y1="155" x2="365" y2="155" stroke="#ff7849" stroke-width="2.4" marker-end="url(#m1n-a)"/>
  <rect x="365" y="115" width="160" height="80" rx="6" fill="#fff4ed" stroke="#7b341e" stroke-width="1.8"/>
  <rect x="380" y="125" width="130" height="20" rx="2" fill="#7b341e"/>
  <text x="445" y="140" text-anchor="middle" font-size="9" font-weight="700" fill="#fff">ARS · 자동 안내</text>
  <text x="445" y="165" text-anchor="middle" font-size="9" fill="#7b341e">"콜센터로</text>
  <text x="445" y="180" text-anchor="middle" font-size="9" fill="#7b341e">연결합니다..."</text>
  <line x1="525" y1="155" x2="580" y2="155" stroke="#ff7849" stroke-width="2.4" marker-end="url(#m1n-a)"/>
  <rect x="580" y="105" width="180" height="100" rx="8" fill="#fff4ed" stroke="#ff7849" stroke-width="2"/>
  <rect x="595" y="115" width="150" height="20" rx="2" fill="#7b341e"/>
  <text x="670" y="130" text-anchor="middle" font-size="9" font-weight="700" fill="#fff">☎ 본사 콜센터 ☎</text>
  <text x="670" y="160" text-anchor="middle" font-size="10" fill="#7b341e">(전화가 콜센터에</text>
  <text x="670" y="175" text-anchor="middle" font-size="10" fill="#7b341e">연결됨)</text>
...
이 비유를 IT로 옮기면 네 가지가 짝지어집니다.

| 비유 | IT 용어 | 한 줄 설명 |
|:---:|:---|:---|
| 외부 손님 | **외부 호스트** | 브라우저를 통해 요청을 보내는 사용자 |
| 본사 대표번호 | **NodePort** | 클러스터로 진입하기 위한 공개 포트 |
| 본사 자동 안내 | **kube-proxy (1차)** | 노드 커널의 iptables 규칙으로 들어온 요청을 Ingress Controller Pod로 전달 |
| 본사 콜센터 | **Ingress Controller** | 다음 단계에서 URL을 읽고 판단을 내리는 Pod |

브라우저에서 `http://localhost/order`로 요청을 보냅니다. 요청은 노드에 뚫린 NodePort(클라우드라면 LoadBalancer)를 통해 클러스터 안으로 들어옵니다. 5.2.3에서 띄워 둔 `minikube tunnel`이 이 입구를 호스트와 이어 주는 통로입니다. 그 직후 같은 노드의 kube-proxy가 iptables 규칙으로 요청을 NodePort Service의 backend Pod인 Ingress Controller로 변환합니다.
...
<svg viewBox="0 0 800 250" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="2단계 비유 — 콜센터 상담원이 부서 명부에서 결정된 부서의 대표 내선을 조회">
  ...
  <text x="400" y="22" text-anchor="middle" font-size="13" font-weight="700" fill="#1f2937">콜센터 상담원이 부서 명부 조회 → 결정된 부서 대표 내선으로</text>
  <rect x="20" y="50" width="180" height="130" rx="8" fill="#fff" stroke="#cbd5e1" stroke-width="1.6"/>
  <rect x="35" y="60" width="150" height="20" rx="2" fill="#94a3b8"/>
  <text x="110" y="75" text-anchor="middle" font-size="9" font-weight="700" fill="#fff">☎ 본사 콜센터 ☎</text>
  <g transform="translate(90, 110)">
    <circle cx="20" cy="0" r="14" fill="#fff" stroke="#94a3b8" stroke-width="1.4"/>
    <path d="M 6 -6 Q 6 -22 20 -22 Q 34 -22 34 -6" fill="none" stroke="#1565c0" stroke-width="2"/>
    <ellipse cx="6" cy="-4" rx="3" ry="4" fill="#1565c0"/>
    <ellipse cx="34" cy="-4" rx="3" ry="4" fill="#1565c0"/>
    <path d="M -3 22 Q -3 50 20 50 Q 43 50 43 22 L 43 60 L -3 60 Z" fill="#fff" stroke="#94a3b8" stroke-width="1.4"/>
  </g>
  <text x="110" y="170" text-anchor="middle" font-size="9" fill="#94a3b8">상담원 (용건 들음)</text>
  <line x1="205" y1="100" x2="265" y2="100" stroke="#475569" stroke-width="1.4" stroke-dasharray="4,3" marker-end="url(#m2n-s)"/>
  <text x="235" y="90" text-anchor="middle" font-size="9" font-style="italic" fill="#475569">"주문 부서?"</text>
  <line x1="265" y1="130" x2="205" y2="130" stroke="#ff7849" stroke-width="1.6" stroke-dasharray="4,3" marker-end="url(#m2n-a)"/>
  <text x="235" y="145" text-anchor="middle" font-size="9" font-style="italic" fill="#7b341e">"내선 1234"</text>
  <rect x="265" y="50" width="220" height="140" rx="4" fill="#fff" stroke="#7b341e" stroke-width="1.6"/>
  <rect x="265" y="50" width="14" height="140" fill="#7b341e"/>
  <text x="272" y="122" text-anchor="middle" font-size="8" font-weight="700" fill="#fff" transform="rotate(-90 272 122)">DIRECTORY</text>
  <text x="382" y="70" text-anchor="middle" font-size="11" font-weight="700" fill="#7b341e">부서 명부</text>
  <line x1="290" y1="80" x2="475" y2="80" stroke="#94a3b8" stroke-width="0.6"/>
  <rect x="285" y="88" width="195" height="22" rx="2" fill="#fff4ed" stroke="#ff7849" stroke-width="1.4"/>
  <text x="295" y="104" font-size="10" font-family="monospace" font-weight="700" fill="#7b341e">▶ 주문 부서</text>
  <text x="470" y="104" text-anchor="end" font-size="10" font-family="monospace" font-weight="700" fill="#7b341e">내선 1234</text>
  <text x="295" y="129" font-size="9" font-family="monospace" fill="#94a3b8">결제 부서</text>
  <text x="470" y="129" text-anchor="end" font-size="9" font-family="monospace" fill="#94a3b8">내선 5678</text>
  <text x="295" y="151" font-size="9" font-family="monospace" fill="#94a3b8">매장 부서</text>
  <text x="470" y="151" text-anchor="end" font-size="9" font-family="monospace" fill="#94a3b8">내선 9012</text>
  <text x="295" y="173" font-size="9" font-family="monospace" fill="#94a3b8">CS 부서</text>
  <text x="470" y="173" text-anchor="end" font-size="9" font-family="monospace" fill="#94a3b8">내선 3456</text>
  <line x1="490" y1="99" x2="555" y2="99" stroke="#ff7849" stroke-width="2.4" marker-end="url(#m2n-a)"/>
  <rect x="555" y="78" width="225" height="42" rx="21" fill="#fff4ed" stroke="#ff7849" stroke-width="2.4"/>
  <text x="667" y="104" text-anchor="middle" font-size="12" font-weight="700" fill="#7b341e">주문 부서 대표 내선 (1234)</text>
  <rect x="565" y="130" width="205" height="28" rx="14" fill="#f8fafc" stroke="#cbd5e1" stroke-width="1.2"/>
  <text x="667" y="148" text-anchor="middle" font-size="10" font-weight="600" fill="#94a3b8">반품 부서 대표 내선 (2345)</text>
  <rect x="565" y="165" width="205" height="28" rx="14" fill="#f8fafc" stroke="#cbd5e1" stroke-width="1.2"/>
  <text x="667" y="183" text-anchor="middle" font-size="10" font-weight="600" fill="#94a3b8">결제 부서 대표 내선 (5678)</text>
  <rect x="565" y="200" width="205" height="28" rx="14" fill="#f8fafc" stroke="#cbd5e1" stroke-width="1.2"/>
...
| 비유 | IT 용어 | 한 줄 설명 |
|:---:|:---|:---|
| 콜센터 상담원 | **Ingress Controller** | URL 경로와 도메인 헤더로 어느 백엔드 Service에 보낼지 결정 |
| 부서 명부 (이름 → 내선) | **클러스터 DNS (CoreDNS)** | Service 이름을 ClusterIP로 풀이 |
...
`nginx-ingress` 같은 실제 컨트롤러는 DNS 조회 대신 API Server를 watch해서 Pod IP 목록을 직접 들고 있다가 곧바로 보내는 경우가 많습니다. DNS 조회는 일반 Pod끼리 Service 이름으로 부를 때 더 자주 쓰입니다.
:::

<div class="svg-figure">
<svg viewBox="0 0 800 260" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="2단계 IT — Ingress Controller가 DNS 조회로 결정된 Service의 ClusterIP로 전송">
  <defs>
  ...
  <text x="400" y="22" text-anchor="middle" font-size="13" font-weight="700" fill="#1f2937">Ingress Controller → DNS 조회 → 결정된 Service ClusterIP로 전송</text>
  <rect x="20" y="55" width="200" height="120" rx="8" fill="#fff" stroke="#cbd5e1" stroke-width="1.6"/>
  <text x="120" y="95" text-anchor="middle" font-size="13" font-weight="700" fill="#0f172a">Ingress Controller</text>
  <text x="120" y="115" text-anchor="middle" font-size="11" fill="#6b7280">Pod</text>
  <text x="120" y="143" text-anchor="middle" font-size="10" fill="#7b341e">URL·Host 읽음</text>
  <line x1="225" y1="100" x2="285" y2="100" stroke="#475569" stroke-width="1.4" stroke-dasharray="4,3" marker-end="url(#fl2n-s)"/>
  <text x="255" y="90" text-anchor="middle" font-size="9" font-style="italic" font-family="monospace" fill="#475569">"order-service?"</text>
  <line x1="285" y1="135" x2="225" y2="135" stroke="#ff7849" stroke-width="1.4" stroke-dasharray="4,3" marker-end="url(#fl2n-a)"/>
  <text x="255" y="150" text-anchor="middle" font-size="9" font-style="italic" font-family="monospace" fill="#7b341e">"10.96.0.20"</text>
  <rect x="285" y="55" width="220" height="120" rx="8" fill="#fff4ed" stroke="#ff7849" stroke-width="2"/>
  <text x="395" y="95" text-anchor="middle" font-size="13" font-weight="700" fill="#7b341e">클러스터 DNS</text>
  <text x="395" y="115" text-anchor="middle" font-size="10" fill="#7b341e">(CoreDNS)</text>
  <text x="395" y="143" text-anchor="middle" font-size="10" fill="#7b341e">이름 → ClusterIP</text>
  <line x1="510" y1="99" x2="575" y2="99" stroke="#ff7849" stroke-width="2.4" marker-end="url(#fl2n-a)"/>
...
*그림 5-23. Ingress Controller가 URL을 읽어 Service를 선택, DNS가 이름을 ClusterIP로 변환*

### 5.3.3 3단계 직원 응답 — 가용 직원에게 자동 분배
...
<div class="svg-figure">
<svg viewBox="0 0 800 340" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="3단계 비유 — 자동 전화 분배가 응대 직원 한 명에게 연결, 부서 매니저가 응대 직원 명단 갱신">
  <defs>
    <marker id="m3n-a" markerWidth="10" markerHeight="10" refX="8" refY="3" orient="auto"><path d="M0,0 L0,6 L8,3 z" fill="#ff7849"/></marker>
    <marker id="m3n-s" markerWidth="10" markerHeight="10" refX="8" refY="3" orient="auto"><path d="M0,0 L0,6 L8,3 z" fill="#475569"/></marker>
    <marker id="m3n-bi" markerWidth="10" markerHeight="10" refX="5" refY="3" orient="auto-start-reverse"><path d="M0,0 L0,6 L8,3 z" fill="#7b341e"/></marker>
  </defs>
  <text x="400" y="22" text-anchor="middle" font-size="13" font-weight="700" fill="#1f2937">부서 대표 내선 → 자동 전화 분배 → 응대 직원 응답</text>
  <rect x="20" y="80" width="160" height="40" rx="20" fill="#fff" stroke="#cbd5e1" stroke-width="1.6"/>
  <g transform="translate(45, 100)">
    <rect x="-8" y="-9" width="18" height="18" rx="2" fill="#1565c0"/>
    <path d="M -5 -7 Q -5 -11 1 -11 Q 7 -11 7 -7" fill="none" stroke="#fff" stroke-width="1.4"/>
    <circle cx="-3" cy="-2" r="0.8" fill="#fff"/>
    <circle cx="1" cy="-2" r="0.8" fill="#fff"/>
    <circle cx="5" cy="-2" r="0.8" fill="#fff"/>
  </g>
  <text x="115" y="105" text-anchor="middle" font-size="11" font-weight="600" fill="#94a3b8">부서 대표 내선</text>
  <line x1="180" y1="100" x2="240" y2="100" stroke="#ff7849" stroke-width="2.4" marker-end="url(#m3n-a)"/>
  <rect x="240" y="60" width="540" height="180" rx="10" fill="#fff" stroke="#7b341e" stroke-width="1.8"/>
  <rect x="270" y="46" width="120" height="24" rx="3" fill="#7b341e"/>
  <text x="330" y="63" text-anchor="middle" font-size="10" font-weight="700" fill="#fff">━ 주문 부서 ━</text>
  <rect x="260" y="90" width="180" height="80" rx="6" fill="#fff4ed" stroke="#ff7849" stroke-width="2.4"/>
  <g transform="translate(285, 130)">
    <rect x="-10" y="-12" width="26" height="22" rx="3" fill="#1565c0"/>
    <rect x="-7" y="-9" width="20" height="6" rx="1" fill="#fff"/>
    ...
    <line x1="18" y1="-5" x2="26" y2="-12" stroke="#7b341e" stroke-width="1.2"/>
    <line x1="18" y1="0" x2="28" y2="0" stroke="#7b341e" stroke-width="1.2"/>
    <line x1="18" y1="5" x2="26" y2="12" stroke="#7b341e" stroke-width="1.2"/>
  </g>
  <text x="380" y="115" text-anchor="middle" font-size="12" font-weight="700" fill="#7b341e">자동 전화 분배</text>
  <text x="380" y="138" text-anchor="middle" font-size="9" fill="#7b341e">가용 인원 중 자동 분배</text>
  <text x="380" y="153" text-anchor="middle" font-size="9" fill="#7b341e">→ 한 명에게 연결</text>
  <rect x="495" y="85" width="270" height="105" rx="8" fill="#fff4ed" stroke="#ff7849" stroke-width="2"/>
  <text x="630" y="103" text-anchor="middle" font-size="11" font-weight="700" fill="#7b341e">응대 직원 그룹</text>
  <line x1="440" y1="138" x2="544" y2="138" stroke="#ff7849" stroke-width="2" marker-end="url(#m3n-a)"/>
  <g transform="translate(560, 138)">
    <circle cx="0" cy="0" r="11" fill="#fff" stroke="#7b341e" stroke-width="1.4"/>
    <path d="M -11 0 Q -11 -13 0 -13 Q 11 -13 11 0" fill="none" stroke="#1565c0" stroke-width="2"/>
    <ellipse cx="-11" cy="2" rx="2.5" ry="3.5" fill="#1565c0"/>
    <ellipse cx="11" cy="2" rx="2.5" ry="3.5" fill="#1565c0"/>
    <path d="M 11 4 Q 16 8 14 12" fill="none" stroke="#1565c0" stroke-width="1.4"/>
    <circle cx="14" cy="13" r="1.6" fill="#1565c0"/>
    <path d="M -3 18 Q -3 32 0 32 Q 3 32 3 18 L 11 32 L -11 32 Z" fill="#fff" stroke="#7b341e" stroke-width="1.4"/>
  </g>
  <text x="560" y="180" text-anchor="middle" font-size="10" font-weight="700" fill="#7b341e">직원 1</text>
  ...
  <text x="700" y="180" text-anchor="middle" font-size="10" font-weight="700" fill="#7b341e">직원 2</text>
  <rect x="20" y="250" width="280" height="80" rx="8" fill="#fff" stroke="#7b341e" stroke-width="2" stroke-dasharray="5,3"/>
  <g transform="translate(55, 280)">
    <circle cx="0" cy="0" r="12" fill="#fff" stroke="#475569" stroke-width="1.4"/>
    <path d="M -3 16 Q -3 32 0 32 Q 3 32 3 16 L 12 32 L -12 32 Z" fill="#fff" stroke="#475569" stroke-width="1.4"/>
    <rect x="14" y="-6" width="20" height="26" rx="2" fill="#fff" stroke="#7b341e" stroke-width="1.2"/>
    <rect x="20" y="-9" width="8" height="5" rx="1" fill="#7b341e"/>
    <line x1="17" y1="3" x2="31" y2="3" stroke="#0f6f3f" stroke-width="0.8"/>
    <line x1="17" y1="8" x2="31" y2="8" stroke="#dc2626" stroke-width="0.8"/>
    <line x1="17" y1="13" x2="31" y2="13" stroke="#0f6f3f" stroke-width="0.8"/>
  </g>
  <text x="180" y="278" text-anchor="middle" font-size="13" font-weight="700" fill="#7b341e">부서 매니저</text>
  <text x="180" y="298" text-anchor="middle" font-size="10" fill="#7b341e">팀 근태 관리 (출퇴근·휴가)</text>
  <text x="180" y="313" text-anchor="middle" font-size="10" fill="#7b341e">→ 가용 인력 명단 관리</text>
  <line x1="305" y1="248" x2="345" y2="172" stroke="#7b341e" stroke-width="1.4" stroke-dasharray="5,3" marker-start="url(#m3n-bi)" marker-end="url(#m3n-bi)"/>
  <text x="280" y="215" font-size="9" font-style="italic" fill="#7b341e">watch (구독·알림)</text>
</svg>
</div>

*그림 5-24. 매니저가 명단을 갱신하면 자동 분배가 살아있는 직원에게 전화 연결*

이 비유를 IT로 옮기면 네 가지가 짝지어집니다.

| 비유 | IT 용어 | 한 줄 설명 |
|:---:|:---|:---|
| 자동 전화 분배 | **kube-proxy (2차)** | ClusterIP를 살아있는 Pod IP로 변환하는 L4 로드밸런서 |
| 응대 직원 명단 | **EndpointSlice** | 현재 살아있는 Pod들의 IP 목록 |
| 부서 매니저 | **Endpoint Controller** | Pod 변동을 감지해 위 명단을 즉시 갱신하는 컨트롤 플레인 컴포넌트 |
| 응대 직원 | **Pod** | 실제 비즈니스 로직을 처리하는 앱 |

ClusterIP로 향한 요청은 같은 노드의 kube-proxy(2차)가 가로채서, EndpointSlice의 살아있는 Pod 중 하나의 IP로 바꿔 보냅니다. 요청이 백엔드 Pod에 닿으면 애플리케이션이 비즈니스 로직을 실행해 응답을 돌려보냅니다.

이 모든 게 가능한 건 Endpoint Controller가 뒤에서 받쳐 주기 때문입니다. Service의 selector에 매칭되는 Pod가 새로 뜨거나 죽으면 즉시 감지해 Pod IP 목록을 갱신합니다. 그래야 kube-proxy(2차)가 항상 살아있는 Pod에만 트래픽을 보낼 수 있습니다. 갱신이 늦어지면 죽은 Pod IP로 트래픽이 흘러가 요청이 실패할 수 있습니다.
...
:::note
**Endpoint Controller와 kube-proxy의 통신 — API Server가 매개**

엄밀히 말하면 Endpoint Controller가 kube-proxy에게 직접 명단을 주는 게 아닙니다. EC가 EndpointSlice를 갱신하면 그 결과는 API Server에 저장되고, 각 노드의 kube-proxy가 API Server를 watch(구독)해서 변경 알림을 받습니다. 즉 두 컴포넌트 사이에 API Server가 중간 매개체 역할을 합니다. 비유 그림에서는 이 매개를 생략하고 매니저와 분배기 사이의 양방향 화살표로 단순화했습니다.
:::
...
| 단계 | 역할 주체 | 하는 일 | 판단 기준 |
|:----:|:---------|:--------|:----------|
| **1. 진입** | NodePort + kube-proxy | 입구 통과 및 인그레스로 전달 | IP와 포트 |
| **2. 결정** | Ingress Controller + DNS | URL 분석 및 서비스 결정 | URL 경로, Host 헤더 |
| **3. 응답** | kube-proxy + Pod | 실제 Pod IP로 변환 및 처리 | 실시간 IP 목록 |

같은 "전달"처럼 보이지만 단계별로 보는 정보의 깊이가 다릅니다. NodePort와 kube-proxy는 봉투 겉면만 보고 옮기고, Ingress Controller는 봉투를 열어 안의 내용까지 읽고 어디로 보낼지 결정합니다.

따로 보면 복잡해 보이던 프로그램들이 한 줄로 늘어놓이니 그제야 머릿속에서 하나의 흐름으로 묶였습니다.

*'본사 콜센터가 부서 명부를 펼치고 부서 대표 내선을 부른다. 그 뒤에서 부서 매니저가 직원 상태를 챙긴다. 각자 맡은 일이 단순하니 관리하기도 편하겠다.'*

## 이것만은 기억하자

- **Service는 Pod의 변하지 않는 직통 전화번호입니다.** Pod는 소모품이라 IP가 수시로 바뀌지만, Service는 변하지 않는 주소를 제공하며 트래픽을 골고루 분산합니다.
- **Service 뒤에는 세 조력자가 있습니다.** 이름을 풀이하는 **DNS**, 실시간 Pod 명단을 관리하는 **Endpoint Controller**, 그리고 실제 길을 닦는 **kube-proxy**가 협력하여 패킷을 배달합니다.
