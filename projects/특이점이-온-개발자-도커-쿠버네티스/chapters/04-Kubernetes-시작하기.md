...
그 질문에 아무 대답도 할 수 없었습니다. 사흘 전 새벽, **"[ALERT] 백엔드 서버 응답 없음"** 알람에 잠에서 깬 일이 떠올랐습니다. 노트북을 열어 보니 백엔드 컨테이너가 메모리 부족으로 죽어 있었고, docker compose up으로 급히 살리는 사이 사용자 수십 명이 이미 빠져나간 뒤였습니다. 그날 노트북을 덮으며 스스로에게 던졌던 *'만약 새벽 두 시에 컨테이너가 죽으면 누가 살려주지?'* 그 질문이 팀장의 입에서 그대로 돌아왔습니다. 컴포즈는 "지금 이 순간 띄워라"는 도구였지, 죽은 컨테이너를 실시간으로 감시하며 살려주는 기능은 없었으니까요. 팀장의 질문은 계속 이어졌습니다.
...
**상황 1. 새벽 3시, 컨테이너가 죽었습니다**

백엔드 컨테이너가 메모리 부족으로 종료됐습니다. 사용자는 "서버 오류" 화면만 보게 됩니다. 컴포즈 환경에서는 개발자가 알림을 확인하고 직접 docker compose up을 다시 실행해야 합니다. 그 사이 서비스는 멈춰 있습니다.
...
세 상황 모두 컴포즈로는 막을 수 없는 순간이었습니다. 답을 찾다 보니 도커 컴포즈와 쿠버네티스가 컨테이너를 대하는 태도부터 다르다는 사실이 보였습니다.
...
또 한 가지 차이가 있습니다. 도커가 한 대의 호스트(노트북·서버 한 대) 안에서 컨테이너를 관리하는 도구라면, 쿠버네티스는 여러 대의 호스트에 걸쳐 컨테이너를 관리하는 도구입니다. 한 대가 죽어도 다른 대가 살아남아 서비스가 이어집니다.
...
이름이 조금 낯설어도 괜찮습니다. **"본사는 계획(선언)을 받고, 상가 건물은 그 계획대로 가맹점을 운영한다"** 는 흐름만 챙기면 됩니다. 본사 안에서 어떤 부서들이 일을 나누는지 한 단계 더 들여다보겠습니다.

본사 안에는 네 개의 핵심 부서가 있습니다. 안내데스크가 외부 명령을 받으면 장부 보관함에 상태를 기록·조회하고, 입지 선정 부서와 운영 감시 부서가 협력해 새 가맹점을 어느 상가 건물에 어떻게 둘지 결정합니다.
...
<svg viewBox="0 0 760 320" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="컨트롤 플레인 내부 구성: Kube API Server, etcd, Scheduler, Controller Manager">
  ...
  <text x="190" y="260" text-anchor="middle" font-size="9" fill="#7b341e">입지 선정 (어느 노드?)</text>
...
| **Scheduler** | 매장 입지 선정 부서 | 새 Pod를 어느 워커 노드에 배치할지 결정합니다. |
| **Controller Manager** | 운영 감시 부서 | 원하는 상태와 실제 상태를 끊임없이 비교하다가 차이가 생기면 복구 작업을 지시합니다. |

워커 노드 쪽에는 본사 지시를 받아 컨테이너를 띄우는 **kubelet**(현장 슈퍼바이저)과 손님 동선이 끊기지 않게 길을 잡아 주는 **kube-proxy**(동선 안내 시스템)가 일합니다. kube-proxy는 챕터 2에서 본 iptables 포트포워딩과 같은 도구로, Docker가 호스트 한 대에 적용한 것을 클러스터 전체로 확장한 형태입니다. 자세한 동작은 5장에서 다룹니다.
...
*그림 4-6. Minikube는 단일 노드 안에 본사와 상가 건물 기능이 함께 들어간 구조*
...
> 실습 코드: https://github.com/metacoding-10-linux-docker/docker/tree/master/ex09
...
> 실습 코드: https://github.com/metacoding-10-linux-docker/docker/tree/master/ex10
