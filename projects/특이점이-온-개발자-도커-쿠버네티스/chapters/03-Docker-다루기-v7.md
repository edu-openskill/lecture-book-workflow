...
화요일 오전 열 시. 회의실에 사람이 모였습니다. 팀장이 화이트보드 앞에서 새 프로젝트의 윤곽을 잡기 시작했습니다. 사내에서 쓸 작은 통합 사이트 하나를 새로 만든다는 이야기였습니다. 회원 정보가 들어가야 하니 DB가 한 자리, 화면을 그릴 프론트엔드가 한 자리, 그 사이를 받쳐 줄 백엔드가 한 자리. 화이트보드 위에 네모 세 개가 나란히 그려졌습니다.
...
컨테이너를 삭제하더라도 이미지는 그대로 남아 있습니다. 같은 환경이 필요할 때마다 언제든 다시 띄울 수 있습니다. 지난주에 commit 명령어를 일일이 쳐서 만들었던 그 자리를 이제 Dockerfile이 채웁니다.
...
DB까지 컨테이너로 잡아 두고 나서 터미널 히스토리를 위로 올려 봤습니다. 지난 며칠 사이에 빌드와 실행 명령이 꽤 길게 늘어서 있었습니다. 같은 패턴이 거듭 나오고 있었습니다. `docker build`, 그 다음에 `docker run`, 같은 네트워크에 묶어 주는 옵션, 포트 매핑. 컨테이너가 한 대씩 늘어날 때마다 이 패턴이 한 묶음씩 더 붙었습니다.
...
여기까지 컨테이너 한 대씩 띄우는 법, 그리고 Compose로 묶는 법을 따로따로 손에 익혔습니다. 이제 그 조각을 모아 회의실에서 받았던 그 통합 사이트의 한 세트를 만들어 볼 차례였습니다. 프론트엔드 자리에 NGINX, 백엔드 자리에 Spring Boot, DB 자리에 MySQL. 목표는 단순했습니다. `docker compose up` 한 줄로 셋이 같이 올라가는 모습을 만드는 일이었습니다.
