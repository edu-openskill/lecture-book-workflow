...
**2. 판단 — 컨트롤 플레인이 명령을 어떻게 처리할지 정한다**
...
다만 Minikube는 노드 하나로 동작합니다. 이 노드가 컨트롤 플레인 역할과 워커 노드 역할을 함께 맡습니다.
...
이렇게 구조가 단순해 리소스도 적게 먹습니다. Minikube에서 작성한 설정 파일은 실제 운영 환경에서도 그대로 사용할 수 있습니다.
...
앞의 준비하기를 참고해 Minikube를 먼저 설치합니다. 그 다음 아래 한 줄로 노트북 안에 클러스터를 시작합니다.
...
선언형은 **원하는 상태**를 YAML 파일로 작성하여 클러스터에 반영하는 방식입니다. 만드는 과정을 명령어로 일일이 지시하는 대신, 쿠버네티스가 YAML을 읽어 원하는 상태로 리소스를 생성합니다.
...
*그림 4-15. Pod 삭제 후 목록을 조회하면 hello-pod2가 사라져 있습니다*
...
Deployment는 "**Pod를 몇 개 유지할지, 문제가 생기면 어떻게 교체할지**"를 적어 두는 **매뉴얼**과 같습니다. 선언한 상태와 실제 상태를 비교해, 차이가 생기면 Pod를 다시 만들어 원래대로 되돌립니다.
...
Deployment가 관리할 Pod를 생성할 때, Pod의 **labels** 영역에 **app: nginx** 라는 이름표를 붙입니다. 그리고 Deployment의 **selector** 에 똑같이 **app: nginx** 라고 지정해 두면, 쿠버네티스는 이 이름표를 가진 Pod들만 식별하여 관리합니다.
...
이제 Deployment가 정말로 지침을 지키는지 확인하기 위해, 현재 실행 중인 Pod를 전부 지웁니다.
...
hello-pod1은 그대로 사라졌지만 Deployment가 만든 Pod는 잠깐 사라졌다가 **새 이름으로 다시 실행되었습니다**.
...
운영 환경에서는 부하를 분산하기 위해 Pod를 여러 개 실행합니다. 이때 Pod의 개수를 일정하게 유지하는 리소스가 바로 **ReplicaSet(레플리카셋)** 입니다.

에어컨이 설정된 온도를 자동으로 유지하는 것처럼, ReplicaSet은 사용자가 선언한 개수와 실제 작동 중인 Pod의 개수를 지속적으로 비교합니다. 만약 Pod가 모자라면 즉시 새 Pod를 생성해 개수를 조절합니다.
...
ReplicaSet은 쿠버네티스에 별도로 존재하는 리소스이지만, **롤링 업데이트나 롤백 같은 배포 기능이 없습니다**. 그래서 실무에서는 ReplicaSet을 직접 사용하지 않고, **Deployment의 replicas 속성**을 통해 Pod의 개수까지 함께 관리합니다.
