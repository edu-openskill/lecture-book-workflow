...
도커가 컨테이너 단위로 관리한다면, 쿠버네티스는 **하나 이상의 컨테이너를 한데 묶은 Pod(파드)** 단위로 관리합니다.
...
에어컨이 설정된 온도를 자동으로 유지하는 것처럼, ReplicaSet은 사용자가 선언한 개수와 실제 작동 중인 Pod의 개수를 지속적으로 비교합니다. 만약 Pod가 모자라면 즉시 새 Pod를 생성해 개수를 맞춥니다.
...
ReplicaSet은 쿠버네티스에 별도로 존재하는 리소스이지만, **롤링 업데이트나 롤백 같은 배포 기능이 없습니다**. 그래서 실무에서는 ReplicaSet을 직접 만들지 않고 Deployment를 사용합니다. **Deployment가 내부적으로 ReplicaSet을 자동으로 만들어** Pod 개수를 유지하고, 그 위에 롤링 업데이트·롤백을 더합니다.
...
운영 중에는 서비스를 멈추지 않고 새 버전으로 교체하는 작업이 자주 일어납니다. 하지만 여러 Pod를 사람이 일일이 수동으로 내리고 올리는 방식은 비효율적일 뿐만 아니라 실수의 위험도 따릅니다. 쿠버네티스는 이 과정을 자동화하는 **롤링 업데이트(Rolling Update)** 를 제공합니다.
...
**maxSurge를 줄이면** 한 번에 띄울 새 Pod 수가 제한되어 천천히 교체됩니다. **maxUnavailable을 늘리면** 기존 Pod를 더 줄이며 빠르게 교체됩니다. 이렇게 두 값을 조정해 원하는 배포 방식을 설정합니다.
