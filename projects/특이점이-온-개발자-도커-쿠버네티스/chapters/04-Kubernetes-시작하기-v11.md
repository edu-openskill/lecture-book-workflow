...
Deployment는 "**Pod를 몇 개 유지할지, 문제가 생기면 어떻게 교체할지**"를 적어 두는 **매뉴얼**과 같습니다. 선언한 상태와 실제 상태를 비교해, 차이가 생기면 Pod를 다시 만들어 원하는 상태로 되돌립니다.
...
hello-pod1은 그대로 사라졌지만 Deployment가 만든 Pod는 삭제된 후 **새 이름으로 다시 실행되었습니다**.
...
운영 중인 서비스는 사용자가 중단 없이 이용하도록, 서비스를 유지하며 새 버전으로 교체하는 작업이 빈번하게 일어납니다. 하지만 여러 Pod를 사람이 일일이 수동으로 내리고 올리는 방식은 비효율적일 뿐만 아니라 실수의 위험도 따릅니다. 쿠버네티스는 이 과정을 자동화하는 **롤링 업데이트(Rolling Update)** 를 제공합니다.
...
예를 들어 replicas: 4, maxSurge: 4, maxUnavailable: 0이라면, 기존 4개는 그대로 둔 상태에서 새 버전 4개를 한꺼번에 더 만듭니다. 새 Pod가 Ready 상태가 되는 대로 기존 Pod를 차례로 종료시킵니다.
...
*그림 4-22. 롤링 업데이트 진행 - 새 Pod 4개를 띄운 뒤 기존 Pod 4개를 차례로 내립니다*

**maxSurge를 줄이면** 한 번에 띄울 새 Pod 수를 제한해 천천히 교체하고, **maxUnavailable을 늘리면** 기존 Pod를 줄이며 빠르게 교체하는 등 두 값을 조정해 원하는 배포 방식을 설정할 수 있습니다.
