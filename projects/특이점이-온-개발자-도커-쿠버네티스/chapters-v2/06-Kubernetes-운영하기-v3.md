...
오픈이는 백엔드 프로젝트의 `application.yml`을 열었습니다. DB 주소와 비밀번호가 환경별 프로필로 나뉘어 있었지만, 결국 같은 이미지 안에 같이 들어가는 값이었습니다. 비밀번호 한 줄을 바꾸자 `gradlew build`를 다시 돌려야 했고, 이미지를 다시 찍어 Docker Hub에 올려야 했고, Deployment의 이미지 태그를 바꿔 새로 배포해야 했습니다.
...
Secret에는 짚고 가야 할 부분이 하나 있습니다. Secret 값은 **Base64로 인코딩**되어 저장되는데, Base64는 **암호화가 아니라 단순 인코딩**입니다. 누군가 `kubectl get secret -o yaml`로 값을 뽑으면 디코딩해서 원문을 볼 수 있다는 뜻입니다. Secret이라는 이름만 보고 "여기에 넣으면 알아서 안전해지겠지" 하고 넘겨짚으면 안 됩니다.

실제 운영 환경에서 Secret의 보안을 제대로 챙기려면 아래 세 가지 중 하나 이상이 필요합니다.

> **참고: Secret 값의 실제 보호 수단**
> 1. **RBAC(Role-Based Access Control)**: 누가 Secret 리소스를 조회할 수 있는지 권한을 제한. 쿠버네티스 기본 기능으로 제공.
...
>
> 이 세 가지는 이 책에서 직접 실습하지 않으며, 운영 경험이 쌓인 뒤 찾아볼 개념들입니다. 지금은 **"Base64는 진짜 보안이 아니다"** 는 사실 하나만 기억해 두면 됩니다.
...
`upstream backend`의 `server backend-service:8080`이 핵심입니다. `backend-service`라는 이름은 곧 뒤에서 만들 Backend Service의 이름과 정확히 같아야 합니다. Nginx가 시작할 때 이 이름을 CoreDNS로 조회해 ClusterIP로 바꿉니다. 덕분에 Nginx는 바뀌는 Pod IP를 신경 쓰지 않고 고정된 Service 이름만 참조하면 됩니다.
...
`backend-configmap`에는 DB 주소와 Redis 주소 같은 일반 설정이 들어가고, `backend-secret`에는 DB 계정과 비밀번호가 들어갑니다. Spring Boot는 그 환경 변수를 `application.yml`의 placeholder로 자동으로 끌어다 씁니다. 6.1에서 연습한 구조 그대로입니다. 로컬에서 `application-prod.yml`을 따로 두고 환경마다 `-Dspring.profiles.active=prod`로 갈아끼우던 방식이, K8s에서는 **Secret/ConfigMap을 환경마다 다르게 적용**하는 방식으로 올라온 셈입니다.
...
리소스 파일을 다 그려 놓고, 실제로 클러스터에 적용하는 차례입니다. 오픈이는 터미널을 네 칸으로 쪼개 놓고 키보드에 손을 올렸습니다. 이번 절의 흐름은 다음과 같이 이어집니다.

| 단계 | 작업 | 대기 시간 |
|------|------|----------|
| 1 | Ingress Controller 활성화 | 약 30초 |
| 2 | 이미지 네 개 빌드 | 약 5~10분 (이미지당 편차) |
| 3 | namespace + 모든 리소스 일괄 생성 | 약 30초 |
| 4 | Pod Running + 로그로 기동 확인 | 약 4~7분 (backend 빌드 포함) |
| 5 | Ingress ADDRESS 채워짐 + tunnel | 약 1~2분 |
...
Pod가 전부 `Running` 상태가 될 때까지는 잠깐 기다려야 합니다. 특히 backend Pod는 `git clone`(약 1분)과 Gradle 빌드(약 3~5분), Spring 기동(약 1분)을 내부에서 차례로 돌립니다. 로그에서 `Tomcat started on port 8080`이 보일 때까지는 준비 중이라는 뜻입니다. 이 시간 동안 오픈이는 `Running`이 뜨는지 `get pod`를 몇 번 새로고침하며 지켜봤습니다.

Pod의 `STATUS` 칼럼에는 네 가지가 자주 등장합니다. `Running`은 떠 있는 상태, `Pending`은 뜨려고 시도 중인 상태, `CrashLoopBackOff`는 떴다가 죽기를 반복하는 상태, `ImagePullBackOff`는 이미지를 가져오지 못하는 상태입니다. 이 네 단어의 의미가 머릿속에 있어야 6.4의 디버깅이 수월해집니다.
...
Ingress ADDRESS가 채워지기까지 **1~2분** 이 걸립니다. `<none>`이나 빈 값이 보인다면 조금 더 기다려야 합니다. 그동안 `minikube tunnel`을 켜 둡니다. Docker Desktop 드라이버를 쓰는 환경에서는 로컬 PC에서 미니큐브 내부 IP로 직접 접근할 수 없어서 터널이 필요합니다.

> `minikube tunnel`은 포그라운드로 도는 명령입니다. 이 터미널을 그대로 두고 새 창을 열어 다음 명령을 쳐야 합니다. 터널이 꺼지면 접속도 끊깁니다.
...
그 전에 한 가지 용어를 정리하고 갑니다. Pod의 `STATUS` 칼럼에는 `Running`(떠 있음), `Pending`(뜨려고 시도 중), `CrashLoopBackOff`(계속 재시작), `ImagePullBackOff`(이미지 못 가져옴) 네 가지가 자주 등장합니다. 오픈이가 이번에 만난 상태는 두 번째인 `Pending`이었습니다.
...
- **종합 배포는 이전 장의 개념이 한 자리에 합류하는 자리다.** 프론트엔드/백엔드/DB/Redis 네 개의 Pod가 Namespace로 분리되고, Service로 이름으로 불리고, Ingress로 외부와 연결되고, PV/PVC로 DB가 영구해집니다.
...
- **챕터 6.** *"비밀번호와 데이터는 어떻게 안전하게?"* 실제 운영. 설정 분리, 데이터 보존, 네 개의 서비스 종합 배포, 그리고 디버깅. 책을 덮은 뒤에도 남을 습관.
