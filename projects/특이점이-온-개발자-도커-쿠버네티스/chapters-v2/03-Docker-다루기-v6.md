...
챕터 2를 마친 오픈이는 다음 날 저녁 다시 노트북 앞에 앉았습니다. 어제는 내 앱을 Docker 위에 올려 실행까지 해봤습니다. 그런데 `docker commit`은 한 번은 재미있어도 두 번째부터 손이 지쳤습니다. 서비스는 내 앱 하나로 돌아가지도 않았습니다. 프론트엔드, 백엔드, 데이터베이스가 각자 떠야 하고 서로 맞물려야 했습니다.

오늘의 목표를 이렇게 잡았습니다.

**내 앱 이미지를 Dockerfile로 자동 빌드하고, 웹 서버·세션 저장소·DB를 차례로 컨테이너로 세워본 뒤, 마지막에는 웹 서버·백엔드·DB를 묶은 풀스택 구성을 명령 한 줄로 띄우는 상태로 완성한다.**

수동 작업의 피로를 Dockerfile로 지우고, 한 앱이 아니라 **한 서비스** 전체를 다루는 데까지 가보는 여정입니다. 여기까지 밀고 가면 어제 서버에서 막혔던 상황 자체가 다시 생기지 않습니다.

## 3.1 Dockerfile — 환경을 자동으로 만들기

### 3.1.1 매번 수동으로 깔기는 지친다

어제 오픈이는 Tomcat 컨테이너에 들어가 `apt update`를 치고, vim을 깔고, index.html을 만들고, `docker commit`으로 이미지를 남겼습니다. 한 번은 재미있었습니다. 두 번째는 손이 무거웠습니다.
...
컨테이너를 지워도 이미지는 남습니다. 같은 환경을 언제든 다시 띄울 수 있습니다. 어제 수동 commit으로 만든 이미지의 자리를, 이제는 Dockerfile이 자동으로 채워줍니다.
...
같은 조합을 오픈이의 앱에도 그대로 쓸 만했습니다. ENTRYPOINT에 "내 런타임 실행"을 고정하고, CMD에 "실행할 파일명"만 바꿔 끼우면 같은 이미지를 여러 용도로 재활용할 수 있습니다.

내 앱 이미지를 재현 가능하게 만드는 조각은 여기까지 갖춰졌습니다. 그다음은 그 앱이 사용자 요청을 어떻게 받을지였습니다.
...
NGINX는 웹 서버이자 요청을 중계하는 **리버스 프록시**입니다. 사용자의 요청을 NGINX가 중간에서 받아 뒤쪽 서버로 넘기고, 응답도 NGINX가 받아 사용자에게 돌려줍니다. 실제 서버 주소를 바깥에 노출하지 않아 보안에도 좋습니다. 요청을 여러 서버로 나눠주는 **로드밸런싱**도 자연스럽게 얹힙니다. 오늘 풀스택 구성에서 앱 앞에 꼭 서 있어야 할 요소입니다.
...
Redis는 메모리 기반이라 재시작하면 데이터가 사라집니다. 회원 정보, 게시글, 주문 내역처럼 **영구히 보관되어야 하는 데이터**는 관계형 데이터베이스가 필요합니다. 앱에 당연히 DB가 붙어야 하고, MySQL도 컨테이너로 띄울 수 있습니다.
...
대목표였던 "내 앱을 포함한 풀스택 구성을 한 줄 명령에 띄우기"가 실습으로 확인됐습니다. 어제 서버에서 환경 불일치로 막혔던 상황은 이 구조 안에서는 반복되지 않습니다. 이미지에 필요한 모든 것이 들어 있고, Compose가 세 서비스를 하나의 네트워크로 묶어 서비스 이름만으로 서로를 부르게 해주기 때문입니다.
