...
네 대를 만들어 놨는데 실제 요청은 어느 주소로 보내야 하는지가 애매했습니다. Pod마다 IP가 다 다르게 박혀 있을 텐데, 같은 nginx 백엔드인데 어느 쪽으로 요청을 보내야 하는지 감이 오지 않았습니다.
...
답을 못 한 장면이 두 가지 있었습니다. Service를 처음 만들어 두고 Pod를 전부 지웠는데도 같은 주소로 들어가니 새 Pod가 응답을 돌려주던 장면. 그리고 인그레스 YAML에 `order-service` 라는 이름만 적었는데 그 이름이 진짜 Pod까지 닿던 장면. 결과는 봤지만 그 사이에서 누가 무슨 일을 하고 있는지는 본 적이 없었습니다.

오픈이는 그 안쪽을 살펴보기로 했습니다.

### 5.3.1 보이지 않는 손 — kube-proxy와 Endpoint Controller

먼저 인그레스 YAML이 걸렸습니다. 백엔드 자리에 `order-service:5678` 이라는 **이름**을 적었을 뿐인데 컨트롤러는 정확히 그 Pod에 닿았습니다. IP는 한 글자도 없었습니다. 이름과 진짜 주소 사이에서 누군가 그 둘을 잇고 있다는 뜻이었습니다.

Pod를 다 지웠는데도 Service URL로 새 Pod에 연결되던 장면도 마찬가지였습니다. Pod IP는 분명 바뀌었을 텐데, 누군가 그 변화를 따라가며 주소를 갈아 끼우고 있었던 셈입니다.

두 장면 다 결국 같은 길을 묻고 있었습니다. Service 주소로 들어온 요청이 어떻게 진짜 Pod까지 닿는가, 그리고 Pod가 새로 태어났을 때 그 길은 누가 다시 그어 주는가. 답을 미리 짚으면 Service 뒤에는 두 프로그램이 일하고 있었습니다. **kube-proxy** 는 가상 주소로 온 요청을 진짜 Pod로 보내는 길을 까는 쪽이고, **Endpoint Controller** 는 Service에 매달린 살아 있는 Pod 목록을 항상 최신으로 챙기는 쪽입니다. 두 프로그램이 어떻게 협력하는지가 이 절의 핵심입니다.

#### 첫 번째 — kube-proxy가 길을 깐다

먼저 출발점인 Service 주소부터 보겠습니다. Service에 박힌 ClusterIP는 사실 진짜 컴퓨터에 붙어 있는 IP가 아니라, 쿠버네티스가 만들어 낸 **가상 주소** 입니다. 그래서 이 주소로 요청을 보내도 받아 줄 실제 컴퓨터가 따로 없습니다. 누군가 그 요청을 중간에서 가로채 살아 있는 Pod IP로 바꿔 보내 줘야 비로소 도착합니다.

그 일을 맡은 프로그램이 **kube-proxy** 입니다. 모든 노드의 커널에 가로채기 규칙(iptables)을 심어 두고, ClusterIP로 들어온 요청을 자동으로 살아 있는 Pod IP로 바꿔 보냅니다. 

![](../assets/CH05/kube-proxy-route.png)

*kube-proxy가 가상 주소(ClusterIP)를 실제 Pod IP로 바꿔 잇는다*
...
이걸로 첫 번째 의문이 풀립니다. 인그레스 YAML에 `order-service` 라는 이름만 적어 두었는데도 진짜 Pod에 닿은 이유는, 그 사이에 kube-proxy가 깔아 둔 규칙이 가상 주소를 진짜 Pod IP로 바꿔 주고 있었기 때문입니다.

#### 두 번째 — Endpoint Controller가 목록을 챙긴다

그런데 한 가지가 남습니다. Pod IP가 바뀌면 그 규칙은 누가 업데이트할까요. kube-proxy가 매번 클러스터를 직접 살피며 갱신하지는 않습니다. 그 일을 따로 맡은 프로그램이 **Endpoint Controller** 입니다.

Endpoint Controller는 Service의 selector(라벨)에 매칭되는 Pod들의 상태를 지켜보다가, Pod가 새로 뜨거나 사라지면 곧바로 그 Service의 **Pod 목록(Endpoint)** 을 고쳐 씁니다. kube-proxy는 그 목록을 받아 자기 규칙을 갱신합니다.

![](../assets/CH05/endpoint-controller-update.png)

*Endpoint Controller가 Pod 변동을 감시해 목록을 갱신하면, kube-proxy가 그 목록대로 길을 새로 깐다*

이걸로 두 번째 의문도 풀립니다. Pod를 전부 지웠는데도 같은 Service 주소로 새 Pod에 연결됐던 이유는, Endpoint Controller가 새 Pod를 즉시 목록에 반영했고 kube-proxy가 그 목록대로 새 규칙을 깔았기 때문입니다.

#### 셋이 한 팀

정리하면 Service 뒤에는 셋이 한 팀으로 움직이고 있습니다.

- **Service** — 바깥에 내건 고정 주소
- **Endpoint** — 그 주소 뒤에 매달린 살아 있는 Pod 목록
- **kube-proxy** — 그 목록을 보고 실제 길을 깐 규칙

목록은 Endpoint Controller가 뒤에서 자동으로 갱신해 주고, 길은 kube-proxy가 그 목록대로 다시 깔아 줍니다. 셋이 맞물려야 비로소 요청이 목적지를 찾아갑니다.

*'Service가 주소, Endpoint Controller가 목록, kube-proxy가 길. 셋이서 한 팀이네.'*

### 5.3.2 같은 요청, 두 가지 일

Service 안쪽이 kube-proxy와 커널 규칙으로 굴러간다는 걸 보고 나니, 인그레스 실습 때 마음에 남아 있던 또 다른 의문이 떠올랐습니다. `/order` 와 `/stores` 를 갈라 보내려고 인그레스 컨트롤러를 따로 띄웠던 그 이유였습니다. 둘 다 결국 트래픽을 Pod로 흘려보내는 통로인데, Service만으로는 이 갈래길을 만들 수 없었던 까닭이 뭐였을까요.

답은 두 컴포넌트가 맡은 일 자체가 달랐다는 데 있었습니다.

#### 한 요청 안에는 두 가지 정보가 들어 있다

브라우저에서 `http://localhost/order` 를 한 번 누르면 요청 하나가 만들어집니다. 그런데 그 요청 안에는 사실 **두 종류의 정보가 같이 들어 있습니다**. 한쪽은 받을 컴퓨터의 주소, 다른 한쪽은 무엇을 요청하는지에 대한 내용입니다.

봉투에 비유하면 쉽습니다. 봉투 겉면에는 받을 사람의 주소가 적혀 있고, 봉투 안에는 편지가 들어 있습니다.

| | 적혀 있는 것 | 답할 수 있는 것 |
|---|---|---|
| **봉투 겉면 (주소)** | IP, Port | "어느 컴퓨터로 보낼까?" |
| **봉투 안 편지 (내용)** | URL 경로, Host, 메서드 | "`/order` ? `/stores` ? 뭘 요청하는가?" |

겉면 주소만 보면 받는 컴퓨터까지는 닿습니다. 그 안에서 정확히 무엇을 처리할지는 편지를 뜯어서 읽어야 알 수 있습니다.

두 정보는 처리하는 쪽도 다릅니다. 봉투 겉면(IP, Port)은 리눅스 커널이 직접 다룹니다. 주소만 보고 빠르게 분배합니다. 대신 봉투 안 편지는 커널이 보지 않습니다. 봉투를 뜯어 편지를 읽는 일은 별도 프로그램의 몫입니다. HTTP를 읽도록 만들어진 웹 서버나 프록시가 그 일을 맡습니다.

#### Service가 갈래길을 못 만드는 이유

Service가 맡은 일은 단순합니다. ClusterIP든 NodePort든, 어떤 입구로 들어온 요청이든 그 목적지를 살아있는 Pod 주소로 바꿔서 흘려보내는 것. 그뿐입니다.

봉투에 붙은 주소 라벨만 새것으로 갈아 붙이고 다시 보내는 일과 같습니다. 봉투를 뜯을 필요도, 안에 든 편지를 읽을 필요도 없습니다. 주소만 보면 끝나는 일입니다.

봉투의 주소(IP, Port)는 리눅스 커널이 원래 빠르게 다루는 정보입니다. Service가 하는 일이 마침 봉투 수준에서 끝나는 일이라, 커널에 그대로 맡길 수 있었습니다.

대신 봉투 안을 보지 못하니, `/order` 와 `/stores` 처럼 편지 내용을 읽어야 갈라지는 길은 이 자리에서 만들 수 없습니다.

#### Ingress가 따로 필요한 이유

Ingress가 맡은 일은 정반대 성격입니다. 같은 주소로 들어온 요청을, 안에 적힌 내용에 따라 다른 백엔드로 갈라 보내는 것입니다.

`http://localhost/order` 와 `http://localhost/stores` 는 봉투의 주소가 똑같습니다. 갈래를 정하는 기준 — 도메인, URL, 헤더 — 은 모두 봉투 안 편지지에 적혀 있습니다. 봉투를 뜯고, 글을 읽고, 어느 부서로 보낼지 판단해야 합니다.

봉투를 뜯어 글을 읽는 일은 커널이 해주는 일이 아닙니다. 편지를 펼쳐보는 직원이 따로 있어야 합니다. 사용자 공간에서 도는 별도 프로그램이 그 직원에 해당합니다.

그래서 Ingress를 쓰려면 편지를 뜯어 읽는 프로그램, 즉 **Ingress Controller**를 클러스터에 따로 깔아 두어야 합니다. HTTP를 다룰 수 있는 웹 서버나 프록시가 그 역할을 맡습니다.

> **참고: Ingress와 Ingress Controller**
>
> Ingress는 라우팅 규칙을 적어둔 리소스(규격)이고, 그 규격을 해석해 트래픽을 흘려보내는 실행 주체는 Ingress Controller입니다. 쿠버네티스는 컨트롤러를 기본으로 깔아주지 않습니다. NGINX 기반 ingress-nginx, Traefik, HAProxy, 클라우드 LB와 연결되는 ALB/GCE Ingress 등 여러 구현체 중 하나를 골라 직접 설치해야 동작합니다.
...
| | Service | Ingress |
|---|---|---|
| 자리 | 클러스터 안 모든 트래픽의 길목 | 외부 HTTP 진입점 한 곳 |
| 하는 일 | 봉투 주소를 살아있는 Pod 주소로 갈아 끼우기 | 봉투를 뜯어 편지를 보고 어느 부서로 보낼지 정하기 |
| 우선순위 | 속도 | 정교함 |
| 도구 | iptables (커널) | Ingress Controller (별도 프로그램) |
| 보는 것 | 봉투 주소 (IP, Port) | 편지 내용 (URL 경로, Host) |
| 대가 | 편지 내용은 못 봄 | 한 단계 더 거침 |

> **참고: L4와 L7**
>
> 네트워크 용어로는 봉투 수준의 정보(IP, Port)를 다루는 계층을 **L4(전송 계층)** , 봉투 안 내용(URL 경로, 헤더 등)을 다루는 계층을 **L7(애플리케이션 계층)** 이라고 부릅니다. Service는 L4 정보만으로 끝나는 일을 하고, Ingress는 L7 정보까지 읽어야 하는 일을 합니다.

*'결국 봉투까지만 보면 되는 일과, 안을 뜯어 읽어야 하는 일이었구나. 둘이 맡은 일 자체가 달랐던 거야.'*

### 5.3.3 요청의 여정

부품의 정체와 둘이 보는 깊이까지 정리하고 나니 처음 막혔던 한 줄을 이제는 그어 볼 만했습니다. 사용자가 브라우저에 URL을 치는 순간부터 Pod에 도달하기까지, 요청은 여러 손을 거쳐 갑니다. 한꺼번에 보면 복잡하니 한 단계씩 따라가 봤습니다.
...
드디어 요청이 백엔드 Pod에 닿았습니다. 애플리케이션은 비로소 비즈니스 로직을 실행해 주문을 처리하거나 데이터를 응답합니다. 이 모든 과정 내내 Endpoint Controller가 뒤에서 Pod 목록을 최신으로 유지하고 있어, kube-proxy의 길은 늘 살아 있는 Pod를 가리켰습니다.
...
따로 보면 복잡해 보이던 부품들이 한 줄에 늘어놓으니 그제야 한 그림으로 묶였습니다.

*'Service는 통로를 만들고 Ingress는 방향을 잡고. 각자 맡은 일이 단순하니 관리하기도 편하겠네.'*
...
- **kube-proxy와 Endpoint Controller가 네트워크를 실시간으로 관리** : 서비스의 주소(ClusterIP)는 실제 장비에 할당되지 않은 가상 주소입니다. kube-proxy가 각 노드 커널 수준에서 이 주소를 실제 Pod IP로 연결해 주며, Endpoint Controller는 Pod의 상태를 감시하며 살아 있는 Pod 목록을 최신으로 유지합니다.
