...
챕터 2에서 오픈이는 컨테이너 하나를 띄우고 내부를 들여다보고 이미지를 구워 Docker Hub에 올리는 데까지 왔습니다. 그런데 회사의 진짜 서비스는 컨테이너 하나로 끝나지 않았습니다. 결제 서비스만 해도 프론트, 백엔드 API, 세션 저장소, 데이터베이스가 각자 돌아야 했습니다. 오픈이는 컨테이너 하나에서 여러 개로 넘어가는 길에 서 있었습니다.
...
오픈이는 키보드에서 손을 뗐습니다. 레시피. 재료 목록과 순서가 적힌 종이 한 장. 누가 만들어도 같은 맛이 나오는 이유였습니다. 장을 보고 재료를 다듬는 수고 대신, 봉지만 뜯으면 요리가 완성되는 **밀키트** 같은 방법은 없을까요.
...
**[실습]** Dockerfile이 위치한 폴더에서 이미지 빌드
...
**[작성]** `Dockerfile`에 html 파일을 복사하는 설정을 추가합니다.
...
WORKDIR 덕에 컨테이너에 접속하면 기본 경로가 `/app`이고, COPY가 그 안에 index.html을 꽂아 넣습니다.

**[실습]** WORKDIR와 COPY가 적용된 이미지 빌드
...
**[실습]** ENTRYPOINT가 적용된 이미지 빌드 및 실행
...
> **참고: NGINX**
> 웹 서버이자 요청을 중계하는 프록시 서버입니다. 이미지나 HTML 같은 파일을 빠르게 처리하고, 사용자의 요청을 백엔드 서버로 연결해 주는 역할을 합니다. 로드밸런싱, 보안(HTTPS) 처리, 캐싱 기능을 모두 갖추고 있어 대규모 서비스를 운영할 때 필수적으로 사용됩니다.
...
**[참고]** 가장 단순한 nginx.conf

```nginx
upstream backend {                           # 1. backend라는 이름으로 목적지 서버 그룹 등록
    server host.docker.internal:8080;        #    이 그룹에 속한 실제 서버 주소
}

server {
    listen 80;                               # 2. 80번 포트로 들어오는 요청을 받음
    server_name localhost;

    location / {                             # 3. 모든 경로(/) 요청에 대해
        proxy_pass http://backend;           # 4. backend upstream으로 전달
    }
}
```

네 줄만 보면 됩니다. 1번에서 목적지 이름을 등록하고, 2번에서 어떤 포트로 들어오는 요청을 받을지 정하고, 3번에서 어떤 경로를 처리할지 정하고, 4번에서 그 경로로 들어온 요청을 목적지로 넘깁니다. 이 네 줄이 NGINX 설정의 뼈대입니다. 이후에 등장하는 로드밸런싱, 경로별 라우팅, 캐싱은 전부 이 뼈대 위에 옵션이 붙는 형태입니다.
...
app1, app2, lb 폴더에 있는 Dockerfile은 개별 이미지를 생성하며, 각각 독립적인 컨테이너로 실행됩니다. EX01은 **서로 다른 이미지(app1, app2)를 경로별로 분배**하는 예제입니다. 뒤에 나올 EX02는 **동일 이미지로 컨테이너 2개를 라운드 로빈**하는 예제이므로, 두 실습의 목적 자체가 다릅니다.
...
**[실습]** EX01 폴더에서 app1, app2, lb 이미지 빌드 및 실행
...
이 예제에서는 컨테이너를 `docker run`으로 각각 따로 실행했습니다. 챕터 2에서 본 것처럼 개별 실행된 컨테이너는 **기본 bridge 네트워크**에 들어가 있고, 이 네트워크에서는 이름 기반 통신이 되지 않습니다. lb 컨테이너가 "app1"이라는 이름으로 상대를 찾아갈 방법이 없었습니다. 그래서 호스트 PC를 거쳐가는 우회 경로를 썼습니다.
...
>
> **주의**: `host.docker.internal`은 Docker Desktop(Windows/macOS) 환경에서 기본으로 지원되는 주소입니다. **Linux에서는 기본으로 해석되지 않습니다.** Linux에서는 `--add-host=host.docker.internal:host-gateway` 옵션을 줘야 쓸 수 있으며, 실무에서는 이 우회 대신 3.3에서 소개하는 **사용자 정의 네트워크**로 컨테이너를 묶어 서비스 이름으로 통신하는 방식을 권장합니다.

lb 컨테이너가 `host.docker.internal:8000`으로 요청을 보내면 호스트 PC의 8000번 포트에 도착합니다. 이 포트는 포트포워딩(`-p 8000:80`)을 통해 app1 컨테이너와 연결되어 있으므로, 최종적으로 app1이 응답을 돌려줍니다.

### 3.2.3 라운드 로빈 : 요청을 골고루 나누다

3.2.2에서는 URL 경로별로 요청을 나눴습니다. 오픈이의 타임세일 문제는 조금 달랐습니다. 하나의 URL로 들어오는 요청을 똑같은 백엔드 여러 대로 흩어뿌려야 했습니다. 서버는 같은 이미지로 찍어내고, NGINX가 요청을 번갈아 뿌려주는 구성입니다.

NGINX의 upstream은 서버 하나만 등록해야 하는 게 아니었습니다. 한 그룹 안에 서버를 여러 개 등록할 수 있습니다. 외부 사용자는 하나의 주소로 접속하지만 실제로는 NGINX가 뒤에 숨어 있는 여러 대의 서버로 요청을 나눠 보냅니다.
...
요청을 순차적으로 회전시키며 분배하면 특정 서버에 부하가 몰리는 상황을 피할 수 있고, 전체 서비스의 안정성이 올라갑니다.

#### 실습해보기

> 실습 코드는 https://github.com/metacoding-10-linux-docker/docker/tree/master/ex02 에서 확인할 수 있습니다.

EX01에서는 **서로 다른 이미지 두 개(app1, app2)**를 각각의 경로(`/app1`, `/app2`)로 보냈습니다. EX02는 **동일한 이미지 하나**로 컨테이너 2개를 서로 다른 포트(8000, 8001)에 띄우고, 하나의 경로(`/app1`)로 들어온 요청을 그 둘에 번갈아 뿌립니다. "같은 서비스를 여러 대로 복제한 뒤 앞에 로드밸런서를 세우는" 전형적인 확장 패턴입니다.
...
EX02는 EX01과 `lb/nginx.conf` 파일을 제외하고 동일한 구조이므로 app1 Dockerfile 코드는 생략합니다.

#### lb 이미지

nginx.conf에서 upstream 블록이 달라졌습니다. 두 줄의 `server`가 같은 upstream 아래 나란히 있습니다.

NGINX는 별도의 로드 밸런싱 알고리즘을 지정하지 않으면 기본적으로 라운드 로빈 방식을 씁니다.
...
동일한 app1 이미지로 컨테이너 2개를 서로 다른 포트(8000, 8001)로 실행하는 방식입니다.

**[실습]** EX02 폴더에서 app1 이미지 빌드, 컨테이너 2개 및 lb 실행
...
오픈이가 브라우저로 `localhost:80/app1` 주소에 동일한 요청을 반복해서 보냈습니다. 요청이 두 서버로 번갈아 전달됐습니다.
...
새로고침할 때마다 요청이 두 서버로 번갈아 전달됐습니다.
...
> **참고: 정적 서버와 캐싱**
> 정적 서버는 한 번 제공한 정적 파일을 일정 기간 저장해 둡니다. 이후 동일한 요청이 오면 서버에 다시 조회하지 않고 저장된 파일을 즉시 반환합니다. 이 저장 과정을 캐싱이라고 부릅니다.

클라이언트가 서버에 처음 이미지 파일을 요청하면 NGINX는 백엔드 서버에 요청을 전달해 이미지 파일을 응답받습니다.
...
nginx.conf 파일입니다. 캐시 저장 경로를 지정하는 `proxy_cache_path`와 캐시 사용 여부를 제어하는 `proxy_cache`가 포함되어 있습니다.
...
아래 명령은 API 서버를 먼저 띄운 뒤 NGINX 캐시 서버를 이어서 띄우는 순서입니다.

**[실습]** EX03 폴더에서 api 서버와 NGINX 캐시 서버 빌드 및 실행
...
오픈이는 브라우저의 **F12 > Network > Headers** 탭을 열었습니다. 브라우저 자체에도 캐시 기능이 있어서 결과가 헷갈릴 수 있었습니다. 브라우저의 Disable cache를 체크해 브라우저 캐싱을 먼저 껐습니다.

Response Headers의 `X-Cache-Status` 값이 `MISS`입니다. `MISS`는 캐시에 요청 데이터가 없어 원본 서버에 요청을 보내 값을 가져온 상태입니다.

![](../assets/CH03/chap02-33.png)

*X-Cache-Status: MISS 확인*

새로고침을 한 번 더 하면 `X-Cache-Status` 값이 `HIT`로 바뀝니다. `HIT`는 캐시에 저장된 파일을 서버 요청 없이 그대로 응답한 상태입니다.
...
재현해 봤습니다. 로그인을 하고 장바구니에 상품을 담고 결제 페이지로 넘어가면 정상이었습니다. 그런데 결제 API를 호출하는 순간 로그인이 풀려 있었습니다. 화면을 새로고침하면 어떤 때는 로그인이 유지되고 어떤 때는 풀렸습니다.

*뭐지. 내가 한 요청이 서로 다른 서버로 갔나.*

Java 개발자로 Spring을 만질 때 `HttpSession`에 값을 담고 꺼내 쓰던 기억이 떠올랐습니다. 서블릿 컨테이너 안에 세션이 있었고, 서버가 한 대일 때는 아무 문제가 없었습니다. 그런데 방금 자기가 손수 만든 게 뭔지 되짚어봤습니다. 결제 API 컨테이너 두 대에 NGINX가 라운드 로빈으로 요청을 뿌려주는 구성. 로그인 요청은 1번 서버로, 그 다음 결제 요청은 2번 서버로 간 겁니다. 1번 서버 메모리에 저장된 세션을 2번 서버는 모릅니다. 로드밸런싱을 붙였더니 세션이 끊어진 셈이었습니다.
...
문제는 서버가 여러 대일 때 발생합니다. 클라이언트가 로그인을 요청하면 NGINX는 이 요청을 '서버 1'로 전달합니다. 이때 로그인 정보는 오직 '서버 1'의 메모리에만 기록됩니다. 다음 요청이 NGINX에 의해 '서버 2'로 전달되면 '서버 2'는 해당 사용자의 세션 정보를 모르기 때문에 다시 로그인을 요구합니다.
...
서버 1이 세션 정보를 자신의 메모리가 아닌 외부 저장소인 Redis에 기록하는 방식입니다. 이후 요청이 서버 2로 전달되더라도 서버 2가 Redis를 확인해 동일한 세션 정보를 읽어옵니다. 어떤 서버로 요청이 가든 로그인이 유지됩니다.
...
app = Flask(__name__)
# host='redis'는 Docker 사용자 정의 네트워크 안의 컨테이너 이름
...
`host='redis'`가 핵심입니다. IP를 직접 쓴 게 아니라 "redis"라는 **컨테이너 이름**으로 상대를 찾고 있습니다. 이게 되려면 같은 사용자 정의 네트워크 안에 둘 다 들어 있어야 합니다.
...
3.2에서는 컨테이너끼리 통신하려면 `host.docker.internal`로 호스트를 경유해야 했습니다. 챕터 2에서 예고한 **사용자 정의 네트워크**로 컨테이너를 묶으면 컨테이너 이름만으로 직접 통신할 수 있습니다.

**[실습]** EX04 폴더에서 네트워크 생성과 Redis, API 컨테이너 실행
...
오픈이는 Redis로 세션 문제를 막고 한숨을 돌렸는데, 금요일에 본부장의 회의 자료 요청이 왔습니다. "지난달 가입한 회원 명단 뽑아와." 그 순간 오픈이는 멈칫했습니다.
...
**[실습]** EX05 폴더에서 DB 이미지 빌드와 컨테이너 실행
...
`init.sql`에 작성한 데이터가 그대로 조회됐습니다.
...
**순서.** 어떤 컨테이너를 먼저 띄울지 `depends_on`으로 지정할 수 있습니다. 단, 컨테이너가 **시작된 시점**을 보장할 뿐, 내부 서비스가 **준비 완료**된 시점까지 보장하지는 않습니다.
...
**[실습]** EX06 폴더에서 docker compose up 명령으로 모든 서비스 실행
...
**[실습]** docker compose down 명령으로 모든 서비스 종료
...
> 이 실습은 학습용입니다. 컨테이너가 뜰 때마다 git clone과 Gradle 빌드를 다시 돌리므로 기동 시간이 길고, 외부 네트워크 상태에 의존합니다. **실무에서는 Dockerfile 빌드 시점에 JAR를 굽고 이미지에 포함시키는 방식**이 일반적입니다. 이미지 빌드는 한 번만 일어나고, 이후 컨테이너 기동은 빌드된 JAR를 `java -jar`로 실행하기만 하므로 빠르고 재현 가능합니다.
...
3.5.1에서 언급한 `depends_on`을 여기서 처음으로 씁니다. 프론트는 백엔드가 준비된 뒤에, 백엔드는 DB가 준비된 뒤에 띄우도록 순서를 박아둡니다.
...
`depends_on`은 **시작 순서**를 보장합니다. db → backend → frontend 순서로 컨테이너가 **켜지는** 것까지는 책임져 줍니다. 그러나 MySQL 프로세스가 떠 있다고 해서 3306 포트로 JDBC 접속을 받을 준비가 끝난 것은 아닙니다. **"컨테이너 시작"과 "서비스 준비 완료"는 다른 시점**입니다. 이 간격 때문에 실무에서는 healthcheck를 별도로 걸거나, 애플리케이션 쪽에서 재시도 로직을 넣어 DB가 뜰 때까지 기다리는 방식을 함께 씁니다. 학습 단계에서는 `depends_on`만으로 충분합니다.

Spring Boot는 `SPRING_DATASOURCE_URL` 환경변수의 `jdbc:mysql://db:3306/...`을 읽어 DB에 접속합니다. 여기서 `db`가 IP가 아닌 **Compose 서비스 이름**이라는 점이 중요합니다. 같은 네트워크에 있는 서비스끼리 이름으로 DNS가 해석되는 구조 덕입니다.

### 3.6.6 통합 실행

오픈이는 EX07 폴더에서 `docker compose up` 한 줄을 쳤습니다.

**[실습]** EX07 폴더에서 docker compose up 명령으로 전체 서비스 실행
...
페이지에 접속하면 프론트의 fetch가 `/api/users`를 호출하고, NGINX가 backend 서비스로 프록시하고, 백엔드는 `db` 서비스 이름으로 MySQL에 붙어 회원 행을 긁어옵니다. `docker compose up` 한 줄이 이 전체 흐름을 올렸습니다. 오픈이는 모니터 앞에서 잠시 의자에 기대앉았습니다. 아침에 여덟 줄을 두드리던 일이 한 줄로 끝나는 순간이었습니다.

## 이것만은 기억하자

- **Dockerfile**에 환경을 한 번 정의하면 어디서든 동일한 컨테이너를 만들 수 있습니다.
- **NGINX**는 URL 경로에 따라 요청을 분기하고, 여러 서버에 트래픽을 분산하며, 정적 파일을 캐싱합니다. 핵심은 `upstream`·`location`·`proxy_pass` 세 지시어의 연결입니다.
- **Redis**는 여러 서버가 공유하는 외부 세션 저장소로, 로드밸런싱으로 인한 세션 불일치를 해결합니다.
- **MySQL**은 컨테이너로도 올릴 수 있고, 환경변수와 init.sql로 초기 상태를 구성합니다.
- **Docker Compose**는 여러 컨테이너를 하나의 파일에 정의하고 `docker compose up` 한 줄로 실행합니다. `depends_on`으로 시작 순서를 잡되, 서비스 준비 완료까지는 보장하지 않는다는 점을 기억해야 합니다.
