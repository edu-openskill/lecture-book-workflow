...
다음 날 아침, 사무실 자리에 앉은 오픈이는 어제 띄워 둔 Pod 네 개를 화면에 다시 펼쳐 놓고 잠시 뿌듯함을 느꼈습니다. 같은 서버가 네 대나 돌아가고 있으니 하나가 죽어도 세 대가 버텨주고, 롤링 업데이트로 새 버전까지 끊김 없이 올라가는 것을 직접 확인했기 때문입니다. 모니터에 나란히 찍힌 네 줄의 'Running' 상태는 식어 가는 커피잔 너머에서 꽤 든든해 보였습니다.
...
챕터 2에서 한 호스트 안의 푸드코트로 그렸던 컨테이너 네트워크가, 이번 챕터에서는 클러스터 규모로 펼쳐집니다. 첫 단추는 매번 흔들리는 Pod IP를 대신해 주는 고정 주소였습니다.
...
다음은 두 번째 의문 차례였습니다. 인그레스 YAML에 `order-service` 라는 이름만 적었는데 진짜 Pod까지 닿던 일이었습니다.
...
> **참고: 클러스터 DNS**
>
> 쿠버네티스는 클러스터 안에 DNS 서버를 함께 띄워 둡니다. Pod가 다른 Service를 이름으로 부르면 이 DNS가 자동으로 ClusterIP를 알려 줍니다. 별도 설정 없이 기본으로 동작합니다.
