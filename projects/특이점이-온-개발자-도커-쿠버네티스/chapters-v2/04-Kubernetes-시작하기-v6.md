...
**Compose로 짠 구성을 Kubernetes에 올려, 자동 복구와 무중단 배포가 가능한 상태까지 간다.**
...
이번 챕터에서는 **Pod**와 **Deployment**만 먼저 다루고, 나머지는 챕터 5와 6에서 순서대로 풉니다. 오늘 목표인 자동 복구·스케일링·무중단 배포를 체험하려면 이 둘로 충분합니다.
...
Pod를 1개가 아니라 여러 개 유지하고 싶다면 `replicas`를 올리면 됩니다. 이 역할을 담당하는 친구가 **ReplicaSet**입니다. Deployment가 자동으로 생성해서 관리하는 리소스입니다. 오늘 풀려는 과제 중 또 한 축인 **스케일링**이 이 지점에서 해결됩니다.
...
자동 복구·스케일링·무중단 배포는 맞춰놨지만, **브라우저에서 내 앱에 안정적으로 접속 가능한 상태**는 아직 아닙니다. 가맹점 주방이 바뀌어도 손님이 늘 거는 **대표 전화번호**가 있어야 합니다. 그게 다음 챕터의 주제인 **Service**의 역할입니다.
