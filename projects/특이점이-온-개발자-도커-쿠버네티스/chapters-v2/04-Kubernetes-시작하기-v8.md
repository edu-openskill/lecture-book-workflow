...
그 질문에 아무 대답도 할 수 없었습니다. 컴포즈는 "지금 이 순간 띄워라"는 도구였지, 죽은 컨테이너를 실시간으로 감시하며 살려주는 기능은 없었으니까요. 팀장의 질문은 계속 이어졌습니다.
...
챕터 1에서 K8s를 항만 관제 센터에 비유했지만, 이번 챕터부터는 더 가까운 비유인 프랜차이즈 본사로 풀어 봅니다. 컨테이너 한 무더기를 다루는 자리에서 가맹점 한 무더기를 다루는 자리로 옮겨 온 셈입니다.
...
*본사가 "가맹점 4개 유지"를 선언하면 시스템이 가맹점 개수를 자동으로 맞추는 구조*
...
*Minikube는 단일 노드 안에 본사와 상가 건물 기능이 함께 들어간 구조*
...
*selector가 지정한 라벨(app: nginx)을 가진 Pod만 골라 관리, 다른 라벨은 건드리지 않음*
...
*Pod를 다 지워도 Deployment가 자동으로 새 Pod를 띄운다*
...
*Pod 하나가 종료되면 ReplicaSet이 설정 개수를 맞추기 위해 새 Pod를 자동 생성*
