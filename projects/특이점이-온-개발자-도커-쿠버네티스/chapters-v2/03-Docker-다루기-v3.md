...
챕터 2에서 오픈이는 컨테이너 하나를 띄우고, 안으로 들어가 보고, 이미지를 구워 Docker Hub에 올리는 데까지 왔습니다. 그런데 회사의 진짜 서비스는 컨테이너 하나로 끝나지 않았습니다. 결제 서비스만 해도 프론트, 백엔드 API, 세션 저장소, 데이터베이스가 각자 돌아야 했습니다. 오픈이는 컨테이너 하나에서 여러 개로 넘어가는 길에 서 있었습니다.
...
app1, app2, lb 폴더에 있는 Dockerfile은 개별 이미지를 생성하며, 각각 독립적인 컨테이너로 실행됩니다. 이 챕터에는 비슷해 보이는 NGINX 예제가 EX01, EX02, EX03 세 개 등장합니다. 목적이 서로 다르므로 들어가기 전에 한 번 비교하고 시작하는 편이 혼선을 줄여줍니다.

| 예제 | 주제 | 이미지 구성 | 경로/서버 구성 | 한 줄 요약 |
|------|------|-------------|----------------|-----------|
| EX01 | 경로 기반 라우팅 | 서로 다른 이미지 2개 (app1, app2) | `/app1 → app1`, `/app2 → app2` | URL 경로로 각기 다른 서비스로 분배 |
| EX02 | 라운드 로빈 로드밸런싱 | 동일 이미지 1개 → 컨테이너 2개 | `/app1` 하나에 서버 2대 묶음 | 같은 서비스를 여러 대로 복제 |
| EX03 | 캐싱 | api + nginx | `/`는 프록시, `/image.png`만 캐시 | 자주 조회되는 파일을 앞에 보관 |
...
Response Headers의 `X-Cache-Status` 값이 `MISS`입니다. 캐시에 요청 데이터가 없어 원본 서버까지 다녀와 가져온 상태입니다.

![](../assets/CH03/chap02-33.png)

*X-Cache-Status: MISS 확인*

새로고침을 한 번 더 하면 `X-Cache-Status` 값이 `HIT`로 바뀝니다. 캐시에 저장된 파일을 서버 요청 없이 그대로 응답한 상태입니다.
...
`host='redis'`가 또 하나의 핵심입니다. IP를 직접 쓴 게 아니라 "redis"라는 **컨테이너 이름**으로 상대를 찾고 있습니다. 이게 되려면 같은 사용자 정의 네트워크 안에 둘 다 들어 있어야 합니다.
...
3.2에서는 `docker run`으로 각각 따로 띄운 컨테이너가 **기본 bridge 네트워크**에 들어가 있어서 이름으로 서로를 못 찾았습니다. 그래서 `host.docker.internal`로 호스트 PC를 한 번 거쳤습니다. 여기서는 **사용자 정의 네트워크**로 넘어갑니다. "내가 직접 만든 네트워크"라는 뜻이고, `docker network create`로 그룹 하나를 만든 뒤 컨테이너를 띄울 때 `--network` 옵션으로 그 그룹에 집어넣습니다. 같은 그룹 안에 들어간 컨테이너는 서로의 이름으로 DNS가 해석돼 바로 통신합니다. 호스트를 경유할 필요가 사라집니다.
...
- **NGINX**는 URL 경로에 따라 요청을 분기하고, 여러 서버에 트래픽을 분산하며, 정적 파일을 캐싱합니다. 핵심은 `upstream`·`location`·`proxy_pass` 세 지시어의 연결입니다. NGINX의 역할 이름이 **리버스 프록시**였습니다.
