...
- Postman과 브라우저 DevTools Network 탭에서 `video.m3u8` 다음에 `video0.ts`, `video1.ts`, ...가 순서대로 내려오는 모습을 직접 확인합니다
- HLS·DASH·WebRTC·RTMP·RTSP를 "지연·방향·용도" 세 축으로 한 장짜리 비교표로 정리하고, 챕터 6의 WebSocket과 대비합니다
:::

:::preview
**이번 챕터는 "영화관 영사기가 한 통의 필름을 열두 통으로 나눠 순서대로 돌리는 이야기"입니다**

챕터 6 마지막에서 오픈이는 세 방식(Polling · SSE · WebSocket)을 나란히 놓고 "어느 자리에 무엇이 맞는가"를 정리했습니다. 수첩을 덮으려는데 동료가 다음 주 회의를 꺼냈습니다. 강의 VOD를 호스팅해야 하는데, 업로드한 mp4를 그대로 내려 주는 구현이 3분째 버퍼링으로 멈춘다는 것이었습니다. 이 챕터에서는 그 3분을 끊어 보겠습니다. 서버가 영상 한 덩어리를 통째로 밀어 주는 대신, FFmpeg가 10초짜리 필름 조각으로 잘라 두고, 브라우저가 조각을 순서대로 집어 가게 만듭니다. 같은 영상이, 같은 회선에서, 거의 끊기지 않고 돕니다. 끝에서는 HLS가 놓인 자리를 WebRTC·RTMP와 함께 지도 위에 놓고, "언제 무엇을 고를지"를 한 장으로 정리합니다.
:::
...
:::note
**`upload/` 폴더는 레포에 커밋되지 않습니다.** 첫 업로드가 들어올 때 `mkdirs()`로 자동 생성되고, 그 아래로 `original/video.mp4`와 `hls/720p/video.m3u8`·`video0.ts` 같은 파일들이 쌓입니다. 실습이 끝난 뒤 용량이 부담되면 이 폴더를 통째로 삭제하면 됩니다.
:::
...
오픈이는 챕터 6을 닫고 수첩을 서랍에 넣었습니다. 월요일 아침, 평소보다 한 잔 더 진한 커피를 뽑아 자리로 돌아오는데, 동료가 노트북을 들고 쫓아왔습니다.

**동료**: "업로드한 영상 재생 누르면 3분째 버퍼링이에요."
...
오픈이는 그 자리에서 자기 노트북에 같은 파일을 열어 봤습니다. 재생 버튼을 눌렀습니다. 비디오 요소의 `currentTime`은 0 근처에서 움직이지 않았습니다. 30초를 기다렸다가 타임라인을 정확히 30분 지점으로 드래그했습니다. 스피너가 다시 10초를 돌았습니다.

**동료**: "그리고요, 모바일에서 열면 아예 로딩도 안 돼요."
...
**팀장**: "유튜브가 동영상 한 덩어리를 통으로 내려 줄까요? 아니면 잘라서 줄까요?"

오픈이는 바로 대답이 나오지 않았습니다.

**팀장**: "유튜브 영상을 볼 때, 앞부분은 봤는데 뒷부분은 아직 안 내려왔을 수도 있잖아요. 그때 사용자가 뒤로 감으면요."
...
오픈이는 자리에 앉아 지금 구현을 다시 봤습니다. 서버에는 `@GetMapping("/videos/{name}")` 컨트롤러 하나가 있었고, 그 안에서 `FileSystemResource`로 mp4 파일 하나를 응답으로 돌려주고 있었습니다. 브라우저의 `<video src="/videos/lecture.mp4">`가 그 URL을 걸어 두면, HTTP는 처음부터 끝까지 바이트를 순서대로 흘려 보냈습니다. Range 헤더로 일부만 요청할 수는 있었지만, 플레이어는 그 방식을 영리하게 쓰지 못하고 있었습니다. 네트워크가 조금이라도 불안하면 이어받기가 제대로 안 됐습니다.
...
그런데 문제는 남았습니다. 여전히 사용자 한 명당 풀 대역폭이 할당됐고, 네트워크가 끊기면 이어받기가 불안정했습니다. `moov` 아톰의 위치에 따라 시크 속도가 갈렸고(영상을 `faststart` 옵션으로 재인코딩해야 앞쪽으로 옮겨집니다), 모바일에서 공용 와이파이가 자주 끊기는 상황은 여전히 풀리지 않았습니다.
...
<div class="sp-figure">
  ...
  <div class="sp-compare">
    <div class="sp-compare-block bad">
      <span class="sp-compare-label">한 덩어리 (mp4)</span>
    ...
    <div class="sp-compare-block good">
      <span class="sp-compare-label">여러 조각 (HLS)</span>
      <div class="sp-compare-content">
        m3u8 1회 + ts N회(10초당 1개). 1~2개 조각만 받으면 재생 시작. 시크는 해당 조각만 바로 요청. 끊겨도 실패한 한 조각만 재시도. m3u8에 여러 화질 등록 시 네트워크에 따라 자동 전환.
      </div>
    </div>
  </div>
  <div style="text-align:center;font-size:var(--fs-xs);color:var(--color-text-muted);margin-top:12px;">같은 영상, 다른 배달 방식이 만드는 차이입니다</div>
</div>
...
먼저 FFmpeg가 HLS를 만들 때 쓰는 옵션을 익히고, 그 옵션들이 어떻게 자바 코드로 번역되는지 살펴보겠습니다.
...
이 짧은 메서드에서 일어나는 일은 셋입니다. Postman이 `form-data`의 `file` 키로 업로드한 바이너리를 `MultipartFile`이 받아 들고, `saveOriginalVideo`가 그걸 `upload/original/video.mp4`로 떨어뜨립니다. 그 다음 `convertToHlsAsync`가 호출되자마자 메서드는 바로 `ResponseEntity.ok(...)`를 반환합니다. **인코딩은 뒤에서 따로 돕니다.** 업로드 요청은 몇 밀리초 안에 응답받고, 사용자는 다른 걸 하러 갈 수 있습니다.
...
<div class="sp-figure">
  ...
  <div style="text-align:center;font-size:var(--fs-xs);color:var(--color-text-muted);margin-top:12px;">응답은 ④에서 끝나고, ⑤·⑥은 백그라운드에서 이어집니다</div>
</div>
...
핵심은 `application/vnd.apple.mpegurl`이라는 MIME 타입 한 줄입니다. 브라우저의 `Hls.js`나 사파리 네이티브 플레이어가 이 타입을 보고 "아, m3u8이니까 목록을 파싱해서 ts를 이어 받자"라고 판단합니다. 타입이 잘못되면 플레이어는 이 응답을 일반 텍스트로 오해하고 재생을 시도하지 않습니다.
...
Key는 반드시 `file`이어야 합니다. 컨트롤러의 `@RequestParam("file")`과 이름이 맞아야 바인딩됩니다. Type은 드롭다운에서 Text가 아니라 **File**을 고릅니다. 드롭다운이 안 보이면 Key 셀에 마우스를 올리면 나타나는 회색 `Text` 버튼을 클릭해서 전환합니다.
...
응답이 **즉시** 돌아옵니다. 몇 백 밀리초 안에 `HLS 변환 시작: ...` 메시지가 뜨고, 그 사이 IntelliJ 콘솔이나 `tail -f` 중인 로그에는 FFmpeg가 방금 막 돌기 시작했다는 라인이 찍히고 있습니다. 여기서 Postman은 돌려줬지만 **변환은 아직 끝나지 않았습니다.**
...
`http://localhost:8080/hls/1080p/video.m3u8` URL로 바꿔 들어가면 1080p 버전도 같은 방식으로 동작합니다. 두 화질 중 어떤 걸 쓸지는 플레이어 설정이나 사용자 선택에 따라 달라집니다. HLS 표준은 m3u8 한 장에 **여러 화질을 목록으로 묶어 두는 마스터 플레이리스트** 개념도 제공합니다. 네트워크 상태에 따라 플레이어가 자동으로 720p↔1080p를 전환하는 **적응형 비트레이트(ABR)**가 이 구조 위에서 돕니다. 이번 실습은 화질별 m3u8을 따로 제공하는 단순 구조이지만, 상용 서비스는 거의 다 마스터 플레이리스트를 씁니다.
...
<div class="sp-figure">
  ...
  <div style="text-align:center;font-size:var(--fs-xs);color:var(--color-text-muted);margin-top:12px;">같은 "영상 전달"이라도 지연·방향·용도가 갈립니다</div>
</div>
...
**동료**: "오픈이 님, 버퍼링 없이 재생되네요. 근데요, 이번엔 다른 문제예요."
...
오픈이는 수첩을 넘겼습니다. 업로드·재생·스트리밍까지 길이 이어졌고, 이제 그 영상 위에 앉을 **검색창**이 남아 있었습니다. 다음 챕터에서 Elasticsearch를 얹습니다.
...
- **다음 문제는 검색이다.** 영상을 올리고 재생하는 파이프라인까지 왔으니, 영상에 딸린 제목·태그·설명을 빠르게 찾는 것이 이어집니다. DB의 `LIKE`로는 10만 건 앞에서 무너집니다. 챕터 8에서 Elasticsearch를 얹습니다
