...
- MSA의 핵심 과제인 분산 트랜잭션과 그 해법(보상 트랜잭션·Saga)을 이해할 수 있습니다.
...
화면을 열었다. 모니터링 대시보드가 빨갛게 차 있었다. 에러 그래프가 가파른 절벽처럼 솟아 있었다. 주문 API가 죽었고, 그 옆에 로그인 API도 같이 죽어 있었다.
...
<svg viewBox="0 0 1040 250" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="서비스끼리 직접 호출하고 실패 시 거꾸로 직접 보상 요청">
  ...
  <line x1="416" y1="135" x2="264" y2="135" stroke="#4f46e5" stroke-width="1.6" stroke-dasharray="4,3" marker-end="url(#c1f8h-o)"/>
...
**방식 2. 중앙 지휘자에게 맡긴다 — Saga 패턴 (챕터 4, 챕터 5)**

이 방식에서는 **지휘자(orchestrator)** 라는 별도 서비스가 전체 흐름을 책임집니다. 각 서비스는 자신의 일만 하고 결과만 보고하고, 주문 서비스는 Kafka 토픽에 이벤트만 발행하면 곧장 자유로워집니다.
...
*그림 1-7. 비동기 Kafka - 지휘자가 흐름을 조율하는 구조*
...
<div class="evolve-flow">
  ...
  <div class="ev-card">
    ...
    <div class="ev-name">DDD + Clean Arch + K8s</div>
    <div class="ev-add">도메인을 중심에 두고 구조를 다듬어 운영 환경에 올린다</div>
  </div>
  <div class="ev-arrow">→</div>
  <div class="ev-card">
...
**챕터 3**에서는 챕터 2 코드를 DDD와 클린 아키텍처로 다시 정리하고, Kubernetes에 올려 운영 환경을 경험합니다.
...
- 분산 트랜잭션을 두 가지 방식으로 풉니다. 챕터 2~3에서는 **보상 트랜잭션**으로 서비스끼리 직접 호출·보상하고, 챕터 4~5에서는 **Saga 패턴(Orchestration)** 으로 중앙 지휘자가 흐름을 맡습니다.
