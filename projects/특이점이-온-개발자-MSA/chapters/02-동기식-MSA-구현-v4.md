...
깨달음: 분산 트랜잭션은 자동 안 됨. 보상은 직접 짠다 (Saga)
...
청사진은 머리에 들어왔습니다. 이제 만들어 봅니다. user·product·order·delivery 각각의 프로젝트를 만들고, JWT 인증 필터를 공통 코드로 두고, 4개를 Docker Compose에 묶었습니다. user에서 로그인하고 JWT 받기까지는 어렵지 않았습니다.

이제 주문 서비스 차례입니다. 주문을 만들려면 상품 서비스의 재고를 줄이고, 배달 서비스에 배달을 만들어야 합니다. RestClient로 두 서비스를 차례로 호출하는 코드를 짜기 시작했습니다.
...
*재고는 줄었고, 그 다음 배달 생성이 실패하면... 어떻게 되돌리지?*

모놀리식이라면 `@Transactional` 한 줄이 다 롤백해줬습니다. 그런데 상품 서비스에 보낸 HTTP 요청은 이미 떠났습니다. 상품 DB에 재고는 줄어든 채로 남습니다. 자동으로는 돌아오지 않습니다.

챕터 1에서 머리로 알던 분산 트랜잭션 문제가 손에 잡혔습니다. 선배 자리로 갔습니다.

**오픈이**: "선배님, 재고는 줄였는데 그 다음 배달 생성이 실패하면요? 자동으로 안 돌아가요."

**선배**: "기억나죠? Saga. 자동으로 안 되니 직접 짜는 거예요. 재고를 줄였으면 다시 늘리고, 배달을 만들었으면 취소하고. 어디까지 진행됐는지 플래그로 기록하고. 그게 **보상 트랜잭션**이에요."

*Saga. 챕터 1에서 배운 그거. 그걸 손으로 짜는 거였구나.*
...
| 클래스 | 역할 |
|---|---|
| `User` | 사용자 엔티티. `username`(unique)·`email`·`password`·`roles` |
| `UserRepository` | Spring Data JPA |
| `UserService` | 로그인 검증 + JWT 발급 + 사용자 조회 |
| `UserController` | `POST /login`·`GET /api/users/{userId}` |

### 2.3.2 더미 데이터

개발 환경에서는 H2 in-memory DB에 자동으로 테스트 데이터를 삽입합니다. `db/data.sql`에 `ssar`·`cos`·`love` 세 계정이 비밀번호 `1234`로 등록됩니다.
...
| 클래스 | 역할 |
|---|---|
| `Product` | 상품 엔티티. `productName`·`quantity`·`price`. `decreaseQuantity()`·`increaseQuantity()` 메서드로 재고 증감 로직 캡슐화 |
| `ProductRepository` | Spring Data JPA |
| `ProductService` | 재고 감소 전 상품 존재·재고·가격 검증 후 `Product.decreaseQuantity()` 호출 |
| `ProductController` | `GET /api/products/{id}`·`PUT /api/products/{id}/decrease`·`PUT /api/products/{id}/increase` |
...
| 클래스 | 역할 |
|---|---|
| `Delivery` | 배달 엔티티. `orderId`·`address`·`status(DeliveryStatus)`. `create()`·`complete()`·`cancel()` 정적 메서드와 상태 전이 메서드로 캡슐화. 상태 전이: `PENDING → COMPLETED` 또는 `PENDING → CANCELLED` |
| `DeliveryRepository` | Spring Data JPA. `findByOrderId(int)` 추가 |
| `DeliveryService` | `createDelivery`(주소 검증 후 즉시 완료)·`cancelDelivery`(중복 취소 방어) |
| `DeliveryController` | `POST /api/deliveries`·`GET /api/deliveries/{id}`·`PUT /api/deliveries/{orderId}` |

### 2.5.2 더미 데이터

주문 3건에 대한 배달 데이터가 모두 `COMPLETED` 상태로 등록됩니다.
...
| 클래스 | 역할 |
|---|---|
| `Order` | 주문 엔티티. `userId`·`productId`·`quantity`·`price`·`status(OrderStatus)`. 상태 전이: `PENDING → COMPLETED` 또는 `PENDING → CANCELLED`. `create()`·`complete()`·`cancel()` 메서드로 캡슐화 |
| `OrderStatus` | enum (PENDING·COMPLETED·CANCELLED) |
| `OrderRepository` | Spring Data JPA |
| `OrderRequest` / `OrderResponse` | 요청 DTO(`productId`·`quantity`·`price`·`address`) / 응답 DTO(Order 필드를 펼친 record) |
| `OrderController` | `POST /api/orders`·`GET /api/orders/{id}`·`PUT /api/orders/{id}` |
| `OrderService` | **[작성]** 보상 트랜잭션 핵심 — `createOrder`(try/catch + 플래그)·`findById`·`cancelOrder` |

### 2.6.2 더미 데이터

사용자별 주문 3건(완료·취소·대기)이 등록됩니다. 한 행에 상품 정보까지 함께 들어갑니다.

### 2.6.3 보상 트랜잭션 - 실패하면 되돌린다
...
### 2.6.7 RestClient - JWT 헤더를 실어 다른 서비스를 호출하다
...
### 2.6.8 ProductClient와 DeliveryClient
...
### 2.6.9 OrderService - 보상 트랜잭션의 핵심
...
OrderService의 나머지 두 메서드는 `findById`(주문 한 건 조회)와 `cancelOrder`(createOrder의 역과정)입니다. `cancelOrder`는 재고 복구(`increaseQuantity`) → 배달 취소(`cancelDelivery`) → 주문 상태 변경(`Order.cancel()`) 순서로 처리합니다. 자세한 구현은 완성 레포를 참고하세요.
...
보상 트랜잭션 덕분에 데이터 일관성을 유지할 수 있었습니다. 하지만 이 구조에는 눈에 잘 보이지 않는 문제가 있습니다. 모든 서비스 호출이 동기적이라 상품 서비스가 1초 걸리면 주문 서비스도 1초를 기다리고, 보상 트랜잭션 코드가 비즈니스 로직과 섞여 try-catch 중첩이 점점 깊어집니다. 게다가 새 비즈니스 규칙이 들어오면 어디 손대야 할지 막막한 코드 구조도 같이 따라옵니다.
