...
오픈이는 배달 코드를 따라가 봤습니다. 배달은 만들어지는 그 줄에서 곧바로 완료로 바뀌었습니다. 정작 물건이 진짜 전달되는 순간은 코드 어디에도 없었습니다. 그렇게 찍힌 완료조차 화면에 바로 뜨지 않아, 동료는 끝났는지 직접 확인할 수밖에 없었습니다.

이 문제를 들고 선배에게 갔습니다.

**오픈이**: "배달이 만들어지자마자 완료로 찍혀요. 지금 이대로 사용자한테 '완료'라고 알리면 거짓말입니다. 어디부터 손대야 하죠?"
...
반대로 택배가 도착하면 초인종이 한 번 울립니다. 안에 있는 사람은 문을 여닫으며 확인할 필요 없이, 울리는 순간 도착을 압니다. 이렇게 클라이언트가 묻지 않아도 서버가 변화가 생긴 순간 그 사실을 먼저 보내는 방식이 **푸시(Push)** 입니다. 클라이언트는 반복해서 묻지 않고, 변화가 생기면 그 즉시 화면까지 전달됩니다.
...
동료가 주문 내역을 몇 번이고 다시 열어 봐야 했던 건 화면에 초인종이 없었기 때문입니다. 사용자에게 끝난 사실을 곧바로 알리려면, 서버가 먼저 보내는 푸시가 필요합니다.

### 5.2.2 WebSocket

푸시를 구현하는 기술이 **WebSocket**입니다.

HTTP 요청-응답은 편지와 같습니다. 한 통 보내고 답장이 오면 그 왕복은 끝나고, 또 궁금하면 다시 한 통을 보내야 합니다. 그래서 보내는 쪽이 묻지 않으면 상대는 아무것도 전할 수 없습니다.

WebSocket은 전화 통화에 가깝습니다. 한 번 연결되면 끊지 않고 그대로 이어 두고, 어느 쪽이든 먼저 말할 수 있습니다. 서버는 변화가 생긴 순간 "주문 완료"라고 먼저 전합니다.

<div class="svg-figure">
<svg viewBox="0 0 900 320" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="편지와 전화 비유로 본 HTTP와 WebSocket. 양쪽 다 사용자와 서버 박스는 같다. 왼쪽은 편지를 한 번 주고받으면 연결이 끊겨 다시 보내야 한다. 오른쪽은 전화 연결이 유지되어 서버가 먼저 주문 완료를 알린다.">
  <defs>
  ...
  <line x1="450" y1="54" x2="450" y2="300" stroke="#e2e8f0" stroke-width="1.4"/>
  ...
  <text x="240" y="266" text-anchor="middle" font-size="13" fill="#475569">보내고 답이 오면 끝.</text>
  <text x="240" y="288" text-anchor="middle" font-size="13" fill="#475569">또 궁금하면 다시 보냅니다.</text>
  ...
  <text x="690" y="266" text-anchor="middle" font-size="13" fill="#475569">한 번 연결하면 끊지 않고,</text>
  <text x="690" y="288" text-anchor="middle" font-size="13" fill="#475569">서버가 먼저 말합니다.</text>
</svg>
</div>
...
:::term-box
**WebSocket이란?** 클라이언트와 서버가 한 번 연결을 맺으면 그 연결을 끊지 않고 유지하는 통신 방식입니다. 연결이 살아 있는 동안에는 서버가 요청을 기다릴 필요 없이 클라이언트에 데이터를 보낼 수 있습니다. 상태가 바뀌면 그 즉시 알림이 전달됩니다.
:::
...
이제부터는 접수하면 배달이 **PENDING**으로 남고, 배달 기사가 전달한 뒤에야 **COMPLETED**가 됩니다. 배달이 만들어진 뒤 완료되기까지를 순서대로 봅니다.
...
<svg viewBox="260 30 1320 560" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="680" y="50" text-anchor="middle" font-size="16" font-weight="700" fill="#0f172a">1단계 — 배달 생성 이벤트 발행 → 오케스트레이터 수신 후 대기</text>
  ...
  <text x="1146" y="128" font-size="20" font-weight="700" fill="#0f172a">1. 배달 생성 이벤트 발행</text>
  <text x="1146" y="162" font-size="18" fill="#475569">배달 서비스가 배달을</text>
  <text x="1146" y="189" font-size="18" fill="#475569">PENDING으로 만든 뒤 발행합니다.</text>
  <text x="1146" y="240" font-size="20" font-weight="700" fill="#0f172a">2. 수신 후 대기</text>
  <text x="1146" y="274" font-size="18" fill="#475569">오케스트레이터가 받지만</text>
  <text x="1146" y="301" font-size="18" fill="#475569">주문 완료 명령을 발행하지 않고</text>
  <text x="1146" y="328" font-size="18" fill="#475569">배달 완료를 기다립니다.</text>
</svg>
</div>
...
<svg viewBox="260 30 1320 560" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="680" y="50" text-anchor="middle" font-size="16" font-weight="700" fill="#0f172a">2단계 — 배달 완료 API → 배달 완료 이벤트 발행 → 오케스트레이터 수신</text>
  <rect x="345" y="64" width="170" height="70" rx="8" fill="#fff" stroke="#cbd5e1" stroke-width="1.4"/>
  <text x="430" y="94" text-anchor="middle" font-size="16" font-weight="700" fill="#94a3b8">Order</text>
  <text x="430" y="116" text-anchor="middle" font-size="12" fill="#cbd5e1">:8081 주문</text>
  <rect x="575" y="64" width="170" height="70" rx="8" fill="#eef2ff" stroke="#4f46e5" stroke-width="1.8"/>
  ...
  <text x="587" y="429" text-anchor="start" font-size="13" font-weight="700" fill="#3730a3">4. 오케스트레이터가 수신</text>
  <rect x="1090" y="40" width="460" height="510" rx="10" fill="#fffdf5" stroke="#e7d9a8" stroke-width="1.5"/>
  <line x1="1132" y1="40" x2="1132" y2="550" stroke="#f0e6c8" stroke-width="1.3"/>
  <text x="1146" y="84" font-size="22" font-weight="700" fill="#9a3412">2단계</text>
  <text x="1146" y="124" font-size="18" font-weight="700" fill="#0f766e">배달 완료 API</text>
  <text x="1146" y="152" font-size="18" fill="#475569">배달 기사가 배달 완료를 처리하면</text>
  <text x="1146" y="179" font-size="18" fill="#475569">배달이 COMPLETED가 됩니다.</text>
  <text x="1146" y="230" font-size="20" font-weight="700" fill="#0f172a">3. 배달 완료 이벤트 발행</text>
  <text x="1146" y="264" font-size="18" fill="#475569">배달 서비스가</text>
  <text x="1146" y="291" font-size="18" fill="#475569">배달 완료 이벤트를 발행합니다.</text>
  <text x="1146" y="342" font-size="20" font-weight="700" fill="#0f172a">4. 오케스트레이터가 수신</text>
  <text x="1146" y="376" font-size="18" fill="#475569">그 토픽을 구독하고 있다가</text>
  <text x="1146" y="403" font-size="18" fill="#475569">메시지를 받습니다.</text>
</svg>
</div>
...
<svg viewBox="260 30 1320 560" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="1146" y="128" font-size="20" font-weight="700" fill="#0f172a">5. 주문 완료 명령 발행</text>
  <text x="1146" y="162" font-size="18" fill="#475569">오케스트레이터가</text>
  <text x="1146" y="189" font-size="18" fill="#475569">주문 완료 명령을 발행합니다.</text>
  <text x="1146" y="240" font-size="20" font-weight="700" fill="#0f172a">6. 주문 서비스가 수신</text>
  <text x="1146" y="274" font-size="18" fill="#475569">주문 서비스가 주문을</text>
  <text x="1146" y="301" font-size="18" fill="#475569">COMPLETED로 바꿉니다.</text>
  <text x="1146" y="345" font-size="18" font-weight="700" fill="#6d28d9">WebSocket Push</text>
  <text x="1146" y="377" font-size="18" fill="#475569">사용자 화면에 즉시 알립니다.</text>
</svg>
</div>
...
이제 생성과 완료를 분리합니다. 배달을 만들 때 완료까지 하던 부분을 없애면, 배달은 PENDING으로만 남습니다.
...
이제 완료는 배달 기사가 호출할 때 일어납니다. 완료 요청을 받는 입구부터 만듭니다.

## 5.4 배달 서비스 - 배달 완료 API

배달 기사의 완료 요청은 컨트롤러로 들어와 서비스를 거쳐 완료 이벤트로 나갑니다. 들어오는 순서대로, 입구인 컨트롤러부터 만듭니다.

### 5.4.1 컨트롤러·포트·이벤트 발행

요청을 받는 컨트롤러, 호출이 거치는 포트, 완료 이벤트를 발행하는 코드는 패턴이 단순해 표로 정리합니다. 전체 코드는 깃헙 레포에서 확인합니다.

| 파일 | 역할 |
|------|------|
| **web/DeliveryController.java** | 배달 완료 API 요청을 받아 처리로 넘김 |
| **usecase/CompleteDeliveryUseCase.java** | 배달 완료 기능을 정의한 인터페이스 |
| **adapter/producer/DeliveryEventProducer.java** | 배달 완료를 카프카로 알림 (앞 챕터와 같은 방식) |

### 5.4.2 서비스 - 완료 처리

배달 기사가 호출하면 배달을 완료합니다. `usecase/DeliveryService.java`에 아래 메서드를 추가합니다.
...
배달 기사가 완료를 호출하면 배달이 완료되고 완료 이벤트가 나갑니다. 이제 오케스트레이터가 완료 이벤트를 받도록 수정합니다.

## 5.5 오케스트레이터 - 배달 완료 이벤트 처리 추가

오케스트레이터는 배달 생성 결과를 받던 흐름을 멈추고, 배달 완료를 받아 주문을 끝내도록 바뀝니다.
...
```java [실습 3] handler/OrderOrchestrator.java. deliveryCreated - 성공 시 발행 중단
...
public void deliveryCreated(DeliveryCreatedEvent event) {
    ...
    // 성공
    // 삭제: complete-order-command 발행  ← 전에는 여기서 주문 완료 명령을 보냈다
    states.remove(orderId);
}
```

실패 처리는 앞과 같습니다. 달라진 곳은 성공 분기뿐입니다.

### 5.5.2 deliveryCompleted 추가 - 배달 완료 시 주문 완료

`handler/OrderOrchestrator.java`에 `deliveryCompleted` 리스너를 추가합니다.

```java [실습 4] handler/OrderOrchestrator.java. deliveryCompleted - 주문 완료 명령 발행
...
```java [실습 5] core/config/WebSocketConfig.java. STOMP WebSocket 설정
@Configuration
...
```java [실습 6] usecase/OrderService.java. completeOrder + WebSocket Push
...
| **JwtAuthenticationFilter.java** | WebSocket 연결 경로를 로그인 검사에서 제외 |
| **frontend·gateway nginx.conf** | WebSocket 연결이 끊기지 않도록 헤더 추가 |
| **frontend/index.html** | 서버 알림을 받아 화면에 주문 완료를 표시 |
...
*그림 5-8. 브라우저에서 index.html 접속 화면*
...
*그림 5-10. 브라우저 Console에서 WebSocket 연결 확인*
...
*그림 5-13. 배달 완료 API 호출 결과 - COMPLETED 상태*
...
*그림 5-15. WebSocket 알림 수신 - 클라이언트 화면에 주문 완료 표시*
...
*그림 5-16. 브라우저 Console에서 WebSocket 응답 메시지 로그 확인*
...
- **delivery-completed 토픽 추가**: 배달 서비스가 완료 이벤트를 발행하고 오케스트레이터가 받아 주문 완료 명령을 발행합니다. 이로써 챕터 5에서 실제 사용하는 Kafka 토픽은 9개가 됩니다.
- **오케스트레이터 변경**: 배달 생성(`delivery-created`) 성공 후 대기, 배달 완료(`delivery-completed`) 수신 후 주문 완료 처리. 실패 시에는 재고를 복구합니다.
- **WebSocket Push**: 주문 서비스가 `SimpMessagingTemplate`으로 사용자별 채널에 실시간 알림을 보냅니다.
:::

챕터 1에서 하나의 모놀리식 서비스로 시작했습니다. 챕터 2에서 서비스를 나눴고, 챕터 3에서 클린 아키텍처와 Kubernetes로 운영 환경을 갖췄습니다. 챕터 4에서 Kafka로 비동기 통신과 실패 대응을 완성했고, 이 챕터에서 사용자에게 결과를 즉시 전달하는 실시간 알림을 더했습니다. 동기에서 비동기로, 하나에서 여럿으로. 코드를 한 줄씩 바꾸다 보면 MSA의 구조가 보입니다.
