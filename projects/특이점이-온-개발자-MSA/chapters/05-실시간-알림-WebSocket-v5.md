...
| **frontend/nginx.conf**, **gateway/nginx.conf** | WebSocket 연결을 끊지 않도록 `/api/ws/` 요청에 **업그레이드 헤더** 추가 |
...
*직접 주문을 넣어봤습니다. 화면에 "처리 중"이 표시되었고, 배달 완료 신호를 보내자 1초도 안 되어 화면이 저절로 바뀌었습니다. "주문 완료!" 새로고침을 누르지 않았는데도, 끝났다는 사실이 먼저 와 있었습니다.*

완성된 시스템을 동료에게 보여줬습니다. 같은 화면, 같은 주문, 같은 배달이었지만 이번에는 다음 날 책이 도착할 때까지 동료가 주문 내역을 다시 열어 보지 않았습니다.

**동료**: "가만히 있었는데 화면이 알아서 바뀌네요. 이제 안 들여다봐도 되겠어요."

**오픈이**: "처음엔 '처리 중'에서 안 끝나는 화면을 어디부터 손대야 할지 막막했는데, 생성과 완료를 나누고 알림까지 붙이니 이렇게 되네요."

**선배**: "거짓 완료를 진짜 완료로 바꾼 것도 막막하던 데서 한 단계씩 풀렸잖아요. 다음엔 더 빨리 보일 거예요."
...
처음엔 하나의 모놀리식 서비스로 시작했습니다. 서비스를 나눴고, 클린 아키텍처와 Kubernetes로 운영 환경을 갖췄습니다. Kafka로 비동기 통신과 실패 대응을 완성했고, 이 챕터에서 사용자에게 결과를 즉시 전달하는 실시간 알림을 더했습니다. 동기에서 비동기로, 하나에서 여럿으로. 코드를 한 줄씩 바꾸다 보면 MSA의 구조가 보입니다.
