...
Minikube 내부에 이미지를 빌드합니다. 챕터 4 대비 frontend 서비스가 새로 추가됩니다.
...
그때는 답을 몰랐습니다. 지금은 압니다. 회원도, 상품도, 주문도, 배달도 각자의 서비스로 떨어져 나왔고, 하나가 흔들려도 나머지는 멈추지 않습니다. Kafka에 쌓인 메시지는 사라지지 않고, 오케스트레이터가 실패한 주문을 되돌리며, 완료된 주문은 사용자 화면에 바로 표시됩니다.
