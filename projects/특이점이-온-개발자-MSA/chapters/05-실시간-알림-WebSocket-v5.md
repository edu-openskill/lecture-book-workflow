...
사용자가 끝났는지 직접 확인해야 했던 그 방식을 짚어 보겠습니다.

### 5.2.1 폴링 vs 푸시

서버에 생긴 변화를 알아내는 방법은 크게 두 가지입니다.

택배가 왔는지 현관문을 5분마다 직접 열어 확인하는 것처럼, 클라이언트가 서버에 "완료됐나요?"를 일정 간격으로 반복해서 묻는 방법이 **폴링(Polling)** 입니다. 변화가 없어도 요청은 계속 나가므로 대부분이 헛걸음입니다.
...
동료가 주문 내역을 몇 번이고 다시 열어 봐야 했던 건 화면에 초인종이 없었기 때문입니다. 서버가 먼저 보내는 이 푸시를 구현하는 기술이 **WebSocket**입니다.
...
사용자에게 진짜로 끝났다고 알리려면, 먼저 배달이 진짜 끝나는 지점이 코드에 있어야 합니다. 그 지점부터 만듭니다.

## 5.3 배달 완료 라이프사이클 설계

지금까지는 배달이 생성되는 즉시 완료로 처리됐습니다. 현실에서는 배달 기사가 물건을 전달해야 완료인데, 그 단계가 코드에 없습니다.

이제부터는 접수하면 배달이 "배달 중(PENDING)"으로 남고, 배달 기사가 전달한 뒤 완료 처리를 해야 비로소 "COMPLETED"가 됩니다. 그 완료 신호가 주문을 끝내고 사용자 화면까지 닿는 과정을 한 단계씩 따라가 보겠습니다.
...
<svg viewBox="260 30 1320 560" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="1146" y="162" font-size="18" fill="#475569">delivery-service가 배달을</text>
  ...
  <text x="1146" y="274" font-size="18" fill="#475569">오케스트레이터가 받지만 주문</text>
  <text x="1146" y="301" font-size="18" fill="#475569">완료 명령을 발행하지 않고</text>
  <text x="1146" y="328" font-size="18" fill="#475569">배달 완료를 기다립니다.</text>
  <text x="1146" y="372" font-size="15" fill="#b91c1c">이전에는 여기서 곧바로</text>
  <text x="1146" y="394" font-size="15" fill="#b91c1c">주문 완료를 발행했습니다.</text>
</svg>
</div>

*그림 5-3. 1단계 - 배달 생성 이벤트 발행 → 오케스트레이터 수신 후 대기*
...
<svg viewBox="260 30 1320 560" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="680" y="50" text-anchor="middle" font-size="16" font-weight="700" fill="#0f172a">2단계 — 배달 완료 API(REST) → 배달 완료 이벤트 발행 → 오케스트레이터 수신</text>
  <rect x="345" y="64" width="170" height="70" rx="8" fill="#fff" stroke="#cbd5e1" stroke-width="1.4"/>
  <text x="430" y="94" text-anchor="middle" font-size="16" font-weight="700" fill="#94a3b8">Order</text>
  <text x="430" y="116" text-anchor="middle" font-size="12" fill="#cbd5e1">:8081 주문</text>
  <rect x="575" y="64" width="170" height="70" rx="8" fill="#eef2ff" stroke="#4f46e5" stroke-width="1.8"/>
  ...
  <text x="775" y="56" text-anchor="middle" font-size="11" font-weight="700" fill="#0f766e">PUT /complete</text>
  <rect x="280" y="250" width="780" height="120" rx="9" fill="#fff4ed" stroke="#ff7849" stroke-width="2"/>
  <rect x="280" y="250" width="780" height="22" fill="#ff7849"/>
  <text x="670" y="265" text-anchor="middle" font-size="11" font-weight="700" fill="#fff">Kafka — 토픽별로 메시지를 보관·전달</text>
  ...
  <text x="1146" y="124" font-size="18" font-weight="700" fill="#0f766e">배달 완료 API (REST)</text>
  <text x="1146" y="152" font-size="18" fill="#475569">배달 기사가 PUT /complete를</text>
  <text x="1146" y="179" font-size="18" fill="#475569">호출해 배달을 COMPLETED로 전이합니다.</text>
  <text x="1146" y="230" font-size="20" font-weight="700" fill="#0f172a">3. 배달 완료 이벤트 발행</text>
  <text x="1146" y="264" font-size="18" fill="#475569">delivery-service가</text>
  <text x="1146" y="291" font-size="18" fill="#475569">delivery-completed를 발행합니다.</text>
...
*그림 5-4. 2단계 - 배달 완료 API → 배달 완료 이벤트 발행 → 오케스트레이터 수신*
...
<svg viewBox="260 30 1320 560" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="1146" y="189" font-size="18" fill="#475569">complete-order-command를 발행합니다.</text>
  <text x="1146" y="240" font-size="20" font-weight="700" fill="#0f172a">6. 주문 서비스가 수신</text>
  <text x="1146" y="274" font-size="18" fill="#475569">order-service가 주문을</text>
  ...
  <text x="1146" y="377" font-size="18" fill="#475569">/topic/orders/{userId}로</text>
  <text x="1146" y="404" font-size="18" fill="#475569">사용자 화면에 즉시 알립니다.</text>
</svg>
</div>
...
이제부터는 배달 생성(`delivery-created`)이 성공해도 바로 완료하지 않고 대기합니다. 이후 배달 완료(`delivery-completed`) 이벤트를 받았을 때 비로소 주문 완료 명령(`complete-order-command`)을 발행합니다.

### 5.5.1 deliveryCreated 수정 - 성공 시 대기

지금까지는 배달 생성이 성공하면 곧바로 주문 완료 명령(`complete-order-command`)이 발행됐습니다. 이제는 배달 완료를 기다리며 아무것도 발행하지 않습니다.
...
실패 처리는 앞과 동일합니다. 핵심 변경은 성공 시입니다. 전에는 배달이 생성되면 곧바로 주문 완료 명령(`complete-order-command`)이 발행됐지만, 이제는 아무것도 하지 않고 배달기사의 완료 API 호출을 기다립니다.
...
*그림 5-7. 브라우저에서 index.html 접속 화면*
...
*그림 5-9. 브라우저 Console에서 WebSocket 연결 확인*
...
*그림 5-12. 배달 완료 API 호출 결과 - COMPLETED 상태*
...
*그림 5-14. WebSocket 알림 수신 - 클라이언트 화면에 주문 완료 표시*
...
*그림 5-15. 브라우저 Console에서 WebSocket 응답 메시지 로그 확인*
