...
- 비즈니스 규칙을 도메인 객체에 캡슐화하는 **DDD**를 적용할 수 있습니다.
...
오픈이가 `OrderService`의 주문 취소 메서드를 열었습니다. 30분 검증을 어디에 끼울까요. 코드를 훑어보니 **같은 종류의 유효성 체크가 여기저기 흩어져 있습니다**. `ProductService`에는 재고 부족·가격 일치 체크가, `DeliveryService`에는 주소 빈 값·이미 취소된 배달 체크가 들어 있습니다. 새 규칙이 들어올 때마다 어느 클래스의 어느 메서드에 추가할지 매번 고민입니다.

동료가 옆에서 코드를 봤습니다.

**동료**: "근데 이상하지 않아? 지금은 `Order`가 그냥 데이터 덩어리고 `OrderService`가 비즈니스 로직을 직접 검증하잖아. `Order`가 자기 일이니까 직접 검증해야 하는 거 아니야?"

오픈이가 멈칫했습니다. 그 말이 맞습니다. 게다가 컨트롤러가 `OrderService` 구현체를 직접 들고 있어서, 환경에 따라 다른 구현체로 갈아끼우기도 어렵습니다.

선배 자리로 갔습니다.

**선배**: "그건 `OrderService`가 너무 많은 걸 안고 있어서 그래요. **DDD**로 비즈니스 규칙을 `Order` 객체 안에 모으고, **클린 아키텍처**로 컨트롤러가 구현체 대신 약속(UseCase 인터페이스)에만 의존하게 만들면 풀려요."

*규칙은 `Order` 안으로, 컨트롤러는 인터페이스에.*
...
<svg viewBox="0 0 1200 580" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="챕터 3 한눈에 보기: 외부 클라이언트가 Ingress와 Gateway를 거쳐 User 또는 Order 서비스를 호출하고, Order가 Product와 Delivery를 동기 호출하는 흐름. 응답은 역순으로 박스를 통과. 모든 서비스는 Kubernetes 클러스터로 묶여 있음">
  ...
  <text x="219" y="125" text-anchor="middle" font-size="13" font-weight="600" fill="#4f46e5">① 요청</text>
  <line x1="400" y1="132" x2="478" y2="132" stroke="#4f46e5" stroke-width="1.6" marker-end="url(#c3f1-i)"/>
  <text x="439" y="125" text-anchor="middle" font-size="13" font-weight="600" fill="#4f46e5">② 라우팅</text>
  <line x1="620" y1="132" x2="698" y2="132" stroke="#4f46e5" stroke-width="1.6" marker-end="url(#c3f1-i)"/>
  <text x="659" y="125" text-anchor="middle" font-size="13" font-weight="600" fill="#4f46e5">③ 로그인</text>
  <line x1="698" y1="172" x2="622" y2="172" stroke="#3730a3" stroke-width="1.6" stroke-dasharray="4,3" marker-end="url(#c3f1-i)"/>
  <text x="660" y="185" text-anchor="middle" font-size="13" font-weight="600" fill="#3730a3">④ 응답</text>
  <line x1="478" y1="172" x2="402" y2="172" stroke="#3730a3" stroke-width="1.6" stroke-dasharray="4,3" marker-end="url(#c3f1-i)"/>
  <text x="440" y="185" text-anchor="middle" font-size="13" font-weight="600" fill="#3730a3">⑤ 응답</text>
  <line x1="258" y1="172" x2="182" y2="172" stroke="#3730a3" stroke-width="1.6" stroke-dasharray="4,3" marker-end="url(#c3f1-i)"/>
  <text x="220" y="185" text-anchor="middle" font-size="13" font-weight="600" fill="#3730a3">⑥ JWT 응답</text>
...
도입부에서 본 흩어진 유효성 체크를 가게에 비유하면 이렇습니다. 손님이 "환불 돼요?" 물으면 점원이 두꺼운 매뉴얼북을 펴서 한 줄씩 확인합니다. "이미 취소됐나? 30분 지났나? 결제는 됐나?" 매뉴얼이 두꺼워질수록 점원의 부담은 늘어납니다. 새 규칙(30분 제한)이 들어오면 매뉴얼이 한 줄 더 늘 뿐, 가게는 여전히 점원 한 명에게 모든 부담이 쏠려 있습니다.

해결은 가게에 사장을 합류시키는 일입니다. 점원은 사장에게 묻기만 하고, 사장이 자기 가게 사정을 알고 즉답합니다. 새 규칙이 들어오면 사장의 머릿속만 바꾸면 됩니다. 점원은 손님 응대에만 집중합니다.

<!-- image-prompt: Minimal black line drawing on white background, split comparison, 4:3, 800x600px. Vertical line in middle. Left "DDD 아닐 때 - 점원과 매뉴얼": clerk with thick manual book pointing at if lines, customer waiting. Right "DDD - 점원과 사장": same clerk without book gesturing to shop owner with "사장" name tag, owner speech bubble "환불 가능합니다!", customer relieved. Cartoon textbook style. -->
![](assets/CH03/gemini/01_clerk-vs-owner.png)
*그림 3-2. 점원과 매뉴얼에서 점원과 사장으로*

코드로 옮기면, 점원은 `OrderService`이고 사장은 `Order` (도메인 객체)입니다. 흩어진 유효성 체크가 도메인 메서드로 모입니다. **도메인 객체가 자기 비즈니스 로직을 직접 검증하게 만드는 설계**, 이것이 DDD입니다.

도메인이 자기 일을 알게 된 뒤에도 한 가지가 남습니다. 컨트롤러가 `OrderService` 구현체에 직접 묶여 있는 문제입니다. 다음 절에서 풀어 보겠습니다.

## 3.2 UseCase 인터페이스 - 어댑터 두기

`Order`가 자기 일을 답하게 됐어도 컨트롤러가 `OrderService` 구현체를 직접 들고 있는 한 문제가 남습니다. 구현체가 바뀌면 컨트롤러까지 같이 바꿔야 하고, 단위 테스트할 때도 가짜 구현체로 갈아끼우기가 어렵습니다.

전원 어댑터를 떠올려 보세요. 노트북을 한국·일본·유럽 어디서 쓰든 본체는 그대로입니다. 본체에 콘센트 모양에 맞는 어댑터만 끼우면 충전이 됩니다. 만약 본체가 콘센트 모양에 직접 맞춰져 있다면, 나라가 바뀔 때마다 노트북 자체를 바꿔야 합니다.

컨트롤러를 노트북처럼 만드는 것이 클린 아키텍처의 방향입니다. 컨트롤러는 어댑터(약속)만 보고, 그 뒤에 어떤 구현체가 꽂혀 있는지는 몰라도 됩니다. 이 약속이 **UseCase 인터페이스**이고, 약속에만 의존하게 하는 방식이 **클린 아키텍처(Clean Architecture)** 의 의존성 역전입니다.
...
*그림 3-3. UseCase 인터페이스 - 전원 어댑터 비유*

어댑터를 코드로 옮기면, 가운데 어댑터가 `CreateOrderUseCase`(UseCase 인터페이스)이고 그 뒤에 꽂히는 플러그가 `OrderService`의 여러 구현체입니다. 컨트롤러는 인터페이스만 알고, 어떤 구현체가 주입됐는지는 신경 쓰지 않습니다.
...
UseCase 인터페이스가 왜 필요한지 이해했으니, 이제 실제 코드에 적용해보겠습니다. 패키지 구조부터 바꾸겠습니다. 챕터 2의 단순 레이어드 구조에서 `domain` 패키지를 안쪽에 두는 구조로 바꿉니다.
...
`domain` 패키지가 안쪽에 자리잡고, 외부(`web`·`adapter`·`repository`)가 그 바깥에서 도메인에 맞춰지는 구조입니다.
...
| 서비스 | UseCase 책임 | 도메인 검증 |
|---|---|---|
| **product** | 상품 조회, 재고 증감 | 재고 충분성·가격 일치 |
| **delivery** | 배달 생성·조회·취소 | 주소 유효성·취소 가능 여부 |
| **user** | 로그인, 사용자 조회 | 비밀번호 일치 |
...
| 리소스 | 파일 | 역할 |
|---|---|---|
| **ConfigMap** | `order-configmap.yml` | 일반 환경변수(DB 주소 등)를 외부에서 주입합니다. |
| **Secret** | `order-secret.yml` | 민감 정보(DB 계정·비밀번호)를 분리해 안전하게 관리합니다. |
| **Deployment** | `order-deploy.yml` | Pod 실행 정의. 이미지·포트·환경변수 묶음을 명시합니다. |
| **Service** | `order-service.yml` | Pod에 고정 DNS를 부여하여 클러스터 내부 통신을 제공합니다. |
| **Ingress** | `gateway-ingress.yml` | 외부 요청을 gateway-service로 라우팅합니다. |
...
오픈이가 답을 못 했습니다. K8s가 30초 만에 다시 띄워준다고 들었으니 괜찮을 것 같기도 하고, 그 30초 동안 들어온 요청이 어떻게 될지는 아직 떠올려 본 적이 없었습니다.
...
- **DDD**: 비즈니스 규칙을 도메인 객체에 캡슐화. `Order.validateCancelable()`·`Order.cancel()`로 도메인이 자기 일을 안다.
