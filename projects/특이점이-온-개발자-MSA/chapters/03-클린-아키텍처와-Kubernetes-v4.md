...
USB 허브가 `CreateOrderUseCase` 같은 **UseCase 인터페이스**이고, 그 허브에 꽂히는 USB 장치들이 `OrderService`의 다양한 코드입니다. 컨트롤러는 인터페이스만 알고, 어떤 코드가 주입됐는지는 신경 쓰지 않습니다.
...
`OrderService` 코드가 아닌 인터페이스를 주입받도록 컨트롤러를 수정합니다. 앞으로 `OrderService`를 다른 코드로 바꿔도 이 컨트롤러는 전혀 수정하지 않아도 됩니다. API는 챕터 2와 동일합니다(POST 생성, GET 조회, PUT 취소).
...
**선배**: "Kubernetes는 원하는 상태를 적어두면, 알아서 그 상태를 유지해줘요. 서비스가 죽어도 자동으로 다시 띄워줍니다."
...
- **MySQL + 프로파일 분리**로 개발은 H2, 운영은 MySQL을 씁니다. 환경변수로 설정을 주입합니다.
