...
배포 다음 날 아침, 모니터에 알림이 울렸습니다. 상품 서버가 잠깐 죽은 탓이었습니다. 에러 로그가 쌓이다가, 쿠버네티스가 상품 서버를 복구하자 멈췄습니다.

문제는 그 사이에 들어온 주문이 전부 실패한 것입니다. 주문 서버가 상품 서버를 호출했지만, 상품 서버가 죽어 있어 주문 실패가 발생했습니다.

*서로 직접 호출하니까, 하나가 죽으면 호출한 쪽도 같이 실패하는구나.*

상황을 확인한 선배가 다가왔습니다.

**선배**: "이거 어제 얘기했던 상황이네요. 동기적으로 직접 호출하니까 그래요. 호출한 쪽은 상대가 응답할 때까지 기다리는데, 상대가 잠깐 죽으면 호출한 쪽도 같이 멈춰요. 이럴 땐 호출을 기다리지 말고 비동기적으로 메시지를 주고받아야 해요."

**오픈이**: "메시지요? 받는 서비스가 죽어 있으면 그 메시지는 어떻게 돼요?"

**선배**: "메시지는 중간에 쌓여 있다가, 받는 서비스가 살아나면 그때 가져가서 처리해요. 보내는 쪽은 응답을 기다리지 않으니 그대로 자기 일을 이어 가죠. 그게 동기 호출과 비동기 메시지의 차이예요."
...
<svg viewBox="0 0 1200 880" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="챕터 4 한눈에 보기: 챕터 3과 동일하게 Client는 클러스터 밖에 있고, 1단계 로그인은 Client가 Ingress·Gateway를 거쳐 User에 로그인하고 JWT를 응답받는다. 2단계 주문은 Client가 Ingress·Gateway를 거쳐 Order에 동기 REST로 주문하고 즉시 PENDING을 응답받는다. 그 뒤 Order·Product·Delivery 세 서비스는 가운데 Orchestrator와 event·command를 주고받고, Orchestrator가 그 모든 메시지를 아래 Kafka 토픽으로 비동기 전달한다. 서비스끼리는 직접 호출하지 않는다.">
  ...
  <text x="650" y="603" text-anchor="middle" font-size="11" font-weight="700" fill="#fff">Kafka — 모든 메시지가 토픽을 거쳐 비동기적으로 전달</text>
...
- Kafka 이벤트로 연결해 서비스 간 결합을 끊는 이유를 이해할 수 있습니다.
- 오케스트레이터가 흐름을 조율하고 실패를 자동 보상하는 Saga를 이해할 수 있습니다.
:::
...
카운터 앞에 서서 커피를 주문하면, 나올 때까지 그 앞에서 기다립니다. 내 커피가 나와야 다음 손님이 주문할 수 있습니다. 만약 커피 머신이 고장 나면 뒤에 줄 선 손님 전부가 기다려야 합니다. 이렇게 요청을 보낸 쪽이 응답이 올 때까지 기다리는 방식이 **동기(synchronous)** 호출입니다.

반대로 커피 주문 후 진동벨을 받으면 자리에 앉아 다른 일을 할 수 있습니다. 커피가 완성되면 벨이 울립니다. 뒤에 줄 선 손님도 곧바로 주문할 수 있습니다. 커피 머신이 잠깐 멈춰도 주문은 그대로 쌓여 있다가, 머신이 다시 돌아가면 밀린 주문부터 이어서 만듭니다. 이렇게 요청을 보낸 쪽이 기다리지 않고 결과가 준비되면 따로 받는 방식이 **비동기(asynchronous)** 호출입니다.
...
그렇다면 MSA 구조에서 비동기 메시지는 어떤 방식으로 주고받을까요?
...
- **RabbitMQ 같은 큐와의 차이**: 전통적인 메시지 큐는 컨슈머가 메시지를 가져가 처리하면 큐에서 사라집니다. Kafka는 읽어도 남아 있다는 점이 가장 큰 차이입니다.
:::

**오픈이**: "각 서비스가 서로 메시지를 발행하고 구독하면서 알아서 자기 할 일을 처리하는 건가요?"

**선배**: "두 가지 방식이 있어요. 방금 말한 것처럼 각 서비스가 서로 발행하고 구독하면서 다음 단계를 스스로 이어 갈 수도 있고, 전체 흐름을 관리하는 **지휘자**를 둘 수도 있어요. **서비스끼리 주고받으면** 중간에 문제가 생겼을 때 어디까지 됐는지 알기 어려워요. 그래서 **지휘자를 두면**, 한 단계가 실패해도 어디까지 진행됐는지 알고 있으니까 **이미 끝난 단계만 복구**할 수 있어요."


## 4.3 Orchestration Saga - 지휘자가 흐름을 조율하다

하나의 주문을 여러 서비스가 단계별로 나눠 처리하다 보면, 중간 한 단계가 실패해도 앞 단계는 이미 끝나 있습니다. 이때 끝난 단계를 취소하는 **보상**으로 데이터를 맞추는 방식을 **Saga 패턴**이라고 합니다.
...
이번 챕터에서는 조율 역할만 전담하는 **별도 orchestrator 서비스**를 두는 구조를 살펴보겠습니다. 주문 요청이 들어오면 orchestrator가 재고 차감, 배달 생성, 주문 완료 단계를 순차적으로 지휘합니다.
...
이 챕터에서는 주문 흐름에 총 8개의 토픽을 사용합니다. 토픽은 orchestrator가 서비스에 일을 시키는 **명령**과, 서비스가 결과를 돌려주는 **이벤트** 두 종류입니다.
...
서비스들이 orchestrator를 사이에 두고 메시지를 주고받기 때문에, 주문 요청이 들어오면 주문 서비스는 우선 **PENDING** 상태로 응답합니다. 이후 orchestrator가 아래 단계를 차례로 진행해 모두 성공하면 주문 상태가 **COMPLETED**로 바뀝니다.
...
만약 상품 서비스가 재고 차감에 실패하면 재고 차감 실패 이벤트를 발행합니다. 오케스트레이터는 **이미 처리된 단계만** 역순으로 되돌립니다.
...
<svg viewBox="260 30 1320 560" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="보상 주문 취소: 오케스트레이터가 주문 취소 명령을 발행하고 주문 서비스가 CANCELLED 처리. 앞 편지들은 비활성으로 남고 주문 취소 명령만 활성이다.">
  ...
  <text x="752" y="429" text-anchor="start" font-size="13" font-weight="700" fill="#4f46e5">3. 주문 취소 명령 발행</text>
  <line x1="730" y1="248" x2="430" y2="136" stroke="#3730a3" stroke-width="2.6" stroke-dasharray="5,3" marker-end="url(#rb2b)"/>
  <text x="605" y="196" text-anchor="start" font-size="13" font-weight="700" fill="#3730a3">4. 주문 서비스가 수신해 CANCELLED 처리</text>
...
챕터 3에서 6개 서비스를 Kubernetes에 올렸습니다. Kafka를 도입하면서 추가되는 건 **Kafka 서버**와 **orchestrator 서버**입니다.
...
모든 서비스는 Kafka 서버의 주소 `kafka-service:9092`로 접근합니다. 각 서비스는 ConfigMap에 접속할 주소를, Kafka 서버는 자기 주소를 지정하는데, 두 값이 같아야 연결됩니다.
...
Kafka 서버는 두 가지 일을 합니다. 하나는 메시지를 받아 두고 전달하는 역할(**브로커**), 다른 하나는 브로커 목록이나 토픽 설정 같은 클러스터 정보를 관리하는 역할(**컨트롤러**)입니다. 예전에는 이 관리 역할을 **ZooKeeper**라는 별도 서비스에 맡겨야 했지만, **KRaft(Kafka Raft)** 모드는 Kafka가 직접 맡습니다. 그래서 ZooKeeper 없이 Kafka 컨테이너 하나가 브로커와 컨트롤러 역할을 모두 처리합니다. 운영 환경에서는 브로커와 컨트롤러를 여러 노드로 나눠 안정성을 높입니다.
:::
...
**오픈이**: "PENDING이었다가 COMPLETED로 바뀌었어요! 그런데 중간에 실패하면 어떻게 되나요?"
...
- **Saga 패턴**으로 분산 트랜잭션을 단계별로 처리하고, 실패한 단계는 **보상 트랜잭션**으로 되돌립니다.
- 주문 요청에는 즉시 `PENDING`으로 응답하고, 처리가 끝나면 최종 상태로 갱신됩니다.
