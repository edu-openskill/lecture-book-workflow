...
## 4.7 Outbox 패턴 - 이벤트가 사라지지 않게

**오픈이**: "그런데 한 가지 걸려요. 주문을 저장하고 Kafka로 발행하는데, 발행이 실패하면 어떻게 되죠? 주문은 DB에 남아 있는데 이벤트만 사라지는 거 아닌가요?"

**선배**: "정확히 봤어요. 그냥 넘기면 안 되는 부분이에요. 그걸 막는 게 **Outbox 패턴**이에요."

MSA에서는 보통 DB 변경과 메시지 발행을 함께 처리합니다. 문제는 이 두 작업을 하나의 트랜잭션으로 묶을 수 없다는 점입니다. DB와 Kafka가 서로 다른 시스템이기 때문입니다. 그래서 주문 생성은 성공했는데 네트워크 오류나 서버 다운으로 발행만 실패하면, 오케스트레이터가 주문 생성을 알지 못해 상품·배달 처리가 시작되지 않고, 데이터가 어긋납니다.
...
그다음 **발송기(Relay)** 가 outbox 테이블을 주기적으로 확인합니다. 아직 발행하지 않은 메시지가 있으면 Kafka로 발행한 뒤, outbox 테이블에서 그 메시지를 보냄으로 표시합니다. 덕분에 Kafka가 잠시 멈춰도 메시지는 outbox에 남아 있다가 다음 차례에 다시 발행됩니다.

:::tip
**Outbox만으로 충분할까?**

Outbox 패턴은 이벤트 유실을 막아주지만, 중복 발송까지 막지는 못합니다. 발송기가 이벤트를 보낸 직후 완료 처리를 하기 전에 멈추면, 다음 차례에 같은 이벤트를 또 보내기 때문입니다. 그래서 이벤트를 받는 쪽은 똑같은 요청을 여러 번 받아도 한 번만 처리한 것과 같은 결과를 내도록 방어해야 합니다. 이러한 성질을 **멱등성**이라고 합니다. 실무에서는 보통 이미 처리한 이벤트 ID를 기록해 두고, 같은 ID가 다시 들어오면 가볍게 건너뛰는 방식으로 구현합니다.
