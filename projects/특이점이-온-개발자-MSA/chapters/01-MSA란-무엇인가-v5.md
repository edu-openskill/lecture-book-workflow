...
새벽 세 시. 휴대폰 알람이 울렸습니다. 가까운 데서 한 번, 멀리서 한 번. 침대 옆 협탁의 알람과 거실 노트북의 알람이 동시에 울려대고 있었습니다.

화면을 열었습니다. 모니터링 대시보드가 빨갛게 차 있었습니다. 에러 그래프가 가파른 절벽처럼 솟아 있었습니다. 주문 API가 죽었고, 그 옆에 로그인 API도 같이 죽어 있었습니다.

*주문이랑 로그인이 무슨 상관이야?*

서버 한 대를 재시작했습니다. 그래프가 잠시 떨어지더니 다시 솟았습니다. 또 한 대. 또. 마지막 한 대를 재시작하자 알람이 조용해졌습니다. 시계를 보니 새벽 네 시 반이었습니다.

월요일 아침, 팀장이 자리로 왔습니다.

**팀장**: "왜 같이 죽었어요? 주문이 몰린 건데 로그인이 왜 같이 멈춰요? 방법 좀 찾아 봐요."
...
**한 덩어리로 묶여 있어서 한 곳을 손대도 전체가 흔들립니다.** 주문 기능만 수정해도 회원·상품·배달까지 **전부 재시작**해야 하고, 배달 기능 버그가 무관한 **회원 로그인까지 느리게** 만듭니다. 주문에만 부하가 몰려도 **전체 서버를 복제**해야 합니다.
...
*그림 1-6. 동기 REST - Order가 두 서비스를 직접 호출*
