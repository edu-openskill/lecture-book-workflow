...
밤 열한 시, 휴대폰이 울렸습니다. 운영팀이었습니다.

**운영팀**: "사이트가 전부 멈췄습니다. 확인 좀 해 주세요."

잠이 확 달아났습니다. 노트북을 열자 모니터링 대시보드가 에러 로그로 가득했습니다. 저녁 일곱 시쯤 시작한 할인 이벤트로 트래픽이 몰리면서, 이를 버티지 못한 서버가 다운되어 있었습니다.

오픈이는 서버를 재시작했습니다. 서버는 트래픽이 빠지기 시작한 새벽 한 시가 되어서야 겨우 안정을 되찾았습니다.
...
- 이 책은 서비스 간 통신을 두 가지 방식으로 구현합니다. 챕터 2~3에서는 **보상 트랜잭션**으로 서비스끼리 직접 호출·보상하고, 챕터 4~5에서는 메시지로 비동기 통신합니다.
:::
