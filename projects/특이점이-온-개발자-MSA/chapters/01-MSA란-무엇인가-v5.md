...
새벽 세 시, 휴대폰이 울렸습니다. 운영팀이었습니다.

**운영팀**: "사이트가 전부 멈췄습니다. 확인 좀 해 주세요."

잠이 확 달아났습니다. 노트북을 열자 모니터링 대시보드가 에러 로그로 가득했습니다. 전날 저녁에 시작한 할인 이벤트로 트래픽이 몰리면서, 이를 버티지 못한 서버가 다운돼 있었습니다.

오픈이는 서버를 재시작했습니다. 치솟던 그래프는 트래픽이 빠지기 시작한 새벽 네 시 반이 되어서야 겨우 잦아들었습니다.

*트래픽이 몰렸다고 사이트가 통째로 죽어버리다니.*

다음 날 아침, 팀장이 자리로 왔습니다.

**팀장**: "이번처럼 트래픽 한 번에 사이트 전체가 다운되면 곤란해요. 재발 방지 대책 좀 찾아봐요."

오픈이는 바로 답을 하지 못했습니다. 새벽에 서버를 재시작해 급한 불은 껐지만, 같은 상황을 막을 근본적인 방법은 떠오르지 않았습니다. 결국 선배를 찾아갔습니다.

**오픈이**: "선배님, 이번에 트래픽이 몰렸다고 사이트가 통째로 뻗어버렸는데요. 이거 어떻게 해야 할까요?"
...
- 마이크로서비스가 역할을 나눠 그 한계를 푸는 방식을 이해할 수 있습니다.
- 서비스를 나누며 떠오르는 핵심 과제, 분산 트랜잭션을 이해할 수 있습니다.
:::
...
<!-- [FLOW CARD: ch1-arc]
사건: 새벽 3시 장애 — 트래픽 폭증으로 서버 다운, 사이트 전체가 멈춤
깨달음: 모든 기능이 한 서버에 있어서 한 번 몰리면 전체가 같이 내려간다 (모놀리식 한계)
...
소프트웨어에서도 동일하게 적용됩니다. 처음에는 하나의 서버에 모든 기능을 넣는 **모놀리식** 구조가 단순하고 빠릅니다.

### 1.1.2 성장하면서 균열이 생긴다

한 공간에 모든 매장이 모여 있는 백화점은 시간이 지나면서 문제가 보이기 시작합니다. 한 매장에 불이 나면 전 층이 함께 대피해야 합니다. 또 정전이 발생하면 건물 전체가 함께 멈춥니다.
...
모든 기능이 한 서버에서 함께 돌아가다 보니, **한 곳의 문제가 곧 전체의 문제가 됩니다.** 주문에 트래픽이 몰리면 회원·상품·배달까지 함께 느려지고, 한 기능에서 생긴 오류가 다른 기능까지 영향을 줍니다. 또 작은 수정에도 전체를 재배포해야 합니다.

*하나의 서버에 다 모여 있으니, 새벽에 서버가 전부 같이 멈춘 거였구나.*
...
**방식 1. 직접 호출로 동기적 처리 (챕터 2, 챕터 3)**

첫 번째는 각 서비스가 다른 서비스를 직접 호출하는 방식입니다. 주문 서비스가 상품 서비스에 "재고 줄여줘"라고 요청한 후 응답이 올 때까지 기다리고, 응답이 돌아오면 다시 배달 서비스를 호출하는 방식입니다. 호출자가 응답까지 멈춰 있는 동기 호출 방식이라, 한 호출이 5초가 걸리면 그만큼 응답도 지연됩니다.
...
다음은 서비스끼리 직접 호출하지 않고, 메시지로 요청을 주고받는 방식입니다. 보내는 쪽은 결과를 기다리지 않으므로(비동기), 한 서비스가 느려도 다른 서비스가 멈추지 않습니다.
...
그런데 MSA에서는 각 서비스가 **독립된 데이터베이스**를 가집니다. 각 서버는 다른 서버에서 처리한 결과를 알 수 없기 때문에, 한 트랜잭션으로 묶을 수 없습니다. 이렇게 여러 DB에 걸친 작업을 하나로 묶어야 하는 상황을 **분산 트랜잭션**이라고 합니다.
...
*재고는 줄였는데 배달이 실패하면... 재고를 다시 원래대로 되돌려야 하는데.*

**오픈이**: "선배님, 재고나 배달 상태를 되돌리는 방법은 없나요?"

**선배**: "자동으로 안 되니 직접 되돌려야 해요. 재고를 줄였으면 다시 늘리고, 배달을 만들었으면 취소하고. 한 단계씩 거꾸로 되돌리는 거예요. 보상 트랜잭션이라고 해요."
...
이 책은 하나의 시스템이 단계별로 진화하는 여정입니다. 각 챕터는 이전 챕터의 한계를 느끼는 것에서 시작합니다.
...
<svg viewBox="0 0 720 400" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="산 등반으로 본 학습 흐름. REST 보상 트랜잭션, DDD 클린 K8s, Kafka 지휘자, WebSocket Push 순으로 네 캠프를 올라 정상에서 완성한다. 각 캠프에 한 줄 요약이 있다.">
  ...
  <text x="72" y="370" font-size="11.5" fill="#6b7280">서비스 직접 연결, 실패 시 보상</text>
  ...
  <text x="266" y="255" font-size="14" font-weight="800" fill="#0f172a">DDD · 클린 + K8s</text>
  <text x="244" y="272" font-size="11.5" fill="#6b7280">구조 정리 후 K8s에 올려 운영</text>
  ...
  <text x="416" y="189" font-size="11.5" fill="#6b7280">메시지로 분리, 지휘자가 조율</text>
...
*이제 어떤 것부터 해야 할지 보이네. 하나씩 해결해 보자.*
...
- 분산 트랜잭션을 두 가지 방식으로 풉니다. 챕터 2~3에서는 **보상 트랜잭션**으로 서비스끼리 직접 호출·보상하고, 챕터 4~5에서는 메시지로 비동기 통신해 서비스를 분리합니다.

챕터 2에서는 4개의 서비스를 REST로 연결하고, 보상 트랜잭션을 구현해 봅니다. 처음에는 단순하게 시작합니다. 그 단순함이 나중에 어떤 문제를 만드는지, 챕터 2에서 직접 느껴 봅니다.
:::
