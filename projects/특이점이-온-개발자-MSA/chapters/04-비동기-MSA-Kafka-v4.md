...
배포 다음 날 오전이었습니다. 모니터링 대시보드에서 5xx 그래프가 짧게 솟구쳤습니다. product-service Pod이 OOM으로 한 번 죽었다가 K8s가 새로 띄우는 그 **30초** 사이였습니다. 챕터 3에서 그토록 자랑하던 자동 복구. 그런데 그 30초 동안 들어온 주문 요청은 전부 504로 떨어졌습니다. 재고 조회·차감 호출이 product-service에 닿지 못해서, 호출하던 order-service까지 함께 멈춘 것이었습니다.
...
며칠 뒤, 같은 동료가 다시 자리로 왔습니다.

**동료**: "어제 주문하고 끝났는지를 모르겠어서, 새로고침을 한 30번 눌렀어요. 그리고 받기도 전에 배달 상태가 '완료'더라고요. 이거 가짜 아니에요?"

폴링과 가짜 배달. 챕터 5의 사건이 또 도착했습니다.
