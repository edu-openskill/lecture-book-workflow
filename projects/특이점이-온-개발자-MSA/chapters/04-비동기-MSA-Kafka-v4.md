...
"아메리카노 하나요"라고 주문하면 진동벨을 받고 자리에 앉습니다. 바리스타는 주문을 순서대로 처리하고, 커피가 완성되면 벨이 울립니다. 손님은 기다리는 동안 다른 일을 할 수 있고, 뒤에 줄 선 손님도 바로 주문할 수 있습니다. 머신이 잠깐 멈춰도 주문 목록은 사라지지 않습니다.


아침에 겪은 그 잠깐의 멈춤이 바로 카운터 대기의 풍경이었습니다. 고장 난 머신이 상품 서버, 그 앞에 멈춰 선 손님이 주문 서버입니다. 머신이 멈추자 카운터 앞에 선 손님까지 함께 멈췄고, 그 사이 들어온 주문이 전부 실패했습니다. 진동벨 방식이라면 손님은 주문만 남기고 자리로 돌아가 있으니, 머신이 잠깐 멈춰도 주문은 그대로 남아 있다가 머신이 돌아온 뒤 처리됩니다. 동기로 서로를 직접 호출하며 응답을 기다리면 한 서비스의 멈춤이 곧 전체의 멈춤이 되지만, 비동기로 메시지를 주고받으면 그렇지 않습니다.



## 4.2 Kafka - 메시지를 전달하는 우체국

선배가 진동벨에 빗대 말한 그 방식, 주문하는 쪽은 메시지만 남기고 돌아가고 담당자는 자기 차례에 쌓인 메시지를 꺼내 처리하는 그 비동기 통신을 실제로 맡아 주는 것이 Kafka입니다. 담당자가 잠깐 자리를 비워도 메시지가 그 자리에 남아 있던 이유도 여기에 있습니다.

Kafka는 우체국입니다. 발송처가 편지를 부치면 우체국이 이름 붙은 분류함에 보관해 두고, 집배원이 자기 분류함에서 편지를 가져갑니다. 부치는 쪽과 가져가는 쪽이 같은 시각에 마주 앉아 있지 않아도 되는 건, 그 사이를 우체국이 떼어 놓기 때문입니다.

<div class="svg-figure">
<svg viewBox="0 0 1180 470" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Kafka 우체국: 프로듀서가 편지를 부치면 우체국이 이름 붙은 분류함에 보관하고 집배원(컨슈머)이 자기 분류함에서 가져간다">
  <defs>
  ...
  <text x="108" y="362" text-anchor="middle" font-size="12.5" fill="#64748b">발송처</text>
  ...
  <g>
    ...
    <text x="407" y="238" text-anchor="middle" font-size="12.5" font-weight="700" fill="#9a3412" font-family="monospace">토픽 A</text>
    ...
    <text x="590" y="238" text-anchor="middle" font-size="12.5" font-weight="700" fill="#9a3412" font-family="monospace">토픽 B</text>
    ...
    <text x="773" y="238" text-anchor="middle" font-size="12.5" font-weight="700" fill="#9a3412" font-family="monospace">토픽 C</text>
  ...
  <text x="590" y="356" text-anchor="middle" font-size="12.5" font-weight="700" fill="#9a3412">이름 붙은 분류함 · 토픽</text>
...
:::term-box
**Apache Kafka**는 서비스 사이에 메시지를 안전하게 저장하고 전달하는 분산 메시지 플랫폼입니다.
:::

그림 4-3의 네 구성요소를 우체국 비유와 함께 정리하면 다음과 같습니다. 정의는 이 표 한 곳에 모읍니다.

| 구성요소 | 우체국 비유 | 하는 일 |
|---|---|---|
| Kafka | 우체국 | 메시지를 받아 보관하고 전달 |
| 토픽(Topic) | 이름 붙은 분류함 | 메시지를 목적별로 나눠 담음 |
| 프로듀서(Producer) | 발송처 | 토픽에 메시지를 부침 |
| 컨슈머(Consumer) | 집배원 | 자기 토픽에서 메시지를 가져감 |

프로듀서와 컨슈머가 동시에 실행 중이지 않아도 메시지는 유실되지 않습니다.

### 4.2.1 컨슈머 그룹

product-service를 2대로 늘려 운영한다고 가정해 보겠습니다. 두 인스턴스가 같은 토픽을 구독하면 "재고 1개 차감" 메시지를 둘 다 받아 처리해, 재고가 2개 줄어듭니다. 이 중복 처리를 막아 주는 것이 **컨슈머 그룹**입니다. 같은 일을 하는 인스턴스를 한 그룹으로 묶으면, 그 메시지는 그룹 안 한 인스턴스에만 전달됩니다. 그림 4-4가 그 차이를 보여 줍니다.
...
<svg viewBox="0 0 980 560" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="컨슈머 그룹 비교: 그룹으로 안 묶으면 두 컨슈머가 같은 메시지를 각각 처리해 재고가 두 번 깎이고, 같은 컨슈머 그룹으로 묶으면 한 컨슈머만 처리해 재고가 한 번만 깎인다">
  ...
  <text x="111" y="104" text-anchor="middle" font-size="11.5" font-weight="700" fill="#fff">토픽 · 분류함</text>
  ...
  <text x="111" y="352" text-anchor="middle" font-size="11.5" font-weight="700" fill="#fff">토픽 · 분류함</text>
...
이번 챕터에서 사용하는 토픽, 그러니까 우체국 안에 만들 분류함의 목록을 먼저 펼쳐 둡니다. 지금 다 외울 필요는 없습니다. 구현하면서 하나씩 다시 나옵니다.
...
**선배**: "**지휘자**를 만드는 거예요. 전체 악보를 보고 순서를 정하는 것처럼, 각 서비스는 자기 일만 하고 결과만 보고하게 해요."

:::tip
**Kafka 더 알아두기**

- **컨슈머가 읽어도 메시지는 남는다**: 우체국에서 편지를 꺼내 읽어도 그 편지는 함에 그대로 보관됩니다(기본 7일). 그래서 서로 다른 컨슈머가 같은 메시지를 각자 자기 속도로 찾아 읽을 수 있고, 장애가 나도 다시 읽어 처리할 수 있습니다.
...
지휘자를 두면 주문 흐름이 어떤 모양이 되는지부터 그려 보겠습니다.
...
가운데 지휘자가 있고 주문, 상품, 배달이 그 둘레에 앉아 있습니다. 이 구조를 **Orchestration Saga**라고 부릅니다. 지휘자 역할을 하는 별도 서비스가 **orchestrator**입니다. orchestrator만 전체 악보를 들고 다음에 누구를 부를지 결정합니다. 각 서비스는 명령을 받아 자기 일을 처리하고 결과만 돌려줄 뿐, 다음 단계가 무엇인지 알 필요가 없습니다. 워크플로우 상태가 orchestrator 한 곳에 모이므로 어디까지 진행됐는지 추적할 수 있고, 실패하면 정확히 그 지점부터 거꾸로 롤백할 수 있습니다.

챕터 2의 `createOrder`가 떠안았던 try/catch 보상 블록이, 이제 orchestrator 한 곳으로 모입니다. 각 서비스의 코드에서는 "다음에 뭐 해야 하지", "실패하면 뭐 되돌려야 하지"가 사라집니다.

:::note
**Saga를 구현하는 방식에는 Choreography도 있습니다.** 중앙 지휘자 없이 각 서비스가 이벤트를 발행하고 구독하며 다음 단계를 스스로 이어 갑니다. 단계가 적을 때는 가볍지만, 단계가 늘거나 보상이 복잡해질수록 전체 흐름이 여러 서비스에 흩어져 추적이 어렵습니다. 이 책은 상태를 한 곳에서 추적할 수 있는 Orchestration을 선택합니다.
:::
...
<svg viewBox="0 0 1080 420" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="주문 실패 시 롤백 시퀀스: 배달 생성 실패 후 재고 복구·주문 취소 보상">
  ...
  <text x="516" y="142" text-anchor="middle" font-size="13" font-weight="600" fill="#3730a3" font-family="JetBrains Mono, monospace">1. product-decreased {success}</text>
  <line x1="920" y1="210" x2="382" y2="210" stroke="#4f46e5" stroke-width="1.6" stroke-dasharray="4,3" marker-end="url(#c4f6-o)"/>
  <text x="651" y="202" text-anchor="middle" font-size="13" font-weight="600" fill="#3730a3" font-family="JetBrains Mono, monospace">2. delivery-created {success: false}</text>
  <line x1="380" y1="270" x2="558" y2="270" stroke="#4f46e5" stroke-width="1.6" marker-end="url(#c4f6-i)"/>
  <text x="469" y="262" text-anchor="middle" font-size="13" font-weight="600" fill="#4f46e5" font-family="JetBrains Mono, monospace">3. increase-product-command (보상)</text>
  <line x1="380" y1="335" x2="132" y2="335" stroke="#4f46e5" stroke-width="1.6" marker-end="url(#c4f6-i)"/>
  <text x="256" y="327" text-anchor="middle" font-size="13" font-weight="600" fill="#4f46e5" font-family="JetBrains Mono, monospace">4. cancel-order-command → CANCELLED</text>
...
*그림 4-13. 클라이언트는 kafka-service:9092로 Kafka에 접근합니다*
...
*그림 4-15. Pod 상태 확인 (kubectl get pods)*
