...
<!-- [FLOW CARD: ch4-arc]
path: assets/CH04/diagram/00_ch4-arc.png
desc: 챕터 4의 사건-깨달음-결과 한 장 요약.
  사건: product-service Pod이 30초 살아나는 동안 order-service까지 모두 504로 막힘. try/catch 보상 코드가 단계마다 깊어지고 있음.
  깨달음: 두 서비스가 직접 전화를 걸어 기다리는 구조 자체가 원인. 메시지를 사이에 두면 한쪽이 죽어도 다른 쪽은 살아간다.
  결과: REST 호출을 Kafka 이벤트로 교체하고, 흐름은 orchestrator가 지휘. ex03 — 한 서비스가 잠시 죽어도 주문이 멈추지 않는다.
-->
![](assets/CH04/diagram/00_ch4-arc.png)
*그림 4-0. 챕터 4 한눈에 보기*
...
배포 다음 날 오전이었습니다. 모니터링 대시보드에서 5xx 그래프가 짧게 솟구쳤습니다. product-service Pod이 OOM으로 한 번 죽었다가 K8s가 새로 띄우는 그 **30초** 사이였습니다. 챕터 3에서 그토록 자랑하던 자동 복구. 그런데 그 30초 동안 들어온 주문 요청은 전부 504로 떨어졌습니다. 재고 조회·차감 호출이 product-service에 도달하지 못해서, 호출하던 order-service까지 함께 멈춘 것이었습니다.

*Pod은 살아났는데 왜 주문이 죽었지?*

오픈이는 `OrderService.createOrder` 메서드를 다시 열어봤습니다.

```
productClient.decrease(...);    ← 여기서 30초 기다림
deliveryClient.create(...);     ← 거기서 또 실패하면
catch (...) {                   ← 보상하러 다시 product로
    productClient.increase(...);
}
```

한 서비스가 잠시 죽었을 뿐인데 호출 사슬 전체가 같이 멈춥니다. 그리고 그 멈춤을 막으려고 만든 try/catch 보상 코드가, 단계가 늘어날수록 점점 깊어지고 있었습니다. 챕터 2에서 두 단계로 끝났던 보상 블록은, 새 비즈니스 요구가 들어올 때마다 한 칸씩 더 들여쓰기되고 있습니다.
