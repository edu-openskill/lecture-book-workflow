...
<svg viewBox="0 0 1200 880" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="챕터 4 한눈에 보기: 챕터 3과 동일하게 Client는 클러스터 밖에 있고, 1단계 로그인은 Client가 Ingress·Gateway를 거쳐 User에 로그인하고 JWT를 응답받는다. 2단계 주문은 Client가 Ingress·Gateway를 거쳐 Order에 동기 REST로 주문하고 즉시 PENDING을 응답받는다. 그 뒤 Order·Product·Delivery 세 서비스는 가운데 Orchestrator와 event·command를 주고받고, Orchestrator가 그 모든 메시지를 아래 Kafka 토픽으로 비동기 전달한다. 서비스끼리는 직접 호출하지 않는다.">
  ...
  <line x1="615" y1="676" x2="615" y2="714" stroke="#4f46e5" stroke-width="2.4" marker-end="url(#c4f0-a)"/>
  <text x="603" y="699" text-anchor="end" font-size="12" font-weight="700" fill="#4f46e5">발행</text>
  <line x1="645" y1="714" x2="645" y2="678" stroke="#4f46e5" stroke-width="2.4" stroke-dasharray="4,3" marker-end="url(#c4f0-a)"/>
...
*그림 4-0. 챕터 4 한눈에 보기 - 서비스·Orchestrator·Kafka 3층 구조*
...
서비스마다 패키지 구조가 다르므로, 서비스별 상세 구조는 각 실습 직전(4.4.1·4.5.1·4.6.1)에서 다룹니다.
...
배포 다음 날 오전이었습니다. 모니터링 대시보드에서 에러 그래프가 짧게 솟구쳤습니다. product-service Pod이 OOM으로 한 번 죽었다가 K8s가 새로 띄우는 그 **30초** 사이였습니다. 챕터 3에서 그토록 자랑하던 자동 복구. 그런데 그 30초 동안 들어온 주문 요청은 전부 실패했습니다. 재고 조회·차감 호출이 product-service에 도달하지 못해서, 호출하던 order-service의 주문 처리까지 통째로 터진 것이었습니다.

*서로 동기적으로 호출하니까, 하나가 죽으면 그 주문이 통째로 실패하는구나.*

그 멈춤을 막으려고 만든 try/catch 보상 코드는, 단계가 늘어날수록 점점 깊어지고 있었습니다. 챕터 2에서 두 단계로 끝났던 보상 블록은, 새 비즈니스 요구가 들어올 때마다 한 칸씩 더 들여쓰기되고 있습니다.

**선배**: "서로 직접 전화를 걸어서 기다려주니까 그래요. 이제 전화를 끊고 **메시지를 남기는** 방식으로 바꿔 봐요. Kafka라는 녀석이 있어요."

**오픈이**: "메시지요?"

**선배**: "주문은 진동벨을 받고 자리에 앉아요. 재고 담당자는 자기 일이 끝나면 다음 사람에게 벨을 울리고요. 누가 잠깐 자리를 비워도 메시지가 사라지지 않아요."

방금 마주친 30초 다운은 **MSA의 목표**인 *"각 서비스를 독립적으로 배포하고 장애를 격리한다"* 와 정면으로 충돌합니다. 직접 호출하는 한 격리는 환상입니다.
...
앞에서 본 30초 다운이 정확히 카운터 대기의 풍경이었습니다. product-service가 머신, order-service가 줄 선 손님입니다. 진동벨을 도입하면 머신이 잠깐 멈춰도 줄은 흩어지지 않습니다.
...
챕터 2·3에서 깊어지던 try/catch 보상 블록이, 이제 한 곳(orchestrator)으로 모입니다. 각 서비스의 코드에서는 "다음에 뭐 해야 하지", "실패하면 뭐 되돌려야 하지"가 사라집니다.
...
<svg viewBox="0 0 1080 460" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="주문 성공 시퀀스: Orchestrator가 Order, Product, Delivery에 차례로 command와 event를 주고받음">
  ...
  <text x="254" y="142" text-anchor="middle" font-size="13" font-weight="600" fill="#3730a3" font-family="JetBrains Mono, monospace">1. order-created 발행</text>
  <line x1="380" y1="200" x2="558" y2="200" stroke="#4f46e5" stroke-width="1.6" marker-end="url(#c4f5-i)"/>
  <text x="469" y="192" text-anchor="middle" font-size="13" font-weight="600" fill="#4f46e5" font-family="JetBrains Mono, monospace">2. decrease-product-command</text>
  <line x1="650" y1="250" x2="382" y2="250" stroke="#4f46e5" stroke-width="1.6" stroke-dasharray="4,3" marker-end="url(#c4f5-o)"/>
  <text x="516" y="242" text-anchor="middle" font-size="13" font-weight="600" fill="#3730a3" font-family="JetBrains Mono, monospace">3. product-decreased</text>
  <line x1="380" y1="300" x2="828" y2="300" stroke="#4f46e5" stroke-width="1.6" marker-end="url(#c4f5-i)"/>
  <text x="604" y="292" text-anchor="middle" font-size="13" font-weight="600" fill="#4f46e5" font-family="JetBrains Mono, monospace">4. create-delivery-command</text>
  <line x1="920" y1="350" x2="382" y2="350" stroke="#4f46e5" stroke-width="1.6" stroke-dasharray="4,3" marker-end="url(#c4f5-o)"/>
  <text x="651" y="342" text-anchor="middle" font-size="13" font-weight="600" fill="#3730a3" font-family="JetBrains Mono, monospace">5. delivery-created</text>
  <line x1="380" y1="400" x2="132" y2="400" stroke="#4f46e5" stroke-width="1.6" marker-end="url(#c4f5-i)"/>
  <text x="256" y="392" text-anchor="middle" font-size="13" font-weight="600" fill="#4f46e5" font-family="JetBrains Mono, monospace">6. complete-order-command</text>
...
├── resources/
│   ├── application-dev.properties       ← [참고] Kafka 설정 추가 (H2 로컬)
│   └── application-prod.properties      ← [참고] Kafka 설정 추가 (MySQL 운영)
└── java/.../
    ├── adapter/
    │   ├── message/
...
각 서비스의 `build.gradle`에 다음 한 줄을 추가합니다.
...
각 서비스의 `src/main/resources/application-dev.properties`와 `application-prod.properties` 모두에 Kafka 설정을 추가합니다. Kafka 주소는 환경변수로 주입받습니다.
...
order-service는 토픽 3개에 대응하는 DTO 3개를 가집니다. 주문 1건에 상품 1건이라는 챕터 2의 단순화 가정을 그대로 따르므로 모두 `record` 한 줄짜리 단순한 형태입니다.

| DTO 클래스 | 방향 | 토픽 | 주요 필드 |
|---|---|---|---|
| `OrderCreatedEvent` | 발행 | `order-created` | orderId, userId, productId, quantity, price, address |
| `CompleteOrderCommand` | 수신 | `complete-order-command` | orderId |
| `CancelOrderCommand` | 수신 | `cancel-order-command` | orderId |
...
챕터 2·3에서 보았던 try-catch 보상 트랜잭션이 사라졌습니다. order-service가 더 이상 product-service·delivery-service를 직접 호출하지 않으므로, 실패도 직접 처리하지 않습니다. orchestrator가 Kafka 메시지로 흐름을 조율하고, 실패 시 order-service에는 `cancel-order-command`만 도착합니다.

OrderService에는 `createOrder` 외에 `completeOrder(int orderId)`와 `cancelOrder(int orderId)` 두 메서드가 함께 존재합니다. 두 메서드 모두 주문 상태만 변경(`complete()`·`cancel()`)하고 즉시 반환합니다. 챕터 4.4.8의 `OrderCommandConsumer`가 orchestrator의 명령을 받아 이 두 메서드를 호출합니다.
...
product-service는 재고 감소·복구 명령을 Kafka로만 받고, 결과를 Kafka로 발행합니다. 챕터 3까지 있던 PUT 엔드포인트(`/api/products/{id}/decrease`, `/increase`)는 제거되고, 컨트롤러에는 GET 조회 엔드포인트만 남습니다. delivery-service도 마찬가지로 배달 생성은 Kafka로만 받고, 컨트롤러에는 GET 엔드포인트만 남습니다.
...
<svg viewBox="0 0 880 320" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="1단계 시퀀스: order-created 수신 후 WorkflowState 생성, decrease-product-command 발행">
  ...
  <text x="289" y="142" text-anchor="middle" font-size="13" font-weight="600" fill="#3730a3" font-family="JetBrains Mono, monospace">1. order-created 발행</text>
  <path d="M430 200 L490 200 L490 218 L432 218" fill="none" stroke="#4f46e5" stroke-width="1.6" marker-end="url(#c4f7-o)"/>
  <text x="500" y="213" text-anchor="start" font-size="13" font-weight="600" fill="#3730a3">2. WorkflowState 생성</text>
  <line x1="430" y1="265" x2="708" y2="265" stroke="#4f46e5" stroke-width="1.6" marker-end="url(#c4f7-i)"/>
  <text x="569" y="257" text-anchor="middle" font-size="13" font-weight="600" fill="#4f46e5" font-family="JetBrains Mono, monospace">3. decrease-product-command</text>
</svg>
</div>
...
*그림 4-10. CLUSTER_ID로 Kafka 브로커들을 하나의 클러스터로 묶습니다*
...
이번에는 product-service를 일부러 꺼봤습니다. 챕터 3에서 30초 동안 504를 쏟아냈던 그 자리에, 이제 주문 요청이 그대로 들어왔습니다. 메시지는 Kafka 토픽에 쌓였고, product-service Pod이 다시 살아나자 밀려 있던 명령이 순서대로 처리됐습니다. order-service의 try/catch 보상 블록은 사라졌고, 그 자리에는 `orderEventProducer.publishOrderCreated(...)` 한 줄만 남았습니다. 두 서비스가 강하게 결합되어 있던 문제가 해결됐습니다.
