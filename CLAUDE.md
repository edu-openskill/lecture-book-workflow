...
**★ 어느 버전을 읽나 (해석 규칙 — "최신본 자동 선택")**: 집필·검증 시 기획 산출물은 **`planning/`에서 가장 높은 번호의 `-vN` 파일(최신본)을 읽는다**(`outline-v3`가 있으면 `outline-v2`가 아니라 `outline-v3`). 즉 *최신 = 최고 번호*가 단일 해석 규칙이고, **특정 버전 번호를 글자로 하드코딩한 문구에 의존하지 않는다.** 그래서 새 버전을 만들면 다음 세션이 자동으로 그 최신본을 집어 집필한다. 단:
- `progress.json`의 `step_*.artifact`/`artifact_version`도 새 최신 경로로 함께 올려 `현재 상태` 출력과 일치시킨다(사람이 읽는 기록).
- 다른 문서가 outline을 가리킬 땐 version-agnostic하게 쓴다("최신 `outline-v*`" 또는 "기준 outline"). 부득이 특정 버전을 적었다면, 새 버전 생성 시 그 문구도 함께 갱신한다(스테일 포인터 금지).
- 이 규칙은 `seed`/`scenario`/`analysis` 등 다른 `-vN` 산출물에도 동일 적용(항상 최신 번호).
